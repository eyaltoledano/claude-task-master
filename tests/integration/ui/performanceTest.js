...
async function runPerformanceTest() {
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        console.log(`Iteration ${i + 1}: ${duration.toFixed(2)}ms ${duration < 200 ? '✅' : '❌'}`);
    ...
    console.log('\n--- Performance Summary ---');
    console.log(`Average: ${(measurements.reduce((a, b) => a + b, 0) / measurements.length).toFixed(2)}ms`);
    console.log(`Min: ${Math.min(...measurements).toFixed(2)}ms`);
    console.log(`Max: ${Math.max(...measurements).toFixed(2)}ms`);
    console.log(`Under 200ms: ${measurements.filter(m => m < 200).length}/${iterations}`);
    ...
    console.log(`${batchDuration < 1000 ? '✅ PASS' : '❌ FAIL'}: Batch completed in reasonable time`);
