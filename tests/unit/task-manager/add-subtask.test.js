/**
 * Tests for the addSubtask function in task-manager.js
 */

import { jest } from '@jest/globals';
import path from 'path'; // Need path for dirname

// Mocks - Add necessary mocks used by the tests and the helper function
const mockReadJSON = jest.fn();
const mockWriteJSON = jest.fn();
const mockIsTaskDependentOn = jest.fn();
const mockGenerateTaskFiles = jest.fn();
const mockDirname = jest.fn(); // Mock for path.dirname used in testRemoveSubtask
...
jest.mock('../../../scripts/modules/dependency-manager.js', () => ({
	...
	// Add other dependency-manager mocks if needed
}));
jest.mock('path', () => ({
	dirname: mockDirname,
...
// Define the test version of addSubtask locally
const testAddSubtask = (
	...
	if (generateFiles) {
		// Mock the path.dirname call if generateTaskFiles needs it
		mockGenerateTaskFiles(tasksPath, mockDirname(tasksPath));
	}
...
// This is needed because the testAddSubtask helper calls it directly.
