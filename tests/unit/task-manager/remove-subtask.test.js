/**
 * Tests for the removeSubtask function in task-manager.js
 */
...
const mockDirname = jest.fn().mockReturnValue('tasks'); // Mock path.dirname to return base dir
...
// This is needed because the testRemoveSubtask helper calls it directly.
jest.mock('../../../scripts/modules/task-manager.js', () => ({
    generateTaskFiles: mockGenerateTaskFiles
}));


// Define the test version of removeSubtask locally
const testRemoveSubtask = (
	...
	if (convertToTask) {
		...
		const highestId = Math.max(0, ...data.tasks.map((t) => t.id)); // Ensure highestId is at least 0
