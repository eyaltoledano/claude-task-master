...
jest.mock('path', () => {
    ...
    return {
        ...originalPath,
        join: jest.fn((...args) => originalPath.join(...args)), // Use real join
        resolve: jest.fn((...args) => originalPath.resolve(...args)), // Use real resolve
        dirname: jest.fn((p) => originalPath.dirname(p)), // Use real dirname
        basename: jest.fn((p) => originalPath.basename(p)), // Use real basename
    };
});
jest.mock('@anthropic-ai/sdk', () => ({
    Anthropic: jest.fn(() => ({ messages: { create: jest.fn() } }))
}));
jest.mock('openai', () => ({
    OpenAI: jest.fn(() => ({
...
jest.mock('boxen', () => jest.fn(text => text)); // Simple mock for boxen
jest.mock('ora', () => {
...
jest.mock('../../scripts/modules/utils.js', () => ({
    ...
    debug: false,
    defaultSubtasks: 3,
        // Add other necessary defaults used by task-manager
    },
    log: jest.fn(),
    readJSON: jest.fn(),
    writeJSON: jest.fn(),
    findTaskById: jest.fn(),
    sanitizePrompt: jest.fn(p => p),
...
jest.mock('../../scripts/modules/ui.js', () => ({
    ...
    formatDependenciesWithStatus: jest.fn(() => '[Dep Info]'),
    getComplexityWithColor: jest.fn(() => 'Medium'),
...
jest.mock('../../scripts/modules/ai-services.js', () => ({
    ...
    generateComplexityAnalysisPrompt: jest.fn(() => 'Complexity Prompt'),
    getAvailableAIModel: jest.fn(() => 'claude-mock-model'),
    handleClaudeError: jest.fn(err => err.message),
    processClaudeResponse: jest.fn(),
    parseSubtasksFromText: jest.fn(),
}));

jest.mock('../../scripts/modules/dependency-manager.js', () => ({
    validateTaskDependencies: jest.fn(() => []), // Return empty array for no issues
...
// --- Import Mocks Needed for Verification --- 
// Import directly from the mock definition
import { readJSON, writeJSON, findTaskById, log } from '../../scripts/modules/utils.js';
import { callClaude, generateSubtasks } from '../../scripts/modules/ai-services.js';
import { validateAndFixDependencies } from '../../scripts/modules/dependency-manager.js';

// --- Test Suite --- 
describe.skip('Task Manager Module', () => {

    const mockTasksPath = 'tasks.json';
    const mockTask = { id: 1, title: 'Task 1', description: 'Desc 1', status: 'pending', dependencies: [], subtasks: [] };
    const mockTasksData = { tasks: [mockTask] };

    // Import the actual module inside beforeAll AFTER mocks are set
    beforeAll(async () => {
        // Wrap in try-catch as the suite is skipped
        try { 
            taskManagerModule = await import('../../scripts/modules/task-manager.js');
        } catch(e) {}
    });
  ...
  beforeEach(() => {
    jest.clearAllMocks();
        ...
        readJSON.mockReturnValue(JSON.parse(JSON.stringify(mockTasksData))); // Return deep copy
        findTaskById.mockImplementation((tasks, id) => tasks.find(t => t.id === parseInt(id)));
        fs.existsSync.mockReturnValue(true); // Default assumption
        fs.readFileSync.mockReturnValue(''); // Default return for readFileSync
        // Reset process.exit mock if used
        jest.spyOn(process, 'exit').mockImplementation(() => {}); 
        jest.spyOn(console, 'error').mockImplementation(() => {}); // Silence console.error
    });

    afterEach(() => {
         // Restore mocks spied on process or console
         jest.restoreAllMocks();
    });

    describe('parsePRD', () => {
        const prdPath = 'prd.txt';
        const prdContent = 'PRD Content';
        const numTasks = 5;
        const mockClaudeResponse = { tasks: [{ id: 1, title: 'Generated Task' }], metadata: {} };
    
    beforeEach(() => {
            fs.readFileSync.mockReturnValue(prdContent);
            callClaude.mockResolvedValue(mockClaudeResponse);
            // We don't need to spy on generateTaskFiles anymore if we import it directly
        });

        test('should read PRD, call Claude, write tasks, and generate files', async () => {
            // Mock generateTaskFiles directly from the imported module
            taskManagerModule.generateTaskFiles = jest.fn().mockResolvedValue();

            await taskManagerModule.parsePRD(prdPath, mockTasksPath, numTasks);

            expect(fs.readFileSync).toHaveBeenCalledWith(prdPath, 'utf8');
            expect(callClaude).toHaveBeenCalledWith(prdContent, prdPath, numTasks);
            expect(writeJSON).toHaveBeenCalledWith(mockTasksPath, mockClaudeResponse);
            expect(log).toHaveBeenCalledWith('success', expect.stringContaining('Successfully generated'));
            // Check if the mocked generateTaskFiles was called
            expect(taskManagerModule.generateTaskFiles).toHaveBeenCalledWith(mockTasksPath, path.dirname(mockTasksPath));
        });

        test('should create tasks directory if it doesnt exist', async () => {
             fs.existsSync.mockReturnValue(false); // Mock directory doesnt exist
             taskManagerModule.generateTaskFiles = jest.fn().mockResolvedValue(); // Mock needed again
             await taskManagerModule.parsePRD(prdPath, mockTasksPath, numTasks);
             expect(fs.mkdirSync).toHaveBeenCalledWith(path.dirname(mockTasksPath), { recursive: true });
        });

        test('should handle errors during PRD parsing', async () => {
            const error = new Error('Claude Error');
            callClaude.mockRejectedValue(error);
            ...
            expect(log).toHaveBeenCalledWith('error', expect.stringContaining('Error parsing PRD'), error.message);
            expect(consoleErrSpy).toHaveBeenCalledWith(expect.stringContaining('Error: Claude Error'));
    ...
    describe('generateTaskFiles', () => {
      const outputDir = 'tasks';
        const tasksData = {
        tasks: [
                { id: 1, title: "Task One", description: "...", status: "pending", dependencies: [], priority: "medium", details: "d1", testStrategy: "t1" },
                { id: 2, title: "Task Two", description: "...", status: "pending", dependencies: [1], priority: "high", details: "d2", testStrategy: "t2" }
            ]
        };

    beforeEach(() => {
             readJSON.mockReturnValue(tasksData);
             fs.existsSync.mockReturnValue(true); // Assume output dir exists by default
             validateAndFixDependencies.mockImplementation((data, path) => {}); // Mock implementation
        });

        test('should read tasks, validate deps, and write individual files', async () => {
            await taskManagerModule.generateTaskFiles(mockTasksPath, outputDir);

            expect(readJSON).toHaveBeenCalledWith(mockTasksPath);
            expect(validateAndFixDependencies).toHaveBeenCalledWith(tasksData, mockTasksPath);
            expect(fs.writeFileSync).toHaveBeenCalledTimes(tasksData.tasks.length); // One file per task

            // Check content of one file
            const expectedFilePath1 = path.join(outputDir, 'task_001.txt');
            const expectedContent1 = expect.stringContaining(`# Task ID: 1`) &&
                                     expect.stringContaining(`# Title: Task One`) &&
                                     expect.stringContaining(`# Status: pending`);
            expect(fs.writeFileSync).toHaveBeenCalledWith(expectedFilePath1, expectedContent1);

            const expectedFilePath2 = path.join(outputDir, 'task_002.txt');
             const expectedContent2 = expect.stringContaining(`# Task ID: 2`) &&
                                     expect.stringContaining(`# Dependencies: 1`);
             expect(fs.writeFileSync).toHaveBeenCalledWith(expectedFilePath2, expectedContent2);
        });

         test('should create output directory if it doesnt exist', async () => {
             fs.existsSync.mockReturnValue(false); // Mock directory doesnt exist
             await taskManagerModule.generateTaskFiles(mockTasksPath, outputDir);
             expect(fs.mkdirSync).toHaveBeenCalledWith(outputDir, { recursive: true });
         });

         test('should throw error if tasks file is invalid', async () => {
            readJSON.mockReturnValue(null); // Simulate invalid file
            await expect(taskManagerModule.generateTaskFiles(mockTasksPath, outputDir)).rejects.toThrow('No valid tasks found');
         });
    });

    // Example for expandTask
    describe('expandTask', () => {
        const taskId = 1;
        const numSubtasks = 3;
        const mockGeneratedSubtasks = [{title: 'Sub 1'}, {title: 'Sub 2'}, {title: 'Sub 3'}];
  
  beforeEach(() => {
             generateSubtasks.mockResolvedValue(mockGeneratedSubtasks);
        });

        test('should find task, generate subtasks, and update file', async () => {
             // Ensure findTaskById returns a task that can be modified
             const taskToModify = JSON.parse(JSON.stringify(mockTask)); // Deep copy
             findTaskById.mockReturnValue(taskToModify);

             await taskManagerModule.expandTask(taskId, numSubtasks);

             expect(findTaskById).toHaveBeenCalledWith(mockTasksData.tasks, taskId);
             expect(generateSubtasks).toHaveBeenCalledWith(taskToModify, numSubtasks, false, ''); // Check args
             expect(taskToModify.subtasks).toHaveLength(numSubtasks);
             expect(taskToModify.subtasks[0].title).toBe('Sub 1');
             expect(taskToModify.subtasks[0].id).toBe(1); // Check auto-ID assignment
             expect(taskToModify.subtasks[0].status).toBe('pending'); // Check default status
             expect(writeJSON).toHaveBeenCalledWith(mockTasksPath, expect.objectContaining({
                 tasks: expect.arrayContaining([taskToModify])
             }));
             expect(log).toHaveBeenCalledWith('success', expect.stringContaining(`Expanded task ${taskId}`));
        });
         ...
         test('should handle task not found', async () => {
             ...
             await taskManagerModule.expandTask(99, numSubtasks);

             expect(generateSubtasks).not.toHaveBeenCalled();
             expect(writeJSON).not.toHaveBeenCalled();
             expect(consoleErrSpy).toHaveBeenCalledWith(expect.stringContaining('Task 99 not found'));
         });

          test('should handle errors during subtask generation', async () => {
             findTaskById.mockReturnValue(mockTask);
             const error = new Error('AI Subtask Error');
             generateSubtasks.mockRejectedValue(error);
             const consoleErrSpy = jest.spyOn(console, 'error');

             await taskManagerModule.expandTask(taskId, numSubtasks);

             expect(writeJSON).not.toHaveBeenCalled();
             expect(consoleErrSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to expand task'), error);
          }
...
const testAddSubtask = (tasksPath, parentId, existingTaskId, newSubtaskData, generateFiles = true) => {
  ...
  test('should fall back to Perplexity if Claude is overloaded', async () => {
    ...
    // updateTasks, setTaskStatus, listTasks, expandAllTasks, 
    // clearSubtasks, addTask, addSubtask, removeSubtask, 
    // analyzeTaskComplexity, updateTaskById, updateSubtaskById
