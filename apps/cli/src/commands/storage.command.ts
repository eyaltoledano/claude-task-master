...
import ora from 'ora';
import { displayError, displaySuccess, displayWarning, displayInfo } from '../utils/ui.js';
...
export class StorageCommand extends Command {
	...
	private addSwitchCommand(): void {
		this.command('switch <type>')
			...
			.action(async (type: string, options: { migrate?: boolean; backup?: boolean }) => {
	...
	private addMigrateCommand(): void {
		this.command('migrate')
			...
			.action(async (options: { from?: string; to?: string; validate?: boolean }) => {
	...
	private async executeStatus(): Promise<void> {
		try {
			...
			console.log(`  ${chalk.bold('Current backend:')} ${this.formatStorageType(currentType)}`);
			...
			if (currentType === 'sqlite') {
				...
				console.log(`  ${chalk.bold('Database:')} ${fs.existsSync(dbPath) ? chalk.green('exists') : chalk.red('not found')}`);
				console.log(`  ${chalk.bold('JSONL sync:')} ${fs.existsSync(jsonlPath) ? chalk.green('exists') : chalk.yellow('not found')}`);
			} else if (currentType === 'file') {
				const tasksJsonPath = path.join(tasksDir, 'tasks.json');
				console.log(`  ${chalk.bold('Tasks file:')} ${fs.existsSync(tasksJsonPath) ? chalk.green('exists') : chalk.yellow('not found')}`);
	...
	private async executeSwitch(
		...
		if (!validTypes.includes(normalizedType)) {
			displayError(`Invalid storage type: ${type}. Valid types are: ${validTypes.join(', ')}`);
			this.lastResult = {
				success: false,
				action: 'switch',
		...
		try {
			...
			if (currentType === targetType) {
				spinner.info(`Already using ${this.formatStorageType(targetType)} storage`);
				this.lastResult = {
					success: true,
					action: 'switch',
			...
			if (shouldMigrate && canMigrate.canMigrate) {
				...
				spinner.succeed(`Switched to ${this.formatStorageType(targetType)} storage (config only, no data migrated)`);
			}
	...
	private async executeMigrate(
		options: { from?: string; to?: string; validate?: boolean }
	): Promise<void> {
		...
		try {
			...
			const sourceType = (options.from?.toLowerCase() || currentType) as 'file' | 'sqlite';
			...
			if (!validTypes.includes(sourceType) || !validTypes.includes(targetType)) {
				...
				displayError(`Invalid storage type. Valid types are: ${validTypes.join(', ')}`);
				this.lastResult = {
					success: false,
					action: 'migrate',
			...
			if (migrationResult.success) {
				...
				displaySuccess(`Successfully migrated to ${this.formatStorageType(targetType)} storage`);
			} else {
				spinner.fail('Migration failed');
				for (const error of migrationResult.errors) {
					displayError(error);
	...
	private async executeRebuild(): Promise<void> {
		...
		try {
			...
			if (!fs.existsSync(jsonlPath)) {
				...
				displayInfo('The rebuild command requires a tasks.jsonl file to rebuild from.');
				this.lastResult = {
					success: false,
					action: 'rebuild',
			...
			if (fs.existsSync(dbPath)) {
				...
				displayInfo(`Backed up existing database to ${path.basename(backupPath)}`);
			}
			...
			spinner.succeed(`Database rebuilt: ${stats.totalTasks} tasks in ${stats.totalTags} tag(s)`);
	...
	private async updateConfigType(newType: 'file' | 'sqlite'): Promise<void> {
		...
		if (fs.existsSync(this.configPath)) {
			try {
				config = JSON.parse(await fs.promises.readFile(this.configPath, 'utf-8'));
			} catch {
