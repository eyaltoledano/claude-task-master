...
export class StartCommand extends Command {
	...
	constructor(name?: string) {
		...
		this.description('Start working on a task by launching claude-code with context')
			...
			.option('--dry-run', 'Show what would be executed without launching claude-code')
			.action(
	...
	private async executeCommand(
		...
		try {
			...
			if (!targetTaskId) {
				console.error(chalk.red('Error: No task ID provided and no available tasks found'));
				process.exit(1);
			}
	...
	private buildStandardizedPrompt(task: Task): string {
		...
		// Header
		sections.push(`You are an AI coding assistant with access to this repository's codebase.`);
		sections.push('');
		...
		if (task.dependencies && task.dependencies.length > 0) {
			...
			sections.push('Make sure these dependencies are completed before proceeding.');
			sections.push('');
		}
		...
		if (task.subtasks && task.subtasks.length > 0) {
			...
			task.subtasks.forEach(subtask => {
				...
				sections.push(`  ${statusIcon} ${task.id}.${subtask.id} - ${subtask.title} [${subtask.status}]`);
		...
		sections.push('When complete, print: COMPLETED: <brief summary of changes>');
		sections.push('');
	...
	private async executeClaudeCode(
		...
		try {
			console.log(chalk.blue('🚀 Starting claude-code to implement the task...'));
			...
			if (error.status === 127) {
				...
				console.error(chalk.yellow('Please make sure Claude Code is installed and available in your PATH'));
			...
			console.error(chalk.red(`❌ Error executing claude-code: ${error.message}`));
