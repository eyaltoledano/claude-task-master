...
Task Master supports HTTP/HTTPS proxy for all AI providers when working in corporate or restricted network environments.
...
| `https_proxy` | HTTPS proxy server URL | `https://proxy.company.com:8080` |
| `no_proxy` | Comma-separated list of hosts to bypass proxy | `localhost,127.0.0.1,.company.com` |
...
- Priority: `TASKMASTER_ENABLE_PROXY` environment variable overrides `config.json` setting
- Users without proxy requirements are not affected when proxy is disabled
