...
The `tm/` prefix ensures workflow branches don't conflict with existing project branches. Titles are sanitized by converting to lowercase, replacing non-alphanumeric characters with dashes, and truncating to 50 characters.
...
**BRANCH_SETUP**
- Create workflow branch: `tm/task-{taskId}-{title}` or `tm/{namespace}/task-{taskId}-{title}` for team mode
- Checkout new branch
...
describe('StartCommand', () => {
  it('should initialize workflow and create branch', async () => {
    ...
    expect(result.branchName).toBe('tm/task-7-implement-auth-system');
  });
});
```
...
export class StartCommand {
  async execute(options: { taskId: string }) {
    ...
    return {
      success: true,
      branchName: `tm/task-${options.taskId}-${sanitizedTitle}`
    };
  }
}
```
...
git branch -D tm/task-7-implement-auth-system
