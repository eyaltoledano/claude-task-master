...
- **Optional Proxy Configuration (Corporate Networks):**
  - `TASKMASTER_ENABLE_PROXY`: Set to 'true' to enable proxy support for AI provider requests (default: false).
  - `http_proxy`: HTTP proxy server URL (e.g., `http://proxy.company.com:8080`).
  - `https_proxy`: HTTPS proxy server URL (e.g., `https://proxy.company.com:8080`).  
