    ...
    ```json
    {
        ...
        "llmRequestForAgent": {
            "delegatedCallDetails": {
                "originalCommand": "parse_prd", // The Taskmaster command that initiated this delegation
                "role": "main", // The AI role used (e.g., "main"|"research")
                "serviceType": "generateText", // The type of LLM service (e.g., "generateText"|"generateObject"|"streamText")
                "requestParameters": {
                    // These are the actual LLM parameters the agent needs to make the call
                    ...
                    // "tagInfo": { ... }, // Optional metadata object describing the request (may include task context, tags, etc.)
