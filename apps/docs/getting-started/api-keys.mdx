...
- **Required**: ✅ **Yes** (for REST API) or use [Cortex Code CLI](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-code)
- **Requirements**: Also requires `SNOWFLAKE_BASE_URL` (or `SNOWFLAKE_ACCOUNT` to derive URL)
- **Alternative Auth**: Key pair authentication (`SNOWFLAKE_ACCOUNT` + `SNOWFLAKE_USER` + `SNOWFLAKE_PRIVATE_KEY_PATH`)
...
```bash
SNOWFLAKE_API_KEY="YOUR_SNOWFLAKE_PAT_HERE"
...
- **Snowflake**: Requires `SNOWFLAKE_API_KEY` + `SNOWFLAKE_BASE_URL`, or key pair auth, or Cortex Code CLI
