...
#### All Tools (Default)
Loads all 43 available tools. Use when you need full Task Master functionality.
...
### 7. Autopilot TDD Workflow

-   **`autopilot_start`**: Initialize autonomous TDD workflow for a task.
-   **`autopilot_next`**: Get the next action in the current workflow.
-   **`autopilot_status`**: Check current workflow progress and status.
-   **`autopilot_complete_phase`**: Complete current TDD phase (RED/GREEN/COMMIT).
-   **`autopilot_commit`**: Save workflow progress with generated commit message.
-   **`autopilot_resume`**: Resume an interrupted workflow from checkpoint.
-   **`autopilot_abort`**: Cancel current workflow and cleanup state.