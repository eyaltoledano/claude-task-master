...
-   **`clear_subtasks`**: Clears all subtasks from one or more tasks. *(Local-only - blocked when using Hamster)*
...
<Info>**Local-Only Tools**: These dependency management tools are only available when using local file storage. They will be blocked when authenticated with Hamster (remote storage).</Info>
...
-   **`models`**: Manages AI model configurations. *(Local-only - blocked when using Hamster)*
