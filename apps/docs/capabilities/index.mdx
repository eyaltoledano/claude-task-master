...
-   **`clear-subtasks`**: Clears all subtasks from one or more tasks. *(Local-only)*
...
<Info>All dependency management commands are local-only and blocked when using Hamster.</Info>

-   **`add_dependency`**: Adds a dependency to a task. *(Local-only)*
-   **`remove_dependency`**: Removes a dependency from a task. *(Local-only)*
-   **`validate_dependencies`**: Validates the dependencies of all tasks. *(Local-only)*
-   **`fix_dependencies`**: Fixes any invalid dependencies. *(Local-only)*

### 5. Project and Configuration

-   **`initialize_project`**: Initializes a new project.
-   **`generate`**: Generates individual task files.
-   **`models`**: Manages AI model configurations. *(Local-only)*
-   **`research`**: Performs AI-powered research.
