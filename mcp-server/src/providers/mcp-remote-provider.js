...
export class MCPRemoteProvider extends BaseAIProvider {
	...
	validateAuth(params) {
		...
		if (
			!this.session.clientCapabilities ||
			!this.session.clientCapabilities.sampling
		) {
	...
	async generateText(params) {
		try {
			...
			logger.debug(
				`Generating text with MCP Remote Provider using model: ${modelId}`
			);
	...
	validateParams(params) {
		...
		logger.debug(
			`Validating MCP params: ${JSON.stringify({
