...
class ExpandTaskSaver extends AgentLLMToolSaver {
    ...
    async processAgentOutput(agentOutput, allTasksData, logWrapper, originalToolArgs, delegatedRequestParams) {
        ...
        if (!parentTask) {
            return { success: false, error: `Parent task with ID ${parentTaskIdNum} not found.` };
        }
        ...
        if (typeof agentOutput === 'string') {
            try {
                ...
                subtasksToSave = Array.isArray(parsed) ? parsed : Array.isArray(parsed?.subtasks) ? parsed.subtasks : null;
            } catch (e) {
                return { success: false, error: `Invalid agentOutput string (JSON parse failed): ${e.message}` };
            }
            ...
            return { success: false, error: "Invalid agentOutput format. Expected a JSON string of subtasks, an array of subtasks, or an object with a 'subtasks' array." };
        }

        if (!Array.isArray(subtasksToSave)) {
            return { success: false, error: `Subtask parsing or processing resulted in non-array: ${JSON.stringify(subtasksToSave)}` };
        }
        ...
        let nextId = (originalTaskDetails?.nextSubtaskId ?? (parentTask.subtasks?.length || 0) + 1) | 0;
        const seenIds = new Set(parentTask.subtasks.map((st) => parseInt(String(st.id), 10)));
...
export const agentllmExpandTaskSave = async (agentOutput, parentTaskIdNum, projectRoot, logWrapper, originalTaskDetails, tag = 'master') => {
    const saver = new ExpandTaskSaver();
    const delegatedRequestParams = { parentTaskIdNum, originalTaskDetails };
    return saver.save(agentOutput, projectRoot, logWrapper, null, delegatedRequestParams, tag);
};
