...
class ResearchSaver extends AgentLLMToolSaver {
    ...
    async saveResearchToFile(researchText, query, projectRoot, logWrapper) {
        try {
            ...
            const querySlug = query.toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-').substring(0, 50).replace(/^-+|-+$/g, '');
            const filename = `${timestamp}_${querySlug}.md`;
            const filePath = path.join(researchDir, filename);
            const fileContent = `# Research Query: ${query}\n\n## Date: ${new Date().toLocaleDateString()}\n\n${researchText}`;

            await fs.promises.writeFile(filePath, fileContent, 'utf8');
            logWrapper.info(`${this.toolName} (saveToFile): Research saved to: ${path.relative(projectRoot, filePath)}`);
            return filePath;
        } catch (error) {
            logWrapper.error(`${this.toolName} (saveToFile): Error saving research file: ${error.message}`);
            return null;
        }
    }

    async processAgentOutput(agentOutput, allTasksData, logWrapper, originalToolArgs) {
    ...
    async save(agentOutput, projectRoot, logWrapper, originalToolArgs, delegatedRequestParams, tag = 'master') {
        ...
        if (saveToFile) {
            savedFilePath = await this.saveResearchToFile(agentOutput, query, projectRoot, logWrapper);
        }
        ...
        try {
            const { tasksJsonPath, allTasksData } = await this.loadTasksData(projectRoot, tag, logWrapper);
            const result = await this.processAgentOutput(agentOutput, allTasksData, logWrapper, originalToolArgs);
            ...
            return { success: true, filePath: savedFilePath, taskUpdated: result.data.taskUpdated };
        } catch (error) {
...
export const agentllmResearchSave = async (agentResearchText, originalResearchArgs, projectRoot, log, sessionContext, tag) => {
    const saver = new ResearchSaver();
    return saver.save(agentResearchText, projectRoot, log, originalResearchArgs, null, tag);
};
