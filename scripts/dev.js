...
import { callLLMWithRetry } from './modules/ai-services.js';
...
const CONFIG = {
  ...
  projectVersion: process.env.PROJECT_VERSION || "1.5.0",
...
function displayBanner() {
  ...
  console.log(
    gradient.atlas.multiline(
      figlet.textSync('Task Master AI', { font: 'Small', horizontalLayout: 'fitted' })
    )
  );
}
...
function readJSON(filepath) {
  ...
  try {
    ...
    console.log(`Content read successfully, length: ${content.length} characters`);
    console.log(`First 100 characters: ${content.substring(0, 100)}`);
    try {
      const data = JSON.parse(content);
      console.log(`JSON parsed successfully, structure: ${Object.keys(data).join(', ')}`);
      ...
      if (data && data.tasks && Array.isArray(data.tasks)) {
        ...
        console.log(`No tasks array found or it's not an array: ${data && data.tasks ? typeof data.tasks : 'undefined'}`);
      }
...
Be creative and thorough in your analysis. The subtasks should collectively represent a complete solution to the parent task.`;
        ...
        console.log(chalk.green(`\nRetried expansion for ${retryCount} tasks.`));
      }
    }
    
    return tasksExpanded;
  } catch (error) {
    console.error(chalk.red('Error expanding tasks:'), error);
    return 0;
  }
}
...
async function main() {
  ...
  program
    ...
    .option('-f, --force', 'Force overwrite of existing tasks.json')
    .action(async (file, options) => {
      ...
      // Set force overwrite environment variable if flag is set
      if (options.force) {
...
${progressBar} ${progress}% ${timingInfo}${additionalContext}`;
  ...
  program
    .command('archives')
    .alias('archive-list')
    .description('List archived tasks and PRDs')
    .action(async () => {
      try {
        const projectRoot = process.cwd();
        
        displayBanner();
        console.log(chalk.bold.blue('\nTask Master Archives\n'));
        
        // Get list of archived tasks and PRDs
        const { listArchives } = await import('./modules/task-manager.js');
        const archives = await listArchives(projectRoot);
        
        if (archives.tasks.length === 0 && archives.prds.length === 0) {
          console.log(chalk.yellow('No archives found. Archives are created when tasks or PRDs are overwritten.'));
          return;
        }
        
        // Display archived tasks
        if (archives.tasks.length > 0) {
          console.log(chalk.cyan.bold('\nArchived Tasks:'));
          const table = new Table({
            head: [
              chalk.white.bold('Filename'), 
              chalk.white.bold('Timestamp'), 
              chalk.white.bold('Size (bytes)')
            ],
            colWidths: [40, 25, 15]
          });
          
          archives.tasks.forEach(file => {
            table.push([
              file.filename,
              file.timestamp,
          ...
          console.log(table.toString());
          console.log(chalk.gray(`Archive location: ${path.join(projectRoot, 'tasks', 'archives')}`));
        }
        
        // Display archived PRDs
        if (archives.prds.length > 0) {
          console.log(chalk.cyan.bold('\nArchived PRDs:'));
          const table = new Table({
            head: [
              chalk.white.bold('Filename'), 
          ...
          console.log(table.toString());
          console.log(chalk.gray(`Archive location: ${path.join(projectRoot, 'scripts', 'archives')}`));
        }
        
        console.log(chalk.bold('\nTo restore an archive:'));
        console.log(`1. Copy the archived file from the archives directory.`);
        console.log(`2. Rename it and place it in the original location.\n`);
        console.log(`Or use the ${chalk.yellow('restore-archive')} command:`);
        console.log(`  ${chalk.cyan('task-master restore-archive <archivePath>')}\n`);
        
      } catch (err) {
        console.error(chalk.red('Error listing archives:'), err.message);
      }
    });
  ...
  program
    ...
    .argument('<archive-path>', 'Path to the archived file to restore')
    .option('-d, --destination <path>', 'Custom destination path (if not restoring to original location)')
    .option('-n, --no-backup', 'Do not create a backup of existing destination file')
    .action(async (archivePath, options) => {
      try {
        const projectRoot = process.cwd();
        
        displayBanner();
        console.log(chalk.bold.blue('\nTask Master Archive Restore\n'));
        
        // Import the restore function
        const { restoreArchive } = await import('./modules/utils.js');
        ...
        if (options.destination) {
          destinationPath = path.isAbsolute(options.destination)
            ? options.destination
        ...
        if (!fs.existsSync(resolvedArchivePath)) {
          console.error(chalk.red(`Archive file not found: ${resolvedArchivePath}`));
          console.log(chalk.yellow('\nTip: Run `task-master archives` to list available archives.'));
          process.exit(1);
        }
        
        // Display restore info
        console.log(`Restoring archive: ${chalk.cyan(resolvedArchivePath)}`);
        if (destinationPath) {
          console.log(`To destination: ${chalk.cyan(destinationPath)}`);
        }
        console.log(`Create backup of existing file: ${options.backup ? chalk.green('Yes') : chalk.yellow('No')}`);
        ...
        // Execute restore
        const spinner = startLoadingIndicator('Restoring archive...');
        const result = restoreArchive(resolvedArchivePath, destinationPath, options.backup);
        stopLoadingIndicator(spinner);
        
        if (!result.success) {
          console.error(chalk.red(`Error restoring archive: ${result.error}`));
          process.exit(1);
        }
        
        console.log(chalk.green(`✓ Archive successfully restored to ${result.restoredTo}`));
        
        // Additional info for tasks.json
        if (result.restoredTo.endsWith('tasks.json')) {
          console.log('\nNext steps:');
          console.log(`  1. Run ${chalk.cyan('task-master list')} to view the restored tasks`);
          console.log(`  2. Run ${chalk.cyan('task-master generate')} to regenerate individual task files`);
        }
        
      } catch (err) {
        console.error(chalk.red('Error restoring archive:'), err.message);
        process.exit(1);
      }
    });
