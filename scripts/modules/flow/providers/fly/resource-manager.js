...
export class FlyResourceManager {
  ...
  async trackResource(resource) {
    try {
      ...
      console.warn(`Failed to track Fly.io resource ${resource.id}: ${error.message}`)
      throw error
    }
  }
  ...
  async updateResource(resourceId, updates) {
    try {
      ...
      if (resource.status === 'ready' && updates.status !== 'stopped') {
        ...
        updatedResource.cost = this.calculateCost(uptimeHours, resource.region || 'ord')
      ...
      console.warn(`Failed to update Fly.io resource ${resourceId}: ${error.message}`)
      throw error
    }
  }
    ...
    try {
      ...
      if (resource) {
        ...
        if (resource.status === 'ready') {
          ...
          const finalCost = this.calculateCost(finalUptime, resource.region || 'ord')
      ...
      console.warn(`Failed to untrack Fly.io resource ${resourceId}: ${error.message}`)
      throw error
    }
  }
  ...
  async logExecution(resourceId, execution) {
    try {
      ...
      console.warn(`Failed to log Fly.io execution for ${resourceId}: ${error.message}`)
      throw error
    }
  }
  ...
  async getResourceLogs(resourceId, options = {}) {
    try {
      ...
      console.warn(`Failed to get logs for Fly.io resource ${resourceId}: ${error.message}`)
      return []
    }
  }
  ...
  async archiveResourceData(resourceId, resourceData) {
    try {
      ...
      const archiveData = {
        ...
        finalCost: this.calculateCost(this.calculateUptime(resourceData.createdAt), resourceData.region || 'ord')
      }
      ...
      for (const file of files) {
        if (file.startsWith('resource-') && file.endsWith('.json') && !file.includes('executions-')) {
          ...
          try {
            ...
            console.warn(`Failed to load Fly.io resource ${resourceId}: ${error.message}`)
          }
        }
      }
      ...
      for (const file of files) {
        if (file.startsWith('executions-') && file.endsWith('.json')) {
          ...
          try {
            ...
            console.warn(`Failed to load Fly.io executions for ${resourceId}: ${error.message}`)
          }
        }
      }
    } catch (error) {
