  ...
  async streamMachineLogs(machineId, callback) {
    try {
      ...
      const pollLogs = async () => {
        try {
          const logs = await this.getMachineLogs(machineId, { since: lastTimestamp })
...
export class FlyMachine {
  ...
  async waitForState(desiredState, timeout = 60000) {
    ...
    while (Date.now() - startTime < timeout) {
      ...
      if (status.state === 'failed' && desiredState !== 'failed') {
        throw new FlyError({
          ...
          category: 'resource',
          details: { machineId: this.id, currentState: status.state, desiredState }
        })
      }
    ...
    throw new FlyError({
      ...
      category: 'resource',
      details: { machineId: this.id, currentState: this.state, desiredState, timeout }
    })
  }
