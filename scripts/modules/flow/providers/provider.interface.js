...
export class QuotaExceededError extends Error {
	constructor(resource, current, limit, provider) {
		super(
			`Quota exceeded for ${resource}: ${current}/${limit} in provider ${provider}`
		);
...
export const SandboxProviderInterface = {
	...
	deleteResource: (resourceId) =>
		Effect.succeed(createActionResult(true, `Resource ${resourceId} deleted`)),
	...
	executeAction: (resourceId, action, parameters = {}) =>
		Effect.succeed(
			createActionResult(true, `Executed ${action} on ${resourceId}`)
		),
