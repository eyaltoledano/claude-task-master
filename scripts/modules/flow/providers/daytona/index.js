...
import { DaytonaError, DaytonaConnectionError, DaytonaExecutionError } from './errors.js'
...
export class DaytonaProvider {
  ...
  async updateResource(resourceId, updates = {}) {
    try {
      ...
      if (updates.environment) {
        ...
        await this.client.updateWorkspaceEnvironment(resourceId, updates.environment)
        resource.config.env = { ...resource.config.env, ...updates.environment }
      }
  ...
  async executeAction(resourceId, action, parameters = {}) {
    try {
      ...
      const { code, language = 'bash', timeout = 60000, workingDir = '/workspace' } = parameters
