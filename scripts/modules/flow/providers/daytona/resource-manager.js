...
export class DaytonaResourceManager {
  ...
  async trackResource(resource) {
    try {
      ...
      console.warn(`Failed to track Daytona resource ${resource.id}: ${error.message}`)
      throw error
    }
  }
  ...
  async updateResource(resourceId, updates) {
    try {
      ...
      if (resource.status === 'ready' && updates.status !== 'stopped') {
        ...
        updatedResource.cost = this.calculateCost(uptimeHours, resource.profile || 'default')
      ...
      console.warn(`Failed to update Daytona resource ${resourceId}: ${error.message}`)
      throw error
    }
  }
    ...
    try {
      ...
      if (resource) {
        ...
        if (resource.status === 'ready') {
          ...
          const finalCost = this.calculateCost(finalUptime, resource.profile || 'default')
      ...
      console.warn(`Failed to untrack Daytona resource ${resourceId}: ${error.message}`)
      throw error
    }
  }
  ...
  async logExecution(resourceId, execution) {
    try {
      ...
      console.warn(`Failed to log Daytona execution for ${resourceId}: ${error.message}`)
      throw error
    }
  }
  ...
  async getResourceLogs(resourceId, options = {}) {
    try {
      ...
      console.warn(`Failed to get logs for Daytona resource ${resourceId}: ${error.message}`)
      return []
    }
  }
  ...
  async persistExecutionData(resourceId, executionData) {
    try {
      ...
      console.warn(`Failed to persist Daytona execution data: ${error.message}`)
    }
  }
  ...
  async archiveResourceData(resourceId, resourceData) {
    try {
      ...
      const archiveData = {
        ...
        finalCost: this.calculateCost(this.calculateUptime(resourceData.createdAt), resourceData.profile || 'default')
      }
  ...
  async loadPersistedData() {
    try {
      ...
      for (const file of files) {
        if (file.startsWith('resource-') && file.endsWith('.json') && !file.includes('executions-')) {
          const resourceId = file.replace('resource-', '').replace('.json', '')
          const filePath = path.join(this.dataDir, file)
          try {
            const resourceData = JSON.parse(await fs.readFile(filePath, 'utf-8'))
            this.resources.set(resourceId, resourceData)
          } catch (error) {
            console.warn(`Failed to load Daytona resource ${resourceId}: ${error.message}`)
          }
        }
      }
      ...
      for (const file of files) {
        if (file.startsWith('executions-') && file.endsWith('.json')) {
          const resourceId = file.replace('executions-', '').replace('.json', '')
          const filePath = path.join(this.dataDir, file)
          try {
            const executionData = JSON.parse(await fs.readFile(filePath, 'utf-8'))
            this.executions.set(resourceId, executionData)
          } catch (error) {
            console.warn(`Failed to load Daytona executions for ${resourceId}: ${error.message}`)
          }
        }
      }
    } catch (error) {
