...
export const availableProviders = {
	mock: {
		...
		metadata: {
			description:
				'Mock provider for testing and development with realistic behavior simulation',
			stability: 'stable',
			documentation:
				'Built-in mock provider supporting all languages for development'
		}
	},
	...
	daytona: {
		...
		type: 'daytona',
		factory: () =>
			import('./daytona/index.js').then((m) => m.DaytonaProviderFactory),
		config: {
			name: 'daytona',
			type: 'daytona',
		...
		metadata: {
			description:
				'Daytona cloud-based development environments with full Linux support',
			stability: 'stable',
	...
	modal: {
		...
		type: 'modal',
		factory: () =>
			import('./modal/index.js').then((m) => m.ModalProviderFactory),
		config: {
			name: 'modal',
			type: 'modal',
		...
		metadata: {
			description:
				'Modal serverless compute platform with GPU support and auto-scaling',
			stability: 'stable',
...
export class ProviderRegistry {
	...
	async loadProviderFactory(providerKey) {
		...
		try {
			...
			throw new Error(
				`Failed to load provider '${providerKey}': ${error.message}`
			);
		}
	}
	...
	validateProviderConfig(providerKey, config) {
		...
		if (authConfig.type === 'api_key') {
			...
			if (!apiKey) {
				errors.push(
					`Missing API key for ${providerKey}. Set ${authConfig.envKey} environment variable or provide apiKey in config.`
				);
			}
		}
