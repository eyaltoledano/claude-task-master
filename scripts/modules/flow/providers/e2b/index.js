...
export class E2BProvider {
  ...
  async getResourceStatus(resourceId) {
    try {
      ...
      let isAlive = true
      try {
        if (resource.sandbox && typeof resource.sandbox.isRunning === 'function') {
  ...
  async executeAction(resourceId, action, parameters = {}) {
    try {
      ...
      if (language === 'javascript' || language === 'node') {
        ...
        result = await resource.sandbox.commands.run(`echo '${code.replace(/'/g, "\\'")}' | ${this.getLanguageCommand(language)}`, {
          timeoutMs: timeout
        })
      }
  ...
  async getResourceLogs(resourceId, options = {}) {
    try {
      ...
      const logs = await this.resourceManager.getResourceLogs(resourceId, options)
    ...
    try {
      ...
      const streamInterval = setInterval(async () => {
        try {
          ...
          callback({ type: 'error', message: error.message, timestamp: new Date().toISOString() })
        }
      }, 1000)
