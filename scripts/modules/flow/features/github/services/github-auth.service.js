...
export class GitHubAuthService {
  constructor(options = {}) {
    ...
    if (!this.clientId || !this.clientSecret) {
      throw new Error('GitHub Client ID and Secret are required. Set GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET environment variables.');
    }
    
    this.github = new GitHub(this.clientId, this.clientSecret);
    this.tokenPath = path.join(os.homedir(), '.taskmaster', 'github-token.json');
  }
  ...
  async pollForToken(deviceCode, interval = 5) {
    ...
    return new Promise((resolve, reject) => {
      const pollInterval = setInterval(async () => {
        try {
          const tokenResponse = await this.exchangeDeviceCodeForToken(deviceCode);
          ...
          if (error.message.includes('authorization_pending')) {
            ...
            clearInterval(pollInterval);
            setTimeout(() => {
              this.pollForToken(deviceCode, interval + 5).then(resolve).catch(reject);
            }, 5000);
  ...
  async authenticate(options = {}) {
    ...
    try {
      ...
      if (openBrowser) {
        try {
          const urlToOpen = deviceFlow.verificationUriComplete || deviceFlow.verificationUri;
      ...
      // Poll for token
      const tokenResult = await this.pollForToken(deviceFlow.deviceCode, deviceFlow.interval);
