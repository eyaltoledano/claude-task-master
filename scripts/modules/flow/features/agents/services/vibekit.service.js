...
export class VibeKitService {
	...
	async generateCodeAndCreatePR(
		...
		try {
			...
			if (onProgress)
				onProgress('Creating sandbox', `Setting up ${agent} environment...`);
			vibeKit = await this.createInstanceForSubtask(agent, subtask.id);

			// Step 2: Generate code with streaming
			if (onProgress)
				onProgress('Generating code', 'AI is writing implementation...');
			...
			if (onProgress)
				onProgress('Creating PR', 'Creating pull request with labels...');
			const pr = await this.createLabeledPR(
				vibeKit,
				subtask,
				agent,
				parentTask
			...
			if (vibeKit) {
				try {
					...
					await vibeKit.kill();
					await this.configService.recordSandboxKilled(
						agent,
						subtask.id,
						duration
					);
					if (onProgress)
						onProgress('Cleanup', 'Sandbox cleaned up successfully');
				} catch (cleanupError) {
	...
	async addAgentLabel(pr, agent) {
		try {
			...
			await this.createGitHubLabel(
				githubAuthConfig.token,
				repository,
				agentLabel
			);

			// Add label to PR
			await this.addLabelToPR(
				githubAuthConfig.token,
				repository,
				pr.number,
				agentLabel.name
	...
	async executeCompleteWorkflow(
		...
		try {
			...
			if (generateCode) {
				...
				const codeResult = await this.generateCodeAndCreatePR(
					agent,
					executionTarget,
					context,
					(phase, message) => {
				...
				validation: {
					warnings:
						availableEnvs.length === 0
							? ['No sandbox environments configured']
							: []
				}
			};
		} catch (error) {
