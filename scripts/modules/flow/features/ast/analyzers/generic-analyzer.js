...
export class GenericAnalyzer {
	...
	detectFileType(filePath, content = '') {
		...
		const isTestFile =
			/test|spec|__tests__|\.test\.|\.spec\.|_test\.|_spec\./i.test(fileName);

		// Check for config file patterns
		const isConfigFile =
			/config|settings|\.env|\.config\.|package\.json|tsconfig|webpack/i.test(
				fileName
			);

		// Check for documentation
		const isDocFile = /readme|changelog|license|contributing|docs?\//i.test(
			filePath
		);

		const type = isTestFile
			? 'test'
			: isConfigFile
				? 'config'
				: isDocFile
	...
	detectFilePurpose(fileName, content) {
		...
		if (name.includes('middleware') || name.includes('interceptor'))
			return 'middleware';
		...
		if (content) {
			if (/export\s+default|module\.exports|public\s+class/.test(content))
				return 'module';
			if (
				/function\s+main|if\s+__name__\s*==\s*['""]__main__['""]/.test(content)
			)
				return 'entry_point';
			if (/import.*test|describe\s*\(|it\s*\(|def\s+test_/.test(content))
				return 'testing';
		}
	...
	async analyzeStructure(astData, content) {
		...
		if (astData.functions && astData.functions.length > 0) {
			...
			structure.complexity.distribution =
				this.getComplexityDistribution(complexities);
		}
	...
	analyzeDuplication(content) {
		...
		const duplicatedLines = Object.entries(lineCount).filter(
			([line, count]) => count > 1
		);

		if (duplicatedLines.length > 0) {
			const totalDuplication = duplicatedLines.reduce(
				(sum, [line, count]) => sum + count - 1,
				0
			);
			patterns.push({
	...
	calculateMaxNestingDepth(content) {
		...
		lines.forEach((line) => {
			...
			const opens = (
				trimmed.match(/[{([]|\bif\b|\bfor\b|\bwhile\b|\btry\b/g) || []
			).length;
	...
	analyzeGenericErrorHandling(content) {
		...
		// Try-catch patterns
		const tryCatchCount = (
			content.match(/try\s*{|except\s*:|catch\s*\(/g) || []
		).length;
		if (tryCatchCount > 0) {
			patterns.push({
		...
		const throwCount = (content.match(/throw\s+|raise\s+|panic\s*\(/g) || [])
			.length;
		if (throwCount > 0) {
			patterns.push({
				type: 'error_throwing',
	...
	analyzeLoggingPatterns(content) {
		...
		const consoleCount = (
			content.match(/console\.|print\s*\(|log\.|logger\./g) || []
		).length;
		if (consoleCount > 0) {
			patterns.push({
		...
		// Debug statements
		const debugCount = (content.match(/debug|DEBUG|console\.debug/g) || [])
			.length;
		if (debugCount > 0) {
			patterns.push({
	...
	analyzeSecurityPatterns(content) {
		...
		const secretPatterns = (
			content.match(/password|secret|key|token|auth/gi) || []
		).length;
		if (secretPatterns > 2) {
			patterns.push({
	...
	analyzeTesting(content, filePath) {
		...
		analysis.isTestFile =
			/test|spec|__tests__|\.test\.|\.spec\.|_test\.|_spec\./i.test(filePath);
		...
		const testPatterns = [
			{
				...
				name: 'describe_it_blocks'
			},
			{
				pattern: /def\s+test_|class.*Test|@test/i,
				framework: 'pytest/unittest',
				name: 'python_tests'
			},
			{
				pattern: /func\s+Test\w+|t\s*\*testing\.T/,
				framework: 'go_testing',
				name: 'go_tests'
			},
			{
				pattern: /\[Test\]|Assert\.|Should\./,
				framework: 'nunit/xunit',
				name: 'dotnet_tests'
			},
			{
				pattern: /expect\s*\(|assert\s*\(|should\s*\(/,
				framework: 'assertion_library',
				name: 'assertions'
			}
		];
	...
	analyzeConfiguration(content, filePath) {
		...
		const fileName = filePath.split('/').pop() || '';
		analysis.isConfigFile =
			/config|settings|\.env|package\.json|tsconfig|webpack|babel/i.test(
				fileName
			);
		...
		analysis.hasEnvironmentVars =
			/process\.env|os\.environ|ENV\[|getenv\(/i.test(content);
	...
	async analyzeCodeQuality(astData, content, filePath) {
		...
		const commentDensity =
			structure.lines.code > 0
				? (structure.lines.comments / structure.lines.code) * 100
				: 0;
		...
		if (commentDensity < 5) {
			...
			quality.suggestions.push(
				'Add more comments to explain complex logic and intent'
			);
		}
		...
		if (structure.complexity.maximum > 10) {
			...
			quality.suggestions.push(
				'Refactor complex functions into smaller, simpler ones'
			);
		}
		...
		if (patterns.duplication && patterns.duplication.length > 0) {
			...
			quality.suggestions.push(
				'Extract duplicated code into reusable functions or constants'
			);
		}
