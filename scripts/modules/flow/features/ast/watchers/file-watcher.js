...
export class UniversalFileWatcher extends EventEmitter {
	...
	async initialize() {
		try {
			...
			console.debug(
				`[FileWatcher] Watching extensions: ${Array.from(this.supportedExtensions).join(', ')}`
			);
			console.debug(
				`[FileWatcher] Ignore patterns: ${this.ignorePatterns.length} patterns`
			);
	...
	async startWatching() {
		...
		try {
			...
			this.watcher
				...
				.on('addDir', (dirPath) =>
					this._handleDirectoryChange(dirPath, 'addDir')
				)
				.on('unlinkDir', (dirPath) =>
					this._handleDirectoryChange(dirPath, 'unlinkDir')
				)
	...
	async _processBatchChanges() {
		...
		try {
			...
			if (cpuUsage > this.options.cpuThrottleThreshold) {
				console.warn(
					`[FileWatcher] High CPU usage (${cpuUsage}%), throttling batch processing`
				);
			...
			if (this.activeAnalysis.size >= this.options.maxConcurrentAnalysis) {
				console.debug(
					`[FileWatcher] Max concurrent analysis reached, queuing changes`
				);
			...
			console.debug(
				`[FileWatcher] Processing batch of ${changes.length} changes`
			);
			...
			console.error(
				`[FileWatcher] Error processing batch changes: ${error.message}`
			);
	...
	async _processBatch(changes) {
		...
		console.debug(
			`[FileWatcher] Batch processing delegated to change processor`
		);
	}
	...
	async _setupGitWatcher() {
		...
		if (!(await fs.pathExists(gitDir))) {
			console.debug(
				'[FileWatcher] No .git directory found, skipping git integration'
			);
			return;
		}
		...
		try {
			...
			this.gitWatcher
				...
				.on('error', (error) =>
					console.error(`[FileWatcher] Git watcher error: ${error.message}`)
				);

			console.debug('[FileWatcher] Git integration enabled');
		} catch (error) {
			console.warn(
				`[FileWatcher] Failed to setup git watcher: ${error.message}`
			);
		}
	}
	...
	async _handleGitChange(filePath) {
		try {
			...
			console.error(
				`[FileWatcher] Error handling git change: ${error.message}`
			);
		}
	}
	...
	async _buildIgnorePatterns() {
		...
		try {
			...
			console.debug(
				`[FileWatcher] Could not read .gitignore: ${error.message}`
			);
		}
