...
class WorktreeCoordinator extends EventEmitter {
	constructor(options = {}) {
		...
		console.log(
			'WorktreeCoordinator initialized for cross-worktree coordination'
		);
	}
	...
	async executeGitOperation(operation) {
		...
		console.log(
			`Executing git command: ${operation.command} in ${operation.worktreePath}`
		);
	...
	async detectConflicts(operation) {
		...
		if (conflicts.length > 0) {
			...
			console.log(
				`Detected ${conflicts.length} conflicts for operation ${operation.id}`
			);
		}
	...
	async resolveConflicts(conflicts, operation) {
		console.log(
			`Resolving ${conflicts.length} conflicts for operation ${operation.id}`
		);
	...
	async resolveConcurrentGitConflict(conflict, operation) {
		...
		console.log(
			`Waiting for conflicting git operation ${conflictingOp.id} to complete`
		);
		...
		while (
			this.activeOperations.has(conflictingOp.id) &&
		...
		if (this.activeOperations.has(conflictingOp.id)) {
			throw new Error(
				`Timeout waiting for conflicting git operation to complete`
			);
		}

		console.log(
			`Conflicting git operation ${conflictingOp.id} completed, proceeding`
		);
	}
	...
	completeOperation(operation, result) {
		...
		console.log(
			`Completed git operation ${operation.id}: ${operation.command}`
		);
	...
	async handleOperationFailure(operation, error) {
		...
		if (
			...
			console.log(
				`Retrying git operation ${operation.id} (attempt ${operation.retryCount}/${this.config.maxRetries})`
			);
			...
			console.error(
				`Git operation ${operation.id} failed permanently after ${operation.retryCount} retries`
			);
	...
	cancelWorktreeOperations(worktreePath) {
		...
		for (const opId of opsToCancel) {
			...
			operation.reject(
				new Error(`Operation cancelled due to worktree unregistration`)
			);
	...
	releaseWorktreeResources(worktreePath) {
		...
		for (const resource of context.resources) {
			...
			if (usage) {
				...
				usage.exclusiveUsers = usage.exclusiveUsers.filter(
	...
	getStatistics() {
		return {
			...
			successRate:
				this.stats.operationsCompleted > 0
					? (this.stats.operationsCompleted /
							(this.stats.operationsCompleted + this.stats.conflictsDetected)) *
						100
					: 100
		};
	}
	 ...
	 */
	getWorktreeContexts() {
		return Array.from(this.worktreeContexts.entries()).map(
			([path, context]) => ({
				path,
