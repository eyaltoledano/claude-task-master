...
class ResourceMonitor extends EventEmitter {
	...
	start() {
		...
		console.log(
			`Resource monitoring started (${this.config.monitoringInterval}ms interval)`
		);
	}
	...
	updateWatcherMetrics(watcherId, metrics) {
		...
		if (!watcher) {
			console.warn(
				`Attempt to update metrics for unknown watcher: ${watcherId}`
			);
			return;
		}
	...
	checkWatcherThresholds(watcherId, watcher) {
		...
		if (metrics.cpuPercent > 15) {
			// 15% per watcher limit
		...
		if (metrics.eventRate > 1000) {
			// 1000 events/sec per watcher
			this.emit('watcherAlert', {
				watcherId,
				type: 'eventRate',
	...
	triggerGracefulDegradation(metrics) {
		console.log(
			'🔄 Graceful degradation triggered - switching to polling mode'
		);
