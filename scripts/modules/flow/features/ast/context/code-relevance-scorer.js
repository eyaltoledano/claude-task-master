...
function scoreFileNameRelevance(filePath, tasks) {
	...
	for (const task of tasks) {
		...
		if (taskText.includes('auth') && fileName.includes('auth'))
			fileScore += 0.3;
		if (taskText.includes('api') && fileName.includes('api')) fileScore += 0.3;
		if (taskText.includes('database') && fileName.includes('db'))
			fileScore += 0.3;
		if (taskText.includes('user') && fileName.includes('user'))
			fileScore += 0.3;
		if (taskText.includes('config') && fileName.includes('config'))
			fileScore += 0.3;
		if (taskText.includes('test') && fileName.includes('test'))
			fileScore += 0.2;
...
function scoreSymbolRelevance(ast, tasks) {
	...
	for (const task of tasks) {
		...
		for (const symbol of allSymbols) {
			...
			if (taskText.includes('create') && symbolLower.includes('create'))
				symbolScore += 0.3;
			if (taskText.includes('update') && symbolLower.includes('update'))
				symbolScore += 0.3;
			if (taskText.includes('delete') && symbolLower.includes('delete'))
				symbolScore += 0.3;
			if (taskText.includes('get') && symbolLower.includes('get'))
				symbolScore += 0.2;
			if (taskText.includes('validate') && symbolLower.includes('validate'))
				symbolScore += 0.3;
			if (taskText.includes('auth') && symbolLower.includes('auth'))
				symbolScore += 0.3;
			if (taskText.includes('hash') && symbolLower.includes('hash'))
				symbolScore += 0.3;
			if (taskText.includes('encrypt') && symbolLower.includes('encrypt'))
				symbolScore += 0.3;
	...
	for (const task of tasks) {
		...
		for (const importItem of ast.imports) {
			...
			if (language === 'javascript' || language === 'typescript') {
				if (taskText.includes('react') && importName.includes('react'))
					importScore += 0.4;
				if (taskText.includes('express') && importName.includes('express'))
					importScore += 0.4;
				if (
					taskText.includes('database') &&
					(importName.includes('mongoose') ||
						importName.includes('sequelize') ||
						importName.includes('prisma'))
				)
					importScore += 0.4;
				if (
					taskText.includes('test') &&
					(importName.includes('jest') ||
						importName.includes('mocha') ||
						importName.includes('chai'))
				)
					importScore += 0.3;
				if (
					taskText.includes('auth') &&
					(importName.includes('passport') ||
						importName.includes('jwt') ||
						importName.includes('bcrypt'))
				)
					importScore += 0.4;
			} else if (language === 'python') {
				if (
					taskText.includes('web') &&
					(importName.includes('flask') ||
						importName.includes('django') ||
						importName.includes('fastapi'))
				)
					importScore += 0.4;
				if (
					taskText.includes('database') &&
					(importName.includes('sqlalchemy') ||
						importName.includes('django.db'))
				)
					importScore += 0.4;
				if (
					taskText.includes('test') &&
					(importName.includes('pytest') || importName.includes('unittest'))
				)
					importScore += 0.3;
				if (taskText.includes('request') && importName.includes('requests'))
					importScore += 0.3;
			} else if (language === 'go') {
				if (
					taskText.includes('web') &&
					(importName.includes('gin') ||
						importName.includes('echo') ||
						importName.includes('fiber'))
				)
					importScore += 0.4;
				if (
					taskText.includes('database') &&
					(importName.includes('gorm') || importName.includes('sql'))
				)
					importScore += 0.4;
				if (taskText.includes('test') && importName.includes('testing'))
					importScore += 0.3;
			}

			// Generic patterns
			if (taskText.includes('http') && importName.includes('http'))
				importScore += 0.3;
			if (taskText.includes('json') && importName.includes('json'))
				importScore += 0.2;
			if (taskText.includes('crypto') && importName.includes('crypto'))
				importScore += 0.3;
			if (taskText.includes('time') && importName.includes('time'))
				importScore += 0.2;
