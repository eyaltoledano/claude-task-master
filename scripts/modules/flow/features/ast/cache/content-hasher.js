...
export class ContentHasher {
	...
	async generateHash(filePath, content, language = null) {
		try {
			...
			this.stats.errors++;
			console.error(
				`[ContentHasher] Error generating hash for ${filePath}: ${error.message}`
			);
			return null;
		}
	}
	...
	async getFileHash(filePath) {
		try {
			...
			const entry = cacheEntries.find(([key]) =>
			...
			this.stats.errors++;
			console.error(
				`[ContentHasher] Error getting file hash for ${filePath}: ${error.message}`
			);
			return null;
		}
	}
