...
export class ASTDebugTools extends EventEmitter {
	...
	async performDebugging(error, filePath, content, language, debugId, options) {
		...
		debug.analysis.errorSpecific = await this.analyzeError(
			error,
			content,
			language
		);
	...
	async analyzeSyntax(content, language) {
		...
		analysis.stats.codeLines = lines.filter(
	...
	async analyzeError(error, content, language) {
		...
		const locationMatch = error.message.match(
			/line\s+(\d+)|position\s+(\d+)|at\s+(\d+)/i
		);
		if (locationMatch) {
		...
		switch (analysis.type) {
			case 'syntax_error':
				analysis.relatedIssues = this.findSyntaxRelatedIssues(
					content,
					language
				);
				break;
	...
	generateIssuesAndSuggestions(debug) {
		...
		debug.suggestions = this.generateSuggestions(
			debug.error.type,
			debug.issues
		);
	...
	getStatistics() {
		...
		return {
			...
			averageIssuesPerFile:
				this.session.filesAnalyzed.size > 0
					? this.session.issuesFound / this.session.filesAnalyzed.size
					: 0
		};
	}
	...
	async runDiagnostic(filePath) {
		try {
			...
			return await this.debugParsingFailure(
				simulatedError,
				filePath,
				content,
				language,
