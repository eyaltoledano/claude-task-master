...
const CodeGenerationFlow = ({ subtask, onComplete, onCancel }) => {
	...
	const initializeFlow = async () => {
		try {
			...
			setProgress({
				step: 'Checking available agents',
				details: 'Verifying API keys and configuration...'
			});
			...
			if (agents.length === 0) {
				throw new Error(
					'No agents configured with API keys. Please set ANTHROPIC_API_KEY, OPENAI_API_KEY, or GOOGLE_API_KEY in your environment.'
				);
			}
			...
			if (agents.length === 1) {
				...
				const selectedAgent =
					await vibeKitService.selectAgentForSubtask(subtask);
				setAgent(selectedAgent);
				setState('agent-selected');
				setProgress({
					step: 'Agent selected',
					details: `Using ${selectedAgent} for this task`
				});
	...
	const startCodeGeneration = async (selectedAgent) => {
		try {
			...
			setProgress({
				...
				details: 'Analyzing project structure and subtask requirements...'
			});
			...
			setProgress({
				...
				details: 'AI is analyzing requirements and writing implementation...'
			});
	...
	const renderContent = () => {
		switch (state) {
			...
			case 'complete':
				return (
					<SuccessDisplay result={result} agent={agent} subtask={subtask} />
				);
