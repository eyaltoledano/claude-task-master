  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor="red" padding={1}>
      ...
      {agent && (
        <Box marginBottom={1}>
          <Text>
            <Text bold>Agent:</Text> {agent.charAt(0).toUpperCase() + agent.slice(1)}
          </Text>
        </Box>
      )}
      ...
      <Box flexDirection="column" marginBottom={1}>
        ...
        <Text dimColor wrap="wrap">{suggestion}</Text>
      </Box>

      {errorType === 'api-key' && (
        <Box flexDirection="column" marginBottom={1}>
          <Text bold color="cyan">🔧 API Key Setup:</Text>
      ...
      {errorType === 'github' && (
        <Box flexDirection="column" marginBottom={1}>
          <Text bold color="cyan">🔧 GitHub Setup:</Text>
      ...
      <Box marginTop={1}>
        <Text dimColor>
          Fix the issue above and retry, or cancel to return to the subtask view.
        </Text>
      </Box>
    </Box>
  );
