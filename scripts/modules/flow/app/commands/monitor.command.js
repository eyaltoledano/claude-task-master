...
export async function launchMonitor(options = {}) {
	try {
		...
		const carousel = new contrib.carousel(pages, {
			screen: screen,
		...
		console.log(
			chalk.gray('   • ↑/↓ arrows or J/K: Scroll through tasks table')
		);
		console.log(
			chalk.gray('   • PgUp/PgDn: Fast scroll | Home/End: Top/Bottom')
		);
		console.log(
			chalk.gray('   • F key: Filter tasks by status (Tasks dashboard)')
		);
		...
		if (
			...
			console.log(
				chalk.yellow('💡 Tip: Try enlarging your terminal window and try again')
			);
...
function loadTaskData(projectRoot) {
	try {
		...
		const result = listTasks(tasksPath, null, null, true, 'json', null, {
			projectRoot
		});
...
function generateProjectDashboardContent(taskData) {
	...
	const taskProgressBar =
		'█'.repeat(Math.floor(taskProgress / 3)) +
		'▓'.repeat(Math.floor((100 - taskProgress) / 6));
	...
	const subtaskProgressBar =
		'█'.repeat(Math.floor(subtaskProgress / 3)) +
		'▓'.repeat(Math.floor((100 - subtaskProgress) / 6));
...
• Add tasks with 'task-master add-task'`;
	...
	const tasksWithNoDeps = taskData.tasks.filter(
		(t) =>
			t.status !== 'done' &&
			t.status !== 'completed' &&
	...
	const tasksReadyToWork = taskData.tasks.filter(
		(t) =>
			t.status !== 'done' &&
			t.status !== 'completed' &&
			(!t.dependencies ||
	...
	const tasksBlocked = taskData.tasks.filter(
		(t) =>
			t.status !== 'done' &&
			t.status !== 'completed' &&
			t.dependencies &&
...
function createTasksDashboardPage(screen, projectRoot) {
	...
	// Create filterable task table
	const taskTable = createFilterableTaskTable(
		grid,
		taskData,
		currentStatusFilter
	);
	...
	screen.key(['f', 'F'], () => {
		try {
			...
			// Update title
			title.setContent(
				`{center}📊 TASK MASTER - TASKS DASHBOARD (Filter: ${currentStatusFilter.toUpperCase()}){/center}`
			);
	...
	const refreshInterval = setInterval(() => {
		try {
			...
			helpBox.setContent(
				`{center}↑/↓/J/K: Scroll tasks | PgUp/PgDn: Fast scroll | Home/End: Top/Bottom | F: Filter | ←/→: Pages | Q/ESC: Exit | Updated: ${newTimestamp}{/center}`
			);
...
function createVibeKitMainDashboard(screen) {
	...
	const updateDashboard = (telemetryData = sampleTelemetryData) => {
		...
		try {
			responseTime.setData(
				['Avg Response Time'],
				[telemetryData.responseTimes || []]
			);
		} catch (error) {
	...
	const refreshInterval = setInterval(() => {
		...
		const dynamicData = {
			...sampleTelemetryData,
			codexRequests: sampleTelemetryData.codexRequests.map(
				(v) => v + Math.floor(Math.random() * 5) - 2
			),
			claudeRequests: sampleTelemetryData.claudeRequests.map(
				(v) => v + Math.floor(Math.random() * 3) - 1
			),
