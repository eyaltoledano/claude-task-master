...
async function checkExistingAuth(options = {}) {
	const spinner = options.verbose
		? null
		: ora('Checking existing authentication...').start();
	...
	try {
		...
		if (authStatus.authenticated) {
			log(
				`✅ Found existing authentication for user: ${authStatus.user.login}`,
				'success'
			);
			if (options.verbose) {
...
async function performGitHubAuth(options = {}) {
	...
	const spinner = options.verbose
		? null
		: ora('Initializing authentication...').start();
...
async function testAuthenticatedRequests(authResult, options = {}) {
	...
	try {
		...
		if (userResponse.ok) {
			...
			throw new Error(
				`User API call failed: ${userResponse.status} ${userResponse.statusText}`
			);
		}
		...
		if (reposResponse.ok) {
			...
			throw new Error(
				`Repositories API call failed: ${reposResponse.status} ${reposResponse.statusText}`
			);
		}
...
export async function githubStatusCommand(options = {}) {
	try {
		...
		if (authStatus) {
			...
			console.log(
				`Email: ${c.cyan}${authStatus.user.email || 'N/A'}${c.reset}`
			);
			console.log(
				`Public Repos: ${c.cyan}${authStatus.user.public_repos}${c.reset}`
			);
			...
			if (options.verbose) {
				...
				console.log(
					`Created: ${c.gray}${new Date(authStatus.user.created_at).toLocaleDateString()}${c.reset}`
				);
				console.log(
					`Token: ${c.gray}${authStatus.token.substring(0, 20)}...${c.reset}`
				);
			}
			...
			console.log(
				`${c.gray}💡 Run 'task-master flow github login' to authenticate${c.reset}`
			);
		}
		...
		if (options.json) {
			...
			console.log(
				JSON.stringify(authStatus || { authenticated: false }, null, 2)
			);
		}
	} catch (error) {
		console.error(
			`${c.red}❌ Failed to check GitHub status: ${error.message}${c.reset}`
		);
		if (options.verbose) {
			console.error(`${c.gray}${error.stack}${c.reset}`);
		}
...
export async function githubLoginCommand(options = {}) {
	try {
		...
		if (authResult && !options.force) {
			...
			console.log(
				`${c.gray}💡 Use --force to re-authenticate or 'logout' to clear existing auth${c.reset}`
			);
			return;
		}
		...
		if (options.test) {
			...
			if (!apiResult.success) {
				console.log(
					`${c.yellow}⚠️  Authentication succeeded but API tests failed${c.reset}`
				);
			}
		}
		...
		console.log(
			`   Email: ${c.cyan}${authResult.user.email || 'N/A'}${c.reset}`
		);
		...
		console.log(
			`${c.gray}💡 Token is stored securely in ~/.taskmaster/github-token.json${c.reset}`
		);
		console.log(
			`${c.gray}💡 Use 'task-master flow github status' to check current status${c.reset}`
		);
	} catch (error) {
		console.error(
			`${c.red}💥 Authentication failed: ${error.message}${c.reset}`
		);
		if (options.verbose) {
			console.error(`${c.gray}${error.stack}${c.reset}`);
		}
...
export async function githubLogoutCommand(options = {}) {
	try {
		...
		if (!authStatus) {
			console.log(
				`${c.yellow}⚠️  Not currently authenticated with GitHub${c.reset}`
			);
		...
		console.log(
			`${c.blue}📝 Currently authenticated as: ${authStatus.user.login}${c.reset}`
		);
		...
		if (!options.force && !options.yes) {
			...
			console.log(
				`${c.yellow}⚠️  Use --force or --yes to confirm logout${c.reset}`
			);
			console.log(
				`${c.gray}💡 This will remove your stored GitHub token${c.reset}`
			);
			return;
		}
		...
		if (logoutResult.success) {
			...
			console.log(
				`${c.gray}💡 Your authentication token has been removed${c.reset}`
			);
		} else {
			console.error(
				`${c.red}❌ Logout failed: ${logoutResult.error}${c.reset}`
			);
			process.exit(1);
		}
	} catch (error) {
	...
	try {
		switch (subcommand.toLowerCase()) {
			...
			default:
				...
				console.log('');
				console.log(
					`${c.gray}💡 Use --help for detailed information about each command${c.reset}`
				);
		}
	} catch (error) {
		console.error(
			`${c.red}💥 GitHub command failed: ${error.message}${c.reset}`
		);
		if (options.verbose) {
			console.error(`${c.gray}${error.stack}${c.reset}`);
		}
