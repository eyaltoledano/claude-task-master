...
import { FlowConfigManager } from '../../shared/config/managers/flow-config-manager.js';
...
export async function showConfig(options = {}) {
	try {
		...
		console.log(chalk.yellow('\n🤖 VibeKit:'));
		console.log(
			`   Enabled: ${config.vibekit.enabled ? chalk.green('Yes') : chalk.red('No')}`
		);
		console.log(
			`   Default Agent: ${chalk.green(config.vibekit.defaultAgent)}`
		);
		console.log(
			`   Streaming: ${config.vibekit.streamingEnabled ? chalk.green('Enabled') : chalk.red('Disabled')}`
		);
		console.log(
			`   GitHub Integration: ${config.vibekit.githubIntegration ? chalk.green('Enabled') : chalk.red('Disabled')}`
		);
		...
		for (const [agent, agentConfig] of Object.entries(config.vibekit.agents)) {
			const status = agentConfig.enabled
				...
				: chalk.gray('Disabled');
			console.log(
				`   ${agent}: ${status} (tokens: ${agentConfig.maxTokens}, temp: ${agentConfig.temperature})`
			);
		}
		...
		console.log(
			`   Stream Output: ${config.execution.streamOutput ? chalk.green('Yes') : chalk.red('No')}`
		);

		// GitHub Settings
		console.log(chalk.yellow('\n📋 GitHub:'));
		console.log(
			`   Auto Detect Repo: ${config.github.autoDetectRepo ? chalk.green('Yes') : chalk.red('No')}`
		);
		console.log(
			`   Default Branch: ${chalk.green(config.github.defaultBranch)}`
		);
...
export async function initConfig(options = {}) {
	try {
		...
		if (
			...
			fs.existsSync(configManager.configFile)
		) {
			console.log(
				chalk.yellow(
		...
		console.log(
			chalk.green(`✅ Configuration initialized: ${configManager.configFile}`)
		);
