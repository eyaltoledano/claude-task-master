...
import ASTConfigManager, { ConfigValidator } from '../../shared/config/ast-config.js';
...
class ConfigCommandHandler {
	constructor(options = {}) {
		...
		this.configManager = new ASTConfigManager({
	...
	async handleResetConfig(section = null) {
		try {
			...
			if (section === true || section === '') {
				...
				console.error(
					'❌ Invalid reset option. Use --config-reset or --config-reset=section'
				);
				return false;
			}
	...
	showConfigHelp() {
		...
		console.log(
			'    task-master flow --config-set="ast.fileWatching.batchDelay=300"'
		);
		console.log(
	...
	async initializeConfiguration() {
		try {
			...
			console.error(
				'❌ Failed to initialize AST configuration:',
				error.message
			);

			// Check if this is a critical error that should prevent startup
			if (
				error.message.includes('Invalid AST configuration prevents startup')
			) {
			...
			console.warn(
				'   Use: task-master flow --config-validate to check issues'
			);
...
export function addConfigOptions(program) {
	program
		.option(
			'--config-show [section]',
			'Show AST configuration (optionally for specific section)'
		)
		.option(
			'--config-set <keyvalue>',
			'Set configuration value using key=value format'
		)
		.option(
			'--config-reset [section]',
			'Reset configuration to defaults (optionally for specific section)'
		)
