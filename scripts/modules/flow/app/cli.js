...
export function registerFlowCommand(programInstance) {
	...
	programInstance
		...
		.option(
			'--dry-run',
			'Preview execution plan without running (execute only)'
		)
		.option(
			'--retry <count>',
			'Number of retries on failure (execute only)',
			'2'
		)
		.option('--output <file>', 'Save generated code to file (generate only)')
		.option(
			'--context <taskId>',
			'Use task context for enhanced generation (generate only)'
		)
		...
		.option(
			'--validate',
			'Validate configuration and environment (config only)'
		)
			...
			try {
				if (!subcommand) {
					...
					const spinner = options.verbose
						? null
						: ora('Starting VibeKit interface...').start();
					try {
				...
				switch (subcommand.toLowerCase()) {
					case 'execute': {
						...
						try {
							...
							if (spinner) {
								...
								console.log(
									chalk.blue('🔍 Validated:'),
									`Task ${validTaskId}, Agent ${validAgent}`
								);
							}
							...
							if (options.dryRun) {
								...
								console.log(
									chalk.green('✅ Dry run:'),
									`Task ${validTaskId} would execute with agent ${validAgent}`
								);
							...
							await executeTask(validTaskId, {
								...
								onProgress: options.verbose
									? (data) => {
											console.log(
												chalk.blue('📊 Progress:'),
												`${data.progress || 0}% - ${data.message || 'Processing...'}`
											);
										}
									: undefined
							});
							...
							console.log(
								chalk.green('✅ Task execution completed successfully!')
							);
						} catch (error) {
					...
					case 'generate': {
						...
						const spinner = options.verbose
							? null
							: ora('Preparing code generation...').start();
						...
						try {
							...
							if (spinner) {
								...
								console.log(
									chalk.blue('📝 Prompt:'),
									taskIdOrPrompt.substring(0, 100) +
										(taskIdOrPrompt.length > 100 ? '...' : '')
								);
							}
					...
					case 'agents': {
						if (options.check) {
							const spinner = options.verbose
								? null
								: ora('Testing agent connectivity...').start();
							if (spinner) spinner.stop();
						}
					...
					case 'batch': {
						if (!taskIdOrPrompt && !additionalArgs?.length) {
							throw {
								...
								suggestion:
									'Usage: task-master flow batch <taskId1> <taskId2> ...'
							};
						}

						const spinner = options.verbose
							? null
							: ora('Validating batch execution...').start();
					...
					case 'config': {
						if (options.show) {
							const { showConfig } = await import(
								'./commands/config.command.js'
							);
							...
							if (!path || !value) {
								throw {
									...
									suggestion:
										'Use: --set "path value" (e.g., --set "vibekit.defaultAgent claude")'
								};
							}
							...
							console.log(
								chalk.cyan('🔧 VibeKit Flow Configuration Commands:')
							);
							console.log('');
							...
							console.log(
								'  task-master flow config --set vibekit.defaultAgent claude'
							);
							console.log('');
					...
					case 'monitor': {
						const { launchMonitor } = await import(
							'./commands/monitor.command.js'
						);
					...
					case 'test-config': {
						...
						const { FlowConfigManager } = await import(
							'./src/config/managers/flow-config-manager.js'
						);
						const { loadFlowConfig } = await import(
						...
						try {
							...
							const configManager = new FlowConfigManager({
							...
							if (options.verbose) {
								...
								console.log(
									'- vibekit.defaultAgent:',
									config.vibekit?.defaultAgent
								);
								console.log(
									'- vibekit.agents:',
									Object.keys(config.vibekit?.agents || {})
								);
								console.log(
									'- vibekit.environments:',
									Object.keys(config.vibekit?.environments || {})
								);
							...
							console.log(
								`- Streaming Enabled: ${config.vibekit?.streamingEnabled}`
							);
							console.log(
								`- Telemetry Enabled: ${config.vibekit?.telemetry?.enabled}`
							);
							console.log(
								`- Session Management: ${config.vibekit?.sessionManagement?.enabled}`
							);
							console.log(
								`- Secrets Provider: ${config.vibekit?.secrets?.provider}`
							);
						} catch (error) {
							throw {
								code: 'VALIDATION_ERROR',
