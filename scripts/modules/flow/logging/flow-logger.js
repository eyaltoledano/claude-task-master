  ...
  async log(entry) {
    ...
    switch (this.format) {
      ...
      case 'detailed':
        ...
        break
      default:
        output = this.colorize ? entry.toSimpleString() : entry.toSimpleString().replace(/\u001b\[[0-9;]*m/g, '')
        break
    }
...
export function initializeLogging(config = {}) {
  ...
  flowLogger.addTransport(new ConsoleTransport({
    level,
    format,
  ...
  if (logToFile) {
    flowLogger.addTransport(new FileTransport({
      level,
  ...
  if (enableMemoryTransport) {
    flowLogger.addTransport(new MemoryTransport({
      level,
