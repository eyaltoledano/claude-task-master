...
export class WorkflowValidator {
		...
		try {
			...
			if (!['in-progress', 'done'].includes(task.status)) {
				...
				validationResults.errors.push(
					`Task status should be 'in-progress' or 'done', currently '${task.status}'`
				);
			}
			...
			if (!task.details || task.details.trim().length < 50) {
				validationResults.warnings.push(
					'Task has minimal implementation details. Consider adding more context.'
				);
			}

			// Check subtasks if they exist
			if (task.subtasks && task.subtasks.length > 0) {
				const subtaskValidation = await this._validateSubtasksForPR(
					task.subtasks
				);
			...
			const patternValidation =
				await this.validateSubtaskImplementationPattern(taskId);
	...
	async validateSubtaskImplementationPattern(subtaskId) {
		try {
			...
			if (
				details.includes('### Progress Update') ||
			...
			if (
				details.includes('Implementation') ||
			...
			if (
				details.includes('Implementation completed') ||
			...
			if (!validation.phases.exploration) {
				validation.warnings.push(
					`${subtaskId}: Missing exploration phase documentation. Consider logging initial research and planning.`
				);
			}

			if (!validation.phases.implementation && task.status === 'in-progress') {
				validation.warnings.push(
					`${subtaskId}: Missing implementation progress logs. Consider updating with findings and decisions.`
				);
			}

			if (!validation.phases.completion && task.status === 'done') {
				validation.warnings.push(
					`${subtaskId}: Missing completion summary. Consider adding final implementation notes.`
				);
			}

			// Determine if pattern is sufficiently followed
			const completedPhases = Object.values(validation.phases).filter(
				Boolean
			).length;
			if (completedPhases === 0) {
				...
				validation.errors.push(
					`${subtaskId}: No implementation pattern detected. Please follow dev_workflow.mdc guidelines.`
				);
			}
	...
	validateCommitMessageFormat(message) {
		...
		if (!basicFormatRegex.test(message)) {
			...
			validation.suggestions.push(
				'Example: feat(task-4): Complete subtask 4.1 - Initialize Express server'
			);
			return validation;
		}
		...
		if (!taskScopeRegex.test(message)) {
			validation.warnings.push(
				'Consider including task reference in scope (e.g., task-4)'
			);
		}
		...
		if (message.includes('subtask')) {
			const hasSubtaskDetails =
				message.includes('Subtask') && message.includes('Relates to Task');
			if (!hasSubtaskDetails) {
				validation.warnings.push(
					'Subtask commits should include "Subtask X.Y:" and "Relates to Task X:" sections'
				);
			}
		}
	...
	async validateWorkflowPrerequisites(workflowType, context = {}) {
		try {
			...
			switch (workflowType) {
				...
				case 'local-merge':
					return await this._validateLocalMergePrerequisites(
						context,
						validation
					);
	...
	async generateWorkflowRecommendations(taskId, currentState = {}) {
		try {
			...
			switch (task.status) {
				case 'pending':
					recommendations.immediate.push(
						'Start implementation by setting status to in-progress'
					);
				...
				case 'in-progress':
					recommendations.immediate.push(
						'Log implementation progress regularly'
					);
					recommendations.suggested.push(
						'Commit progress incrementally following dev_workflow.mdc patterns'
					);
					break;

				case 'done':
					recommendations.immediate.push(
						'Consider creating PR or merging locally'
					);
					break;
			}
			...
			if (currentState.hasUncommittedChanges) {
				recommendations.immediate.push(
					'Commit current changes before proceeding'
				);
			}
			...
			if (!patternValidation.phases.exploration) {
				recommendations.suggested.push(
					'Document exploration findings and implementation plan'
				);
			}
			if (
				!patternValidation.phases.implementation &&
				task.status === 'in-progress'
			) {
				recommendations.suggested.push(
					'Log implementation progress with findings and decisions'
				);
			}
	...
	async _validateSubtasksForPR(subtasks) {
		...
		if (completionPercentage < 80) {
			...
			validation.errors.push(
				`Only ${completionPercentage.toFixed(0)}% of subtasks completed. Consider completing more before PR.`
			);
		} else if (completionPercentage < 100) {
			validation.warnings.push(
				`${completionPercentage.toFixed(0)}% of subtasks completed. Some subtasks remain.`
			);
		}
	...
	_generatePRReadinessSuggestions(validationResults, task) {
		if (task.status === 'in-progress') {
			validationResults.suggestions.push(
				'Consider marking task as done if implementation is complete'
			);
		}
		...
		if (validationResults.warnings.length > 0) {
			validationResults.suggestions.push(
				'Address warnings to improve PR quality'
			);
		}
	}
	...
	async _validatePRPrerequisites(context, validation) {
		...
		try {
			...
			validation.isReady = false;
			validation.errors.push(
				'GitHub CLI (gh) not available. Install it to create PRs.'
			);
		...
		try {
			...
			if (!validation.prerequisites.githubRemote) {
				validation.isReady = false;
				validation.errors.push(
					'Repository is not hosted on GitHub. Use local merge instead.'
				);
			}
		} catch (error) {
			validation.isReady = false;
		...
		if (context.worktreePath) {
			try {
				...
				if (!validation.prerequisites.cleanWorkingDirectory) {
					validation.warnings.push(
						'Uncommitted changes detected. Commit before creating PR.'
					);
				}
			} catch (error) {
		...
		if (context.worktreePath) {
			try {
				...
				if (!validation.prerequisites.cleanWorkingDirectory) {
					validation.isReady = false;
					validation.errors.push(
						'Uncommitted changes detected. Commit before merging.'
					);
				}
			} catch (error) {
				validation.isReady = false;
