...
import { TaskEnhancer } from './task-enhancer.js';
import { GitContextGenerator } from './git-context-generator.js';
...
export class TaskContextGenerator {
  constructor(options = {}) {
    ...
    this.projectAnalyzer = new ProjectStructureAnalyzer(this.projectRoot, this.astMode);
    ...
    try {
      ...
      if (tasks.length > 0) {
        contextPromises.push(
          this.enhanceTasks(tasks).then(result => ({ type: 'tasks', data: result }))
        );
      }
      ...
      if (includeGitContext) {
        contextPromises.push(
          this.gitGenerator.generateGitContext().then(result => ({ type: 'git', data: result }))
        );
      }
      ...
      if (includeProjectStructure) {
        contextPromises.push(
          this.projectAnalyzer.analyzeProject().then(result => ({ type: 'project', data: result }))
        );
      }
  ...
  async enhanceTasks(tasks) {
    try {
      ...
      const taskHierarchy = this.taskEnhancer.organizeTaskHierarchy(enhancedTasks);
  ...
  extractProjectName() {
    ...
    try {
      const packagePath = require('path').join(this.projectRoot, 'package.json');
  ...
  async generateTaskSpecificContext(taskIds, options = {}) {
    try {
      ...
      if (this.astAvailable && taskKeywords.length > 0) {
        try {
          const taskSpecificProject = await this.projectAnalyzer.getTaskSpecificContext(taskKeywords);
          context.data.projectAnalysis = {
            ...context.data.projectAnalysis,
            taskSpecific: taskSpecificProject
          };
        } catch (error) {
          console.warn('Failed to get task-specific project context:', error.message);
        }
      }

      return context;
    } catch (error) {
      throw new Error(`Task-specific context generation failed: ${error.message}`);
    }
  }
