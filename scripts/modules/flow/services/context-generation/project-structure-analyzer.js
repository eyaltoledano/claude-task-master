...
export class ProjectStructureAnalyzer {
  ...
  async buildDirectoryTree(dirPath, maxDepth = 3, currentDepth = 0) {
    ...
    try {
      ...
      const filteredItems = items.filter(item => 
        ...
        !['node_modules', 'dist', 'build', 'coverage', '.git'].includes(item)
      );

      for (const item of filteredItems.slice(0, 20)) { // Limit items per directory
  ...
  generateStructureSummary(structure) {
    ...
    const traverse = (node) => {
      if (node.type === 'file') {
        ...
        if (['package.json', '.gitignore', 'tsconfig.json', 'webpack.config.js'].includes(node.name)) {
        ...
        if (['src', 'lib', 'components', 'services', 'utils', 'tests', 'docs'].includes(node.name)) {
  ...
  async analyzeDependencies() {
    ...
    const frameworkPatterns = ['react', 'vue', 'angular', 'svelte', 'next', 'nuxt', 'express', 'fastify'];
    const uiPatterns = ['styled', 'emotion', 'tailwind', 'bootstrap', 'material', 'antd', 'chakra'];
    const testPatterns = ['jest', 'mocha', 'vitest', 'cypress', 'playwright', 'testing-library'];
    const buildPatterns = ['webpack', 'vite', 'rollup', 'parcel', 'babel', 'typescript', 'esbuild'];
    ...
    allDeps.forEach(dep => {
      ...
      } else if (['lodash', 'axios', 'moment', 'date-fns', 'uuid'].includes(dep)) {
  ...
  summarizeASTContext(astContext) {
    ...
    return {
      ...
      languages: astContext.analysisResults?.reduce((langs, result) => {
        if (result.language && !langs.includes(result.language)) {
          langs.push(result.language);
        }
        return langs;
      }, []) || [],
      hasComponents: astContext.analysisResults?.some(result => 
        result.components && result.components.length > 0) || false,
      hasFunctions: astContext.analysisResults?.some(result => 
        result.functions && result.functions.length > 0) || false,
      hasImports: astContext.analysisResults?.some(result => 
        result.imports && result.imports.length > 0) || false
    };
  }
    ...
    if (projectAnalysis.astContext) {
      ...
      if (ast.available) {
        ...
        if (ast.summary) {
          ...
          if (ast.summary.hasComponents) markdown += `- Contains React/Vue components\n`;
          if (ast.summary.hasFunctions) markdown += `- Contains functions/methods\n`;
        }
        markdown += '\n';
      } else {
