...
export class ExecutionState {
	...
	forceProgress(progress, phase = null, message = null) {
		...
		if (this.streamingActive) {
			streamingService.forceProgress(
				this.executionId,
				this.progress,
				this.phase,
				message
...
export class SimpleExecutionService {
	...
	async executeTask(config, options = {}) {
		...
		try {
			...
			state.setPhase(
				EXECUTION_PHASES.ACQUIRING,
				'Acquiring execution resources'
			);
			...
			state.addLog('debug', 'Output generated', {
			...
			state.addLog(
				'info',
				`Task execution completed in ${this._calculateDuration(state)}ms`
			);
	...
	async cancelExecution(executionId, reason = 'User requested cancellation') {
		...
		if (
			...
			throw new Error(
				`Cannot cancel execution ${executionId}: already ${state.status}`
			);
		}
	...
	async *streamExecution(executionId) {
		...
		try {
			for await (const message of streamingService.streamMessages(
				executionId
			)) {
				yield message;
			}
			...
			throw new Error(
				`Streaming failed for execution ${executionId}: ${error.message}`
			);
		}
	}
