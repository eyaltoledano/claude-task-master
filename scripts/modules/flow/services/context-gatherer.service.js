...
export class ContextGathererService {
  ...
  async gatherFullContext(subtask, options = {}) {
    try {
      ...
      const claudeMdContent = await this.generateClaudeContext(subtask, options);
      ...
      throw new Error(`Failed to gather context for subtask ${subtask.id}: ${error.message}`);
    }
  }
    ...
    const getStructure = async (currentPath, depth = 0) => {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (['.js', '.jsx', '.ts', '.tsx', '.json', '.md', '.css', '.scss'].includes(ext)) {
  ...
  async findRelatedFiles(subtask, options) {
    ...
    const searchPaths = ['src', 'lib', 'components', 'pages', 'utils', 'services', 'hooks'];
    
    for (const searchPath of searchPaths) {
      const fullPath = path.join(this.projectRoot, searchPath);
      try {
        const pathFiles = await this.searchFilesInPath(fullPath, searchTerms, options);
  ...
  async analyzeDependencies(subtask) {
    ...
    const commonDeps = [
      'react', 'redux', 'axios', 'lodash', 'moment', 'dayjs',
      'express', 'mongoose', 'prisma', 'sequelize', 'typeorm',
      'jest', 'mocha', 'chai', 'cypress', 'testing-library'
    ];
  ...
  async findTestFiles(testPath) {
    ...
    for (const entry of entries) {
      if (entry.isFile()) {
        ...
        const subFiles = await this.findTestFiles(path.join(testPath, entry.name));
        files.push(...subFiles);
      }
    }
...
${Object.entries(sections.implementationGuidelines).map(([key, value]) => `- **${key}**: ${value}`).join('\n')}

## Related Files
${sections.relatedFiles.files.slice(0, 5).map(file => `- ${file.name}: ${file.preview.substring(0, 100)}...`).join('\n')}
`;
