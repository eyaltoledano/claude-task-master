...
export class SandboxCleaner {
  ...
  async start() {
    ...
    console.log(`Sandbox cleaner started. Checking every ${this.checkInterval / 1000 / 60} minutes.`);
  }
  ...
  async cleanupOldSandboxes() {
    ...
    try {
      ...
      console.log(`Found ${orphanedSandboxes.length} orphaned sandboxes to evaluate`);

      for (const sandbox of orphanedSandboxes) {
        const ageHours = (new Date() - new Date(sandbox.createdAt)) / (1000 * 60 * 60);
        ...
        if (ageHours > maxAgeHours) {
          ...
          if (killed) {
            ...
            console.log(`✅ Cleaned up orphaned sandbox: ${sandbox.sandboxId} (age: ${ageHours.toFixed(1)}h)`);
          } else {
          ...
          console.warn(`⚠️  Sandbox ${sandbox.sandboxId} will be cleaned up in ${remainingMinutes} minutes`);
        }
      }
  ...
  async cleanupUntracked() {
    try {
      ...
      const untrackedSandboxes = sandboxes.filter(sandbox => 
        !trackedSandboxes.includes(sandbox.sandboxID)
      );

      if (untrackedSandboxes.length > 0) {
        console.log(`Found ${untrackedSandboxes.length} untracked E2B sandboxes`);
        ...
        for (const sandbox of untrackedSandboxes) {
          ...
          if (createdAt < sixHoursAgo) {
            const killed = await this.killE2BSandbox(sandbox.sandboxID);
            if (killed) {
              console.log(`✅ Cleaned up untracked sandbox: ${sandbox.sandboxID}`);
            }
          }
        }
      }
  ...
  async killE2BSandbox(sandboxId) {
    try {
      ...
      if (response.ok) {
        ...
        console.error(`Failed to delete sandbox ${sandboxId}: ${response.status}`);
        return false;
      }
    } catch (error) {
  ...
  async forceCleanupAll() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        for (const [sandboxId, info] of Object.entries(config.tracking.sandboxHistory)) {
          if (!info.killedAt) {
            updates[sandboxId] = {
              ...info,
