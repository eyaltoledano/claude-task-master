...
const KEY_MAPPINGS = {
  'cmd': navigator.platform.toUpperCase().indexOf('MAC') >= 0 ? 'metaKey' : 'ctrlKey',
  'ctrl': 'ctrlKey',
  'alt': 'altKey',
  'shift': 'shiftKey',
  'meta': 'metaKey',
...
export const useKeyboardShortcuts = (config = {}) => {
  ...
  const getShortcutForAction = useCallback((action) => {
    for (const [shortcut, shortcutAction] of Object.entries(mergedShortcuts)) {
