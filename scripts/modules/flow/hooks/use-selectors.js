...
export const useCurrentScreen = () => useNavigationStore(state => state.getCurrentScreen())
...
export const useInputState = () => useUIStore(
  useShallow(state => ({
...
export const useHasTasksFile = () => useDataStore(state => state.hasTasksFile)

// Git Info Selectors - using shallow comparison
export const useGitInfo = () => useDataStore(
  useShallow(state => ({
...
export const useCurrentModel = () => useDataStore(state => state.currentModel)

export const useVersion = () => useDataStore(state => state.version)

// Preferences Selectors
export const useTheme = () => usePreferencesStore(state => state.currentTheme)

export const useEffectiveTheme = () => usePreferencesStore(state => state.getEffectiveTheme())

export const useCurrentBackend = () => usePreferencesStore(state => state.currentBackend)

export const usePerformanceSettings = () => usePreferencesStore(
  useShallow(state => ({
...
export const useUIPreferences = () => usePreferencesStore(
  useShallow(state => ({
...
export const useDataActions = () => useDataStore(
  useShallow(state => ({
...
export const useNavigationActions = () => useNavigationStore(
  useShallow(state => ({
...
export const usePreferencesActions = () => usePreferencesStore(
  useShallow(state => ({
...
export const useTaskStats = () => useDataStore(state => {
  const tasks = state.tasks;
  if (!tasks.length) return { total: 0, pending: 0, completed: 0, inProgress: 0 };
...
export const useAvailableTasks = () => useDataStore(state => 
  state.tasks.filter(task => task.status === 'pending')
)

export const useCompletedTasks = () => useDataStore(state => 
  state.tasks.filter(task => task.status === 'done' || task.status === 'completed')
)
