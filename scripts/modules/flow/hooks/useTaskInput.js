  ...
  const handleListViewInput = (input, key) => {
    if (key.downArrow) {
      ...
      if (newIndex < scrollOffset) {
        ...
        const newIndex = Math.min(selectedIndex + VISIBLE_ROWS, visibleTasks.length - 1);
        setSelectedIndex(newIndex);
      ...
      cycleTaskStatus(task);
        ...
        statusFilter === 'status' ? setStatusFilter('all') : setPriorityFilter('all');
    } else if (input === '2') {
        statusFilter === 'status' ? setStatusFilter('pending') : setPriorityFilter('high');
    } else if (input === '3') {
        statusFilter === 'status' ? setStatusFilter('in-progress') : setPriorityFilter('medium');
    } else if (input === '4') {
        statusFilter === 'status' ? setStatusFilter('done') : setPriorityFilter('low');
    } else if (input === '/') {
      setIsSearching(true);
    }
  };
