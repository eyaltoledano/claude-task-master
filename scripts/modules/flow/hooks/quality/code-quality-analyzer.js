...
export class CodeQualityAnalyzer {
	...
	async analyzeSession(session, task, worktree, services) {
		...
		try {
			...
			analysis.aggregateMetrics = this.calculateAggregateMetrics(analysis.files);
	...
	async analyzeFile(change, task) {
		...
		try {
			...
			if (this.options.enableComplexityAnalysis) {
				analysis.metrics.complexity = this.calculateComplexity(change.content, analysis.language);
			}
	...
	analyzeJavaScript(code) {
		return {
			functionCount: (code.match(/function\s+\w+|=>\s*{|async\s+function/g) || []).length,
			...
			asyncFunctionCount: (code.match(/async\s+function|\basync\s*\(/g) || []).length,
			promiseUsage: (code.match(/\.then\(|\.catch\(|await\s+/g) || []).length,
			consoleUsage: (code.match(/console\.(log|error|warn|debug)/g) || []).length,
			todoComments: (code.match(/\/\/\s*TODO|\/\*\s*TODO|\*\s*TODO/gi) || []).length
		};
	}
	...
	async analyzeTaskAlignment(changes, task) {
		...
		const codeContent = changes.map(c => c.content).join('\n').toLowerCase();
	...
	calculateAggregateMetrics(fileAnalyses) {
		...
		return {
			...
			averageCommentRatio: totals.totalComments / Math.max(totals.totalLines, 1),
