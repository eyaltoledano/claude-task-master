...
export function ExecutionManagementScreen({ onBack }) {
	...
	useInput((input, key) => {
		...
		if (key.ctrl && input === 'k' && selectedExecution) {
			// Kill execution
			telemetry.trackEvent('EXECUTION_KILL', {
		...
		if (key.ctrl && input === 'r' && selectedExecution) {
			// Restart execution
			telemetry.trackEvent('EXECUTION_RESTART', {
				executionId: selectedExecution.id
			});
			setNotification({
		...
		if (key.ctrl && input === 's' && selectedExecution) {
			...
			if (isStreaming) {
				disconnect();
				telemetry.trackEvent('STREAMING_DISCONNECT', {
					executionId: selectedExecution.id
				});
				setNotification({
					message: 'Streaming disconnected',
					type: 'warning',
					duration: 2000
				});
			} else {
				connect();
				telemetry.trackEvent('STREAMING_CONNECT', {
					executionId: selectedExecution.id
				});
				setNotification({
			...
			<BaseModal
				title={
					<Box flexDirection="row" justifyContent="space-between" width="100%">
						<Text>
							Execution Management - Traditional Mode (
							{optimizedExecutions.length})
						</Text>
						<Box flexDirection="row">
							...
							<Box marginLeft={1}>
								<Badge
									color={performanceMetrics.memoryUsage > 100 ? 'red' : 'green'}
								>
				...
				<Box flexDirection="column" padding={1}>
					...
					<Box marginBottom={2}>
						...
						{executionItems.length > 0 ? (
							<SelectInput items={executionItems} onSelect={handleSelect} />
						) : (
							<Box marginTop={1}>
						...
						<Box flexDirection="column">
							...
							<Box borderStyle="single" padding={1} marginBottom={1}>
								<Box flexDirection="column">
									...
									<Text color="gray">
										Started:{' '}
										{new Date(selectedExecution.startTime).toLocaleTimeString()}
									</Text>

									<Box marginTop={1}>
										{selectedExecution.status === 'running' &&
										(currentProgress > 0 || selectedExecution.progress) ? (
											<Box flexDirection="column">
												<Box flexDirection="row" justifyContent="space-between">
													<Text color="cyan">Progress:</Text>
													<Text color="gray">
														{isStreaming ? '🔴 Live' : '⚫ Static'} | Stream:{' '}
														{currentStatus}
													</Text>
												</Box>
												<ProgressBar
													value={currentProgress || selectedExecution.progress}
												/>
												<Text color="gray">
													{Math.round(
														(currentProgress || selectedExecution.progress) *
															100
													)}
													%
												</Text>
							...
							<Box borderStyle="single" padding={1}>
								...
								<Box marginTop={1} flexDirection="column">
									<Text color="gray">
										[Ctrl+K] Kill execution | [Ctrl+R] Restart | [Ctrl+S] Toggle
										streaming
									</Text>
									<Text color="gray">
										Streaming:{' '}
					...
					<Box marginTop={2}>
						<Text color="gray">
							[↑/↓] Navigate | [Enter] Select | [i] Interactive Mode | [h/?]
							Help | [Ctrl+C] Settings | [Esc]{' '}
							{selectedExecution ? 'Deselect' : 'Back'}
						</Text>
					</Box>
				</Box>
