...
export function ClaudeSessionActions({
  ...
  const renderSessionActions = () => {
    ...
    return (
      <Box flexDirection="column" marginY={1}>
        ...
        <Box flexDirection={isNarrow ? 'column' : 'row'} gap={isNarrow ? 0 : 2} marginTop={1}>
          <Text color={theme.text.secondary}>
            [Enter/r] Resume
          </Text>
          <Text color={theme.text.secondary}>
  ...
  const renderConfigStatus = () => {
    if (!config) {
      return (
        <Box marginY={1}>
          <Text color="red">
            ⚠️  No Claude Code configuration found
          </Text>
        </Box>
      );
    }
    ...
    if (config.allowedTools && config.allowedTools.length > 0) {
      statusItems.push(`Tools: ${config.allowedTools.slice(0, 3).join(', ')}${config.allowedTools.length > 3 ? '...' : ''}`);
    }
    ...
    return (
      <Box marginY={1}>
        <Text color={theme.text.tertiary}>
          Config: {statusItems.length > 0 ? statusItems.join(' • ') : 'Default settings'}
        </Text>
      </Box>
    );
  };
  ...
  const renderQuickMenu = () => {
    ...
    const menuItems = [
      { key: 'n', label: 'Start New Query', action: onNewQuery, disabled: isProcessing },
      { key: 'r', label: 'Resume Selected', action: () => selectedSession && onResumeSession?.(selectedSession.sessionId), disabled: !selectedSession || isProcessing },
      { key: 'v', label: 'View Messages', action: () => selectedSession && onViewSession?.(selectedSession), disabled: !selectedSession },
      { key: 'c', label: 'Update Config', action: onConfigUpdate, disabled: isProcessing },
      { key: 'q', label: 'Back to Main', action: onBack, disabled: isProcessing },
    ];

    return (
      <Box flexDirection="column" borderStyle="round" borderColor={theme.accent} padding={1} marginY={1}>
        <Text color={theme.accent} bold>
          Quick Actions Menu:
        </Text>
        {menuItems.map((item) => (
          <Box key={item.key} marginTop={1}>
            <Text color={item.disabled ? theme.text.tertiary : theme.text.secondary}>
              [{item.key}] {item.label}
