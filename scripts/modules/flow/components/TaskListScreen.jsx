...
export function TaskListScreen() {
  ...
  // Status cycle order
  const statusCycle = ['pending', 'in-progress', 'done', 'blocked', 'deferred', 'cancelled'];
  ...
  useInput((input, key) => {
    ...
    if (input === 'n' && filteredTasks[selectedIndex]) {
      ...
      backend.setTaskStatus(task.id, nextStatus)
        .then(() => reloadTasks())
  ...
  return (
    <Box flexDirection="column" flexGrow={1}>
      ...
      >
        ...
        <Text color={filter === 'in-progress' ? 'cyan' : 'gray'}>[3] In Progress </Text>
