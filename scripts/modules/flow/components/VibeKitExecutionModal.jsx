...
export function VibeKitExecutionModal({
			...
			try {
				...
				const loadConfigFile = (filename) => {
					...
					try {
						...
						throw new Error(
							`Config file ${filename} not found in either ${projectPath} or ${fallbackPath}`
						);
					} catch (err) {
	...
	useInput((input, key) => {
		...
		if (step === 'config') {
			if (key.upArrow) {
				...
				setSelectedOption(
					Math.min(configOptions.length - 1, selectedOption + 1)
				);
			} else if (key.return) {
	...
	const handleConfigSelection = () => {
		...
		switch (selected.key) {
			case 'agent': {
				...
				setConfig((prev) => ({
					...prev,
					agent: enabledAgents[nextAgentIndex]
				}));
				break;
			}
			...
			case 'sandbox': {
				...
				const nextSandboxIndex =
					(currentSandboxIndex + 1) % enabledSandboxes.length;
				setConfig((prev) => ({
					...prev,
					sandbox: enabledSandboxes[nextSandboxIndex]
				}));
				break;
			}
	...
	const createVibeKitConfig = async (agentType) => {
		...
		const baseConfig = {
			...
			environment: {
				e2b: {
					...
					templateId:
						getE2BTemplateForAgent(agentType) ||
		...
		try {
			...
			try {
				...
				if (tokenData.access_token) {
					...
					console.log(
						'GitHub configuration added using OAuth token from ~/.taskmaster/github-token.json'
					);
				}
			} catch (tokenError) {
		...
		console.log('VibeKit config created:', {
			agent: {
				type: baseConfig.agent.type,
				model: { ...baseConfig.agent.model, apiKey: '[REDACTED]' }
			},
			environment: {
				e2b: { ...baseConfig.environment.e2b, apiKey: '[REDACTED]' }
			},
			github: baseConfig.github
				? { token: '[REDACTED]', repository: baseConfig.github.repository }
				: 'NOT_CONFIGURED'
		});
	...
	const executeWithVibeKit = async () => {
		...
		console.log('Environment variables check:', {
			ANTHROPIC_API_KEY: process.env.ANTHROPIC_API_KEY
				? `[PRESENT - ${process.env.ANTHROPIC_API_KEY.substring(0, 10)}...]`
				: 'MISSING',
			OPENAI_API_KEY: process.env.OPENAI_API_KEY
				? `[PRESENT - ${process.env.OPENAI_API_KEY.substring(0, 10)}...]`
				: 'MISSING',
			E2B_API_KEY: process.env.E2B_API_KEY
				? `[PRESENT - ${process.env.E2B_API_KEY.substring(0, 10)}...]`
				: 'MISSING',
			E2B_TEMPLATE_ID: process.env.E2B_TEMPLATE_ID || 'NOT_SET',
			GITHUB_TOKEN: process.env.GITHUB_TOKEN
				? `[PRESENT - ${process.env.GITHUB_TOKEN.substring(0, 10)}...]`
				: 'MISSING'
		});
		...
		try {
			...
			console.log('Creating VibeKit instance with config:', {
				...
				environment: {
					e2b: {
						...
						apiKey: vibeKitConfig.environment.e2b.apiKey
							? '[REDACTED]'
							: 'MISSING'
					}
				},
			...
			const createComprehensivePrompt = () => {
				...
				const details = subtask ? subtask.details || '' : task?.details || '';
			...
			if (vibeKitConfig.github) {
				...
				try {
					...
					console.log(
						'Note: VibeKit SDK will automatically add /feature/ to create full branch name like:',
						`${uniqueBranchPrefix}/feature/task-name`
					);
		...
		>
			<Box justifyContent="center" marginBottom={1}>
				<Text bold color="blue">
					🤖 Agent Execution -{' '}
					{subtask ? `Subtask ${task.id}.${subtask.id}` : `Task ${task?.id}`}
				</Text>
			</Box>
			...
			{step === 'config' && (
				<>
					...
					{enabledAgents.length === 0 || enabledSandboxes.length === 0 ? (
						<Box marginBottom={1}>
							<Text color="red">
								...
								{enabledSandboxes.length === 0 &&
									'❌ No active sandboxes found in sandboxes.json'}
							</Text>
						</Box>
					) : (
						<>
					...
					<Text color="gray" marginTop={1}>
						Use ↑↓ to navigate, Enter to select/toggle, d to debug/preview
			...
			{step === 'debug' && (
				<Box flexDirection="column" height={25}>
					...
					>
						...
						<Box flexDirection="column" height={18}>
							{(() => {
								...
								const dependencies =
									subtask?.dependencies || task?.dependencies || [];
								if (dependencies.length > 0) {
									prompt += `Dependencies:\n`;
									prompt += `- This task depends on: ${dependencies.join(', ')}\n`;
									prompt += `- Ensure these dependencies are considered in the implementation\n\n`;
								...
								return lines.slice(0, 16).map((line, index) => (
									<Box
										key={`debug-line-${index}-${line.substring(0, 20).replace(/\s/g, '_')}`}
									>
										<Text color="green">
							...
							<Box marginTop={1}>
								<Text color="yellow">
									... showing first 16 lines. Full prompt will be sent to
									VibeKit.
								</Text>
							</Box>
						</Box>
					...
					<Box marginTop={1} flexDirection="column">
						...
						<Box marginLeft={2}>
							<Text>
								Task ID: {subtask ? `${task.id}.${subtask.id}` : task?.id}
							</Text>
						</Box>
						<Box marginLeft={2}>
					...
					<Box marginTop={1}>
						<Text color="gray">
							Press Enter or Esc to return to configuration
						</Text>
					</Box>
				</Box>
			)}
			...
			{step === 'executing' && (
				<Box flexDirection="column" height={25}>
					...
					<Box marginBottom={1}>
						<Text>
							Agent: {config.agent} | Sandbox: {config.sandbox} | Template:{' '}
							{config.agent}
						</Text>
					</Box>
						...
						<Box flexDirection="column" height={12}>
							{streamingMessages.length === 0 ? (
								<Box justifyContent="center" alignItems="center" height={12}>
									<Text color="gray">
										Waiting for agent to start streaming...
									</Text>
								</Box>
							) : (
								streamingMessages.slice(-15).map((messageObj, index) => {
									const preview =
										typeof messageObj.content === 'string'
											? messageObj.content.length > 90
												? messageObj.content.substring(0, 90) + '...'
												: messageObj.content
									...
									return (
										<Box
											key={`stream-${messageObj.timestamp}-${index}`}
											marginBottom={0}
										>
											<Text color="green">[{messageObj.time}]</Text>
											<Text color={messageObj.isError ? 'red' : 'white'}>
												{' '}
												{preview}
											</Text>
										</Box>
							...
							{streamingMessages.length > 15 && (
								<Box>
									<Text color="yellow">
										... showing last 15 of {streamingMessages.length} messages
									</Text>
								</Box>
							)}
						</Box>
					...
					<Box marginTop={1} flexDirection="column">
						...
						<Box marginLeft={2}>
							<Text color="white">
								b - Move to background (continue execution)
							</Text>
						</Box>
						<Box marginLeft={2}>
			...
			{step === 'results' && results && (
				<Box flexDirection="column" height={25}>
					...
					<Box marginBottom={1} flexDirection="column">
						...
						<Box marginLeft={2}>
							<Text>
								Agent: {results.agent} | Environment: {results.environment}
							</Text>
						</Box>
					</Box>
					...
					{results.success ? (
						<Box flexDirection="column">
							...
							<Box marginBottom={1} flexDirection="column">
								...
								<Box marginLeft={2} flexDirection="column">
									{results.streamingUpdates && (
										<Text>
											📡 Received {results.streamingUpdates} streaming updates
										</Text>
									)}
									{results.codeResult && (
									...
									{results.pullRequest ? (
										...
										<Text color="yellow">
											⚠️ No pull request created (GitHub not configured)
										</Text>
									)}
							...
							{results.pullRequest && (
								<Box marginBottom={1} flexDirection="column">
									...
									<Box marginLeft={2} flexDirection="column">
										<Text color="green">
											PR #{results.pullRequest.number}:{' '}
											{results.pullRequest.title || 'Code generation'}
										</Text>
										<Text>
											Branch: {results.pullRequest.head?.ref || 'unknown'}
										</Text>
										<Text>
											Commit:{' '}
											{results.pullRequest.head?.sha?.substring(0, 8) ||
												'unknown'}
										</Text>
										<Text color="cyan">
											URL: {results.pullRequest.html_url}
										</Text>
									</Box>
								</Box>
							)}
								...
								<Box marginBottom={1} flexDirection="column">
									...
									<Box marginLeft={2} flexDirection="column">
										{results.codeResult.code && (
											<Text>
												Generated {results.codeResult.code.length} characters of
												code
											</Text>
										)}
										{results.codeResult.files && (
											<Text>
												Created {results.codeResult.files.length} files
											</Text>
										)}
										<Text color="gray">
											Mode: code generation with streaming
										</Text>
									</Box>
								</Box>
							)}
						...
						<Box flexDirection="column" marginBottom={1}>
							...
							<Box marginTop={1} marginLeft={2}>
								...
								<Text color="gray">
									• Verify E2B template '{results.agent}' exists
								</Text>
								<Text color="gray">
