    ...
    >
      ...
      <Box alignItems="center" marginBottom={1}>
        ...
        <Box marginLeft={2}>
          <StatusMessage 
            variant={
              currentStatus === 'completed' || execution.status === 'completed' ? 'success' :
              currentStatus === 'failed' || execution.status === 'failed' ? 'error' :
              currentStatus === 'running' || execution.status === 'running' ? 'info' : 'default'
            }
          >
...
const DetailedProgressView = ({ executions, theme }) => {
  const runningExecutions = useMemo(() => {
    return executions.filter(e => e.status === 'running' || e.status === 'in-progress');
  }, [executions]);
    ...
    <Box flexDirection="column" height="100%">
      ...
      <Box flexGrow={1} padding={1}>
        {viewMode === 'overview' ? (
          ...
          <DetailedProgressView executions={optimizedExecutions} theme={theme} />
        )}
      </Box>
