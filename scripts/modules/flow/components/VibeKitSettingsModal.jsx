...
export function VibeKitSettingsModal({ isVisible, onClose, projectRoot }) {
		...
		if (isVisible && projectRoot) {
			try {
				...
				setConfig({
					...
					enableGitHub: !!(
						vibeKitConfig.github?.token || process.env.GITHUB_API_KEY
					),
					enableSessionManagement:
						vibeKitConfig.sessionManagement?.enabled ?? true,
	...
	const getDefaultSandbox = (vibeKitConfig) => {
		if (vibeKitConfig.environments?.e2b?.apiKey || process.env.E2B_API_KEY)
			return 'e2b';
		if (
			vibeKitConfig.environments?.northflank?.apiKey ||
			process.env.NORTHFLANK_API_KEY
		)
			return 'northflank';
		if (
			vibeKitConfig.environments?.daytona?.apiKey ||
	...
	const handleOptionSelect = () => {
		...
		switch (selected.key) {
			case 'defaultAgent': {
				...
				setConfig((prev) => ({
					...prev,
					defaultAgent: agentOptions[nextIndex]
				}));
				break;
			}
			...
			case 'defaultSandbox': {
				...
				setConfig((prev) => ({
					...prev,
					defaultSandbox: sandboxOptions[nextIndex]
				}));
				break;
			}
			...
			case 'enableSessionManagement':
				setConfig((prev) => ({
					...prev,
					enableSessionManagement: !prev.enableSessionManagement
				}));
				break;
		...
		>
			...
			{editingField && (
				...
				>
					<Box flexDirection="column">
						<Text color="yellow">
							Editing:{' '}
							{configOptions.find((opt) => opt.key === editingField)?.label}
						</Text>
			...
			{configStatus && (
				...
				>
					...
					<Text>
						Active Agent: {configStatus.agent.type}{' '}
						{configStatus.agent.configured ? '✅' : '❌'}
					</Text>
					<Text>
						Environments: {configStatus.environments.count} configured
					</Text>
					<Text>GitHub: {configStatus.github.configured ? '✅' : '❌'}</Text>
					<Text>
						Session Management:{' '}
						{configStatus.sessionManagement.enabled ? '✅' : '❌'}
					</Text>
					...
					{configStatus.validation.issues.length > 0 && (
						<Box flexDirection="column" marginTop={1}>
							...
							{configStatus.validation.issues.map((issue, idx) => (
								<Text key={`issue-${idx}-${issue.slice(0, 10)}`} color="red">
									{' '}
									• {issue}
								</Text>
							))}
					...
					{configStatus.validation.warnings.length > 0 && (
						<Box flexDirection="column" marginTop={1}>
							...
							{configStatus.validation.warnings.map((warning, idx) => (
								<Text
									key={`status-warning-${idx}-${warning.slice(0, 10)}`}
									color="yellow"
								>
									{' '}
									• {warning}
