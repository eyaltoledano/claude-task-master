...
export function SandboxControlPanel({
	...
	useInput((input, key) => {
		...
		if (key.upArrow) {
			...
			setSelectedOption(
				Math.min(controlOptions.length - 1, selectedOption + 1)
			);
		} else if (key.return) {
			handleControlAction();
		} else if (key.escape) {
		...
		switch (sandboxStatus) {
			case 'ready':
				return {
					color: 'green',
					text: '✅ Ready',
					description: 'Sandbox is configured and ready for use'
				};
			case 'active':
				return {
					color: 'green',
					text: '🟢 Active',
					description: 'Sandbox is running and available'
				};
			case 'paused':
				return {
					color: 'yellow',
					text: '⏸️  Paused',
					description: 'Sandbox is paused to save resources'
				};
			case 'terminated':
				return {
					color: 'red',
					text: '🛑 Terminated',
					description: 'Sandbox has been terminated'
				};
			case 'not-configured':
				return {
					color: 'yellow',
					text: '⚠️  Not Configured',
					description: 'Sandbox environment needs configuration'
				};
			case 'error':
				return {
					color: 'red',
					text: '❌ Error',
					description: 'Error accessing sandbox'
				};
			default:
				return {
					color: 'gray',
					text: '❓ Unknown',
					description: 'Sandbox status unknown'
				};
		}
	};
	...
	return (
		...
		>
			...
			>
				...
				<Text color={statusDisplay.color}>Sandbox: {statusDisplay.text}</Text>
				<Text color="gray">{statusDisplay.description}</Text>

				{sessionInfo && (
					<Box flexDirection="column" marginTop={1}>
						<Text>
							Session: {sessionInfo.hasActiveSession ? '✅ Active' : '❌ None'}
						</Text>
						...
						<Text>
							Management:{' '}
							{sessionInfo.sessionManagement.enabled
								? '✅ Enabled'
								: '❌ Disabled'}
						</Text>
					</Box>
			...
			{sandboxStatus === 'not-configured' && (
				...
				>
					...
					<Text color="yellow">
						• Set up sandbox environment (E2B, Northflank, or Daytona)
					</Text>
					<Text color="yellow">
						• Configure API keys in environment variables
					</Text>
					<Text color="yellow">
						• Use VibeKit Settings to configure defaults
					</Text>
				</Box>
			)}
