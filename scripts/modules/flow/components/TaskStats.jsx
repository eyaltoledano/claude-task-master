...
export function TaskStats({
			...
			(acc, task) => {
				if (task.subtasks && task.subtasks.length > 0) {
					...
					task.subtasks.forEach((subtask) => {
						acc.subtaskStatus[subtask.status] =
							(acc.subtaskStatus[subtask.status] || 0) + 1;
					});
				}
				return acc;
			},
		...
		return (
			...
			>
				...
				{isGitRepo && branchName && !isNarrow && (
					<Box>
						<Text color={hasChanges ? 'yellow' : 'green'}>🌿 {branchName}</Text>
					</Box>
				)}
			</Box>
		);
	...
	const renderTaskCounts = () => {
		if (isLoading) {
			return <Text color={theme.text.secondary}>Loading tasks...</Text>;
		}
	...
	const renderComplexityInfo = () => {
		...
		const highComplexityCount =
			complexityReport.tasks?.filter((task) => task.score >= 8).length || 0;

		if (highComplexityCount === 0) return null;

		return (
			<Box marginTop={1}>
				<Text color={theme.text.secondary}>High complexity:</Text>
				<Text color="yellow"> {highComplexityCount} tasks</Text>
				<Text color={theme.text.secondary}> (score ≥ 8)</Text>
			</Box>
		);
	};
