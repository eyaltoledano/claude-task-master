...
const LogMessage = ({ message, theme }) => {
  const getLogColor = (level) => {
    switch (level?.toLowerCase()) {
      case 'error': return theme.colors?.error || 'red';
      case 'warn': case 'warning': return theme.colors?.warning || 'yellow';
      case 'info': return theme.colors?.info || 'blue';
      case 'debug': return theme.colors?.muted || 'gray';
      case 'success': return theme.colors?.success || 'green';
      default: return theme.colors?.text || 'white';
    }
  };
...
const ExecutionDetailsPanel = ({ execution, theme }) => {
  const { messages, currentStatus, currentProgress, isStreaming } = useStreamingExecution(
    execution?.id,
    { maxMessages: 50, autoConnect: true }
  );
  ...
  return (
    <Box flexDirection="column" padding={1} height="100%">
        ...
        <Box alignItems="center">
          ...
          <StatusMessage 
            variant={
              ...
              currentStatus === 'running' ? 'info' : 'default'
            }
          >
      ...
      {/* Metadata */}
      <Box marginBottom={1} flexDirection="column">
        <Text color={theme.colors?.muted || 'gray'} bold>Metadata:</Text>
      ...
      <Box flexDirection="column" flexGrow={1}>
        ...
        >
          {messages.length === 0 ? (
            <Text color={theme.colors?.muted || 'gray'}>
              {isStreaming ? 'Waiting for log messages...' : 'No log messages available'}
            </Text>
          ) : (
            messages.slice(-10).map((message, index) => (
              <LogMessage key={message.id || `${message.timestamp}-${index}`} message={message} theme={theme} />
            ))
          )}
        </Box>
      </Box>
...
export function InteractiveExecutionList({ onBack }) {
  ...
  const { executions, loading, error, connectionStatus, refetch } = useExecutions({
    pollInterval: 2000,
    maxRetries: 3,
  ...
  return (
    <Box flexDirection="column" height="100%">
      ...
      >
        <Text color={theme.colors?.muted || 'gray'}>
          [i] Toggle Interactive Mode • [r] Refresh • [ESC] {interactiveMode ? 'Exit Interactive' : 'Back'}
        </Text>
      </Box>
    </Box>
  );
