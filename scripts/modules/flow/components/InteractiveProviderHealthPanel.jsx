...
const ProviderHealthSummary = ({ providers, theme }) => {
  ...
  const unhealthyCount = providers.filter(p => p.health && !p.health.success).length;
...
const ProviderDetailView = ({ provider, theme }) => {
  ...
  return (
    <Box flexDirection="column" padding={1} height="100%">
      ...
      {/* Health Status */}
      <Box marginBottom={1} flexDirection="column">
        <Text bold color={theme.colors?.accent || 'yellow'}>Health Status:</Text>
      ...
      {/* Capabilities */}
      <Box marginBottom={1} flexDirection="column">
        <Text bold color={theme.colors?.accent || 'yellow'}>Capabilities:</Text>
        <Box paddingLeft={1} marginTop={1} flexDirection="column">
                     {formatCapabilities(provider.capabilities).map((capability, index) => (
             <Text key={capability || `capability-${index}`} color={theme.colors?.text || 'white'}>
               • {capability}
             </Text>
           ))}
        </Box>
      ...
      {/* Configuration */}
      <Box marginBottom={1} flexDirection="column">
        <Text bold color={theme.colors?.accent || 'yellow'}>Configuration:</Text>
        <Box paddingLeft={1} marginTop={1} flexDirection="column">
          {provider.config ? (
      ...
      {provider.health && !provider.health.success && provider.health.error && (
        <Box flexDirection="column">
          <Text bold color={theme.colors?.error || 'red'}>Error Details:</Text>
          <Box 
            paddingLeft={1} 
            marginTop={1} 
            borderStyle="round" 
...
export function InteractiveProviderHealthPanel({ onBack }) {
  ...
  const { renderCount } = usePerformanceMonitor('InteractiveProviderHealthPanel');
  ...
  const providerOptions = useMemo(() => {
    return optimizedProviders.map(provider => ({
      label: `${provider.name} (${provider.key}) - ${
        provider.health?.success ? '✓ Healthy' : 
        provider.health ? '✗ Unhealthy' : '? Unknown'
      }`,
      value: provider.key,
      provider: provider
    }));
  ...
  const selectedProvider = useMemo(() => {
    return optimizedProviders.find(provider => provider.key === selectedProviderId);
  ...
  return (
    <Box flexDirection="column" height="100%">
      ...
      >
        <Text color={theme.colors?.muted || 'gray'}>
          [r] Refresh All • [a] Toggle Auto-refresh • [s] Refresh Selected • [ESC] Back
        </Text>
      </Box>
    </Box>
  );
