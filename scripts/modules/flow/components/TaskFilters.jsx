...
export function TaskFilters({
  ...
  const renderStatusFilters = () => {
    const statusOptions = [
      ...
      { key: 'in-progress', label: 'In Progress', count: taskCounts['in-progress'] || 0 },
  ...
  const renderSearchBox = () => {
    ...
    return (
      <Box>
        ...
        />
        <Text color={theme.text.secondary}> [Enter] to search, [Esc] to cancel</Text>
      </Box>
    );
  };
    ...
    <Box flexDirection="column" width={maxContentWidth} marginBottom={1}>
      ...
      <Box flexDirection={isNarrow ? 'column' : 'row'} justifyContent="space-between">
        <Box flexDirection="column">
          ...
          {filterMode === 'status' ? renderStatusFilters() : renderPriorityFilters()}
        </Box>
