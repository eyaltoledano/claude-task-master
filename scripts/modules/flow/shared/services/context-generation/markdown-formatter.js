...
export class MarkdownFormatter {
  ...
  generateHeader(projectInfo) {
    ...
    const description = projectInfo.description || 'Generated context for development assistance';
  ...
  formatExecutionContext(executionOptions) {
    ...
    if (executionOptions.requirements && executionOptions.requirements.length > 0) {
    ...
    if (executionOptions.constraints && executionOptions.constraints.length > 0) {
    ...
    if (gitContext.currentStatus && !gitContext.currentStatus.error) {
      ...
      if (status.hasUncommittedChanges) {
        ...
        if (summary.modified.length > 0) {
          ...
          if (summary.modified.length > 10) markdown += ` (+${summary.modified.length - 10} more)`;
          markdown += '\n';
        }
        
        if (summary.added.length > 0) {
          markdown += `- Added: ${summary.added.slice(0, 10).join(', ')}`;
          if (summary.added.length > 10) markdown += ` (+${summary.added.length - 10} more)`;
          markdown += '\n';
        }
        
        if (summary.untracked.length > 0) {
          markdown += `- Untracked: ${summary.untracked.slice(0, 10).join(', ')}`;
          if (summary.untracked.length > 10) markdown += ` (+${summary.untracked.length - 10} more)`;
          markdown += '\n';
        }
  ...
  formatProjectStructure(projectAnalysis) {
    ...
    if (projectAnalysis.structure && !projectAnalysis.structure.error) {
      ...
      if (fileTypes.length > 0) {
        markdown += `**File Types:** `;
        markdown += fileTypes.map(([ext, count]) => `${ext}(${count})`).join(', ');
        markdown += '\n';
      }
