...
export class TaskEnhancer {
	...
	extractTaskKeywords(task) {
		...
		const filePattern =
			/([a-zA-Z0-9\-_/]+\.(js|jsx|ts|tsx|json|md|css|scss))/gi;
		const componentPattern =
			/\b([A-Z][a-zA-Z0-9]*(?:Component|Service|Manager|Handler)?)\b/g;
		const actionPattern =
			/\b(implement|create|add|update|modify|fix|refactor|test)\s+([a-zA-Z\s]+)/gi;

		const files = [...text.matchAll(filePattern)].map((match) => match[1]);
		const components = [...text.matchAll(componentPattern)].map(
			(match) => match[1]
		);
