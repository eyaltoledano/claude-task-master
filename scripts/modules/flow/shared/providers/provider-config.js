...
export class ProviderConfigManager {
	...
	async loadConfig() {
		try {
			...
			console.warn(
				'⚠️ Invalid provider configuration, using defaults:',
				error.message
			);
			this.currentConfig = ProviderConfigSchema.parse({});
			await this.saveConfig();
	...
	getEnvironmentConfig() {
		...
		return (
			...
			this.currentConfig.environments.development
		);
	}
	...
	getProviderConfig(providerName = 'vibekit') {
		return (
			...
			this.currentConfig.providers.vibekit
		);
	}
	...
	getAgentConfig(providerName = 'vibekit', agentName = 'claude') {
		...
		return (
			providerConfig?.agentConfigs?.[agentName] ||
			providerConfig.agentConfigs.claude
		);
	}
	...
	async updateProviderConfig(providerName, updates) {
		...
		const providerSchema =
			ProviderConfigSchema.shape.providers.shape[providerName];
	...
	async updateAgentConfig(providerName, agentName, updates) {
		...
		// Validate the updated configuration
		const agentSchema =
			ProviderConfigSchema.shape.providers.shape[providerName].shape
				.agentConfigs.shape[agentName];
		const validatedConfig = agentSchema.parse(updatedConfig);

		if (!this.currentConfig.providers[providerName]) {
			this.currentConfig.providers[providerName] =
				ProviderConfigSchema.shape.providers.shape[providerName].parse({});
		}
		...
		this.currentConfig.providers[providerName].agentConfigs[agentName] =
			validatedConfig;
		await this.saveConfig();
