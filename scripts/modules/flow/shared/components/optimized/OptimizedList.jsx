...
export const OptimizedList = memo(({
  ...
  const { visibleItems, totalHeight, handleScroll, scrollTop } = useVirtualScroll(
    items,
    maxHeight,
    itemHeight
  );
      ...
      <Box {...containerStyle}>
        <Box {...contentStyle}>
          {visibleItems.map((item) => (
            <Box
              key={keyExtractor(item, item.index)}
              {...item.style}
            >
  ...
  return (
    <Box flexDirection="column">
      ...
      {renderedCount < items.length && (
        <Text color="gray">Loading more... ({renderedCount}/{items.length})</Text>
      )}
    </Box>
  );
});
