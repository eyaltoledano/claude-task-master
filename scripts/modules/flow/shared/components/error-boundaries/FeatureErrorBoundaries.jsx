...
export const TaskOperationErrorBoundary = ({ children, operation }) => (
	<ErrorBoundaryWithLogger
		...
		fallback={(props) => (
			<TaskOperationErrorFallback {...props} operation={operation} />
		)}
		allowReset
	>
		{children}
...
export const MCPErrorBoundary = ({ children, serverName }) => (
	<ErrorBoundaryWithLogger
		...
		fallback={(props) => (
			<MCPErrorFallback {...props} serverName={serverName} />
		)}
		allowReset
	>
		{children}
...
export const FileOperationErrorBoundary = ({ children, operation }) => (
	<ErrorBoundaryWithLogger
		...
		fallback={(props) => (
			<FileOperationErrorFallback {...props} operation={operation} />
		)}
		allowReset
	>
		{children}
...
const FileOperationErrorFallback = ({ error, operation, onReset }) => (
	<Box flexDirection="column" padding={1}>
		...
		{error?.code === 'EACCES' && <Text color="gray">Permission denied</Text>}
