...
export function useAgentSession(backend, taskId = null, initialAgent = 'claude') {
  ...
  useEffect(() => {
    const loadSessions = async () => {
      setLoading(true);
      try {
        const availableSessions = await serviceRef.current.getSessions(initialAgent); // Assuming service extension for getSessions
  ...
  const createNewSession = async () => {
    setLoading(true);
    try {
      const newSession = await serviceRef.current.createSession(initialAgent, taskId);
