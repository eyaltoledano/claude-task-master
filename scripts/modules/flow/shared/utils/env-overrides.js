...
export function applyEnvironmentOverrides(configData) {
	...
	if (env.VIBEKIT_STREAMING_ENABLED !== undefined) {
		configData.vibekit.streamingEnabled =
			env.VIBEKIT_STREAMING_ENABLED === 'true';
	}

	if (env.VIBEKIT_GITHUB_INTEGRATION !== undefined) {
		configData.vibekit.githubIntegration =
			env.VIBEKIT_GITHUB_INTEGRATION === 'true';
	}
	...
	if (env.VIBEKIT_TELEMETRY_ENABLED !== undefined) {
		configData.vibekit.telemetry.enabled =
			env.VIBEKIT_TELEMETRY_ENABLED === 'true';
	}
	...
	if (env.VIBEKIT_TELEMETRY_SAMPLING_RATE) {
		configData.vibekit.telemetry.samplingRate = parseFloat(
			env.VIBEKIT_TELEMETRY_SAMPLING_RATE
		);
	}

	// Session management overrides
	if (!configData.vibekit.sessionManagement)
		configData.vibekit.sessionManagement = {};

	if (env.VIBEKIT_SESSION_ENABLED !== undefined) {
		configData.vibekit.sessionManagement.enabled =
			env.VIBEKIT_SESSION_ENABLED === 'true';
	}

	if (env.VIBEKIT_SESSION_PERSIST !== undefined) {
		configData.vibekit.sessionManagement.persistSessions =
			env.VIBEKIT_SESSION_PERSIST === 'true';
	}
	...
	if (env.VIBEKIT_ASK_MODE_ENABLED !== undefined) {
		configData.vibekit.askMode.enabled =
			env.VIBEKIT_ASK_MODE_ENABLED === 'true';
	}
	...
	if (env.VIBEKIT_STREAMING_BUFFER_SIZE) {
		configData.vibekit.streaming.bufferSize = parseInt(
			env.VIBEKIT_STREAMING_BUFFER_SIZE,
			10
		);
	}

	if (env.VIBEKIT_STREAMING_COMPRESSION !== undefined) {
		configData.vibekit.streaming.compression =
			env.VIBEKIT_STREAMING_COMPRESSION === 'true';
	}
	...
	// E2B overrides
	if (!configData.vibekit.environments.e2b)
		configData.vibekit.environments.e2b = {};
	if (env.E2B_API_KEY) {
	...
	if (!configData.vibekit.environments.northflank)
		configData.vibekit.environments.northflank = {};
	...
	if (env.NORTHFLANK_PROJECT_ID) {
		configData.vibekit.environments.northflank.projectId =
			env.NORTHFLANK_PROJECT_ID;
	}
	...
	// Daytona overrides
	if (!configData.vibekit.environments.daytona)
		configData.vibekit.environments.daytona = {};
	...
	if (env.DAYTONA_WORKSPACE_ID) {
		configData.vibekit.environments.daytona.workspaceId =
			env.DAYTONA_WORKSPACE_ID;
	}
	...
	if (!configData.vibekit.agents.opencode)
		configData.vibekit.agents.opencode = {};
