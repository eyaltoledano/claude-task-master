...
export class PerformanceMonitor {
	...
	endTimer(operation) {
		...
		if (metric && metric.startTime) {
			...
			metric.avgDuration =
				((metric.avgDuration || 0) * (metric.count - 1) + duration) /
				metric.count;
...
export class RenderTracker {
	...
	trackRender(componentName, props) {
		...
		if (current.count > 50) {
			console.warn(
				`[Performance] ${componentName} has rendered ${current.count} times`
			);
		}
	}
...
export class MemoryMonitor {
	...
	snapshot(label) {
		...
		if (delta.heapUsed > 50 * 1024 * 1024) {
			// 50MB
			console.warn(
				`[Memory] Heap usage increased by ${(delta.heapUsed / 1024 / 1024).toFixed(2)}MB since baseline`
			);
		}
