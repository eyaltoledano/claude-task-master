...
export class FlowConfigManager {
	...
	getDefaultConfig() {
		try {
			...
			console.warn(
				'Failed to load Flow config, using fallback:',
				error.message
			);
			return {
				nodeEnv: 'development',
		...
		return {
			...
			vibekit: {
				...
				sessionManagement: {
					...
					sessionDir:
						flowConfig.session?.sessionDir || '.taskmaster/flow/sessions',
	...
	validateEnvironment() {
		...
		if (this.config?.vibekit?.agents) {
			for (const [agent, agentConfig] of Object.entries(
				this.config.vibekit.agents
			)) {
				if (agentConfig.enabled) {
					const requiredKey = agentKeyMap[agent];
					if (requiredKey && !process.env[requiredKey]) {
						issues.push(
							`Missing API key: ${requiredKey} (required for ${agent} agent)`
						);
					}
				}
			}
		...
		if (this.config?.vibekit?.environments) {
			...
			if (
				this.config.vibekit.environments.e2b?.enabled &&
			...
			if (this.config.vibekit.environments.northflank?.enabled) {
				if (!process.env.NORTHFLANK_API_KEY) {
					warnings.push(
						'Missing NORTHFLANK_API_KEY (required for Northflank environment)'
					);
				}
				if (!process.env.NORTHFLANK_PROJECT_ID) {
					warnings.push(
						'Missing NORTHFLANK_PROJECT_ID (recommended for Northflank)'
					);
				}
			}
			...
			if (this.config.vibekit.environments.daytona?.enabled) {
				if (!process.env.DAYTONA_API_KEY) {
					warnings.push(
						'Missing DAYTONA_API_KEY (required for Daytona environment)'
					);
				}
				if (!process.env.DAYTONA_WORKSPACE_ID) {
					warnings.push(
						'Missing DAYTONA_WORKSPACE_ID (recommended for Daytona)'
					);
				}
			}
		}
		...
		if (
			this.config?.vibekit?.githubIntegration ||
			this.config?.github?.enabled
		) {
			if (!process.env.GITHUB_API_KEY) {
				warnings.push(
					'Missing GITHUB_API_KEY (required for GitHub integration and PR automation)'
				);
			}
		}
		...
		if (this.config?.vibekit?.telemetry?.enabled) {
			if (
				!this.config.vibekit.telemetry.endpoint &&
				!process.env.VIBEKIT_TELEMETRY_ENDPOINT
			) {
				warnings.push(
					'Missing telemetry endpoint (VIBEKIT_TELEMETRY_ENDPOINT)'
				);
			}
			if (
				!this.config.vibekit.telemetry.apiKey &&
				!process.env.VIBEKIT_TELEMETRY_API_KEY
			) {
		...
		if (this.config?.vibekit?.secrets?.enabled) {
			if (
				this.config.vibekit.secrets.provider === 'vault' &&
				!this.config.vibekit.secrets.vaultUrl
			) {
				warnings.push('Missing vault URL for secrets management');
			}
			if (
				this.config.vibekit.secrets.provider === 'aws-secrets' &&
				!this.config.vibekit.secrets.awsRegion
			) {
		...
		if (this.config?.vibekit?.enabled) {
			...
			if (!hasEnvironment) {
				warnings.push(
					'No sandbox environment enabled (E2B, Northflank, or Daytona required for VibeKit)'
				);
			}

			// At least one agent should be available
			const hasAgent = Object.values(this.config.vibekit.agents || {}).some(
				(agent) => agent.enabled
			);

			if (!hasAgent) {
				warnings.push(
					'No AI agent enabled (at least one agent required for VibeKit)'
				);
			}
		}
...
export const flowConfig = {
	...
	async initialize(projectRoot) {
		this._manager = new FlowConfigManager({
...
export class FlowConfig {
	...
	async updateVibeKitConfig(newConfig) {
		...
		this.manager.setValue('vibekit', {
			...this.getVibeKitConfig(),
			...newConfig
		});
