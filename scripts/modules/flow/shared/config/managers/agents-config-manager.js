...
export class AgentsConfigManager {
	...
	async getGitHubConfig() {
		...
		try {
			const { autoLoadGitHubToken } = await import(
				'../commands/github-auth-demo.js'
			);
	...
	async recordSandboxKilled(agent, subtaskId, duration) {
		return this.updateConfig((config) => {
			...
			if (sandbox) {
				...
				const currentAvg =
					config.tracking.statistics.averageSandboxDuration || 0;
				config.tracking.statistics.averageSandboxDuration = Math.round(
	...
	async getPRsForTask(taskId) {
		const config = await this.loadConfig();
		return Object.values(config.tracking.pullRequests).filter(
			(pr) => pr.taskId === taskId
		);
	}
	...
	async selectBestAgent(subtask) {
		...
		const preferredAgents =
			config.preferences.taskTypes[taskType]?.preferredAgents || [];
	...
	async getOrphanedSandboxes() {
		...
		return Object.values(config.tracking.sandboxHistory).filter((sandbox) => {
