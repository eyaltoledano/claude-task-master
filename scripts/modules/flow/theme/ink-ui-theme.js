  ...
  return {
    components: {
      ...
      StatusMessage: {
        styles: {
          icon: ({ variant }) => ({
            color: variant === 'success' ? currentTheme.success || 'green' : 
                  variant === 'error' ? currentTheme.error || 'red' :
                  variant === 'warning' ? currentTheme.warning || 'yellow' : 
      ...
      Select: {
        styles: {
          item: ({ isSelected }) => ({
            color: isSelected ? currentTheme.accent || 'cyan' : currentTheme.text || 'white',
            backgroundColor: isSelected ? currentTheme.highlight || 'gray' : 'transparent'
          }),
          indicator: () => ({
