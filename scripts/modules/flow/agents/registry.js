...
export class AgentRegistry extends EventEmitter {
  constructor(config = {}) {
    ...
    this.config = {
      ...
      defaultStrategy: config.defaultStrategy || SELECTION_STRATEGIES.BEST_QUALITY,
      fallbackChain: config.fallbackChain || ['claude', 'gpt', 'mock'],
      qualityThreshold: config.qualityThreshold || 0.7,
      stateFile: config.stateFile || path.join(process.cwd(), '.taskmaster', 'agent-registry-state.json'),
      ...config
    }
  ...
  async initialize() {
    try {
      ...
      throw new AgentError(`Failed to initialize agent registry: ${error.message}`)
    }
  }
  ...
  async getAgent(providerType, config = {}) {
    try {
      ...
      throw new AgentError(`Failed to get agent ${providerType}: ${error.message}`)
    }
  }
  ...
  async selectAgent(task, options = {}) {
    const requiredCapabilities = task.capabilities || [AGENT_CAPABILITIES.CODE_GENERATION]
    const strategy = options.strategy || this.config.defaultStrategy
    const excludeProviders = options.exclude || []
    
    try {
      // Get available agents with required capabilities
      const candidates = await this._getCandidateAgents(requiredCapabilities, excludeProviders)
      ...
      const selectedAgent = this._applySelectionStrategy(candidates, strategy, task, options)
  ...
  updateQualityMetrics(agentType, taskResult) {
    ...
    current.averageResponseTime = 
      (current.averageResponseTime * (current.totalTasks - 1) + responseTime) / current.totalTasks
  ...
  _saveState() {
    try {
      ...
      this.emit('warning', { message: `Failed to save state: ${error.message}` })
    }
  }
  ...
  _loadState() {
    try {
      ...
      this.emit('warning', { message: `Failed to load state: ${error.message}` })
    }
  }
  ...
  async _ensureAgent(providerType, config = {}) {
    ...
    if (provider.class) {
      ...
      try {
        ...
        throw new AgentError(`Failed to load agent module ${provider.module}: ${error.message}`)
      }
    } else {
      throw new AgentError(`Invalid provider configuration for ${providerType}`)
    }
    ...
    if (this._persistedAgentMetrics && this._persistedAgentMetrics[providerType]) {
      agent.metrics = { ...agent.metrics, ...this._persistedAgentMetrics[providerType] }
    }
  ...
  _selectFastest(candidates) {
    return candidates.reduce((best, current) => 
      current.provider.averageLatency < best.provider.averageLatency ? current : best
    ).agent
  }

  _selectCheapest(candidates) {
    return candidates.reduce((best, current) => 
      current.provider.costPerToken < best.provider.costPerToken ? current : best
    ).agent
  }

  _selectBestQuality(candidates) {
    return candidates.reduce((best, current) => {
      const currentQuality = this.qualityMetrics.get(current.providerType)?.score || current.provider.qualityScore
      const bestQuality = this.qualityMetrics.get(best.providerType)?.score || best.provider.qualityScore
    ...
    setInterval(async () => {
      try {
        ...
        this.emit('health_check_completed', {
          timestamp: new Date().toISOString(),
          healthyCount: Array.from(this.healthStatus.values()).filter(h => h.healthy).length
        })
      } catch (error) {
