...
export class MockAgent extends AIAgent {
	...
	async _doGenerateCode(task, context, options) {
		...
		const mockExplanation = this._generateMockExplanation(
			task,
			mockCode,
			language
		);
	...
	async *_doStreamResponse(task, options) {
		const steps = [
			...
			{
				type: 'progress',
				content: 'Implementing functionality...',
				progress: 40
			},
			{
				type: 'code',
			...
			{
				type: 'result',
				content: 'Mock task execution completed successfully',
				progress: 100
			}
		];
	...
	_generateMockCode(task, context, language) {
		const templates =
			this.responses.codeTemplates[language] ||
			this.responses.codeTemplates.javascript;
