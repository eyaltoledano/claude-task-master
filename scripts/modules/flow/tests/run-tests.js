...
class FlowTestRunner {
    ...
    parseCategory(args) {
        ...
        const validCategories = ['all', 'jest', 'unit', 'integration', 'e2e', 'visual', 'component', 'watch'];
    ...
    async runNonJestTests(specificCategory = null) {
        const categories = specificCategory ? [specificCategory] : ['e2e', 'visual'];
    ...
    async runComponentTests(args) {
        const componentName = args.find(arg => !arg.startsWith('-') && arg !== 'component');
        ...
        const jestArgs = [
            'jest',
            '--config', path.join(this.baseDir, 'jest.config.js'),
            '--testNamePattern', componentName,
            '--passWithNoTests'
        ];
