...
describe('Flow Production Validation E2E Tests', () => {
	...
	describe('Full Workflow Testing', () => {
		...
		test('should execute complete task workflow', async () => {
			...
			const workflowEvents = taskWorkflowTest.events.filter(
				(e) => e.type.startsWith('task_') || e.type.startsWith('workflow_')
			);
	...
	describe('Error Recovery Workflow Validation', () => {
		test('should recover from component crashes', async () => {
			...
			await testEnvironment.simulateComponentCrash(
				crashRecoveryTest.process,
				'TaskPanel'
			);

			// Verify recovery process
			const recoveryEvents = crashRecoveryTest.events.filter(
				(e) => e.type.includes('error') || e.type.includes('recovery')
			);
		...
		test('should recover from Flow connection failures', async () => {
			...
			const connectionEvents = connectionFailureTest.events.filter(
				(e) => e.type.includes('flow') || e.type.includes('connection')
			);
		...
		test('should handle multiple simultaneous errors', async () => {
			...
			const errorSimulations = [
				testEnvironment.simulateComponentCrash(
					multiErrorTest.process,
					'StatusBar'
				),
				testEnvironment.simulateMemoryPressure(multiErrorTest.process, {
					targetUsage: 140 * 1024 * 1024
				}),
				testEnvironment.simulateFlowFailure(multiErrorTest.process, {
					type: 'timeout'
				})
			];
			...
			const finalHealth = await testEnvironment.checkApplicationHealth(
				multiErrorTest.process
			);
	...
	describe('Performance Under Load Testing', () => {
		...
		test('should handle concurrent user sessions', async () => {
			...
			const totalMemoryUsage = sessionResults.reduce(
				(sum, r) => sum + r.peakMemoryUsage,
				0
			);
	...
	describe('Timeout and Shutdown Validation', () => {
		...
		test('should perform clean shutdown on SIGTERM', async () => {
			...
			await testEnvironment.waitForEvent(
				shutdownTest.process,
				'tui_ready',
				10000
			);
		...
		test('should handle graceful shutdown during operations', async () => {
			...
			await testEnvironment.waitForProcessExit(
				operationShutdownTest.process,
				15000
			);
		...
		test('should handle emergency shutdown on resource exhaustion', async () => {
			...
			await testEnvironment.simulateResourceExhaustion(
				emergencyShutdownTest.process,
				{
					type: 'memory',
	...
	describe('VibeKit Integration Validation', () => {
		test('should maintain VibeKit connection throughout session', async () => {
			...
			const connectionLostEvents = vibeKitEvents.filter(
				(e) => e.type === 'vibekit_connection_lost'
			);
...
class E2EPerformanceMonitor {
	...
	generateReport() {
		return {
			...
			summary: {
				avgTestDuration:
					this.metrics.testDurations.reduce((a, b) => a + b, 0) /
						this.metrics.testDurations.length || 0,
				peakMemoryUsage: Math.max(...this.metrics.memoryUsage) || 0,
				avgResponseTime:
					this.metrics.responsesTimes.reduce((a, b) => a + b, 0) /
						this.metrics.responsesTimes.length || 0
			}
		};
	}
}
