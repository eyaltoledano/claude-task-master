...
class FilesystemTester {
    ...
    async setupTestEnvironment() {
        ...
        try {
            ...
            await fs.mkdir(path.join(this.testProjectRoot, 'basic'), { recursive: true });
            await fs.mkdir(path.join(this.testProjectRoot, 'special'), { recursive: true });
            await fs.mkdir(path.join(this.testProjectRoot, 'permissions'), { recursive: true });
            await fs.mkdir(path.join(this.testProjectRoot, 'large'), { recursive: true });
            await fs.mkdir(path.join(this.testProjectRoot, 'concurrent'), { recursive: true });
...
export default function test() {
    ...
    async testBasicFileOperations() {
        ...
        try {
            ...
            try {
                const testFile = path.join(this.testProjectRoot, 'basic', 'write-test.txt');
            ...
            // Test file appending
            try {
                const testFile = path.join(this.testProjectRoot, 'basic', 'append-test.txt');
                
                await fs.writeFile(testFile, 'Initial content');
                await fs.appendFile(testFile, '\nAppended content');
                
                const content = await fs.readFile(testFile, 'utf8');
                const hasAppendedContent = content.includes('Initial content') && 
            ...
            // Test file copying
            try {
                const sourceFile = path.join(this.testProjectRoot, 'basic', 'simple.txt');
                const targetFile = path.join(this.testProjectRoot, 'basic', 'copy-test.txt');
            ...
            try {
                const originalFile = path.join(this.testProjectRoot, 'basic', 'rename-source.txt');
                const renamedFile = path.join(this.testProjectRoot, 'basic', 'rename-target.txt');
                ...
                const renamedExists = await fs.access(renamedFile).then(() => true).catch(() => false);
            ...
            // Test file deletion
            try {
                const testFile = path.join(this.testProjectRoot, 'basic', 'delete-test.txt');
            ...
            try {
                ...
                const hasValidStats = stats.isFile() && 
                                     stats.size > 0 && 
    ...
    async testDirectoryOperations() {
        ...
        try {
            ...
            try {
                const nestedDir = path.join(this.testProjectRoot, 'nested', 'deep', 'directory');
            ...
            try {
                const files = await fs.readdir(path.join(this.testProjectRoot, 'basic'));
            ...
            try {
                const entries = await fs.readdir(path.join(this.testProjectRoot, 'basic'), { withFileTypes: true });
                const hasDetailedInfo = entries.some(entry => entry.isFile() || entry.isDirectory());
            ...
            try {
                ...
                const dirExists = await fs.access(emptyDir).then(() => true).catch(() => false);
            ...
            try {
                ...
                await fs.writeFile(path.join(dirToRemove, 'sub', 'subfile.txt'), 'subfile content');
                
                await fs.rm(dirToRemove, { recursive: true });
                
                const dirExists = await fs.access(dirToRemove).then(() => true).catch(() => false);
    ...
    async testSpecialCharacterHandling() {
        ...
        try {
            ...
            for (const file of specialFiles) {
                try {
                    const filepath = path.join(this.testProjectRoot, 'special', file.name);
            ...
            try {
                const lowerFile = path.join(this.testProjectRoot, 'special', 'case-test.txt');
                const upperFile = path.join(this.testProjectRoot, 'special', 'CASE-TEST.TXT');
                ...
                try {
                    ...
                    const files = await fs.readdir(path.join(this.testProjectRoot, 'special'));
                    caseSensitive = files.includes('case-test.txt') && files.includes('CASE-TEST.TXT');
                } catch {
    ...
    async testPathLengthLimits() {
        ...
        try {
            ...
            try {
                const moderatePath = path.join(this.testProjectRoot, 'path-length-test', 'moderate-length-directory-name-that-is-longer-than-usual');
                await fs.mkdir(moderatePath, { recursive: true });
                
                const testFile = path.join(moderatePath, 'test-file-with-moderate-length-name.txt');
            ...
            try {
                const maxSafeLength = Math.floor(this.filesystemInfo.maxPathLength * 0.8); // Use 80% of max
                ...
                while (longPath.length + segmentLength + 20 < maxSafeLength) { // Leave room for filename
                    const segment = 'a'.repeat(Math.min(segmentLength, maxSafeLength - longPath.length - 20));
    ...
    async testFilePermissions() {
        ...
        try {
            ...
            try {
                const testFile = path.join(this.testProjectRoot, 'permissions', 'permission-test.txt');
            ...
            if (this.filesystemInfo.platform !== 'win32') {
                try {
                    const testFile = path.join(this.testProjectRoot, 'permissions', 'chmod-test.txt');
            ...
            try {
                const testDir = path.join(this.testProjectRoot, 'permissions', 'permission-dir');
                await fs.mkdir(testDir);
        ...
        try {
            ...
            if (this.filesystemInfo.platform !== 'win32') {
                ...
                try {
                    ...
                    const symlinkFile = path.join(this.testProjectRoot, 'symlink-file.txt');
                ...
                try {
                    const targetDir = path.join(this.testProjectRoot, 'symlink-target-dir');
                    const symlinkDir = path.join(this.testProjectRoot, 'symlink-dir');
                    
                    await fs.mkdir(targetDir);
                    await fs.writeFile(path.join(targetDir, 'file-in-dir.txt'), 'File in symlinked directory');
                    ...
                    const fileContent = await fs.readFile(path.join(symlinkDir, 'file-in-dir.txt'), 'utf8');
                ...
                try {
                    ...
                    const symlinkFile = path.join(this.testProjectRoot, 'link-to-resolve.txt');
    ...
    async testFileWatching() {
        ...
        try {
            ...
            try {
                ...
                await this.delay(100);
                await fs.writeFile(path.join(watchDir, 'new-file.txt'), 'New file content');
    ...
    async testLargeFileOperations() {
        ...
        try {
            ...
            try {
                const largeFile = path.join(this.testProjectRoot, 'large', 'large-file.txt');
            ...
            try {
                const streamFile = path.join(this.testProjectRoot, 'large', 'stream-file.txt');
    ...
    async testConcurrentAccess() {
        ...
        try {
            ...
            try {
                const concurrentFile = path.join(this.testProjectRoot, 'concurrent', 'concurrent-writes.txt');
            ...
            try {
                const readFile = path.join(this.testProjectRoot, 'concurrent', 'concurrent-reads.txt');
                await fs.writeFile(readFile, 'Content for concurrent reading\n'.repeat(100));
    ...
    async testFilesystemMetadata() {
        ...
        try {
            ...
            try {
                const timestampFile = path.join(this.testProjectRoot, 'timestamp-test.txt');
            ...
            try {
                ...
                const expectedSize = Buffer.byteLength(initialContent + additionalContent, 'utf8');
    ...
    printResults() {
        ...
        if (!overallSuccess) {
            console.log(`⚠️ Some filesystem compatibility issues detected. Review failed tests above.`);
        }
    }
}
