...
class CrossPlatformCompatibilityTester {
	constructor() {
		...
		this.testProjectRoot = path.join(
			__dirname,
			'../fixtures/cross-platform-test'
		);
		this.platformInfo = {
	...
	async setupTestEnvironment() {
		...
		try {
			...
			await fs.mkdir(path.join(this.testProjectRoot, 'windows-test'), {
				recursive: true
			});
			await fs.mkdir(path.join(this.testProjectRoot, 'unix-test'), {
				recursive: true
			});
			await fs.mkdir(path.join(this.testProjectRoot, 'mixed-case'), {
				recursive: true
			});
	...
	async createPlatformTestFiles() {
		const testFiles = {
			...
			'mixed-case/unicode-文件.txt':
				'Unicode filename test (Chinese characters)',
...
echo "Platform: $(uname -s)"`
		...
		try {
			...
			for (const testPath of testPaths) {
				try {
					...
					console.warn(
						`Path handling issue with "${testPath}": ${error.message}`
					);
				}
			}
		...
		try {
			...
			for (const [key, value] of Object.entries(testEnvVars)) {
				try {
					...
					console.warn(
						`Environment variable error for ${key}: ${error.message}`
					);
				}
			}

			const platformEnvTests =
				this.platformInfo.platform === 'win32'
			...
			const success =
				successCount === Object.keys(testEnvVars).length &&
				platformEnvCount > 0;
	...
	async testProcessSpawning() {
		...
		try {
			...
			if (this.platformInfo.platform === 'win32') {
				try {
					const cmdResult = await this.spawnProcess('cmd', [
						'/c',
						'echo',
						'Windows CMD test'
					]);
				...
				try {
					const bashResult = await this.spawnProcess('bash', [
						'-c',
						'echo "Unix Bash test"'
					]);
					processes.push({
	...
	async testFilePermissions() {
		...
		try {
			...
			try {
				...
				permissionTests.push({
					name: 'File Read',
					success: content.includes('Permission test')
				});
			} catch (error) {
				permissionTests.push({
					name: 'File Read',
			...
			try {
				...
				permissionTests.push({
					name: 'File Write',
					success: content.includes('Appended content')
				});
			} catch (error) {
				permissionTests.push({
					name: 'File Write',
	...
	async testCharacterEncoding() {
		...
		try {
			...
			const testStrings = [
				...
				{
					name: 'UTF-8 Basic',
					content: 'UTF-8 with accents: café, naïve, résumé'
				},
				{
					name: 'UTF-8 Extended',
					content: 'Extended UTF-8: 文件测试, العربية, русский'
				},
			...
			for (const testString of testStrings) {
				try {
					const testFile = path.join(
						...
						`encoding-${testString.name.toLowerCase().replace(/\s+/g, '-')}.txt`
					);
	...
	async testCaseSensitivity() {
		...
		try {
			...
			const testFiles = ['case-test.txt', 'CASE-TEST.TXT', 'Case-Test.txt'];
			...
			try {
				const files = await fs.readdir(
					path.join(this.testProjectRoot, 'case-test')
				);
			...
			const successfulTests = caseSensitivityTests.filter(
				(t) => t.success
			).length;
	...
	async testPlatformDetection() {
		...
		try {
			...
			const validNodeDetection =
				nodeDetection.platform && nodeDetection.arch && nodeDetection.version;
			detectionTests.push({
			...
			const validOsDetection =
				osDetection.platform && osDetection.arch && osDetection.type;
			detectionTests.push({
			...
			const adaptationTests = {
				scriptExtension:
					this.platformInfo.platform === 'win32' ? '.bat' : '.sh',
			...
			const validAdaptation =
				adaptationTests.scriptExtension &&
	...
	printResults() {
		...
		if (!overallSuccess) {
			console.log(
				`⚠️ Some cross-platform compatibility issues detected. Review failed tests above.`
			);
		}
	}
}
