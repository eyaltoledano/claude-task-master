...
describe('Worktree-AST Integration Suite', () => {
	...
	describe('Worktree Discovery and AST Processing', () => {
		test('should discover worktrees and initialize AST processing', async () => {
			...
			expect(mockWorktreeManager.discoverWorktrees).toHaveBeenCalledWith(
				'/project'
			);
		...
		test('should process only active worktree by default', async () => {
			...
			expect(mockASTProcessor.processDirectory).toHaveBeenCalledWith(
				'/project'
			);
		...
		test('should discover nested worktrees correctly', async () => {
			...
			mockWorktreeManager.discoverWorktrees.mockResolvedValueOnce(
				nestedWorktrees
			);
	...
	describe('File Watching and Real-time AST Updates', () => {
		test('should setup file watching for worktree directories', async () => {
			...
			expect(
				integrationManager.state.watchedDirectories.has(worktreePath)
			).toBe(true);
		});
		...
		test('should ignore changes in excluded directories', async () => {
			...
			expect(mockASTProcessor.processFile).not.toHaveBeenCalledWith(
				excludedFile
			);
		});
	});
	...
	describe('Resource Management and Performance', () => {
		...
		test('should enforce memory limits during AST processing', async () => {
			...
			const result = await processWorktreeWithLimits('/project', {
				memoryLimit
			});
		...
		test('should clean up resources on worktree removal', async () => {
			...
			expect(mockResourceMonitor.releaseWorktree).toHaveBeenCalledWith(
				worktreePath
			);
	...
	describe('Git Integration and Branch Management', () => {
		test('should coordinate AST processing with git branch changes', async () => {
			...
			expect(mockGitOperations.checkoutBranch).toHaveBeenCalledWith(
				targetBranch
			);
			expect(mockASTProcessor.processFile).toHaveBeenCalledWith(
				'/project/src/new-component.js'
			);
		});
		...
		test('should optimize AST processing based on git diff', async () => {
			...
			expect(mockASTProcessor.processFile).toHaveBeenCalledTimes(
				changedFiles.length
			);
		});
	...
	describe('Multi-Worktree Coordination', () => {
		...
		test('should balance resource allocation across worktrees', async () => {
			...
			expect(allocation.perWorktree.memory).toBe(
				totalMemoryLimit / worktrees.length
			);
		...
		test('should handle cross-worktree file dependencies', async () => {
			...
			const result = await handleCrossWorktreeDependency(
				mainFile,
				dependentFile
			);
	...
	async function discoverAndInitializeWorktrees(projectPath) {
		try {
			const worktrees =
				await integrationManager.worktreeComponents.manager.discoverWorktrees(
					projectPath
				);

			for (const worktree of worktrees) {
				await integrationManager.astComponents.processor.processDirectory(
					worktree.path
				);
	...
	async function switchWorktreeWithAST(fromBranch, toBranch) {
		try {
			...
			// Switch worktree
			await integrationManager.worktreeComponents.manager.switchWorktree(
				toBranch
			);
	...
	async function setupWorktreeFileWatching(worktreePath, config = {}) {
		try {
			await integrationManager.worktreeComponents.fileWatcher.watch(
				worktreePath,
				{
					extensions: ['.js', '.ts', '.py'],
					ignore: ['node_modules', '.git'],
			...
			setTimeout(async () => {
				try {
					if (eventType === 'modify') {
						...
						await integrationManager.astComponents.processor.processFile(
							filePath
						);
					}
	...
	async function trackWorktreeResources(worktreePath) {
		...
		await integrationManager.worktreeComponents.resourceMonitor.trackWorktree(
			worktreePath,
			{
				limits,
				monitoring: true
	...
	async function processWorktreeWithLimits(worktreePath, limits) {
		const usage =
			integrationManager.worktreeComponents.resourceMonitor.getResourceUsage();
	...
	async function removeWorktreeWithCleanup(worktreePath) {
		...
		await integrationManager.worktreeComponents.fileWatcher.unwatch(
			worktreePath
		);

		// Release resources
		await integrationManager.worktreeComponents.resourceMonitor.releaseWorktree(
			worktreePath
		);
		...
		// Remove worktree
		await integrationManager.worktreeComponents.manager.removeWorktree(
			worktreePath
		);
	...
	async function getWorktreePerformanceMetrics(worktreePath) {
		const resourceUsage =
			integrationManager.worktreeComponents.resourceMonitor.getResourceUsage();
		const astStats =
			integrationManager.astComponents.processor.getProcessingStats();
		...
		return {
			...
			fileWatchingStats: {
				watchedFiles:
					integrationManager.worktreeComponents.fileWatcher.getWatchedFiles()
						.size,
				isWatching:
					integrationManager.worktreeComponents.fileWatcher.isWatching()
			}
		};
	}
	...
	async function handleBranchChangeWithAST(targetBranch) {
		const currentBranch =
			await integrationManager.worktreeComponents.gitOps.getCurrentBranch();

		await integrationManager.worktreeComponents.gitOps.checkoutBranch(
			targetBranch
		);

		const changedFiles =
			await integrationManager.worktreeComponents.gitOps.getChangedFiles();
