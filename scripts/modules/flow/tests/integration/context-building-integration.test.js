...
describe('Context Building Integration Suite', () => {
	...
	beforeEach(() => {
		...
		mockRelevanceCalculator.filterByThreshold.mockImplementation(
			(files) => files
		);
	...
	describe('End-to-End Context Building Pipeline', () => {
		test('should build complete context from file list to Claude format', async () => {
			...
			expect(mockContextBuilder.buildFileContext).toHaveBeenCalledTimes(
				files.length
			);
		...
		test('should handle large project context building efficiently', async () => {
			const largeFileSet = Array.from(
				{ length: 100 },
				(_, i) => `/project/src/component${i}.js`
			);
		...
		test('should prioritize files based on relevance scoring', async () => {
			...
			files.forEach((file) => {
				mockRelevanceCalculator.calculateFileRelevance.mockReturnValueOnce(
					file.expectedRelevance
				);
			});
		...
		test('should handle token limit optimization', async () => {
			...
			const largeFiles = Array.from(
				{ length: 20 },
				(_, i) => `/project/src/large${i}.js`
			);

			// Mock token estimation
			mockContextFormatter.estimateTokenCount.mockReturnValue(200); // 200 tokens per file
	...
	describe('Relevance Calculation and Ranking', () => {
		test('should calculate file relevance based on multiple factors', async () => {
			...
			).toHaveBeenCalledWith(file, task, expect.objectContaining(factors));
		});

		test('should rank files by composite relevance score', async () => {
			const files = [
				{
					path: '/project/src/auth/login.js',
					factors: { path: 0.9, content: 0.8 }
				},
				{
					path: '/project/src/utils/helpers.js',
					factors: { path: 0.2, content: 0.4 }
				},
				{
					path: '/project/src/auth/validation.js',
					factors: { path: 0.9, content: 0.7 }
				}
			];
			...
			const ranked = await rankFilesByRelevance(
				files,
		...
		test('should adjust relevance weights based on task type', async () => {
			const taskTypes = [
				{
					type: 'bug-fix',
					weights: { recentActivity: 1.5, pathRelevance: 1.2 }
				},
				{
					type: 'new-feature',
					weights: { dependencies: 1.3, contentRelevance: 1.4 }
				},
			...
			for (const taskType of taskTypes) {
				...
				expect(
					mockRelevanceCalculator.adjustRelevanceWeights
	...
	describe('Claude Format Optimization', () => {
		...
		test('should estimate and optimize token usage', async () => {
			...
			expect(optimized.optimizedContext.files.length).toBeLessThan(
	...
	describe('Caching and Performance Optimization', () => {
		...
		test('should invalidate cache when files change', async () => {
			...
			affectedContexts.forEach((context) => {
				expect(mockCacheManager.invalidateContext).toHaveBeenCalledWith(
					context
				);
			});
		});
		...
		test('should optimize context building for repeated patterns', async () => {
			...
			for (const task of tasks) {
				const files = [...commonFiles, `/project/src/specific-${task}.js`];
			...
			expect(results.every((r) => r.processingStats.commonFilesOptimized)).toBe(
				true
			);
		});
	...
	describe('Error Handling and Recovery', () => {
		...
		test('should handle memory pressure during large context building', async () => {
			const largeFileSet = Array.from(
				{ length: 1000 },
				(_, i) => `/project/large/file${i}.js`
			);

			// Simulate memory pressure
			const memoryLimit = 100 * 1024 * 1024; // 100MB

			const result = await buildContextWithMemoryManagement(
				largeFileSet,
				memoryLimit
			);
	...
	describe('Project Structure Integration', () => {
		test('should analyze project structure for context enhancement', async () => {
			...
			mockProjectAnalyzer.analyzeProjectStructure.mockResolvedValueOnce(
				expectedStructure
			);

			const result = await buildContextWithProjectAnalysis(projectPath, [
				'src/component.js'
			]);
		...
		test('should identify and include relevant entry points', async () => {
			...
			mockProjectAnalyzer.identifyEntryPoints.mockResolvedValueOnce(
				entryPoints
			);

			const result = await buildContextWithEntryPoints([
				'/project/src/component.js'
			]);

			expect(result.includedEntryPoints).toEqual(entryPoints);
			expect(result.files.some((f) => f.filePath.includes('index.js'))).toBe(
				true
			);
		});
	...
	async function buildCompleteContext(files, taskDescription) {
		...
		const projectContext =
			await contextPipeline.projectAnalyzer.analyzeProjectStructure();

		// Step 2: Build file contexts
		const fileContexts = [];
		for (const file of files) {
			const astResult = await contextPipeline.astAnalyzer.analyzeFile(file);
			const fileContext = await contextPipeline.contextBuilder.buildFileContext(
				file,
				astResult
			);
			fileContext.relevanceScore =
				contextPipeline.relevanceCalculator.calculateFileRelevance(
					file,
					taskDescription
				);
		...
		const rankedFiles =
			contextPipeline.relevanceCalculator.rankFiles(fileContexts);
		const relevantFiles = contextPipeline.relevanceCalculator.filterByThreshold(
			rankedFiles,
			contextPipeline.state.relevanceThreshold
		);

		// Step 4: Build combined context
		const combinedContext =
			await contextPipeline.contextBuilder.buildProjectContext({
				files: relevantFiles,
				projectContext,
				taskDescription
			});
		...
		const claudeFormat =
			await contextPipeline.formatter.formatForClaude(combinedContext);
	...
	async function buildContextWithTokenLimit(files, tokenLimit) {
		...
		for (const file of files) {
			...
			if (estimatedTokens + fileTokens <= tokenLimit) {
				const context =
					await contextPipeline.contextBuilder.buildFileContext(file);
	...
	async function buildMergedContext(contextSources, taskDescription) {
		...
		const mergedContext =
			await contextPipeline.contextBuilder.mergeContexts(allContexts);

		return {
			sources: allContexts,
			totalFiles: allContexts.reduce(
				(sum, source) => sum + source.contexts.length,
				0
			),
			mergedContext
		};
	...
	async function calculateFileRelevance(file, task, factors) {
		return contextPipeline.relevanceCalculator.calculateFileRelevance(
			file,
			task,
			factors
		);
	...
	async function filterFilesByRelevance(files, threshold) {
		...
		return contextPipeline.relevanceCalculator.filterByThreshold(
			fileList,
			threshold
		);
	}
	...
	async function adjustRelevanceWeights(taskType, weights) {
		return contextPipeline.relevanceCalculator.adjustRelevanceWeights(
			taskType,
			weights
		);
	}
	...
	async function buildContextWithDynamicRelevance(files) {
		...
		for (const file of files) {
			const initialRelevance =
				contextPipeline.relevanceCalculator.calculateFileRelevance(file);
	...
	async function buildContextWithMemoryManagement(files, memoryLimit) {
		...
		for (const batch of batches) {
			const batchResult = await buildCompleteContext(
				batch,
				'memory-managed task'
			);
	...
	async function formatContextWithRecovery(context) {
		try {
			const formatted =
				await contextPipeline.formatter.formatForClaude(context);
			return {
				success: true,
				formattingFailed: false,
				claudeFormat: formatted
			};
		} catch (error) {
			// Fallback formatting
	...
	async function buildContextWithEntryPoints(files) {
		const entryPoints =
			await contextPipeline.projectAnalyzer.identifyEntryPoints();
	...
	async function buildContextWithDependencies(files) {
		const dependencyMap =
			await contextPipeline.projectAnalyzer.mapDependencies();
