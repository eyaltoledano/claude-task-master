...
describe('Cache Invalidation Integration Suite', () => {
  ...
  beforeEach(() => {
    ...
    mockASTProcessor.getCacheKey.mockImplementation((filePath, options = {}) => 
  ...
  describe('File-Based Cache Invalidation', () => {
    test('should invalidate AST cache when file is modified', async () => {
      ...
      expect(mockCacheInvalidator.invalidateFile).toHaveBeenCalledWith(filePath);
    ...
    test('should handle file creation with dependency tracking', async () => {
      ...
      expect(mockDependencyTracker.trackDependency).toHaveBeenCalledWith(newFile);
  ...
  describe('Directory-Based Cache Invalidation', () => {
    test('should invalidate cache for entire directory', async () => {
      ...
      expect(mockCacheInvalidator.invalidateDirectory).toHaveBeenCalledWith(directoryPath);
    ...
    test('should handle selective directory invalidation', async () => {
      ...
      expect(mockCacheManager.delete).toHaveBeenCalledTimes(matchingFiles.length);
    ...
    test('should handle nested directory invalidation', async () => {
      ...
      expect(mockCacheManager.delete).toHaveBeenCalledTimes(nestedStructure.length);
  ...
  describe('Dependency-Based Cache Invalidation', () => {
    test('should invalidate dependent files when dependency changes', async () => {
      ...
      expect(mockDependencyTracker.getDependents).toHaveBeenCalledWith(changedFile);
      expect(mockCacheInvalidator.invalidateFile).toHaveBeenCalledWith(changedFile);
    ...
    test('should build and use dependency graph for efficient invalidation', async () => {
      const dependencyGraph = {
        '/project/src/base.js': ['/project/src/child1.js', '/project/src/child2.js'],
        '/project/src/child1.js': ['/project/src/grandchild.js'],
        '/project/src/child2.js': [],
        '/project/src/grandchild.js': []
      };
      
      mockDependencyTracker.buildDependencyGraph.mockReturnValueOnce(dependencyGraph);
      
      const result = await invalidateWithDependencyGraph('/project/src/base.js');
    ...
    test('should optimize invalidation for large dependency trees', async () => {
      ...
      mockDependencyTracker.buildDependencyGraph.mockReturnValueOnce(largeDependencyTree);
  ...
  describe('Claude Service Cache Integration', () => {
    ...
    test('should coordinate AST and Claude cache invalidation', async () => {
      ...
      relatedContexts.forEach(context => {
        expect(mockClaudeService.invalidateContextCache).toHaveBeenCalledWith(context);
      });
    });
  ...
  describe('Performance and Load Testing', () => {
    test('should handle high-frequency invalidation requests', async () => {
      ...
      const files = Array.from({ length: invalidationCount }, (_, i) => 
      ...
      expect(mockCacheInvalidator.invalidateFile).toHaveBeenCalledTimes(invalidationCount);
    });

    test('should optimize memory usage during large cache operations', async () => {
      const largeFileSet = Array.from({ length: 5000 }, (_, i) => 
        `/project/src/large/file${i}.js`
      );
  ...
  describe('Error Handling and Recovery', () => {
    test('should handle cache corruption during invalidation', async () => {
      ...
      const result = await invalidateFileCache('/project/src/corrupted.js', 'modify');
  ...
  async function invalidateFileCache(filePath, changeType) {
    try {
      ...
      if (changeType === 'create' || changeType === 'modify') {
        const dependents = await cacheManager.dependencyTracker.getDependents(filePath);
  ...
  async function invalidateDirectoryCacheWithPattern(directoryPath, pattern) {
    ...
    const matchingKeys = allKeys.filter(key => {
      ...
      return filePath && filePath.includes(directoryPath) && regex.test(filePath);
    });
  ...
  async function invalidateDependencyChain(filePath) {
    ...
    while (queue.length > 0) {
      ...
      const dependents = await cacheManager.dependencyTracker.getDependents(currentFile);
      queue.push(...dependents);
    }
    
    return {
      invalidatedFiles: Array.from(invalidatedFiles),
      circularDependencyDetected: invalidatedFiles.size < queue.length + invalidatedFiles.size
    };
  }
  ...
  async function invalidateClaudeContextCache(contextFiles, taskId) {
    ...
    const cachedResponse = await cacheManager.claudeService.getCachedResponse(contextKey);
  ...
  async function invalidateWithFallback(filePath) {
    try {
      ...
      try {
        ...
        return { success: false, primaryFailed: true, fallbackUsed: true, error: fallbackError.message };
      }
    }
  }
