...
describe('Complete Workflow Integration Tests', () => {
  ...
  beforeAll(async () => {
    testTempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'workflow-integration-'));
  });
  ...
  beforeEach(() => {
    ...
    mockClaudeService.processRequest.mockResolvedValue(createMockClaudeResponse());
  ...
  describe('Basic Workflow Execution', () => {
    test('should execute complete task-to-PR workflow successfully', async () => {
      ...
      const workflowInit = await mockWorkflowManager.initializeWorkflow(workflowConfig);
      expect(workflowInit.state).toBe('initialized');
      expect(mockWorkflowManager.initializeWorkflow).toHaveBeenCalledWith(workflowConfig);

      // Step 2: Build AST context
      const astContext = await mockASTContext.buildContext(workflowConfig.projectPath, mockTaskData);
      ...
      const gitBranch = await mockGitService.createBranch(workflowConfig.branchName);
      ...
      const workflowComplete = await mockWorkflowManager.completeWorkflow(workflowConfig.workflowId, {
        pr: prResult,
        metrics: claudeResponse.qualityMetrics,
    ...
    test('should handle workflow with complex task requirements', async () => {
      ...
      const complexResponse = {
        ...
        implementation: {
          ...
          testFiles: complexTask.files.filter(f => f.includes('test')).map(file => ({
            file,
      ...
      const astContext = await mockASTContext.buildContext(workflowConfig.projectPath, complexTask);
      const sessionId = await mockClaudeService.createSession({ context: astContext, task: complexTask });
      const claudeResponse = await mockClaudeService.processRequest({ sessionId, task: complexTask, context: astContext });
    ...
    test('should manage workflow state and checkpoints', async () => {
      ...
      await mockWorkflowManager.initializeWorkflow(workflowConfig);
      await mockWorkflowManager.saveCheckpoint('ast-context-built', { files: 15 });
      
      let state = await mockWorkflowManager.getWorkflowState(workflowConfig.workflowId);
      expect(state.stage).toBe('ast-context-built');
      expect(state.progress).toBe(20);

      await mockWorkflowManager.saveCheckpoint('claude-processing', { sessionId: 'claude-session-123' });
      state = await mockWorkflowManager.getWorkflowState(workflowConfig.workflowId);
      expect(state.stage).toBe('claude-processing');
      expect(state.progress).toBe(40);

      await mockWorkflowManager.saveCheckpoint('hooks-executed', { hooksRun: 5 });
      state = await mockWorkflowManager.getWorkflowState(workflowConfig.workflowId);
      expect(state.stage).toBe('hooks-executed');
      expect(state.progress).toBe(60);

      await mockWorkflowManager.saveCheckpoint('pr-created', { prId: 'pr-456' });
      state = await mockWorkflowManager.getWorkflowState(workflowConfig.workflowId);
  ...
  describe('Advanced Workflow Scenarios', () => {
    test('should handle workflow with multiple file modifications', async () => {
      ...
      const multiFileResponse = {
        ...
        implementation: {
          codeChanges: multiFileTask.files.map((file, index) => ({
            file,
            type: file.includes('docs') ? 'documentation' : 
                  file.includes('test') ? 'test' :
                  file.includes('config') ? 'modify' : 'create',
      ...
      expect(claudeResponse.implementation.codeChanges).toHaveLength(9);
      expect(claudeResponse.implementation.dependencies.added).toContain('jsonwebtoken');
      expect(claudeResponse.implementation.dependencies.added).toContain('bcrypt');
    ...
    test('should handle workflow with external API integrations', async () => {
      ...
      const hookResults = await mockHookExecutor.executeHooks('api-validation', {
    ...
    test('should integrate with quality analysis and automated reviews', async () => {
      ...
      const qualityResponse = {
        ...
        qualityAnalysis: {
          strengths: ['High test coverage', 'Good separation of concerns', 'Secure implementation'],
          improvements: ['Consider caching for performance', 'Add input validation edge cases'],
          riskFactors: ['Database query complexity', 'Third-party API dependency']
        }
      };
      ...
      expect(claudeResponse.qualityMetrics.testCoverage).toBeGreaterThanOrEqual(95);
      expect(claudeResponse.qualityMetrics.codeComplexity).toBeLessThanOrEqual(7);
      expect(claudeResponse.qualityMetrics.securityScore).toBeGreaterThanOrEqual(9);
  ...
  describe('Performance and Scalability', () => {
    test('should complete workflow within performance targets', async () => {
      ...
      await mockASTContext.buildContext(performanceConfig.projectPath, mockTaskData);
      ...
      expect(timings.totalWorkflow).toBeLessThan(performanceConfig.performanceTargets.totalWorkflow);
      expect(timings.astContext).toBeLessThan(performanceConfig.performanceTargets.astContext);
      expect(timings.claudeProcessing).toBeLessThan(performanceConfig.performanceTargets.claudeProcessing);
      expect(timings.hookExecution).toBeLessThan(performanceConfig.performanceTargets.hookExecution);
      expect(timings.prCreation).toBeLessThan(performanceConfig.performanceTargets.prCreation);
    ...
    test('should handle large codebase workflow efficiently', async () => {
      ...
      const astContext = await mockASTContext.buildContext(workflowConfig.projectPath, largCodebaseTask);
      await mockClaudeService.createSession({ context: astContext, task: largCodebaseTask });
      const claudeResponse = await mockClaudeService.processRequest({
        sessionId: 'claude-session-123',
      ...
      console.log(`Large codebase workflow: ${duration}ms, Memory increase: ${Math.round(memoryIncrease / 1024 / 1024)}MB`);
    });
    ...
    test('should efficiently handle workflow with extensive testing requirements', async () => {
      ...
      expect(claudeResponse.implementation.testMetrics.unitTests).toBeGreaterThan(40);
      expect(claudeResponse.implementation.testMetrics.integrationTests).toBeGreaterThan(30);
      expect(claudeResponse.qualityMetrics.testCoverage).toBeGreaterThanOrEqual(98);

      // Verify all test types are covered
      const testFiles = claudeResponse.implementation.codeChanges.filter(change => 
        change.type === 'test'
      );
      expect(testFiles).toHaveLength(7); // All test files

      console.log('Test Implementation Metrics:', claudeResponse.implementation.testMetrics);
    });
  });
  ...
  describe('Data Persistence and Recovery', () => {
    test('should persist workflow state across interruptions', async () => {
      ...
      await mockWorkflowManager.saveCheckpoint('ast-built', { files: 15, complexity: 7.5 });

      // Simulate interruption and recovery
      const restoredState = await mockWorkflowManager.restoreCheckpoint(persistentWorkflow.workflowId);
      expect(restoredState.stage).toBe('ast-built');
      expect(restoredState.data.files).toBe(15);

      // Continue from checkpoint
      await mockWorkflowManager.saveCheckpoint('claude-complete', { sessionId: 'claude-session-123' });
      await mockWorkflowManager.saveCheckpoint('pr-created', { prId: 'pr-456' });
    ...
    test('should handle workflow data cleanup after completion', async () => {
      ...
      mockWorkflowManager.completeWorkflow.mockImplementation((workflowId, result) => {
        cleanupActions.push({
          action: 'archive-logs',
          workflowId,
          timestamp: Date.now()
      ...
      const completionResult = await mockWorkflowManager.completeWorkflow(cleanupConfig.workflowId, {
        pr: prResult,
