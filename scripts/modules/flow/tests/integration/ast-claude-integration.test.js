...
const mockLanguageDetector = {
  ...
  getSupportedLanguages: jest.fn(() => ['javascript', 'python', 'go', 'typescript'])
};
...
describe('AST-Claude Integration Suite', () => {
  ...
  describe('Core AST-Claude Data Flow', () => {
    test('should successfully process file through complete AST-Claude pipeline', async () => {
      ...
      expect(mockLanguageDetector.detectLanguage).toHaveBeenCalledWith(testFile);
      expect(mockASTParser.parseFile).toHaveBeenCalledWith(testFile, 'javascript');
    ...
    test('should handle cached AST data efficiently', async () => {
      ...
      expect(mockCacheManager.get).toHaveBeenCalledWith(expect.stringContaining(testFile));
    });
    ...
    test('should propagate errors correctly through pipeline', async () => {
      ...
      await expect(processFileThroughPipeline(testFile, 'session-error'))
  ...
  describe('Session Management Integration', () => {
    ...
    test('should handle session cleanup on completion', async () => {
      ...
      expect(integrationManager.state.activeSessions.has(sessionId)).toBe(false);
  ...
  describe('AST Context Building for Claude', () => {
    ...
    test('should optimize context for Claude token limits', async () => {
      ...
      mockContextBuilder.formatForClaude.mockImplementationOnce(async (context) => {
      ...
      const result = await buildClaudeContext(largeContext.files, 'Large project task');
  ...
  describe('Hook Integration with AST Processing', () => {
    ...
    test('should handle hook failures gracefully', async () => {
      ...
      const result = await executeASTProcessingWithHooks('/test/file.js', [hookConfig]);
    ...
    test('should abort processing for required hook failures', async () => {
      ...
      await expect(executeASTProcessingWithHooks('/test/file.js', [hookConfig]))
  ...
  describe('Error Handling and Recovery', () => {
    ...
    test('should implement retry logic for transient failures', async () => {
      ...
      const result = await processFileWithRetry('/test/network-issue.js', { maxRetries: 3 });
    ...
    test('should recover from cache corruption', async () => {
      ...
      const result = await processFileWithCacheRecovery('/test/cached-corrupt.js');
    ...
    test('should handle Claude service unavailability', async () => {
      ...
      const result = await processFileWithServiceFallback('/test/service-down.js');
  ...
  describe('Performance and Load Testing', () => {
    test('should handle high-volume file processing efficiently', async () => {
      ...
      const fileCount = 100;
      const files = Array.from({ length: fileCount }, (_, i) => `/test/file${i}.js`);
    ...
    test('should throttle concurrent AST processing', async () => {
      ...
      const totalFiles = 20;
      const files = Array.from({ length: totalFiles }, (_, i) => `/test/concurrent${i}.js`);
  ...
  async function processFileThroughPipeline(filePath, sessionId) {
    try {
      ...
      const language = await integrationManager.astComponents.languageDetector.detectLanguage(filePath);
      
      // Step 2: Check cache
      const cacheKey = `ast:${filePath}:${language}`;
      let astData = await integrationManager.astComponents.cacheManager.get(cacheKey);
      let cached = false;
      
      if (!astData) {
        // Step 3: Parse AST
        astData = await integrationManager.astComponents.parser.parseFile(filePath, language);
        await integrationManager.astComponents.cacheManager.set(cacheKey, astData);
      } else {
        cached = true;
      }
      
      // Step 4: Build context
      const contextData = await integrationManager.astComponents.contextBuilder.buildContext([{
        path: filePath,
        ast: astData,
        language
      }]);
      ...
      if (!integrationManager.state.activeSessions.has(sessionId)) {
        const claudeSession = await integrationManager.claudeComponents.backgroundService.startSession({
          id: sessionId,
          context: contextData
        });
  ...
  async function createClaudeSessionWithAST(config) {
    ...
    for (const file of config.files) {
      const result = await processFileThroughPipeline(file, `temp-${Date.now()}`);
      astContexts.push(result);
    }
    
    const combinedContext = await integrationManager.astComponents.contextBuilder.buildContext(
      astContexts.map(ctx => ({
  ...
  async function cleanupSession(sessionId) {
    ...
    if (session) {
      await integrationManager.claudeComponents.backgroundService.endSession(sessionId);
  ...
  async function buildClaudeContext(files, taskDescription) {
    const contextData = await integrationManager.astComponents.contextBuilder.buildContext(files);
    const claudeFormatted = await integrationManager.astComponents.contextBuilder.formatForClaude({
  ...
  async function buildFileContext(filePath) {
    const result = await processFileThroughPipeline(filePath, 'context-session');
    return {
      path: filePath,
      ast: result.astData,
  ...
  async function executeASTProcessingWithHooks(filePath, hooks) {
    ...
    for (const hook of hooks.filter(h => h.timing === 'before')) {
      try {
        await integrationManager.claudeComponents.hookExecutor.executeHook(hook.name, {
          filePath,
          stage: 'before-parsing'
        });
      } catch (error) {
    ...
    // Process AST
    const astData = await integrationManager.astComponents.parser.parseFile(filePath, 'javascript');
  ...
  async function processFileWithErrorHandling(filePath) {
    try {
      const result = await processFileThroughPipeline(filePath, 'error-session');
  ...
  async function processFileWithRetry(filePath, options = {}) {
    ...
    while (attempts < maxRetries) {
      attempts++;
      try {
        const result = await processFileThroughPipeline(filePath, `retry-session-${attempts}`);
  ...
  async function processFileWithCacheRecovery(filePath) {
    try {
      const result = await processFileThroughPipeline(filePath, 'cache-recovery-session');
      ...
      if (error.message.includes('cache')) {
        ...
        const result = await processFileThroughPipeline(filePath, 'cache-recovery-retry');
  ...
  async function processFileWithServiceFallback(filePath) {
    try {
      const result = await processFileThroughPipeline(filePath, 'service-fallback-session');
      return {
        success: true,
        astProcessed: true,
  ...
  async function processFilesWithThrottling(files, concurrentLimit) {
    ...
    while (queue.length > 0 || active.size > 0) {
      ...
      while (active.size < concurrentLimit && queue.length > 0) {
        ...
        const promise = integrationManager.astComponents.parser.parseFile(file, 'javascript')
          .then(result => {
