...
describe('ErrorBoundaryWithRecovery Component', () => {
    ...
    describe('Performance Monitoring', () => {
        ...
        test('should meet performance thresholds', async () => {
            ...
            const avgHandlingTime = results.reduce((sum, r) => 
                sum + r.performanceMetrics.errorHandlingTime, 0) / results.length;

            expect(avgHandlingTime).toBeLessThan(performanceTest.maxErrorHandlingTime);
...
class MockErrorBoundaryWithRecovery extends EventEmitter {
    ...
    async attemptRecovery(error, options = {}) {
        ...
        const timer = options.trackPerformance ? this.performanceMonitor.startTimer() : null;

        while (attempts < maxAttempts && !recoverySuccessful) {
            attempts++;
            
            try {
                recoverySuccessful = await this.executeRecoveryStrategy(strategy, error);
