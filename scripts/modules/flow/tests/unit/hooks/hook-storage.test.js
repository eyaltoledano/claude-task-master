...
class MockHookStorage extends EventEmitter {
	...
	matchesCriteria(entry, criteria) {
		...
		if (
			...
			entry.metadata.priority < criteria.minPriority
		) {
			return false;
		}
		...
		if (
			...
			entry.metadata.priority > criteria.maxPriority
		) {
			return false;
		}
		...
		return {
			...
			cacheHitRate:
				totalCacheOperations > 0
					...
					: 0,
			totalDataSize: Array.from(this.storage.values()).reduce(
...
describe('Hook Storage System', () => {
	...
	describe('Hook Storage Operations', () => {
		...
		test('should store hook with custom options', async () => {
			...
			const metadata = await storage.storeHook(
				'custom-hook',
				hookData,
				options
			);
		...
		test('should reject invalid hook ID', async () => {
			...
			await expect(storage.storeHook('', hookData)).rejects.toThrow(
				'Hook ID must be a non-empty string'
			);
			await expect(storage.storeHook(null, hookData)).rejects.toThrow(
				'Hook ID must be a non-empty string'
			);
		});

		test('should reject invalid hook data', async () => {
			await expect(storage.storeHook('test', null)).rejects.toThrow(
				'Hook data must be an object'
			);
			await expect(storage.storeHook('test', 'not-object')).rejects.toThrow(
				'Hook data must be an object'
			);
		});
		...
		test('should require initialized storage', async () => {
			...
			await expect(uninitializedStorage.storeHook('test', {})).rejects.toThrow(
	...
	describe('Hook Retrieval Operations', () => {
		beforeEach(async () => {
			await storage.storeHook('retrieve-test', {
				name: 'retrieve-test',
				data: 'test-data'
			});
		});
	...
	describe('Hook Deletion Operations', () => {
		...
		test('should handle deletion of non-existent hook', async () => {
			await expect(storage.deleteHook('non-existent')).rejects.toThrow(
				"Hook 'non-existent' not found"
			);
		});

		test('should handle silent deletion', async () => {
			const existed = await storage.deleteHook('non-existent', {
				silent: true
			});
	...
	describe('Batch Operations', () => {
		...
		test('should retrieve multiple hooks in batch', async () => {
			...
			const { results, notFound } = await storage.retrieveBatch([
				'batch-retrieve-1',
				'batch-retrieve-2',
				'non-existent'
			]);
	...
	describe('Query Operations', () => {
		beforeEach(async () => {
			await storage.storeHook(
				'query-1',
				{ name: 'query-1' },
				{ type: 'validator', tags: ['test'], priority: 5 }
			);
			await storage.storeHook(
				'query-2',
				{ name: 'query-2' },
				{ type: 'executor', tags: ['test', 'prod'], priority: 10 }
			);
			await storage.storeHook(
				'query-3',
				{ name: 'query-3' },
				{ type: 'validator', tags: ['prod'], priority: 1 }
			);
		});
		...
		test('should sort query results', async () => {
			const results = await storage.queryHooks({
				sortBy: 'priority',
				sortOrder: 'desc'
			});
	...
	describe('Backup Operations', () => {
		...
		test('should reject restore of non-existent backup', async () => {
			await expect(storage.restoreFromBackup('non-existent')).rejects.toThrow(
	...
	describe('Storage Shutdown', () => {
		...
		test('should create final backup on shutdown', async () => {
			storage.config.enableBackups = true;
			await storage.storeHook('final-backup-test', {
				name: 'final-backup-test'
			});
