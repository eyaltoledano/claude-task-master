...
class MockHookContext extends EventEmitter {
  ...
  constructor(initialData = {}) {
    ...
    this.statistics.activeContexts = MockHookContext.globalStatistics.activeContexts;
  ...
  getShared(key, defaultValue = undefined) {
    ...
    const value = this.sharedData.has(key) ? this.sharedData.get(key) : defaultValue;
  ...
  validateData(key, value, validator) {
    if (typeof validator === 'function') {
      ...
      if (typeof value !== validator.type) {
        throw new Error(`Expected ${validator.type} for key '${key}', got ${typeof value}`);
      }
    }
  }
  ...
  merge(otherContext, options = {}) {
    ...
    for (const [key, value] of otherContext.data.entries()) {
      if (this.data.has(key)) {
        ...
        } else if (strategy === 'merge' && typeof value === 'object' && typeof this.data.get(key) === 'object') {
  ...
  getStatistics() {
    return {
      ...
      totalDataSize: Array.from(this.data.values()).reduce((total, value) => 
        total + this.calculateDataSize(value), 0
      )
    };
  }
...
describe('Hook Context System', () => {
  ...
  describe('Data Management', () => {
    ...
    test('should reject invalid keys', () => {
      expect(() => context.set('', 'value')).toThrow('Context key must be a non-empty string');
      expect(() => context.set(null, 'value')).toThrow('Context key must be a non-empty string');
      expect(() => context.get('')).toThrow('Context key must be a non-empty string');
      expect(() => context.has(null)).toThrow('Context key must be a non-empty string');
      expect(() => context.delete('')).toThrow('Context key must be a non-empty string');
    });
  });
  ...
  describe('Data Validation', () => {
    test('should validate data with function validator', () => {
      ...
      expect(validator).toHaveBeenCalledWith('validValue', 'validatedKey', context);
  ...
  describe('Shared Data Management', () => {
    ...
    test('should reject invalid shared keys', () => {
      expect(() => context.setShared('', 'value')).toThrow('Shared context key must be a non-empty string');
      expect(() => context.getShared(null)).toThrow('Shared context key must be a non-empty string');
    });
  });

  describe('Context Isolation', () => {
    test('should create isolated context', () => {
      const isolated = context.createIsolatedContext('testIsolated', { isolatedKey: 'isolatedValue' });
    ...
    test('should reject invalid isolated context names', () => {
      expect(() => context.createIsolatedContext('')).toThrow('Isolated context name must be a non-empty string');
      expect(() => context.getIsolatedContext(null)).toThrow('Isolated context name must be a non-empty string');
      expect(() => context.destroyIsolatedContext('')).toThrow('Isolated context name must be a non-empty string');
    });
  });
  ...
  describe('Context Utilities', () => {
    test('should return keys, values, and entries', () => {
      ...
      expect(context.entries()).toEqual([['key1', 'value1'], ['key2', 'value2']]);
    });
  ...
  describe('Context Serialization', () => {
    ...
    test('should reject invalid JSON data', () => {
      expect(() => context.fromJSON(null)).toThrow('Invalid JSON data for context restoration');
      expect(() => context.fromJSON('not-object')).toThrow('Invalid JSON data for context restoration');
    });
  });
