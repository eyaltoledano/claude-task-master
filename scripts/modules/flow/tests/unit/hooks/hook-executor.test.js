...
class MockHookExecutor extends EventEmitter {
	...
	discoverHooks(pattern = '*') {
		...
		for (const [name, config] of this.hooks) {
			if (
				...
				config.tags.some((tag) => tag.includes(pattern))
			) {
				discovered.push({
					name,
	...
	getStatistics() {
		return {
			...
			enabledHooks: Array.from(this.hooks.values()).filter((h) => h.enabled)
				.length
		};
	}
...
describe('Hook Executor System', () => {
	...
	describe('Hook Registration', () => {
		...
		test('should register hook with custom options', async () => {
			...
			const config = await executor.registerHook(
				'custom-hook',
				mockHook,
				options
			);
		...
		test('should reject invalid hook name', async () => {
			...
			await expect(executor.registerHook('', mockHook)).rejects.toThrow(
				'Hook name must be a non-empty string'
			);
			await expect(executor.registerHook(null, mockHook)).rejects.toThrow(
				'Hook name must be a non-empty string'
			);
		});

		test('should reject invalid hook function', async () => {
			await expect(executor.registerHook('test', null)).rejects.toThrow(
				'Hook function must be a valid function'
			);
			await expect(
				executor.registerHook('test', 'not-a-function')
			).rejects.toThrow('Hook function must be a valid function');
		});
	});

	describe('Hook Discovery', () => {
		beforeEach(async () => {
			await executor.registerHook('high-priority', jest.fn(), {
				priority: 10,
				tags: ['critical']
			});
			await executor.registerHook('medium-priority', jest.fn(), {
				priority: 5,
				tags: ['normal']
			});
			await executor.registerHook('low-priority', jest.fn(), {
				priority: 1,
				tags: ['optional']
			});
			await executor.registerHook('disabled-hook', jest.fn(), {
				enabled: false
			});
		});
		...
		test('should return hooks sorted by priority', () => {
			...
			for (let i = 0; i < discovered.length - 1; i++) {
				expect(discovered[i].priority).toBeGreaterThanOrEqual(
					discovered[i + 1].priority
				);
			}
		});
	});
	...
	describe('Hook Execution', () => {
		...
		test('should handle hook execution timeout', async () => {
			...
			await expect(executor.executeHook('slow-hook')).rejects.toThrow(
		...
		test('should handle hook execution errors', async () => {
			...
			await expect(executor.executeHook('error-hook')).rejects.toThrow(
				'Hook failed'
			);
		...
		test('should reject execution of non-existent hook', async () => {
			await expect(executor.executeHook('non-existent')).rejects.toThrow(
				"Hook 'non-existent' not found"
			);
		});

		test('should reject execution of disabled hook', async () => {
			const mockHook = jest.fn();
			await executor.registerHook('disabled-hook', mockHook, {
				enabled: false
			});

			await expect(executor.executeHook('disabled-hook')).rejects.toThrow(
				"Hook 'disabled-hook' is disabled"
			);
		});
		...
		test('should check hook dependencies', async () => {
			...
			await expect(executor.executeHook('dependent-hook')).rejects.toThrow(
				"Hook dependency 'missing-dependency' not found"
			);
		});
	});
	...
	describe('Multiple Hook Execution', () => {
		beforeEach(async () => {
			...
			await executor.registerHook(
				'hook3',
				jest.fn().mockRejectedValue(new Error('Hook3 failed'))
			);
		});

		test('should execute multiple hooks and return results', async () => {
			const { results, errors } = await executor.executeHooks([
				'hook1',
				'hook2'
			]);
		...
		test('should execute hooks by pattern', async () => {
			await executor.registerHook(
				'validation-hook',
				jest.fn().mockResolvedValue('validated'),
				{ tags: ['validation'] }
			);
	...
	describe('Error Recovery and Resilience', () => {
		test('should continue execution despite individual hook failures', async () => {
			...
			await executor.registerHook(
				'another-good-hook',
				jest.fn().mockResolvedValue('also-success')
			);
		...
		test('should maintain system stability after cleanup', async () => {
			await executor.registerHook(
				'test-hook',
				jest.fn().mockResolvedValue('before-cleanup')
			);
			...
			await executor.registerHook(
				'new-hook',
				jest.fn().mockResolvedValue('after-cleanup')
			);
