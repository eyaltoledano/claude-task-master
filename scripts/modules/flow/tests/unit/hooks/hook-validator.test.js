...
class MockHookValidator extends EventEmitter {
	...
	async validateHook(hookConfig, context = {}) {
		...
		try {
			...
			validationResult.valid =
				...
				validationResult.safetyIssues.filter((issue) => issue.blocking)
					.length === 0;
			...
			this.statistics.securityViolations +=
				validationResult.securityViolations.length;
			this.statistics.safetyCheckFailures +=
				validationResult.safetyIssues.length;
	...
	async validateConfiguration(config) {
		...
		if (
			...
			!validSecurityLevels.includes(config.securityLevel)
		) {
		...
		for (const hookConfig of hookConfigs) {
			try {
				...
				results.set(hookConfig.name, {
					valid: false,
					errors: [
						{ type: 'validation', message: error.message, severity: 'error' }
					],
					warnings: [],
					securityViolations: [],
					safetyIssues: []
...
describe('Hook Validator System', () => {
	...
	describe('Validation Rule Management', () => {
		...
		test('should reject invalid rule name', () => {
			...
			expect(() => validator.addValidationRule('', rule)).toThrow(
				'Validation rule name must be a non-empty string'
			);
			expect(() => validator.addValidationRule(null, rule)).toThrow(
				'Validation rule name must be a non-empty string'
			);
		});

		test('should reject invalid rule object', () => {
			expect(() => validator.addValidationRule('test', null)).toThrow(
				'Validation rule must be an object'
			);
			expect(() => validator.addValidationRule('test', 'not-object')).toThrow(
				'Validation rule must be an object'
			);
		});
	});
		...
		test('should detect invalid security level', async () => {
			...
			expect(result.errors).toContainEqual(
				expect.objectContaining({
					field: 'securityLevel',
					message:
						'Invalid security level. Must be one of: low, medium, high, strict'
				})
			);
		});
