...
class MockPreLaunchValidationHook extends EventEmitter {
	...
	setupDefaultValidators() {
		...
		this.validators.set('permissions', async (context) => {
			...
			return {
				...
				message:
					denied.length > 0
						? `Insufficient permissions: ${denied.join(', ')}`
			...
			return {
				...
				message:
					insufficient.length > 0
						? `Insufficient resources: ${insufficient.join(', ')}`
						: 'All resources available',
				details: {
					insufficient,
					checked: context.requiredResources?.length || 0
				}
			};
		});
	}
	...
	async execute(context = {}) {
		...
		try {
			...
			if (this.config.validateDependencies) {
				validationResult.validations.dependencies =
					await this.validators.get('dependencies')(context);
			}

			if (this.config.validateEnvironment) {
				validationResult.validations.environment =
					await this.validators.get('environment')(context);
			}

			if (this.config.validatePermissions) {
				validationResult.validations.permissions =
					await this.validators.get('permissions')(context);
			}

			if (this.config.validateResources) {
				validationResult.validations.resources =
					await this.validators.get('resources')(context);
			}
	...
	updateAverageTime(validationTime) {
		const total =
			this.statistics.passedValidations + this.statistics.failedValidations;
		this.statistics.averageValidationTime =
			(this.statistics.averageValidationTime * (total - 1) + validationTime) /
			total;
	}
...
describe('Pre-Launch Validation Hook', () => {
	...
	describe('Dependency Validation', () => {
		...
		test('should fail with missing dependencies', async () => {
			const context = {
				requiredDependencies: [{ name: 'missing-dependency', version: '1.0.0' }]
			};

			const result = await validationHook.execute(context);

			expect(result.validations.dependencies.valid).toBe(false);
			expect(result.validations.dependencies.message).toContain(
	...
	describe('Environment Validation', () => {
		...
		test('should fail with missing environment variables', async () => {
			...
			expect(result.validations.environment.message).toContain(
	...
	describe('Permission Validation', () => {
		...
		test('should fail with denied permissions', async () => {
			...
			expect(result.validations.permissions.message).toContain(
	...
	describe('Resource Validation', () => {
		...
		test('should fail with insufficient resources', async () => {
			...
			expect(result.validations.resources.message).toContain(
	...
	describe('Performance', () => {
		test('should complete validation within time limits', async () => {
			const context = {
				requiredDependencies: Array.from({ length: 100 }, (_, i) => ({
					name: `dep-${i}`
				})),
				...
				requiredResources: Array.from({ length: 10 }, (_, i) => ({
					name: `resource-${i}`,
					available: true
				}))
			};
