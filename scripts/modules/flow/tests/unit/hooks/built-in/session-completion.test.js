...
class MockSessionCompletionHook extends EventEmitter {
  ...
  async generateSessionReport() {
    const report = {
      ...
      summary: {
        ...
        successfulResults: this.currentSession.results.filter(r => r.success).length,
        failedResults: this.currentSession.results.filter(r => !r.success).length,
  ...
  calculatePerformanceMetrics() {
    ...
    return {
      ...
      averageResultTime: this.currentSession.results.length > 0 
        ? duration / this.currentSession.results.length 
        : 0,
      errorRate: this.currentSession.results.length > 0 
        ? (this.currentSession.errors.length / this.currentSession.results.length) * 100 
        : 0
    };
  }
  ...
  getStatistics() {
    return {
      ...this.statistics,
      successRate: this.statistics.totalCompletions > 0 
        ? (this.statistics.successfulCompletions / this.statistics.totalCompletions) * 100 
        : 0,
...
describe('Session Completion Hook', () => {
  ...
  describe('Session Completion Execution', () => {
    ...
    test('should emit completion lifecycle events', async () => {
      ...
      expect(finishedSpy).toHaveBeenCalledWith({
        sessionId: 'completion-test-session',
        result: expect.objectContaining({ sessionId: 'completion-test-session' }),
    ...
    test('should reject execution when no session set', async () => {
      ...
      await expect(completionHook.execute()).rejects.toThrow('No session to complete');
    });
  });
  ...
  describe('Report Generation', () => {
    ...
    test('should skip report generation when disabled', async () => {
      const hookWithoutReport = new MockSessionCompletionHook({ generateReport: false });
  ...
  describe('Notification System', () => {
    ...
    test('should include different notification types', async () => {
      ...
      const completionNotification = notifications.find(n => n.type === 'session_completed');
  ...
  describe('Session Archiving', () => {
    ...
    test('should skip archiving when disabled', async () => {
      const hookWithoutArchive = new MockSessionCompletionHook({ archiveSession: false });
  ...
  describe('Performance Benchmarks', () => {
    test('should complete within performance thresholds', async () => {
      const session = {
        ...
        results: Array.from({ length: 100 }, (_, i) => ({ id: `result-${i}`, success: true })),
