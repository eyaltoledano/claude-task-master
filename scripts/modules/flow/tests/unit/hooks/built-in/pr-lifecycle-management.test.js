...
class MockPRLifecycleManagementHook extends EventEmitter {
	...
	async monitorPR(context) {
		...
		if (!monitoringResult.checks.reviewsCompleted) {
			monitoringResult.recommendations.push(
				`Need ${this.config.requireApprovals - pr.approvals} more approvals`
			);
		}
		...
		const readyForMerge = Object.values(monitoringResult.checks).every(
			(check) => check
		);
	...
	async mergePR(context) {
		...
		if (pr.approvals < this.config.requireApprovals) {
			throw new Error(
				`PR #${pr.number} needs ${this.config.requireApprovals - pr.approvals} more approvals`
			);
		}
...
describe('PR Lifecycle Management Hook', () => {
	...
	describe('PR Creation', () => {
		...
		test('should reject creation without required fields', async () => {
			...
			await expect(prHook.execute(context)).rejects.toThrow(
				'Branch and title are required for PR creation'
			);
		});
	});
	...
	describe('PR Merging', () => {
		...
		test('should reject merge of already merged PR', async () => {
			...
			// Try to merge again
			await expect(prHook.execute({ action: 'merge', prId })).rejects.toThrow(
				'is already merged'
			);
		});
	});
	...
	describe('PR Closing', () => {
		...
		test('should reject closing already closed PR', async () => {
			...
			// Try to close again
			await expect(prHook.execute({ action: 'close', prId })).rejects.toThrow(
				'is already closed'
			);
		});
	});
	...
	describe('Error Handling', () => {
		test('should reject execution when hook not active', async () => {
			...
			await expect(prHook.execute({ action: 'create' })).rejects.toThrow(
				'Hook not active'
			);
		});

		test('should reject unknown actions', async () => {
			await expect(prHook.execute({ action: 'unknown' })).rejects.toThrow(
