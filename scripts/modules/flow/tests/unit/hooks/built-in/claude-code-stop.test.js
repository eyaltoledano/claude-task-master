...
class MockClaudeCodeStopHook extends EventEmitter {
  ...
  async performSafetyChecks(context, result) {
    ...
    if (this.currentSession?.activeOperations > 0) {
      if (context.force) {
        result.warnings.push(`${this.currentSession.activeOperations} active operations will be terminated`);
      } else {
        throw new Error(`Cannot stop: ${this.currentSession.activeOperations} active operations in progress`);
      }
    }

    // Check for critical processes
    const criticalProcesses = this.currentSession?.processes?.filter(p => p.critical) || [];
    if (criticalProcesses.length > 0) {
      if (context.force) {
        result.warnings.push(`${criticalProcesses.length} critical processes will be terminated`);
      } else {
        throw new Error(`Cannot stop: ${criticalProcesses.length} critical processes running`);
      }
    }
    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Process termination timeout')), 
  ...
  async cleanupResources(result) {
    ...
    for (const resource of resources) {
      try {
        ...
        result.warnings.push(`Failed to cleanup resource ${resource.id}: ${error.message}`);
      }
    }
  ...
  async cleanupTempFiles(result) {
    ...
    for (const file of tempFiles) {
      try {
        ...
        result.warnings.push(`Failed to cleanup temp file ${file.path}: ${error.message}`);
      }
    }
  ...
  async performForceStop(result) {
    try {
      ...
      result.warnings.push('Force stop performed - some operations may not have completed gracefully');
  ...
  getStatistics() {
    return {
      ...this.statistics,
      successRate: this.statistics.totalStops > 0 
        ? ((this.statistics.gracefulStops + this.statistics.forcedStops) / this.statistics.totalStops) * 100 
        : 0,
      gracefulRate: this.statistics.totalStops > 0 
...
describe('Claude Code Stop Hook', () => {
  ...
  describe('Hook Activation and Deactivation', () => {
    ...
    test('should reject double activation', async () => {
      await expect(stopHook.activate()).rejects.toThrow('Claude Code stop hook already active');
    });

    test('should reject deactivation when not active', async () => {
      const newHook = new MockClaudeCodeStopHook();
      
      await expect(newHook.deactivate()).rejects.toThrow('Claude Code stop hook not active');
    });
  });
  ...
  describe('Session Management', () => {
    ...
    test('should emit sessionSet event', () => {
      ...
      expect(eventSpy).toHaveBeenCalledWith({ sessionId: 'event-test-session' });
    });
    ...
    test('should reject invalid session object', () => {
      expect(() => stopHook.setCurrentSession(null)).toThrow('Invalid session object');
      expect(() => stopHook.setCurrentSession('not-object')).toThrow('Invalid session object');
    });
  });
  ...
  describe('Stop Hook Execution', () => {
    ...
    test('should reject concurrent executions', async () => {
      ...
      await expect(stopHook.execute()).rejects.toThrow('Stop operation already in progress');
  ...
  describe('Safety Checks', () => {
    ...
    test('should handle unsaved work with force flag', async () => {
      ...
      expect(result.graceful).toBe(true);
      expect(result.warnings).toContain('Unsaved work detected but force stop requested');
    });
    ...
    test('should reject stop with unsaved work without force', async () => {
      ...
      await expect(stopHook.execute({ force: false, allowForceStop: false })).rejects.toThrow('Cannot stop: unsaved work detected');
    });
    ...
    test('should handle active operations with force flag', async () => {
      ...
      expect(result.warnings).toContain('3 active operations will be terminated');
    });
    ...
    test('should reject stop with active operations without force', async () => {
      ...
      await expect(stopHook.execute({ force: false, allowForceStop: false })).rejects.toThrow('Cannot stop: 2 active operations in progress');
    });
    ...
    test('should handle critical processes with force flag', async () => {
      ...
      expect(result.warnings).toContain('1 critical processes will be terminated');
    });
  ...
  describe('State Saving', () => {
    ...
    test('should skip state saving when disabled', async () => {
      const hookWithoutStateSave = new MockClaudeCodeStopHook({ saveStateBeforeStop: false });
  ...
  describe('Process Termination', () => {
    ...
    test('should handle process termination timeout', async () => {
      const shortTimeoutHook = new MockClaudeCodeStopHook({ gracefulShutdownTimeout: 1 });
      ...
      expect(result.warnings.some(w => w.includes('Process termination warning'))).toBe(true);
  ...
  describe('Temporary File Cleanup', () => {
    ...
    test('should skip temp file cleanup when disabled', async () => {
      const hookWithoutTempCleanup = new MockClaudeCodeStopHook({ cleanupTempFiles: false });
  ...
  describe('Force Stop Operations', () => {
    test('should perform force stop when graceful stop fails', async () => {
      ...
      expect(result.warnings).toContain('Force stop performed - some operations may not have completed gracefully');
      expect(eventSpy).toHaveBeenCalledWith({
  ...
  describe('Notification System', () => {
    test('should send notifications when enabled', async () => {
      const hookWithNotifications = new MockClaudeCodeStopHook({ notifyOnStop: true });
    ...
    test('should include warning notifications', async () => {
      ...
      const warningNotifications = result.notifications.filter(n => n.type === 'warning');
  ...
  describe('Utility Methods', () => {
    ...
    test('should reject graceful stop with unsafe conditions', async () => {
      ...
      await expect(stopHook.gracefulStop()).rejects.toThrow('Cannot stop: unsaved work detected');
    });
  });
  ...
  describe('Performance Benchmarks', () => {
    ...
    test('should handle multiple concurrent stop attempts gracefully', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        attempts.push(
          stopHook.execute().catch(error => error.message)
        );
      }
      ...
      const successes = results.filter(r => typeof r === 'object' && r.graceful);
      const failures = results.filter(r => typeof r === 'string' && r.includes('already in progress'));
