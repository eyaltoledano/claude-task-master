...
describe('ObservabilityManager', () => {
	...
	describe('OpenTelemetry Integration', () => {
		test('should initialize OpenTelemetry SDK', async () => {
			...
			expect(mockOpenTelemetry.trace.getTracer).toHaveBeenCalledWith(
				'task-master-tui'
			);
			expect(mockOpenTelemetry.metrics.getMeter).toHaveBeenCalledWith(
				'task-master-tui'
			);
		});
		...
		test('should create nested spans for complex operations', async () => {
			const parentSpan =
				await mockObservabilityManager.startTrace('parent-operation');
			const childSpan = await mockObservabilityManager.startTrace(
				'child-operation',
				{
					parent: parentSpan
				}
	...
	describe('Metric Recording and Storage', () => {
		test('should record counter metrics', async () => {
			const counter = mockOpenTelemetry.metrics
				.getMeter()
		...
		test('should record histogram metrics', async () => {
			const histogram = mockOpenTelemetry.metrics
				.getMeter()
				.createHistogram('test-histogram');
		...
		test('should record gauge metrics', async () => {
			const gauge = mockOpenTelemetry.metrics
				.getMeter()
				.createGauge('test-gauge');

			await mockObservabilityManager.recordGauge(
				'memory_usage',
				45 * 1024 * 1024,
				{
					process: 'main',
		...
		test('should enforce metric limits per type', async () => {
			...
			const result = await mockObservabilityManager.recordCounter(
				'counter_overflow',
				1
			);
		...
		test('should retrieve stored metrics', async () => {
			...
			const metrics =
				await mockObservabilityManager.getStoredMetrics('counter');
	...
	describe('Performance Monitoring', () => {
		...
		test('should detect performance anomalies', async () => {
			...
			const anomalies =
				await mockObservabilityManager.detectPerformanceAnomalies(slowEntries);
		...
		test('should correlate performance with user actions', async () => {
			...
			const correlation =
				await mockObservabilityManager.correlatePerformanceWithAction(
					userAction,
					performanceEntry
				);
	...
	describe('Health Check System', () => {
		...
		test('should schedule periodic health checks', async () => {
			...
			expect(mockObservabilityManager.getHealthCheckInterval()).toBe(
				healthCheckInterval
			);
	...
	describe('VibeKit Telemetry Integration', () => {
		...
		test('should handle VibeKit telemetry errors', async () => {
			mockVibeKitTelemetry.exportTraces.mockRejectedValue(
		...
		test('should respect sampling rate', async () => {
			...
			for (let i = 0; i < totalTraces; i++) {
				const shouldExport =
					mockObservabilityManager.shouldSample(samplingRate);
	...
	describe('Error Handling and Recovery', () => {
		test('should handle OpenTelemetry initialization errors', async () => {
			...
			const result =
				await mockObservabilityManager.initializeWithErrorHandling();
		...
		test('should recover from telemetry service failures', async () => {
			...
			const result =
				await mockObservabilityManager.recoverFromTelemetryFailure();
		...
		test('should handle performance observer errors', async () => {
			...
			const result =
				await mockObservabilityManager.startPerformanceMonitoringWithErrorHandling();
	...
	describe('Configuration Management', () => {
		test('should validate configuration parameters', async () => {
			...
			const validation =
				await mockObservabilityManager.validateConfiguration(invalidConfig);
			...
			expect(validation.errors).toContain(
				'healthCheckInterval must be a number'
			);
		});
		...
		test('should apply configuration updates', async () => {
			...
			expect(mockObservabilityManager.getConfiguration().memoryThreshold).toBe(
				200 * 1024 * 1024
			);
			expect(mockObservabilityManager.getConfiguration().samplingRate).toBe(
				0.2
			);
		});
		...
		test('should handle configuration reload', async () => {
			...
			expect(mockObservabilityManager.getConfigurationStatus()).toBe(
				'reloaded'
			);
		});
	});
