...
class MockWorkflowStateManager extends EventEmitter {
  constructor(options = {}) {
    super();
    this.options = { persistenceInterval: 5000, maxWorkflowHistory: 1000, stateTimeout: 300000, maxRetryAttempts: 3, ...options };
    this.workflows = new Map();
    this.workflowHistory = [];
    this.isActive = false;
    this.stats = { workflowsCreated: 0, workflowsCompleted: 0, stateTransitions: 0, persistenceOperations: 0, rollbackOperations: 0, errors: 0 };
  }
  ...
  async createWorkflow(workflowConfig) {
    ...
    const workflow = {
      ...
      steps: steps.map((step, index) => ({
        id: step.id || `step_${index}`, name: step.name, type: step.type, config: step.config || {},
        status: 'pending', dependencies: step.dependencies || [], retryCount: 0,
        startedAt: null, completedAt: null, error: null, result: null
      })),
      status: 'created', currentStep: null, data: { ...initialData },
      metadata: { ...metadata, createdAt: Date.now(), createdBy: metadata.createdBy || 'system' },
      history: [], checkpoints: []
    };
    ...
    this.addToHistory({ workflowId: id, action: 'created', timestamp: Date.now(), details: { name, stepCount: steps.length } });
  ...
  async transitionWorkflowState(workflowId, newStatus, details = {}) {
    ...
    const transition = { workflowId, from: oldStatus, to: newStatus, timestamp: Date.now(), details };
    workflow.history.push(transition);
    this.addToHistory({ workflowId, action: 'state_transition', timestamp: Date.now(), details: transition });
  ...
  async executeStep(workflowId, stepId) {
    ...
    try {
      ...
      if (step.retryCount < this.options.maxRetryAttempts) {
        ...
        this.emit('step:retry', { workflowId, stepId, step, attempt: step.retryCount });
        ...
        await this.transitionWorkflowState(workflowId, 'failed', { failedStep: stepId, error: error.message });
      }
  ...
  async simulateStepExecution(workflow, step) {
    ...
    switch (step.type) {
      case 'data_processing':
        return { type: 'data_processing', data: { processedItems: Math.floor(Math.random() * 100) + 1 }, success: true };
      case 'api_call':
        if (step.config.shouldFail) throw new Error('Simulated API call failure');
        return { type: 'api_call', data: { response: { status: 200, data: 'API response data' } }, success: true };
      case 'validation':
        const isValid = Math.random() > 0.2;
        if (!isValid) throw new Error('Validation failed');
        return { type: 'validation', data: { validationPassed: true }, success: true };
      default:
  ...
  findNextExecutableStep(workflow) {
    return workflow.steps.find(step => step.status === 'pending' && this.checkStepDependencies(workflow, step));
  }
  ...
  async createCheckpoint(workflowId, checkpointName) {
    ...
    const checkpoint = {
      name: checkpointName, timestamp: Date.now(),
  ...
  async rollbackToCheckpoint(workflowId, checkpointName) {
    ...
    const checkpoint = workflow.checkpoints.find(cp => cp.name === checkpointName);
    ...
    this.addToHistory({ workflowId, action: 'rollback', timestamp: Date.now(), details: { checkpointName, checkpointTimestamp: checkpoint.timestamp } });
    this.emit('workflow:rollback', { workflowId, checkpointName, workflow: restoredWorkflow });
  ...
  async persistState() {
    ...
    try {
      ...
      this.emit('state:persisted', { workflowCount: this.workflows.size, historyLength: this.workflowHistory.length });
    } catch (error) {
      this.stats.errors++;
  ...
  getStats() {
    return { ...this.stats, activeWorkflows: this.workflows.size, historyLength: this.workflowHistory.length };
  }
...
describe('WorkflowStateManager Service', () => {
  ...
  beforeEach(() => {
    ...
    eventLog = [];
    manager.on('manager:started', (data) => eventLog.push({ event: 'manager:started', data }));
    manager.on('manager:stopped', (data) => eventLog.push({ event: 'manager:stopped', data }));
    manager.on('workflow:created', (data) => eventLog.push({ event: 'workflow:created', data }));
    manager.on('workflow:state:changed', (data) => eventLog.push({ event: 'workflow:state:changed', data }));
    manager.on('workflow:completed', (data) => eventLog.push({ event: 'workflow:completed', data }));
    manager.on('workflow:rollback', (data) => eventLog.push({ event: 'workflow:rollback', data }));
    manager.on('step:started', (data) => eventLog.push({ event: 'step:started', data }));
    manager.on('step:completed', (data) => eventLog.push({ event: 'step:completed', data }));
    manager.on('step:failed', (data) => eventLog.push({ event: 'step:failed', data }));
    manager.on('step:retry', (data) => eventLog.push({ event: 'step:retry', data }));
    manager.on('checkpoint:created', (data) => eventLog.push({ event: 'checkpoint:created', data }));
    manager.on('state:persisted', (data) => eventLog.push({ event: 'state:persisted', data }));
  });
  ...
  describe('Manager Lifecycle', () => {
    ...
    test('should handle double start attempt', async () => {
      await manager.start();
      await expect(manager.start()).rejects.toThrow('WorkflowStateManager already active');
    });
  ...
  describe('Workflow Creation and Management', () => {
    ...
    test('should create workflow successfully', async () => {
      const workflowConfig = {
        id: 'test-workflow-1', name: 'Test Workflow',
        steps: [
          { id: 'step1', name: 'Data Processing', type: 'data_processing' },
          { id: 'step2', name: 'Validation', type: 'validation', dependencies: ['step1'] }
        ],
        initialData: { input: 'test-data' }, metadata: { createdBy: 'test-user' }
      };
    ...
    test('should get workflows by status', async () => {
      const configs = [
        { id: 'workflow-1', name: 'Workflow 1', steps: [{ id: 'step1', name: 'Step 1', type: 'data_processing' }] },
        { id: 'workflow-2', name: 'Workflow 2', steps: [{ id: 'step1', name: 'Step 1', type: 'validation' }] }
      ];
  ...
  describe('Workflow Execution', () => {
    ...
    test('should start and execute simple workflow', async () => {
      const workflowConfig = {
        id: 'simple-workflow', name: 'Simple Test',
    ...
    test('should execute workflow with dependencies', async () => {
      const workflowConfig = {
        id: 'dependency-workflow', name: 'Dependency Test',
        steps: [
          { id: 'step1', name: 'First Step', type: 'data_processing' },
          { id: 'step2', name: 'Second Step', type: 'validation', dependencies: ['step1'] },
          { id: 'step3', name: 'Third Step', type: 'api_call', dependencies: ['step2'] }
        ]
      };
    ...
    test('should handle step failures and retries', async () => {
      const workflowConfig = {
        id: 'retry-workflow', name: 'Retry Test',
        steps: [{ id: 'step1', name: 'Failing Step', type: 'api_call', config: { shouldFail: true } }]
      };
      ...
      expect(workflow.steps[0].retryCount).toBe(manager.options.maxRetryAttempts);
      expect(eventLog.filter(e => e.event === 'step:retry')).toHaveLength(manager.options.maxRetryAttempts);
    });
  });
  ...
  describe('State Management and Persistence', () => {
    ...
    test('should create and restore checkpoints', async () => {
      const workflowConfig = {
        id: 'checkpoint-workflow', name: 'Checkpoint Test',
        steps: [
          { id: 'step1', name: 'First Step', type: 'data_processing' },
          { id: 'step2', name: 'Second Step', type: 'validation', dependencies: ['step1'] }
        ]
      ...
      // Create checkpoint
      const checkpointResult = await manager.createCheckpoint('checkpoint-workflow', 'after-step1');
      ...
      const rollbackResult = await manager.rollbackToCheckpoint('checkpoint-workflow', 'after-step1');
    ...
    test('should persist state', async () => {
      const workflowConfig = {
        id: 'persistence-workflow', name: 'Persistence Test',
        steps: [{ id: 'step1', name: 'Test Step', type: 'data_processing' }]
      };
    ...
    test('should track statistics accurately', async () => {
      const workflowConfig = {
        id: 'stats-workflow', name: 'Stats Test',
        steps: [{ id: 'step1', name: 'Test Step', type: 'data_processing' }]
      };
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    test('should handle invalid workflow operations', async () => {
      await expect(manager.startWorkflow('nonexistent'))
        .rejects.toThrow('Workflow nonexistent not found');
    });

    test('should handle checkpoint operations on nonexistent workflow', async () => {
      await expect(manager.createCheckpoint('nonexistent', 'test'))
        .rejects.toThrow('Workflow nonexistent not found');
      
      await expect(manager.rollbackToCheckpoint('nonexistent', 'test'))
        .rejects.toThrow('Workflow nonexistent not found');
    });
    ...
    test('should handle concurrent workflow operations', async () => {
      ...
      for (let i = 1; i <= 3; i++) {
        workflowConfigs.push({
          id: `concurrent-workflow-${i}`, name: `Concurrent Workflow ${i}`,
          steps: [{ id: 'step1', name: 'Test Step', type: 'data_processing' }]
        });
      }
      
      const createResults = await Promise.all(workflowConfigs.map(config => manager.createWorkflow(config)));
      ...
      const startResults = await Promise.all(workflowConfigs.map(config => manager.startWorkflow(config.id)));
