...
class MockBackgroundClaudeCode extends EventEmitter {
	...
	getSessionStats(sessionId) {
		...
		return {
			...
			operationsCount: session.stats.operationsCount,
...
describe('BackgroundClaudeCode Service', () => {
	...
	beforeEach(() => {
		...
		service.on('session:created', (data) =>
			eventLog.push({ event: 'session:created', data })
		);
		service.on('session:stopped', (data) =>
			eventLog.push({ event: 'session:stopped', data })
		);
		service.on('operation:queued', (data) =>
			eventLog.push({ event: 'operation:queued', data })
		);
		service.on('operation:completed', (data) =>
			eventLog.push({ event: 'operation:completed', data })
		);
	});
	...
	describe('Background Operations', () => {
		...
		test('should handle operations for inactive sessions', async () => {
			...
			const operation = { type: 'code_analysis' };
			await expect(
				service.queueOperation(sessionId, operation)
