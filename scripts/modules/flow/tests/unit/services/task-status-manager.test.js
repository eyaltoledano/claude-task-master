...
describe('TaskStatusManager - Phase 4/5 Tests', () => {
	...
	describe('Workflow Step Management', () => {
		test('should handle start-implementation step', async () => {
			...
			expect(mockBackend.setTaskStatus).toHaveBeenCalledWith(
				'4',
				'in-progress'
			);
		});
		...
		test('should reject unknown workflow step', async () => {
			await expect(
				statusManager.updateStatusForWorkflowStep('4', 'unknown-step')
	...
	describe('Workflow Steps Tracking', () => {
		...
		test('should handle task not found', async () => {
			...
			await expect(
				statusManager.getWorkflowStepsForTask('nonexistent')
	...
	describe('Error Handling', () => {
		test('should handle backend failures gracefully', async () => {
			...
			await expect(
				statusManager.updateStatusForWorkflowStep('4', 'start-implementation')
		...
		test('should validate required parameters', async () => {
			await expect(
				statusManager.updateStatusForWorkflowStep(null, 'start-implementation')
			).rejects.toThrow();

			await expect(
				statusManager.updateStatusForWorkflowStep('4', null)
			).rejects.toThrow();
		});
	});
	...
	describe('Integration Features', () => {
		test('should support batch status updates', async () => {
			...
			const results = await Promise.all(
				taskIds.map((id) =>
					statusManager.updateStatusForWorkflowStep(
						id,
						'complete-implementation'
					)
				)
		...
		test('should handle concurrent updates', async () => {
			const updates = [
				...
				statusManager.updateStatusForWorkflowStep('4.2', 'commit-progress', {
					commitMessage: 'test'
				}),
				statusManager.updateStatusForWorkflowStep(
					'4.3',
