...
class MockStreamingStateManager extends EventEmitter {
  constructor(options = {}) {
    super();
    this.options = { maxStateHistory: 100, syncInterval: 1000, compressionThreshold: 1000, ...options };
    this.state = new Map();
    this.stateHistory = [];
    this.subscribers = new Map();
    this.isActive = false;
    this.stats = { stateUpdates: 0, syncOperations: 0, compressionEvents: 0, errors: 0 };
  }
  ...
  subscribe(key, callback, options = {}) {
    ...
    const subscription = { id: subscriptionId, callback, options, createdAt: Date.now(), callCount: 0 };
    ...
    if (options.immediate && this.state.has(key)) {
      setImmediate(() => {
        this.notifySubscriber(subscription, key, {
          key, value: this.state.get(key), timestamp: Date.now(), type: 'immediate'
        });
      });
    }
  ...
  async notifySubscriber(subscription, key, stateUpdate) {
    try {
      ...
      if (subscription.options.filter && !subscription.options.filter(stateUpdate)) return;
      await subscription.callback(stateUpdate);
    } catch (error) {
      this.stats.errors++;
      this.emit('subscription:error', { subscriptionId: subscription.id, key, error: error.message });
    }
  }
    ...
    return {
      ...
      state: {
        ...
        subscriberCount: Array.from(this.subscribers.values())
          .reduce((total, keySubscribers) => total + keySubscribers.size, 0)
      }
    };
  }
...
describe('StreamingStateManager Service', () => {
  ...
  beforeEach(() => {
    ...
    manager.on('state:updated', (data) => eventLog.push({ event: 'state:updated', data }));
    manager.on('subscription:created', (data) => eventLog.push({ event: 'subscription:created', data }));
    manager.on('subscription:removed', (data) => eventLog.push({ event: 'subscription:removed', data }));
    manager.on('subscription:error', (data) => eventLog.push({ event: 'subscription:error', data }));
  });
  ...
  describe('Manager Lifecycle', () => {
    ...
    test('should handle double start attempt', async () => {
      await manager.start();
      await expect(manager.start()).rejects.toThrow('StreamingStateManager already active');
    });
  ...
  describe('State Management', () => {
    ...
    test('should handle operations when not active', async () => {
      ...
      await expect(manager.setState('key', 'value')).rejects.toThrow('StreamingStateManager not active');
      expect(() => manager.getState('key')).toThrow('StreamingStateManager not active');
      expect(() => manager.getAllState()).toThrow('StreamingStateManager not active');
    });
  });
  ...
  describe('Subscription Management', () => {
    ...
    test('should create and manage subscriptions', async () => {
      ...
      expect(eventLog.some(e => e.event === 'subscription:created')).toBe(true);
    ...
    test('should unsubscribe successfully', async () => {
      ...
      expect(result.success).toBe(true);
      expect(eventLog.some(e => e.event === 'subscription:removed')).toBe(true);
    ...
    test('should handle nonexistent subscription removal', () => {
      expect(() => manager.unsubscribe('nonexistent')).toThrow('Subscription nonexistent not found');
    });
  });
