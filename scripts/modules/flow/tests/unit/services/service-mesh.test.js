...
class MockServiceMesh extends EventEmitter {
  ...
  async registerService(serviceConfig) {
    ...
    if (this.services.has(id)) throw new Error(`Service ${id} already registered`);
    
    const service = { id, name, endpoint, healthCheckPath, status: "healthy", registeredAt: Date.now() };
  ...
  async unregisterService(serviceId) {
    if (!this.services.has(serviceId)) throw new Error(`Service ${serviceId} not found`);
...
describe("ServiceMesh", () => {
  ...
  beforeEach(() => {
    ...
    eventLog = [];
    mesh.on("mesh:started", (data) => eventLog.push({ event: "mesh:started", data }));
    mesh.on("mesh:stopped", (data) => eventLog.push({ event: "mesh:stopped", data }));
    mesh.on("service:registered", (data) => eventLog.push({ event: "service:registered", data }));
    mesh.on("service:unregistered", (data) => eventLog.push({ event: "service:unregistered", data }));
  });
  ...
  describe("Service Registration", () => {
    ...
    test("should unregister service successfully", async () => {
      ...
      expect(eventLog.some(e => e.event === "service:unregistered")).toBe(true);
    });
    ...
    test("should handle duplicate registration", async () => {
      ...
      await expect(mesh.registerService(serviceConfig))
        .rejects.toThrow("Service duplicate-service already registered");
    });

    test("should handle unregistering nonexistent service", async () => {
      await expect(mesh.unregisterService("nonexistent"))
  ...
  describe("Error Handling", () => {
    test("should handle operations when not running", async () => {
      await expect(mesh.registerService({
        id: "test",
        name: "test",
        endpoint: "http://localhost:3000"
    ...
    test("should handle concurrent operations", async () => {
      ...
      const services = [
        { id: "service1", name: "Service 1", endpoint: "http://localhost:3001" },
        { id: "service2", name: "Service 2", endpoint: "http://localhost:3002" },
