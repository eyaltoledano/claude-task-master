...
class MockPRMonitoringService extends EventEmitter {
	...
	async start() {
		if (this.isRunning)
			throw new Error('PR Monitoring Service already running');
		this.isRunning = true;
	...
	async removePRMonitoring(prId) {
		if (!this.isRunning) throw new Error('Service not running');
		if (!this.monitoredPRs.has(prId))
			throw new Error(`PR ${prId} not being monitored`);
	...
	async handleStatusChange(prId, oldStatus, newStatus) {
		...
		const notification = {
			prId,
			type: 'status_change',
			oldStatus: oldStatus.state,
			newStatus: newStatus.state,
			timestamp: Date.now(),
			details: { mergeable: newStatus.mergeable, checks: newStatus.checks }
		};
	...
	async handleWebhook(payload) {
		...
		if (!config.events.includes(action)) {
			this.emit('webhook:ignored', {
				prId,
				action,
				reason: 'event_not_monitored'
			});
	...
	async healthCheck() {
		return {
			...
			rateLimits: {
				...
				percentage: (this.rateLimits.remaining / this.rateLimits.limit) * 100
			},
			timestamp: Date.now()
		};
	}
...
describe('PRMonitoringService', () => {
	...
	beforeEach(() => {
		...
		service.on('service:started', (data) =>
			eventLog.push({ event: 'service:started', data })
		);
		service.on('service:stopped', (data) =>
			eventLog.push({ event: 'service:stopped', data })
		);
		service.on('pr:monitoring:added', (data) =>
			eventLog.push({ event: 'pr:monitoring:added', data })
		);
		service.on('pr:monitoring:removed', (data) =>
			eventLog.push({ event: 'pr:monitoring:removed', data })
		);
		service.on('pr:status:checked', (data) =>
			eventLog.push({ event: 'pr:status:checked', data })
		);
		service.on('pr:status:changed', (data) =>
			eventLog.push({ event: 'pr:status:changed', data })
		);
		service.on('webhook:received', (data) =>
			eventLog.push({ event: 'webhook:received', data })
		);
		service.on('webhook:processed', (data) =>
			eventLog.push({ event: 'webhook:processed', data })
		);
		service.on('webhook:ignored', (data) =>
			eventLog.push({ event: 'webhook:ignored', data })
		);
		service.on('notification:sent', (data) =>
			eventLog.push({ event: 'notification:sent', data })
		);
	});
	...
	describe('Service Lifecycle', () => {
		...
		test('should handle double start attempt', async () => {
			...
			await expect(service.start()).rejects.toThrow(
				'PR Monitoring Service already running'
			);
		});
	...
	describe('PR Monitoring Management', () => {
		...
		test('should add PR monitoring successfully', async () => {
			...
			expect(eventLog.some((e) => e.event === 'pr:monitoring:added')).toBe(
				true
			);
		...
		test('should remove PR monitoring successfully', async () => {
			...
			expect(eventLog.some((e) => e.event === 'pr:monitoring:removed')).toBe(
				true
			);
		});
		...
		test('should handle duplicate PR monitoring', async () => {
			...
			await expect(service.addPRMonitoring(prConfig)).rejects.toThrow(
				'PR testorg/testrepo#123 already being monitored'
			);
		});
	...
	describe('PR Status Checking', () => {
		...
		test('should handle rate limiting', async () => {
			...
			await expect(service.checkPRStatus(prId)).rejects.toThrow(
		...
		test('should handle non-existent PR check', async () => {
			await expect(
				service.checkPRStatus('nonexistent/repo#999')
	...
	describe('Error Handling and Statistics', () => {
		...
		test('should handle operations when service not running', async () => {
			...
			await expect(
				service.addPRMonitoring({ owner: 'test', repo: 'test', prNumber: 1 })
