...
describe('BatchInvalidation', () => {
  ...
  beforeAll(() => {
    ...
    BatchInvalidation = class MockBatchInvalidation extends EventEmitter {
      ...
      async invalidateBatch(items, options = {}) {
        ...
        try {
          if (this.options.enableProgress) {
            this.progressTracker.startOperation('batch-invalidation', items.length);
          }
          
          const startTime = performance.now();
          const batches = this._createBatches(items, batchSize);
          const results = await this._processBatchesConcurrently(batches, concurrency, operationId);
      ...
      async _processBatchesConcurrently(batches, concurrency, operationId) {
        ...
        for (let i = 0; i < batches.length; i++) {
          const batchPromise = this._processBatch(batches[i], i, operationId)
            .then(result => {
              completed++;
              if (this.options.enableProgress) {
                this.progressTracker.updateProgress(completed * this.options.batchSize, {
                  batchIndex: i,
      ...
      _summarizeResults(results) {
        ...
        summary.averageBatchDuration = results.length > 0 ? totalDuration / results.length : 0;
      ...
      _updateAverageBatchTime(newTime) {
        if (this.stats.averageBatchTime === 0) {
          ...
          this.stats.averageBatchTime = (this.stats.averageBatchTime + newTime) / 2;
        }
      }
      ...
      async optimizeBatchSize(sampleItems) {
        ...
        const optimalSize = Math.floor(this.options.memoryThreshold / (memoryUsage / sampleItems.length));
  ...
  beforeEach(() => {
    ...
    for (let i = 0; i < 500; i++) {
      mockCache.set(`key-${i}`, { data: `value-${i}`, index: i }, {
  ...
  describe('Basic Batch Operations', () => {
    ...
    test('should respect custom batch size', async () => {
      ...
      const result = await batchInvalidator.invalidateBatch(items, { batchSize: 10 });
  ...
  describe('Error Handling and Retries', () => {
    ...
    test('should handle permanent failures after max retries', async () => {
      ...
      mockCache.delete = jest.fn().mockRejectedValue(new Error('Permanent cache error'));
    ...
    test('should continue processing other batches when one fails', async () => {
      ...
      const result = await batchInvalidator.invalidateBatch(items, { batchSize: 1 });
  ...
  describe('Progress Tracking', () => {
    test('should emit progress events during batch processing', async () => {
      ...
      expect(progressEvents[progressEvents.length - 1].percentage).toBeCloseTo(100, 0);
    });
  ...
  describe('Performance Testing', () => {
    test('should handle large batch operations efficiently', async () => {
      ...
      const items = Array.from({ length: 1500 }, (_, i) => `large-key-${i + 500}`);
      const startTime = performance.now();
    ...
    test('should optimize batch size based on memory constraints', async () => {
      ...
      const optimizedSize = await batchInvalidator.optimizeBatchSize(sampleItems);
      
      expect(optimizedSize).toBeGreaterThan(0);
      expect(optimizedSize).toBeLessThanOrEqual(batchInvalidator.options.batchSize);
    });
  ...
  describe('Operation Management', () => {
    ...
    test('should handle cancellation of non-existent operations', async () => {
      const cancelled = await batchInvalidator.cancelOperation('non-existent-id');
  ...
  describe('Edge Cases and Error Scenarios', () => {
    ...
    test('should handle zero concurrency gracefully', async () => {
      const items = ['key-1', 'key-2'];
      const result = await batchInvalidator.invalidateBatch(items, { concurrency: 1 });
