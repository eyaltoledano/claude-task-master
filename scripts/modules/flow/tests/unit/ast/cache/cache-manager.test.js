...
describe('CacheManager', () => {
	...
	beforeAll(() => {
		...
		mockContentHasher =
			require('../../../../src/ast/cache/content-hasher.js').ContentHasher;
		mockKeyGenerator =
			require('../../../../src/ast/cache/cache-key-generator.js').CacheKeyGenerator;
	});
	...
	describe('Basic Cache Operations', () => {
		...
		test('should throw error when operating on uninitialized cache', async () => {
			...
			await expect(uninitializedManager.get('key')).rejects.toThrow(
				'Cache manager not initialized'
			);
			await expect(uninitializedManager.set('key', 'value')).rejects.toThrow(
				'Cache manager not initialized'
			);
			await expect(uninitializedManager.delete('key')).rejects.toThrow(
				'Cache manager not initialized'
			);
			await expect(uninitializedManager.has('key')).rejects.toThrow(
				'Cache manager not initialized'
			);
			await expect(uninitializedManager.clear()).rejects.toThrow(
				'Cache manager not initialized'
			);
		});
	});
	...
	describe('Performance Testing', () => {
		...
		test('should handle high-volume operations efficiently', async () => {
			...
			expect(
				results.every((result, index) => result === `value-${index}`)
			).toBe(true);
		...
		test('should maintain performance under memory pressure', async () => {
			...
			for (let i = 0; i < iterations; i++) {
				await pressureCache.set(
					`pressure-${i}`,
					`large-value-${'x'.repeat(1000)}-${i}`
				);
			...
			expect(pressureCache.getStats().evictionCount).toBeGreaterThan(
				iterations - 10
			);
	...
	describe('Shutdown and Cleanup', () => {
		...
		test('should prevent operations after shutdown', async () => {
			...
			await expect(cacheManager.get('key')).rejects.toThrow(
				'Cache manager not initialized'
			);
			await expect(cacheManager.set('key', 'value')).rejects.toThrow(
				'Cache manager not initialized'
			);
		});
	});
