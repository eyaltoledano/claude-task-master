...
describe('SelectiveInvalidation', () => {
  ...
  beforeAll(() => {
    ...
    SelectiveInvalidation = class MockSelectiveInvalidation extends EventEmitter {
        ...
        for (const dependent of dependents) {
          ...
          for (const key of cacheKeys) {
            if (this.cache.has(key)) {
              await this.cache.delete(key);
              invalidated.push(key);
              this.emit('invalidated', { type: 'cascade', file: dependent, key, depth });
            }
          }
          
          // Recursively cascade if enabled
          if (options.recursive !== false) {
            const cascaded = await this._cascadeInvalidation(dependent, options, depth + 1);
      ...
      async _batchInvalidate(filePaths, options = {}) {
        ...
        for (const batch of batches) {
          const batchResults = await Promise.all(
            batch.map(filePath => this.invalidateFile(filePath, { ...options, strategy: 'direct' }))
          );
      ...
      _findCacheKeysForFile(filePath) {
        ...
        for (const key of this.cache.keys()) {
          if (key.includes(filePath) || key.includes(filePath.replace(/\\/g, '/'))) {
            keys.push(key);
          }
          
          // Check metadata for file references
          const metadata = this.cache.getMetadata(key);
          if (metadata && metadata.dependencies && metadata.dependencies.includes(filePath)) {
            keys.push(key);
          }
        }
  ...
  beforeEach(async () => {
    ...
    mockDependencyTracker.addDependency('/src/components/Header.js', '/src/utils.js');
  ...
  describe('Direct File Invalidation', () => {
    test('should invalidate cache entries for specific file', async () => {
      ...
      const result = await invalidator.invalidateFile('/src/app.js', { strategy: 'direct' });
    ...
    test('should handle non-existent files gracefully', async () => {
      const result = await invalidator.invalidateFile('/src/nonexistent.js', { strategy: 'direct' });
  ...
  describe('Cascading Invalidation', () => {
    test('should cascade invalidation to dependent files', async () => {
      const result = await invalidator.invalidateFile('/src/utils.js', { strategy: 'cascade' });
    ...
    test('should respect cascade depth limits', async () => {
      const limitedInvalidator = new SelectiveInvalidation(mockCache, mockDependencyTracker, {
        maxCascadeDepth: 1
      });
      
      const result = await limitedInvalidator.invalidateFile('/src/config.js', { strategy: 'cascade' });
    ...
    test('should emit cascade events with depth information', async () => {
      ...
      await invalidator.invalidateFile('/src/config.js', { strategy: 'cascade' });
    ...
    test('should handle circular dependencies without infinite loops', async () => {
      ...
      const result = await invalidator.invalidateFile('/src/a.js', { strategy: 'cascade' });
  ...
  describe('Batch Invalidation', () => {
    test('should invalidate multiple files efficiently', async () => {
      const filesToInvalidate = ['/src/app.js', '/src/utils.js', '/src/config.js'];
      
      const results = await invalidator.invalidateFiles(filesToInvalidate, { batch: true });
    ...
    test('should respect batch size limits', async () => {
      const smallBatchInvalidator = new SelectiveInvalidation(mockCache, mockDependencyTracker, {
        batchSize: 2
      });
      
      const filesToInvalidate = ['/src/app.js', '/src/utils.js', '/src/config.js', '/src/components/Header.js'];
      await smallBatchInvalidator.invalidateFiles(filesToInvalidate, { batch: true });
  ...
  describe('Tag-Based Invalidation', () => {
    test('should invalidate cache entries by tag', async () => {
      ...
      await mockCache.set('tagged-entry-2', 'data2', { tags: ['test-tag', 'other-tag'] });
      await mockCache.set('untagged-entry', 'data3', { tags: ['different-tag'] });
  ...
  describe('Scheduled Invalidation', () => {
    ...
    test('should emit queue processing events', async () => {
      ...
      invalidator.on('queueProcessing', (event) => processingEvents.push(event));
    ...
    test('should maintain performance with complex dependency graphs', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        for (let j = 0; j < Math.min(5, i); j++) {
          mockDependencyTracker.addDependency(`/src/file-${i}.js`, `/src/file-${j}.js`);
        }
      }
      ...
      await invalidator.invalidateFile('/src/file-0.js', { strategy: 'cascade' });
  ...
  describe('Statistics and Monitoring', () => {
    test('should track invalidation statistics', async () => {
      await invalidator.invalidateFile('/src/app.js', { strategy: 'direct' });
      await invalidator.invalidateFile('/src/utils.js', { strategy: 'cascade' });
  ...
  describe('Integration Scenarios', () => {
    test('should handle file system change simulation', async () => {
      ...
      const results = await Promise.all(
        changedFiles.map(file => invalidator.invalidateFile(file, { strategy: 'cascade' }))
      );
    ...
    test('should integrate with cache warming', async () => {
      ...
      // Warm up cache
      const warmedFiles = await invalidator.warmupCache(['/src/app.js', '/src/utils.js']);
