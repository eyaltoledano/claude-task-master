...
describe('DependencyTracker', () => {
    ...
    DependencyTracker = class MockDependencyTracker {
      ...
      getDependencies(filePath, options = {}) {
        ...
        if (options.recursive) {
          return this._getRecursiveDependencies(normalizedPath, options.maxDepth || this.options.maxDepth);
        }
      ...
      getDependents(filePath, options = {}) {
        ...
        if (options.recursive) {
          return this._getRecursiveDependents(normalizedPath, options.maxDepth || this.options.maxDepth);
        }
      ...
      _getRecursiveDependencies(filePath, maxDepth = 10, visited = new Set()) {
        ...
        for (const dep of direct) {
          result.push(dep);
          const recursive = this._getRecursiveDependencies(dep, maxDepth - 1, new Set(visited));
      ...
      _getRecursiveDependents(filePath, maxDepth = 10, visited = new Set()) {
        ...
        for (const dependent of direct) {
          ...
          const recursive = this._getRecursiveDependents(dependent, maxDepth - 1, new Set(visited));
          result.push(...recursive);
        }
      ...
      getStats() {
        return {
          ...
          averageDependencies: this.dependencies.size > 0 
            ? Array.from(this.dependencies.values()).reduce((sum, deps) => sum + deps.size, 0) / this.dependencies.size 
            : 0
        };
      }
  ...
  describe('File Management', () => {
    ...
    test('should update file dependencies', async () => {
      ...
      const result = await tracker.updateFileDependencies('/src/app.js', ['/src/config.js', '/src/api.js']);
      
      expect(result).toBe(true);
      expect(tracker.getDependencies('/src/app.js')).toEqual(['/src/config.js', '/src/api.js']);
  ...
  describe('Dependency Queries', () => {
    ...
    test('should get recursive dependents', () => {
      const dependents = tracker.getDependents('/src/constants.js', { recursive: true });
    ...
    test('should respect max depth in recursive queries', () => {
      const deps = tracker.getDependencies('/src/app.js', { recursive: true, maxDepth: 2 });
      expect(deps).toContain('/src/utils.js');
      expect(deps).toContain('/src/config.js');
  ...
  describe('Change Impact Analysis', () => {
    beforeEach(async () => {
      ...
      await tracker.addFile('/src/app.js', ['/src/components/Header.js', '/src/utils.js']);
    ...
    test('should handle multiple changed files', () => {
      const affected = tracker.getAffectedFiles(['/src/utils.js', '/src/config.js']);
  ...
  describe('Graph Import/Export', () => {
    ...
    test('should import dependency graph', async () => {
      ...
      expect(result).toBe(true);
      expect(tracker.getDependencies('/src/new-app.js')).toEqual(['/src/new-utils.js']);
      expect(tracker.getDependencies('/src/new-utils.js')).toEqual(['/src/new-config.js']);
    });
  ...
  describe('Error Handling', () => {
    ...
    test('should handle very deep dependency chains', async () => {
      ...
      const deps = tracker.getDependencies('/src/file-19.js', { recursive: true });
