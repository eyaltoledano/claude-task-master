...
describe('ComplexityScorer', () => {
		...
		ComplexityScorer = class MockComplexityScorer {
			...
			_updateAverageComplexity(score) {
				...
				this.stats.averageComplexity =
					(this.stats.averageComplexity * (count - 1) + score) / count;
			}
				...
				return {
					...
					hitRate:
						this.stats.analysisCount > 0
							? this.stats.cacheHits / this.stats.analysisCount
							: 0
				};
			}
	...
	describe('File Analysis', () => {
		test('should analyze multiple files', () => {
			...
			expect(results['/complex.js'].score).toBeGreaterThan(
				results['/simple.js'].score
			);
		});
	...
	describe('Error Handling', () => {
		...
		test('should handle malformed code gracefully', () => {
			...
			const result = scorer.analyzeComplexity(
				'/malformed.js',
				malformedContent
			);
