...
describe('CodeRelevanceScorer', () => {
	...
	beforeAll(() => {
		...
		CodeRelevanceScorer = class MockCodeRelevanceScorer {
			...
			scoreFile(filePath, content, metadata = {}) {
				...
				scores.context = this._scoreContextRelevance(
					filePath,
					content,
					metadata
				);
			...
			_scoreKeywordRelevance(filePath, content) {
				...
				if (this.taskContext.description) {
					const descWords = this.taskContext.description
						.toLowerCase()
						.split(/\s+/);
			...
			_scoreStructuralRelevance(content, metadata = {}) {
				...
				if (codeLines > 0) {
					const structuralDensity =
						(functions.length + classes.length) / codeLines;
			...
			_updateAverageScore(newScore) {
				...
				this.stats.averageScore =
					(this.stats.averageScore * (count - 1) + newScore) / count;
			}
			...
			getStats() {
				return {
					...
					hitRate:
						this.stats.scoringCount > 0
							? this.stats.cacheHits / this.stats.scoringCount
							: 0
				};
			}
	...
	describe('Structural Relevance Scoring', () => {
      ...
      `;
		...
		test('should give bonus for certain file types', () => {
			...
			expect(jsResult.breakdown.structural).toBeGreaterThan(
				mdResult.breakdown.structural
			);
		});
	...
	describe('Recency Scoring', () => {
		...
		test('should score recently changed files highly', () => {
			const recentResult = scorer.scoreFile(
				'/src/recent.js',
				'console.log("recent");'
			);
			const oldResult = scorer.scoreFile('/src/old.js', 'console.log("old");');

			expect(recentResult.breakdown.recency).toBeGreaterThan(
	...
	describe('Context Relevance Scoring', () => {
		...
		test('should score files based on task priority', () => {
			const highPriorityResult = scorer.scoreFile(
				'/src/app.js',
				'console.log("test");'
			);
			...
			const lowPriorityResult = scorer.scoreFile(
				'/src/app.js',
				'console.log("test");'
			);

			expect(highPriorityResult.breakdown.context).toBeGreaterThan(
				lowPriorityResult.breakdown.context
			);
		});
		...
		test('should give bonus to related files', () => {
			...
			const unrelatedResult = scorer.scoreFile(
				'/src/utils.js',
				'utility functions'
			);

			expect(relatedResult.breakdown.context).toBeGreaterThan(
				unrelatedResult.breakdown.context
			);
		});
	...
	describe('File Scoring and Ranking', () => {
		...
		test('should score multiple files', () => {
			...
			expect(results['/src/auth.js'].score).toBeGreaterThan(
				results['/src/utils.js'].score
			);
		});
