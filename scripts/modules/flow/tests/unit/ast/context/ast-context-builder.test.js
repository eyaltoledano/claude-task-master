...
describe('ASTContextBuilder', () => {
    ...
    ASTContextBuilder = class MockASTContextBuilder {
      constructor(options = {}) {
        this.options = {
          ...
          includePatterns: options.includePatterns || ['**/*.js', '**/*.py', '**/*.go'],
          ...
          for (const item of items) {
            ...
            if (stats.isDirectory()) {
              if (!this._isExcluded(itemPath)) {
                await this._discoverFilesRecursive(itemPath, files, visited, options);
              }
  ...
  describe('File Discovery', () => {
    beforeEach(() => {
      ...
      mockFileSystem.addFile('/project/src/app.js', 'console.log("Hello World");');
      mockFileSystem.addFile('/project/src/utils.js', 'export function helper() {}');
      mockFileSystem.addFile('/project/src/components/Button.jsx', 'export function Button() {}');
      mockFileSystem.addFile('/project/tests/app.test.js', 'test("app", () => {});');
      mockFileSystem.addFile('/project/node_modules/react/index.js', 'module.exports = React;');
    ...
    test('should exclude node_modules and .git directories', async () => {
      ...
      expect(result.context.files).not.toHaveProperty('/project/node_modules/react/index.js');
    ...
    test('should respect include patterns', async () => {
      ...
      expect(result.context.files).toHaveProperty('/project/src/components/Button.jsx');
  ...
  describe('AST Parsing', () => {
    beforeEach(() => {
      mockFileSystem.addFile('/project/app.js', `
        import React from 'react';
        export function App() {
          return <div>Hello World</div>;
        }
      `);
      mockFileSystem.addFile('/project/utils.py', `
        def calculate(x, y):
            return x + y
        ...
        class Calculator:
            pass
      `);
      mockFileSystem.addFile('/project/main.go', `
        package main
    ...
    test('should handle parse errors gracefully', async () => {
      mockFileSystem.addFile('/project/broken.js', 'invalid javascript syntax {{{');
  ...
  describe('Context Structure Building', () => {
    beforeEach(() => {
      mockFileSystem.addFile('/project/src/index.js', `
        import { helper } from './utils.js';
        export function main() {}
      `);
      mockFileSystem.addFile('/project/src/utils.js', `
  ...
  describe('Performance and Memory Management', () => {
    test('should handle large numbers of files efficiently', async () => {
      ...
      for (let i = 0; i < 200; i++) {
        mockFileSystem.addFile(`/project/src/file${i}.js`, `
    ...
    test('should handle concurrent parsing efficiently', async () => {
      ...
      const startTime = performance.now();
      const result = await contextBuilder.buildContext('/project', { concurrency: 10 });
      const endTime = performance.now();
  ...
  describe('Error Handling', () => {
    test('should handle file system errors gracefully', async () => {
      ...
      mockFileSystem.readdir = jest.fn().mockRejectedValue(new Error('Permission denied'));
