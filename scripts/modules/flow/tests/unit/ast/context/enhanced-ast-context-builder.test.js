...
describe('EnhancedASTContextBuilder', () => {
  ...
  beforeAll(() => {
    ...
    const MockRelevanceScorer = class {
      ...
      scoreFile(filePath, content, metadata = {}) {
        ...
        const typeScores = {
          'js': 0.8, 'jsx': 0.8, 'ts': 0.8, 'tsx': 0.8,
          'py': 0.7, 'go': 0.7, 'java': 0.6,
          'md': 0.3, 'txt': 0.2, 'json': 0.4
        };
    ...
    EnhancedASTContextBuilder = class MockEnhancedASTContextBuilder extends EventEmitter {
      constructor(options = {}) {
        super();
        this.options = {
        ...
        this.dependencyTracker = options.dependencyTracker || new MockDependencyTracker();
        this.relevanceScorer = options.relevanceScorer || new MockRelevanceScorer();
      ...
      async buildEnhancedContext(rootPath, taskContext = {}, options = {}) {
        ...
        try {
          ...
          if (this.options.enableIncrementalUpdates && this._canUseIncremental(rootPath, gitContext)) {
            return await this._performIncrementalUpdate(rootPath, gitContext, taskContext);
          }
      ...
      _canUseIncremental(rootPath, gitContext) {
        ...
        if (gitContext && cached.gitContext) {
          const hasNewCommits = gitContext.recentCommits.length > cached.gitContext.recentCommits.length;
          const hasNewChanges = gitContext.changedFiles.length > cached.gitContext.changedFiles.length;
      ...
      async _enhanceWithDependencies(context) {
        ...
        context.dependencyGraph = this.dependencyTracker.buildDependencyGraph(files);
      ...
      getCacheStats() {
        return {
          ...
          hitRate: this.stats.buildsCount > 0 ? this.stats.cacheHits / this.stats.buildsCount : 0,
  ...
  describe('Git Integration', () => {
    beforeEach(() => {
      ...
      mockGitProvider.addCommit('abc123', 'Add new feature', ['/project/src/feature.js']);
    ...
    test('should include Git context in enhanced build', async () => {
      ...
      expect(result.metadata.gitContext.changedFiles).toContain('/project/src/app.js');
    });
  ...
  describe('Dependency Tracking', () => {
    beforeEach(() => {
      mockDependencyTracker.addDependency('/project/src/app.js', '/project/src/utils.js');
      mockDependencyTracker.addDependency('/project/src/utils.js', '/project/src/config.js');
    });
    ...
    test('should include related files in dependencies', async () => {
      ...
      expect(utilsFile.dependencies.dependents).toContain('/project/src/app.js');
  ...
  describe('Relevance Scoring', () => {
    test('should apply relevance scoring to files', async () => {
      ...
      const result = await enhancedBuilder.buildEnhancedContext('/project', taskContext);
  ...
  describe('Incremental Updates', () => {
    test('should perform incremental updates when possible', async () => {
      ...
      mockGitProvider.addCommit('def456', 'Add new file', ['/project/src/new-file.js']);
  ...
  describe('Context Preview', () => {
    test('should provide context preview with limited files', async () => {
      const preview = await enhancedBuilder.getContextPreview('/project', {}, 1);
    ...
    test('should sort preview files by relevance', async () => {
      ...
      const preview = await enhancedBuilder.getContextPreview('/project', taskContext, 1);
  ...
  describe('Error Handling', () => {
    test('should emit error events on build failures', async () => {
      ...
      mockGitProvider.getCurrentBranch = jest.fn().mockRejectedValue(new Error('Git error'));
    ...
    test('should continue without optional features on errors', async () => {
      ...
      mockDependencyTracker.buildDependencyGraph = jest.fn().mockImplementation(() => {
