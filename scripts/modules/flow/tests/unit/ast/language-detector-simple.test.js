...
describe('LanguageDetector (Mocked)', () => {
	beforeEach(() => {
		...
		mockLanguageDetector.getSupportedLanguages.mockReturnValue([
			'javascript',
			'typescript',
			'python',
			'go'
	...
	describe('File Extension Detection', () => {
		test('should detect JavaScript files by extension', () => {
			expect(mockLanguageDetector.detectByExtension('app.js')).toBe(
				'javascript'
			);
			expect(mockLanguageDetector.detectByExtension('component.jsx')).toBe(
				'javascript'
			);
		});

		test('should detect TypeScript files by extension', () => {
			expect(mockLanguageDetector.detectByExtension('app.ts')).toBe(
				'typescript'
			);
			expect(mockLanguageDetector.detectByExtension('component.tsx')).toBe(
				'typescript'
			);
		});

		test('should detect Python files by extension', () => {
			expect(mockLanguageDetector.detectByExtension('script.py')).toBe(
				'python'
			);
		});
