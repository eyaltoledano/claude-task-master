...
class MockResourceMonitor extends EventEmitter {
	...
	registerWorktree(path, options = {}) {
		...
		if (this.worktreeResources.size >= this.options.maxWorktrees) {
			throw new Error(
				`Maximum worktree limit reached: ${this.options.maxWorktrees}`
			);
		}
		...
		const worktreeResource = {
			...
			limits: {
				memory:
					options.memoryLimit ||
					this.options.memoryThreshold / this.options.maxWorktrees,
				cpu:
					options.cpuLimit ||
					this.options.cpuThreshold / this.options.maxWorktrees,
				disk:
					options.diskLimit ||
					this.options.diskThreshold / this.options.maxWorktrees
			},
			violations: [],
	...
	async updateResourceUsage() {
		...
		try {
			...
			this.globalResources.memory.used = Math.floor(
				Math.random() * this.globalResources.memory.total * 0.8
			);
			this.globalResources.cpu.usage = Math.floor(Math.random() * 100);
			this.globalResources.disk.used = Math.floor(
				Math.random() * this.globalResources.disk.total * 0.9
			);
			...
			for (const [path, worktree] of this.worktreeResources) {
				...
				resources.memory.used = Math.floor(
					Math.random() * worktree.limits.memory
				);
				...
				resources.files.totalSize =
					resources.files.count * (Math.floor(Math.random() * 10000) + 1000);

				// Update peaks
				resources.memory.peak = Math.max(
					resources.memory.peak,
					resources.memory.used
				);
				resources.cpu.peak = Math.max(resources.cpu.peak, resources.cpu.usage);
				resources.disk.peak = Math.max(
					resources.disk.peak,
					resources.disk.used
				);
	...
	getResourceUtilization() {
		return {
			memory:
				(this.globalResources.memory.used / this.globalResources.memory.total) *
				100,
			cpu: this.globalResources.cpu.usage,
			disk:
				(this.globalResources.disk.used / this.globalResources.disk.total) * 100
		};
	}
	...
	updateStatistics() {
		...
		if (worktrees.length > 0) {
			const totalMemory = worktrees.reduce(
				(sum, w) => sum + w.resources.memory.used,
				0
			);
			const totalCpu = worktrees.reduce(
				(sum, w) => sum + w.resources.cpu.usage,
				0
			);
			const totalDisk = worktrees.reduce(
				(sum, w) => sum + w.resources.disk.used,
				0
			);
...
describe('ResourceMonitor', () => {
	...
	describe('Resource Monitoring', () => {
		...
		test('should track peak resource usage', async () => {
			...
			expect(secondUpdate.resources.memory.peak).toBeGreaterThanOrEqual(
				firstUpdate.resources.memory.peak
			);
			expect(secondUpdate.resources.cpu.peak).toBeGreaterThanOrEqual(
				firstUpdate.resources.cpu.peak
			);
			expect(secondUpdate.resources.disk.peak).toBeGreaterThanOrEqual(
				firstUpdate.resources.disk.peak
			);
		});
