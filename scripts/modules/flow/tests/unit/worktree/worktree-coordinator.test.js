...
class MockWorktreeCoordinator extends EventEmitter {
	...
	async initializeComponents() {
		...
		this.worktreeManager = {
			...
			getAllWorktrees: () =>
				Array.from(this.worktreeManager.worktrees.values()),
	...
	detectConflicts(managerWorktrees, simpleWorktrees) {
		...
		for (const managerWorktree of managerWorktrees) {
			const simpleWorktree = simpleWorktrees.find(
				(w) => w.path === managerWorktree.path
			);
			...
			try {
				switch (conflict.type) {
					case 'missing_in_simple':
						...
						const managerWorktree = this.worktreeManager
							.getAllWorktrees()
					...
					case 'missing_in_manager':
						...
						const simpleWorktree = this.simpleManager
							.getWorktrees()
							.find((w) => w.path === conflict.path);
						if (simpleWorktree) {
					...
					case 'branch_mismatch':
						...
						const simpleIndex = this.simpleManager.worktrees.findIndex(
							(w) => w.path === conflict.path
						);
						if (simpleIndex >= 0) {
							this.simpleManager.worktrees[simpleIndex].branch =
								conflict.managerBranch;
						}
						break;
				}
	...
	checkSystemHealth() {
		...
		health.components.worktreeManager = this.worktreeManager?.isInitialized
			? 'healthy'
			: 'unhealthy';
		health.components.simpleManager = this.simpleManager?.isInitialized
			? 'healthy'
			: 'unhealthy';
		health.components.resourceMonitor = this.resourceMonitor?.isInitialized
			? 'healthy'
			: 'unhealthy';
...
describe('WorktreeCoordinator', () => {
	...
	describe('Component Synchronization', () => {
		...
		test('should handle sync errors gracefully', async () => {
			...
			// Force an error
			coordinator.worktreeManager.getAllWorktrees = jest
				.fn()
				.mockImplementation(() => {
					throw new Error('Sync test error');
				});

			await expect(coordinator.syncComponents()).rejects.toThrow(
				'Sync test error'
			);
	...
	describe('Coordination Cycle', () => {
		...
		test('should handle coordination cycle errors', async () => {
			...
			coordinator.syncComponents = jest
				.fn()
				.mockRejectedValue(new Error('Cycle test error'));

			await expect(coordinator.coordinationCycle()).rejects.toThrow(
				'Cycle test error'
			);
	...
	describe('Operation Queue Management', () => {
		...
		test('should handle operation failures', async () => {
			...
			await expect(
				coordinator.queueOperation(failingOperation)
	...
	describe('Component Status and Statistics', () => {
		...
		test('should track operation statistics', async () => {
			...
			expect(finalStats.coordinationCycles).toBe(
				initialStats.coordinationCycles + 1
			);
		});
	});
	...
	describe('Cleanup and Destruction', () => {
		...
		test('should perform cleanup when enabled', async () => {
			...
			coordinator.worktreeManager.cleanup = jest
				.fn()
				.mockResolvedValue(['/test/cleaned']);
