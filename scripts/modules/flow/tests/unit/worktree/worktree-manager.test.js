	...
	async initialize() {
		...
		try {
			...
			await new Promise((resolve) =>
				setTimeout(resolve, Math.max(1, Math.random() * 10))
			);
	...
	async discoverWorktrees() {
		...
		try {
			...
			const mockWorktrees = [
				...
				{
					path: '/mock/project-feature-1',
					branch: 'feature/user-auth',
					isMain: false
				},
				{
					path: '/mock/project-feature-2',
		...
		for (const [path, worktree] of this.worktrees) {
			if (!worktree.isMain && worktree.status !== 'locked') {
				...
				if (daysSinceAccess > 7) {
					// Clean up worktrees not accessed for 7 days
...
describe('WorktreeManager', () => {
	...
	describe('Initialization', () => {
		...
		test('should throw error if already initialized', async () => {
			...
			await expect(manager.initialize()).rejects.toThrow(
				'WorktreeManager already initialized'
			);
		});
	...
	describe('Worktree Discovery', () => {
		...
		test('should throw error if discovery already in progress', async () => {
			...
			await expect(discoveryPromise2).rejects.toThrow(
		...
		test('should update discovery statistics', async () => {
			...
			expect(finalStats.discoveredWorktrees).toBeGreaterThan(
				initialStats.discoveredWorktrees
			);
	...
	describe('Worktree Management', () => {
		...
		test('should enforce maximum worktree limit', async () => {
			...
			for (
				let i = 0;
				i < manager.options.maxWorktrees - manager.worktrees.size;
				i++
			) {
			...
			await expect(
				manager.addWorktree('/test/project-overflow', 'feature/overflow')
			).rejects.toThrow(
		...
		test('should throw error when removing non-existent worktree', async () => {
			await expect(
				manager.removeWorktree('/non/existent/path')
			).rejects.toThrow('Worktree not found at path: /non/existent/path');
		});
	});
	...
	describe('File Watching', () => {
		...
		test('should handle file change events', (done) => {
			...
			manager
				.addWorktree('/test/project-file-change', 'feature/file-change')
				.then(() => {
					manager.simulateFileEvent(
						'/test/project-file-change',
						'change',
						'src/index.js'
					);
				});
	...
	describe('Cleanup Operations', () => {
		...
		test('should cleanup old worktrees', async () => {
			...
			oldWorktree.lastAccessed = new Date(
		...
		test('should not cleanup main worktree', async () => {
			const mainWorktree = manager.getMainWorktree();
			mainWorktree.lastAccessed = new Date(
		...
		test('should not cleanup locked worktrees', async () => {
			...
			const worktree = manager.getWorktree(path);
			worktree.lastAccessed = new Date(
				Date.now() - 30 * 24 * 60 * 60 * 1000
			).toISOString(); // 30 days ago
			await manager.lockWorktree(path);
	...
	describe('Operation Queue Management', () => {
		...
		test('should handle operation failures', async () => {
			...
			await expect(manager.queueOperation(operation)).rejects.toThrow(
				'Operation failed'
			);
	...
	describe('Statistics and Monitoring', () => {
		...
		test('should track operation statistics', async () => {
			...
			expect(finalStats.createdWorktrees).toBe(
				initialStats.createdWorktrees + 2
			);
			expect(finalStats.removedWorktrees).toBe(
				initialStats.removedWorktrees + 1
			);
			expect(finalStats.totalOperations).toBeGreaterThan(
				initialStats.totalOperations
			);
		});
	...
	describe('Performance Tests', () => {
		...
		test('should maintain performance with large number of worktrees', async () => {
			...
			for (let i = 0; i < 50; i++) {
				await largeManager.addWorktree(
					`/test/large-${i}`,
					`feature/large-${i}`
				);
			}
