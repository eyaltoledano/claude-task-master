...
(async () => {
	try {
		...
		if (currentScreenFromNav === 'screen1' && canGoBack && canGoForward) {
			...
			console.log(
				`Debug: current screen: ${currentScreenFromNav}, canGoBack: ${canGoBack}, canGoForward: ${canGoForward}`
			);
			console.log(
				`Debug: history: ${JSON.stringify(navState.history)}, currentIndex: ${navState.currentIndex}`
			);
		...
		if (
			resetUIScreen === 'welcome' &&
			resetTasks.length === 0 &&
			resetTheme === 'auto' &&
			resetNavHistory.length === 1
		) {
		...
		console.log(
			`✅ Performance test: 3000 state changes completed in ${duration.toFixed(2)}ms`
		);
