...
class FlowTestRunner {
    ...
    showHelp() {
        ...
        this.log('  integration   Run integration tests (real components)', 'white');
        ...
        this.log('\nExamples:', 'bright');
        this.log('  node test-runner.js                    # Run all tests', 'green');
        this.log('  node test-runner.js unit               # Run all unit tests', 'green');
        this.log('  node test-runner.js ast                # Run AST tests', 'green');
        this.log('  node test-runner.js unit ast           # Run unit AST tests', 'green');
        this.log('  node test-runner.js --watch            # Watch mode', 'green');
        this.log('  node test-runner.js --coverage         # With coverage', 'green');
        this.log('');
    }
    ...
    processJestResult(result, testPath) {
        ...
        const suiteMatch = output.match(/Test Suites:\s*(\d+)\s*failed.*?(\d+)\s*passed.*?(\d+)\s*total/);
    ...
    printSummary() {
        ...
        this.log(`   Success Rate: ${successRate}%`, this.results.failed > 0 ? 'red' : 'green');
