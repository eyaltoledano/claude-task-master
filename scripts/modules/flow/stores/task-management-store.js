...
export const useTaskManagementStore = create(
	subscribeWithSelector((set, get) => ({
		...
		setStatusFilter: (filter) =>
			set({ statusFilter: filter, priorityFilter: 'all' }),
		setPriorityFilter: (filter) =>
			set({ priorityFilter: filter, statusFilter: 'all' }),
		setSearchQuery: (query) => set({ searchQuery: query }),
		setIsSearching: (isSearching) => set({ isSearching }),

		setShowExpandOptions: (show) => set({ showExpandOptions: show }),
		setShowClaudeLauncherModal: (show) =>
			set({ showClaudeLauncherModal: show }),
		setShowBranchConflictModal: (show) =>
			set({ showBranchConflictModal: show }),
