...
const defaultMigrate = (persistedState, version) => {
	console.log(
		`[State Migration] Migrating ${persistedState?.name || 'store'} from version ${version}`
	);
...
export const storeMigrations = {
	...
	preferences: {
		...
		v1_to_v2: (state) =>
			migrationUtils.addFields(state, {
...
export const createMigrationChain = (migrations, storeName) => {
	return (persistedState, version) => {
		...
		for (const migrationKey of migrationKeys) {
			const [fromVersion, toVersion] = migrationKey
				.split('_to_')
				.map((v) => parseInt(v.replace('v', '')));

			if (currentVersion === fromVersion) {
				console.log(
					`[${storeName}] Migrating from v${fromVersion} to v${toVersion}`
				);
...
export const storageUtils = {
	...
	clearAll: () => {
		const keys = Object.keys(localStorage).filter(
			(key) => key.startsWith('taskmaster-') || key.startsWith('tm-')
		);
	...
	importState: (backup) => {
		...
		console.log(
			`[Storage] Imported ${Object.keys(backup.stores).length} stores`
		);
...
export const persistenceMonitor = {
	...
	endTimer: (operation, storeName) => {
		...
		if (startTime) {
			...
			console.log(
				`[Persistence] ${storeName} ${operation} took ${duration.toFixed(2)}ms`
			);
