...
class IntegratedWorktreeManager extends EventEmitter {
	...
	setupComponentIntegration() {
		...
		this.coordinator.on('operationFailed', (data) => {
			console.warn(
				`Git operation failed in worktree coordination: ${data.error.message}`
			);
			this.integrationMetrics.coordinationEvents++;
		});
	...
	getSystemStatus() {
		return {
			...
			uptime: this.integrationMetrics.startTime
				? Date.now() - this.integrationMetrics.startTime
				: 0,
			components: {
				worktreeManager: {
					status: 'active',
	...
	calculateEventsPerMinute() {
		...
		const uptimeMinutes =
			(Date.now() - this.integrationMetrics.startTime) / (1000 * 60);
		const totalEvents =
			this.integrationMetrics.worktreeEvents +
			this.integrationMetrics.resourceEvents +
			this.integrationMetrics.coordinationEvents;
	...
	async healthCheck() {
		...
		try {
			...
			if (coordStats.successRate < 90) {
				health.status = 'degraded';
				health.issues.push(
					`Low coordination success rate: ${coordStats.successRate}%`
				);
			}
		} catch (error) {
			health.status = 'degraded';
