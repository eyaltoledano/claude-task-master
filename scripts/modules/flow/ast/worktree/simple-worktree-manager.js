...
class SimpleWorktreeManager extends EventEmitter {
	constructor(options = {}) {
		...
		console.log(
			'SimpleWorktreeManager initialized with equal treatment for all worktrees'
		);
	}
	...
	async start() {
		try {
			...
			if (this.resourceMonitor) {
				...
				this.resourceMonitor.on('degradation', () =>
			...
			console.log(
				`SimpleWorktreeManager started, managing ${this.worktrees.size} worktrees`
			);
		} catch (error) {
	...
	async processGitQueue() {
		...
		try {
			while (this.gitQueue.length > 0) {
				const { command, workingDirectory, resolve, reject } =
					this.gitQueue.shift();
