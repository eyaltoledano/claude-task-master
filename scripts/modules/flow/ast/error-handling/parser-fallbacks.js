...
export class ParserFallbacks extends EventEmitter {
    constructor(options = {}) {
        ...
        console.log('ParserFallbacks initialized for fast development error recovery');
    }
    ...
    async parseWithFallbacks(filePath, content, language, primaryParser) {
        ...
        try {
            ...
            const result = await this.tryPrimaryParser(primaryParser, filePath, content);
            if (result.success) {
                return result;
            }
            ...
            const fallbackResult = await this.executeFallbackStrategy(filePath, content, language, result.error);
            
            if (fallbackResult.success) {
                this.metrics.successfulFallbacks++;
                const duration = Date.now() - startTime;
                this.metrics.fastestFallback = Math.min(this.metrics.fastestFallback, duration);
                this.metrics.slowestFallback = Math.max(this.metrics.slowestFallback, duration);
    ...
    async executeFallbackStrategy(filePath, content, language, primaryError) {
        ...
        const strategies = cachedStrategy ? [cachedStrategy] : this.getFallbackStrategies(language);
        
        for (const strategy of strategies) {
            try {
                const result = await this.tryFallbackStrategy(strategy, filePath, content, language);
                ...
                console.warn(`Fallback strategy '${strategy.name}' failed:`, error.message);
            }
        }
    ...
    regexJavaScript(content, result) {
        ...
        result.complexity = Math.max(1, Math.floor(result.functions.length * 0.3 + result.classes.length * 0.5));
        return result;
    }
    ...
    regexPython(content, result) {
        ...
        result.complexity = Math.max(1, Math.floor(result.functions.length * 0.2 + result.classes.length * 0.4));
        return result;
    }
    ...
    regexGo(content, result) {
        // Functions
        const funcMatches = content.match(/func\s+(?:\([^)]*\)\s+)?(\w+)\s*\([^)]*\)/g);
        if (funcMatches) {
            result.functions = funcMatches.map(match => ({
        ...
        if (importMatches) {
            result.imports = importMatches.flatMap(match => {
                if (match.includes('(')) {
                    ...
                    return imports ? imports.map(imp => ({
    ...
    regexGeneric(content, result) {
        ...
        result.complexity = Math.max(1, result.functions.length + result.classes.length);
        return result;
    }
    ...
    async contentAnalysisFallback(filePath, content, language) {
        ...
        const stats = {
            ...
            indentedLines: lines.filter(line => line.startsWith('  ') || line.startsWith('\t')).length,
        ...
        result.complexity = Math.max(1, Math.floor(
        ...
        if (stats.functionKeywords > 0) {
            for (let i = 0; i < stats.functionKeywords; i++) {
                result.functions.push({
                    ...
                    lineStart: Math.floor((i + 1) * (stats.totalLines / stats.functionKeywords)),
                    complexity: 1
                });
            }
        }
    ...
    getStatistics() {
        return {
            ...
            averageFallbackTime: this.metrics.successfulFallbacks > 0 ? 
                (this.metrics.slowestFallback + this.metrics.fastestFallback) / 2 : 0
        };
    }
