...
export class ASTValidator extends EventEmitter {
	constructor(options = {}) {
		...
		console.log(
			'ASTValidator initialized for fast development quality assurance'
		);
	}
	...
	async validateAST(result, filePath, originalContent, language) {
		...
		try {
			...
			this.stats.fastestValidation = Math.min(
				this.stats.fastestValidation,
				duration
			);
			this.stats.slowestValidation = Math.max(
				this.stats.slowestValidation,
				duration
			);
			...
			this.stats.averageConfidence =
				(this.stats.averageConfidence * (this.stats.totalValidations - 1) +
	...
	async performValidation(result, filePath, originalContent, language) {
		...
		validation.isValid =
			validation.confidence >= this.config.minConfidenceScore;
	...
	validateFunctions(functions) {
		...
		for (const func of functions) {
			...
			if (
				func.lineStart !== undefined &&
				(typeof func.lineStart !== 'number' || func.lineStart < 1)
			) {
				funcIssues.push('Invalid lineStart value');
			}

			if (
				func.complexity !== undefined &&
				(typeof func.complexity !== 'number' || func.complexity < 1)
			) {
			...
			if (funcIssues.length > 0) {
				...
				validation.issues.push(
					`Function ${func.name || 'unknown'}: ${funcIssues.join(', ')}`
				);
			} else {
	...
	validateClasses(classes) {
		...
		for (const cls of classes) {
			...
			if (classIssues.length > 0) {
				validation.invalidCount++;
				validation.issues.push(
					`Class ${cls.name || 'unknown'}: ${classIssues.join(', ')}`
				);
			} else {
				validation.validCount++;
			}
	...
	validateImports(imports) {
		...
		for (const imp of imports) {
			...
			if (importIssues.length > 0) {
				validation.invalidCount++;
				validation.issues.push(
					`Import ${imp.source || 'unknown'}: ${importIssues.join(', ')}`
				);
			} else {
				validation.validCount++;
			}
	...
	async validateContent(result, originalContent, language) {
		...
		if (result.functions) {
			...
			if (parsedCount < estimatedCount * 0.5) {
				check.warnings.push(
					`Parsed ${parsedCount} functions, estimated ${estimatedCount} - may be missing functions`
				);
				check.confidence -= 0.2;
			} else if (parsedCount > estimatedCount * 2) {
				check.warnings.push(
					`Parsed ${parsedCount} functions, estimated ${estimatedCount} - may have false positives`
				);
				check.confidence -= 0.1;
			}
		}
		...
		if (result.classes) {
			...
			if (parsedCount < estimatedCount * 0.7) {
				check.warnings.push(
					`Parsed ${parsedCount} classes, estimated ${estimatedCount} - may be missing classes`
				);
				check.confidence -= 0.15;
			}
		}
		...
		if (result.imports) {
			...
			if (parsedCount < estimatedCount * 0.5) {
				check.warnings.push(
					`Parsed ${parsedCount} imports, estimated ${estimatedCount} - may be missing imports`
				);
				check.confidence -= 0.1;
			}
		}
		...
		if (result.complexity) {
			...
			if (
				result.complexity < estimatedComplexity * 0.3 ||
				result.complexity > estimatedComplexity * 3
			) {
				check.warnings.push(
					`Complexity ${result.complexity} seems unusual (estimated ${estimatedComplexity})`
				);
				check.confidence -= 0.05;
			}
		}
	...
	analyzeContent(content, language) {
		...
		const analysis = {
			...
			codeLines: lines.filter(
				(line) => line.trim() && !this.isComment(line, language)
			).length,
		...
		switch (language.toLowerCase()) {
			case 'javascript':
			case 'typescript':
				analysis.estimatedFunctions = (
					content.match(
						/(?:function\s+\w+|(?:const|let|var)\s+\w+\s*=\s*(?:function|\([^)]*\)\s*=>))/g
					) || []
				).length;
				analysis.estimatedClasses = (
					content.match(/class\s+\w+/g) || []
				).length;
				analysis.estimatedImports = (
			...
			case 'python':
				analysis.estimatedFunctions = (
					content.match(/def\s+\w+/g) || []
				).length;
				analysis.estimatedClasses = (
					content.match(/class\s+\w+/g) || []
				).length;
				analysis.estimatedImports = (
					content.match(/(?:from\s+\w+\s+import|import\s+\w+)/g) || []
				).length;
				break;

			case 'go':
				analysis.estimatedFunctions = (
					content.match(/func\s+(?:\([^)]*\)\s+)?\w+/g) || []
				).length;
				analysis.estimatedClasses = (
					content.match(/type\s+\w+\s+struct/g) || []
				).length;
				analysis.estimatedImports = (
					content.match(/import\s+(?:\(\s*[^)]+\s*\)|"[^"]+")/) || []
				).length;
				break;
			...
			default:
				// Generic estimation
				analysis.estimatedFunctions = (
					content.match(/(function|def|func)\s+\w+/g) || []
				).length;
				analysis.estimatedClasses = (
					content.match(/(class|struct|type)\s+\w+/g) || []
				).length;
				analysis.estimatedImports = (
					content.match(/(import|include|require)/g) || []
				).length;
		}

		// Estimate complexity
		analysis.estimatedComplexity = Math.max(
			1,
			Math.floor(
	...
	async validateConsistency(result, filePath, language) {
		...
		if (result.functions && result.functions.length > 0) {
			...
			if (duplicates.length > 0) {
				check.warnings.push(
					`Duplicate function names: ${[...new Set(duplicates)].join(', ')}`
				);
				check.confidence -= 0.1;
			}
		}
		...
		if (result.classes && result.classes.length > 0) {
			...
			if (duplicates.length > 0) {
				check.warnings.push(
					`Duplicate class names: ${[...new Set(duplicates)].join(', ')}`
				);
				check.confidence -= 0.1;
			}
		}
		...
		if (result.functions) {
			const invalidLines = result.functions.filter(
				(f) => f.lineStart && f.lineStart < 1
			);
			if (invalidLines.length > 0) {
				check.warnings.push(
					`${invalidLines.length} functions have invalid line numbers`
				);
				check.confidence -= 0.05;
			}
		}
		...
		if (result.complexity && result.functions) {
			const funcComplexity = result.functions.reduce(
				(sum, f) => sum + (f.complexity || 1),
				0
			);
			if (
				result.complexity < funcComplexity * 0.5 ||
				result.complexity > funcComplexity * 3
			) {
				check.warnings.push(
					`Overall complexity (${result.complexity}) inconsistent with function complexities (${funcComplexity})`
				);
				check.confidence -= 0.05;
			}
		}
	...
	async quickValidate(result) {
		if (!result || typeof result !== 'object') {
			return {
				isValid: false,
				confidence: 0,
				message: 'Invalid result object'
			};
		}

		// Basic structure check
		const hasRequiredFields = this.rules.requiredFields.every(
	...
	getStatistics() {
		return {
			...
			averageValidationTime:
				this.stats.totalValidations > 0
					? (this.stats.slowestValidation + this.stats.fastestValidation) / 2
					: 0
		};
	}
