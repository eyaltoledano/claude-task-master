...
export class AnalyzerDispatcher {
    ...
    async analyzeCode(astData, filePath, content, language) {
        ...
        try {
            ...
            if (analyzer === this.analyzers.generic) {
                analysis = await analyzer.analyzeGeneric(astData, filePath, content, language);
            } else {
                // Use language-specific analyzer
                analysis = await this.performLanguageSpecificAnalysis(
                    analyzer, astData, filePath, content, language
                );
            }
            ...
            if (analyzer !== this.analyzers.generic && this.options.fallbackToGeneric) {
                analysis.genericInsights = await this.analyzers.generic.analyzeGeneric(
                    astData, filePath, content, language
                );
            }
            ...
            console.warn(`[AnalyzerDispatcher] Analysis failed for ${language}:`, error.message);
            
            // Fallback to generic analyzer
            if (this.options.fallbackToGeneric) {
                return await this.analyzers.generic.analyzeGeneric(astData, filePath, content, language);
            }
    ...
    selectAnalyzer(language) {
        ...
        if (this.analyzers[normalizedLanguage] && this.options.enableLanguageSpecific) {
        ...
        if (mappedLanguage && this.analyzers[mappedLanguage] && this.options.enableLanguageSpecific) {
     ...
     */
    async performLanguageSpecificAnalysis(analyzer, astData, filePath, content, language) {
        const normalizedLanguage = language.toLowerCase();
        ...
        if (analyzer === this.analyzers.javascript) {
            ...
            return await analyzer.analyzeGeneric(astData, filePath, content, language);
        }
    }
    ...
    getAnalysisSummary(analysisResults) {
        ...
        analysisResults.forEach(result => {
            if (result.success) {
                ...
                summary.analyzerBreakdown[analyzer] = (summary.analyzerBreakdown[analyzer] || 0) + 1;
