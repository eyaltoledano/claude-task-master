...
export class JavaScriptAnalyzer {
    ...
    detectLanguageVariant(filePath, content) {
        ...
        const hasModernFeatures = /(?:import\s+.*from|export\s+(?:default\s+)?(?:class|function|const)|class\s+\w+|async\s+function|\=\>)/.test(content);
    ...
    detectReactFramework(astData, content) {
        ...
        const reactImports = imports.filter(imp => 
            imp.source && (imp.source.includes('react') || imp.source.includes('React'))
        );
        ...
        const componentFunctions = functions.filter(func => 
            func.name && /^[A-Z]/.test(func.name) && func.returns
        );
    ...
    detectExpressFramework(astData, content) {
        ...
        // Express imports
        const imports = astData.imports || [];
        const expressImports = imports.filter(imp => 
            imp.source && imp.source.includes('express')
        );
    ...
    detectNodeJSFramework(astData, content) {
        ...
        const nodeModules = ['fs', 'path', 'http', 'https', 'crypto', 'util', 'stream', 'events'];
        const imports = astData.imports || [];
        
        const nodeImports = imports.filter(imp => 
            imp.source && nodeModules.includes(imp.source)
        );

        if (nodeImports.length > 0) {
            confidence += 0.3;
            evidence.push(`Node.js built-in modules: ${nodeImports.map(i => i.source).join(', ')}`);
        }
    ...
    async analyzeJavaScriptPatterns(astData, content) {
        ...
        const basePatterns = await this.codeAnalyzer.analyzePatterns(astData, 'javascript', null, content);
    ...
    analyzeModernJSPatterns(content) {
        ...
        const modernFeatures = [
            { pattern: /const\s+\w+\s*=/, name: 'const_declarations', description: 'Const variable declarations' },
            { pattern: /let\s+\w+\s*=/, name: 'let_declarations', description: 'Let variable declarations' },
            { pattern: /\([^)]*\)\s*=>/, name: 'arrow_functions', description: 'Arrow functions' },
            { pattern: /`[^`]*\$\{[^}]*\}[^`]*`/, name: 'template_literals', description: 'Template literals with interpolation' },
            { pattern: /\[([^,\]]+),\s*\.\.\.(\w+)\]/, name: 'array_destructuring', description: 'Array destructuring with spread' },
            { pattern: /\{([^}]+)\}\s*=/, name: 'object_destructuring', description: 'Object destructuring' },
            { pattern: /class\s+\w+\s*(?:extends\s+\w+)?/, name: 'es6_classes', description: 'ES6 class declarations' },
            { pattern: /async\s+function|\basync\s*\(|\basync\s+\w+\s*=>/, name: 'async_functions', description: 'Async function declarations' }
        ];
    ...
    analyzeAsyncPatterns(content) {
        ...
        // Promise patterns
        const promiseCount = (content.match(/\.then\s*\(|\.catch\s*\(|new Promise\s*\(/g) || []).length;
        if (promiseCount > 0) {
            patterns.push({
                type: 'promise_usage',
                count: promiseCount,
        ...
        // Callback patterns
        const callbackCount = (content.match(/function\s*\([^)]*err[^)]*\)|callback\s*\(|cb\s*\(/g) || []).length;
        if (callbackCount > 0) {
            patterns.push({
    ...
    analyzeTestPatterns(content) {
        ...
        // Jest patterns
        const jestPatterns = (content.match(/describe\s*\(|it\s*\(|test\s*\(|expect\s*\(/g) || []).length;
        if (jestPatterns > 0) {
            patterns.push({
                type: 'jest_testing',
                count: jestPatterns,
        ...
        // Mocha patterns
        const mochaPatterns = (content.match(/describe\s*\(.*function|it\s*\(.*function|beforeEach|afterEach/g) || []).length;
        if (mochaPatterns > 0) {
            patterns.push({
                type: 'mocha_testing',
                count: mochaPatterns,
    ...
    analyzePerformancePatterns(content) {
        ...
        // Web Performance API
        if (/performance\.now|performance\.mark|performance\.measure/.test(content)) {
            patterns.push({
    ...
    async analyzeComplexity(astData, content) {
        return await this.complexityScorer.calculateComplexity(astData, 'javascript', content);
    }
    ...
    async analyzeCodeQuality(astData, content) {
        ...
        if (longFunctions.length > 0) {
            ...
            quality.suggestions.push('Consider breaking down large functions into smaller, focused functions');
        }
        ...
        if (nestedPatterns && nestedPatterns.length > 5) {
            ...
            quality.suggestions.push('Reduce nesting with early returns and guard clauses');
        }
        ...
        if (consoleLogs > 2) {
            ...
            quality.suggestions.push('Remove debug console.log statements or use proper logging');
        }
    ...
    generateJavaScriptRecommendations(analysis) {
        ...
        const hasModernPatterns = analysis.patterns.modernJS && analysis.patterns.modernJS.length > 0;
