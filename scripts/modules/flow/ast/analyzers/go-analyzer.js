...
export class GoAnalyzer {
    ...
    detectGoVersion(content) {
        ...
        if (/fmt\.Errorf.*%w/.test(content) || /errors\.Is|errors\.As/.test(content)) {
            return '1.13+';
        }
    ...
    detectTestingFramework(content, astData) {
        ...
        const testFunctions = functions.filter(func => 
            func.name && func.name.startsWith('Test')
        );
        ...
        // Benchmark functions
        const benchmarkFunctions = functions.filter(func => 
            func.name && func.name.startsWith('Benchmark')
        );
    ...
    async analyzeGoPatterns(astData, content) {
        ...
        const basePatterns = await this.codeAnalyzer.analyzePatterns(astData, 'go', null, content);
    ...
    analyzeNamingConventions(content, astData) {
        ...
        const privateFunctions = functions.filter(func => 
            func.name && /^[a-z]/.test(func.name)
        );
    ...
    async analyzeComplexity(astData, content) {
        return await this.complexityScorer.calculateComplexity(astData, 'go', content);
    }
    ...
    async analyzeCodeQuality(astData, content, filePath) {
        ...
        if (errorReturns > errorChecks + 2) {
            ...
            quality.suggestions.push('Ensure all errors are properly checked and handled');
        }
        ...
        const goroutines = (content.match(/go\s+\w+\s*\(|go\s+func\s*\(/g) || []).length;
        const contextUsage = (content.match(/context\.|ctx\s+context\.Context/g) || []).length;
        ...
        if (goroutines > 2 && contextUsage === 0) {
            ...
            quality.suggestions.push('Use context.Context for goroutine cancellation and timeouts');
        }
    ...
    generateGoRecommendations(analysis) {
        ...
        const hasConcurrency = analysis.patterns.concurrency && analysis.patterns.concurrency.length > 0;
        if (hasConcurrency) {
            recommendations.push({
        ...
        const hasErrorHandling = analysis.patterns.errorHandling && analysis.patterns.errorHandling.length > 0;
        if (hasErrorHandling) {
            const explicitChecks = analysis.patterns.errorHandling.find(p => p.type === 'explicit_error_handling');
        ...
        if (analysis.goVersion === 'unknown' || !analysis.goVersion.includes('1.1')) {
            recommendations.push({
                type: 'modernization',
                priority: 'medium',
