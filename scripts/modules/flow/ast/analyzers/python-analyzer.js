...
export class PythonAnalyzer {
    ...
    detectDjangoFramework(astData, content) {
        ...
        const djangoImports = imports.filter(imp => 
            imp.source && imp.source.includes('django')
        );

        if (djangoImports.length > 0) {
            confidence += 0.5;
            evidence.push(`Django imports: ${djangoImports.map(i => i.source).join(', ')}`);
        }

        // Django patterns
        const djangoPatterns = [
            { pattern: /class\s+\w+\(models\.Model\):/, description: 'Django model classes' },
            { pattern: /def\s+\w+\(request[^)]*\):/, description: 'Django view functions' },
            { pattern: /@login_required|@permission_required/, description: 'Django decorators' },
            { pattern: /urlpatterns\s*=/, description: 'Django URL patterns' },
            { pattern: /INSTALLED_APPS|DATABASES|SECRET_KEY/, description: 'Django settings' }
        ];
    ...
    detectFastAPIFramework(astData, content) {
        ...
        // FastAPI imports
        const imports = astData.imports || [];
        const fastapiImports = imports.filter(imp => 
            imp.source && imp.source.includes('fastapi')
        );
    ...
    detectFlaskFramework(astData, content) {
        ...
        // Flask imports
        const imports = astData.imports || [];
        const flaskImports = imports.filter(imp => 
            imp.source && imp.source.includes('flask')
        );
    ...
    async analyzePythonPatterns(astData, content) {
        ...
        const basePatterns = await this.codeAnalyzer.analyzePatterns(astData, 'python', null, content);
    ...
    analyzeDecoratorPatterns(content) {
        ...
        const decoratorPatterns = [
            { pattern: /@property/, name: 'property_decorators', description: 'Property decorators' },
            { pattern: /@staticmethod/, name: 'staticmethod_decorators', description: 'Static method decorators' },
            { pattern: /@classmethod/, name: 'classmethod_decorators', description: 'Class method decorators' },
            { pattern: /@dataclass/, name: 'dataclass_decorators', description: 'Dataclass decorators' },
            { pattern: /@functools\./, name: 'functools_decorators', description: 'Functools decorators' },
            { pattern: /@app\.|@bp\./, name: 'route_decorators', description: 'Route decorators' }
        ];
    ...
    analyzeTypeHints(content) {
        const functions = content.match(/def\s+\w+\([^)]*\)(?:\s*->\s*\w+)?:/g) || [];
        const typedFunctions = content.match(/def\s+\w+\([^)]*\)\s*->\s*\w+:/g) || [];
        
        const coverage = functions.length > 0 ? (typedFunctions.length / functions.length) * 100 : 0;
    ...
    async analyzeComplexity(astData, content) {
        return await this.complexityScorer.calculateComplexity(astData, 'python', content);
    }
    ...
    async analyzeCodeQuality(astData, content) {
        ...
        if (longFunctions.length > 0) {
            ...
            quality.suggestions.push('Break down large functions following single responsibility principle');
        }

        // Check for missing docstrings
        const functionsWithoutDocstrings = functions.filter(func => 
        ...
        if (functionsWithoutDocstrings.length > functions.length * 0.5) {
            ...
            quality.suggestions.push('Add docstrings to document function behavior and parameters');
        }
        ...
        if (printStatements > 2) {
            ...
            quality.suggestions.push('Use logging module instead of print statements');
        }
        ...
        if (todoComments > 3) {
            ...
            quality.suggestions.push('Address TODO/FIXME comments or create proper issues');
        }
    ...
    generatePythonRecommendations(analysis) {
        ...
        if (analysis.pythonVersion === 'unknown' || analysis.pythonVersion.startsWith('2.')) {
            recommendations.push({
                type: 'modernization',
                priority: 'high',
        ...
        const hasAsyncPatterns = analysis.patterns.asyncPatterns && analysis.patterns.asyncPatterns.length > 0;
        if (hasAsyncPatterns) {
            recommendations.push({
