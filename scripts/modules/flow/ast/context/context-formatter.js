...
export async function formatASTContext(filteredResults, options = {}) {
	const { config, tasks = [], includeComplexity = true, includeImports = true } = options;
	...
	for (const [language, results] of Object.entries(filteredResults)) {
		...
		const functionsSection = formatFunctionsAndClasses(results, { includeComplexity });
		if (functionsSection) {
	...
	if (tasks.length > 0) {
		const recommendations = generateImplementationRecommendations(filteredResults, tasks);
		if (recommendations) {
...
function generateSummary(filteredResults) {
	...
	for (const results of Object.values(filteredResults)) {
		...
		for (const result of results) {
			if (result.ast.functions) stats.totalFunctions += result.ast.functions.length;
	...
	stats.avgComplexity = complexityCount > 0 ? (complexitySum / complexityCount).toFixed(1) : 0;
...
function categorizeImport(importName, language) {
	...
	// Framework imports
	const frameworks = {
		javascript: ['react', 'vue', 'angular', 'express', 'fastify', 'koa', 'next', 'nuxt'],
	...
	if (importName.startsWith('.') || importName.startsWith('/') || importName.includes('./')) {
		return 'local';
	}
	...
	const builtins = {
		...
		python: ['os', 'sys', 'json', 'time', 'datetime', 'collections', 're', 'math'],
		go: ['fmt', 'os', 'time', 'strings', 'encoding/json', 'net/http', 'context']
	};
	
	if (builtins[language]?.some(builtin => lower === builtin || lower.startsWith(builtin + '/'))) {
		return 'builtin';
	}
...
function detectCodePatterns(results, language) {
	...
	// Async patterns
	const asyncFunctions = allFunctions.filter(f => f.isAsync || f.name.includes('async'));
	if (asyncFunctions.length > 0) {
		patterns.push(`Async operations: ${asyncFunctions.length} async functions found`);
	}
	
	// Error handling patterns
	const errorHandlingFunctions = allFunctions.filter(f => 
	...
	if (errorHandlingFunctions.length > 0) {
		patterns.push(`Error handling: ${errorHandlingFunctions.length} error-related functions`);
	}
	
	// Test patterns
	const testFunctions = allFunctions.filter(f => 
	...
	// CRUD patterns
	const crudPatterns = ['create', 'read', 'update', 'delete', 'get', 'set', 'add', 'remove'];
	...
	if (crudFunctions.length > 0) {
		patterns.push(`CRUD operations: ${crudFunctions.length} data manipulation functions`);
	}
	
	// Language-specific patterns
	if (language === 'javascript' || language === 'typescript') {
		// React patterns
		const reactComponents = allFunctions.filter(f => 
			f.name.match(/^[A-Z]/) || f.name.includes('Component')
		);
		if (reactComponents.length > 0) {
			patterns.push(`React components: ${reactComponents.length} component functions`);
		}
		...
		// Decorator patterns
		const decoratedFunctions = allFunctions.filter(f => f.decorators && f.decorators.length > 0);
		if (decoratedFunctions.length > 0) {
			patterns.push(`Decorators: ${decoratedFunctions.length} decorated functions`);
		}
	} else if (language === 'go') {
		// Interface patterns
		...
		// Goroutine patterns
		const goroutineFunctions = allFunctions.filter(f => 
			f.name.toLowerCase().includes('goroutine') || 
			f.name.toLowerCase().includes('worker')
		);
		if (goroutineFunctions.length > 0) {
			patterns.push(`Concurrency: ${goroutineFunctions.length} goroutine-related functions`);
		}
	}
...
function generateCrossLanguageInsights(filteredResults) {
	...
	if (hasDatabase) {
		section += '- **Data Layer**: Database integration across multiple languages\n';
	}
...
function generateImplementationRecommendations(filteredResults, tasks) {
	...
	if (avgCodeComplexity > 7) {
		section += '- **High Complexity**: Consider breaking down complex functions before adding new features\n';
	}
	...
	if (!hasTestFiles) {
		section += '- **Testing**: No test files detected - consider adding tests for new implementations\n';
	}
	
	// Language-specific recommendations
	const languages = Object.keys(filteredResults);
	if (languages.includes('javascript') && languages.includes('typescript')) {
		section += '- **Type Safety**: Consider migrating JavaScript files to TypeScript for better type safety\n';
	}
	
	// Check for authentication patterns in tasks
	const hasAuthTask = tasks.some(task => 
	...
	if (hasAuthTask) {
		...
		if (!hasAuthCode) {
			section += '- **Authentication**: No existing auth code detected - starting from scratch\n';
		} else {
			section += '- **Authentication**: Existing auth patterns found - follow established conventions\n';
		}
	}
