...
export class ContextBuilder {
    ...
    async buildTaskAwareContext(taskDescription, allFiles, options = {}) {
        try {
            ...
            const fileScores = await this.scoreFileRelevance(allFiles, taskAnalysis, context.analysis);
            ...
            context.insights = this.generateContextInsights(context.analysis, taskAnalysis);
            context.recommendations = this.generateTaskRecommendations(context.analysis, taskAnalysis);
    ...
    async performComprehensiveAnalysis(allFiles) {
        ...
        // Dependency analysis
        analysis.dependencies = await this.dependencyMapper.buildDependencyGraph(allFiles);
    ...
    async scoreFileRelevance(allFiles, taskAnalysis, codeAnalysis) {
        ...
        for (const [filePath, astData] of allFiles) {
            ...
            score.relevance = this.calculateTaskRelevance(filePath, astData, taskAnalysis);
            
            // Complexity scoring
            const complexityData = codeAnalysis.complexity.get(filePath);
            if (complexityData) {
                score.complexity = this.calculateComplexityRelevance(complexityData, taskAnalysis);
            }

            // Dependency scoring
            score.dependencies = this.calculateDependencyRelevance(filePath, codeAnalysis.dependencies);
    ...
    generateContextInsights(analysis, taskAnalysis) {
        ...
        if (highComplexityFiles > 0) {
            insights.push({
                ...
                actionable: 'Be careful when modifying complex files; consider refactoring opportunities',
                confidence: 0.8
            });
        }

        // Dependency insights
        if (analysis.dependencies.circularDependencies && analysis.dependencies.circularDependencies.length > 0) {
            insights.push({
        ...
        if (taskAnalysis.type === 'feature_addition') {
            insights.push({
                ...
                actionable: 'Follow existing patterns and maintain consistency with current architecture',
                confidence: 0.8
            });
        }
    ...
    generateTaskRecommendations(analysis, taskAnalysis) {
        ...
        if (taskAnalysis.language === 'javascript' || taskAnalysis.language === 'typescript') {
    ...
    createContextMetadata(context) {
        return {
            version: '2.1.0',
            contextSize: context.selectedFiles.reduce((sum, f) => sum + f.tokenEstimate, 0),
            fileCount: context.selectedFiles.length,
            compressionRatio: context.selectedFiles.filter(f => f.included === 'summary').length / context.selectedFiles.length,
    ...
    extractTaskKeywords(description) {
        ...
        // Technical keywords
        const techKeywords = [
            'api', 'database', 'frontend', 'backend', 'component', 'function',
            'class', 'interface', 'service', 'controller', 'model', 'view',
        ...
        const identifiers = description.match(/[a-zA-Z][a-zA-Z0-9]*(?:[A-Z][a-zA-Z0-9]*)*|[a-z]+-[a-z-]+/g) || [];
    ...
    determineTaskIntent(description) {
        if (description.includes('add') || description.includes('create') || description.includes('implement')) {
            return 'create';
        }
        ...
        if (description.includes('refactor') || description.includes('improve') || description.includes('optimize')) {
            return 'refactor';
        }
    ...
    estimateTaskComplexity(description) {
        ...
        const highComplexityWords = ['complex', 'advanced', 'multiple', 'integration', 'system'];
    ...
    calculateTaskRelevance(filePath, astData, taskAnalysis) {
        ...
        if (taskAnalysis.framework === 'react') {
            const hasReactImports = imports.some(imp => imp.source.includes('react'));
    ...
    explainFileScore(score, taskAnalysis) {
        ...
        if (score.relevance > 0.5) {
            reasons.push(`High task relevance (${Math.round(score.relevance * 100)}%)`);
        }
    ...
    createFileSummary(astData, scoreData) {
        ...
        if (imports.length > 0) {
            summary += `// Imports: ${imports.slice(0, 3).map(imp => imp.source).join(', ')}\n`;
        }
        ...
        if (classes.length > 0) {
            summary += `// Classes: ${classes.slice(0, 3).map(c => c.name).join(', ')}\n`;
        }
    ...
    calculateAnalysisStatistics(analysis) {
        return {
            ...
            uniqueFrameworks: [...new Set(analysis.frameworks.map(f => f.name))].length,
            averageComplexity: this.calculateAverageComplexity(analysis.complexity),
            dependencyComplexity: analysis.dependencies.statistics?.dependencyComplexity || 'unknown'
        };
    }
    ...
    calculateTokenUtilization(selectedFiles) {
        const totalTokens = selectedFiles.reduce((sum, f) => sum + f.tokenEstimate, 0);
