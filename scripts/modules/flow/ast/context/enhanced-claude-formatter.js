 ...
 */
export async function formatEnhancedClaudeContext(enhancedResults, tasks = [], options = {}) {
    const {
        worktreePath = '',
    ...
    if (includePrioritizedContext) {
        context += generatePrioritizedContextSection(enhancedResults, tasks, options);
    }
    ...
    if (includeImplementationGuidance && tasks.length > 0) {
        context += generateImplementationGuidanceSection(enhancedResults, tasks, options);
    }
...
function generatePrioritizedContextSection(enhancedResults, tasks, options) {
    ...
    const importantFiles = prioritizedFiles.filter(f => f.relevanceScore >= 60 && f.relevanceScore < 85);
    ...
    const contextFiles = prioritizedFiles.filter(f => f.relevanceScore >= 40 && f.relevanceScore < 60);
 ...
 */
function generateImplementationGuidanceSection(enhancedResults, tasks, options) {
...
function analyzeTaskComplexity(tasks, enhancedResults) {
    ...
    const hasComplexOperations = tasks.some(t => 
    ...
    if (enhancedResults.context?.files) {
        relevantFiles.push(...enhancedResults.context.files.map(file => ({
            path: file.path,
            score: 75,
    ...
    if (complexity === 'High') {
        strategy = 'Break down into smaller pieces, implement core functionality first, then add features';
    } else if (complexity === 'Medium') {
        strategy = 'Implement in logical order respecting dependencies, test each component';
    }
...
function identifyImpactAreas(tasks, enhancedResults) {
    ...
    tasks.forEach(task => {
        ...
        if (content.includes('auth') || content.includes('login')) areas.add('Authentication');
        if (content.includes('api') || content.includes('endpoint')) areas.add('API Layer');
        if (content.includes('database') || content.includes('db')) areas.add('Data Layer');
        if (content.includes('ui') || content.includes('component')) areas.add('User Interface');
        if (content.includes('test') || content.includes('spec')) areas.add('Testing');
        if (content.includes('security') || content.includes('permission')) areas.add('Security');
        if (content.includes('performance') || content.includes('optimize')) areas.add('Performance');
    });
...
function determinePrimaryFocus(tasks, enhancedResults) {
    ...
    const commonTheme = Object.entries(themes)
        .sort(([,a], [,b]) => b - a)[0];

    return commonTheme ? `${commonTheme[0]} implementation and related features` : 'Multi-feature implementation';
}
...
function extractFrameworks(analysis) {
    ...
    if (analysis.files) {
        analysis.files.forEach(file => {
            if (file.path.includes('react') || file.path.includes('jsx')) frameworks.push('React');
            ...
            if (file.path.includes('gin') && file.language === 'go') frameworks.push('Gin');
        });
    }
...
function generateArchitectureInsights(analysis) {
    ...
    if (analysis.summary?.languageDistribution) {
        ...
        if (languages.length > 2) {
            insights.push('Multi-language project - ensure consistent patterns across languages');
        }
    }
...
function extractDependencies(analysis) {
    ...
    if (analysis.files) {
        const commonExternalDeps = [
            { name: 'react', description: 'UI library for building interfaces', category: 'framework' },
            { name: 'express', description: 'Web framework for Node.js', category: 'framework' },
...
function generateBestPractices(analysis, frameworks) {
    ...
    if (frameworks.includes('Express.js')) {
        practices.push('Use middleware for cross-cutting concerns like authentication');
...
function getFrameworkGuidance(framework, analysis) {
    const guidance = {
        ...
        'Express.js': 'Follow RESTful API conventions, use middleware for common functionality',
        ...
        'Vue.js': 'Use composition API for complex components, follow Vue style guide',
        'Gin': 'Use middleware for common functionality, follow Go naming conventions'
    };

    return guidance[framework] || 'Follow framework best practices and conventions';
}
...
function generateTaskSpecificTips(tasks, analysis) {
    ...
    tasks.forEach(task => {
        ...
        if (title.includes('api')) {
            ...
            tips.push('Implement proper error responses with meaningful status codes');
        }
        
        if (title.includes('test')) {
            tips.push('Cover both happy path and error scenarios in tests');
            tips.push('Use descriptive test names that explain the expected behavior');
        }
...
function generateTestingStrategy(analysis, tasks) {
    ...
    const hasUiTasks = tasks.some(t => t.title?.toLowerCase().includes('component') || t.title?.toLowerCase().includes('ui'));
