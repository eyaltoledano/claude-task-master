...
try {
	...
	console.warn(
		'Phase 3.2 smart invalidation not available, using basic invalidation'
	);
}
...
export class EnhancedASTContextBuilder {
	...
	async initialize() {
		...
		try {
			...
			console.warn(
				'[Enhanced AST] Failed to initialize context builder:',
				error.message
			);
	...
	async buildWorktreeContext(worktreePath, contextOptions = {}) {
		...
		try {
			...
			console.debug('[Enhanced AST] Discovered files', {
				count: projectFiles.length
			});

			// Phase 2: Enhanced Parsing with Language Detection
			const parseResults = await this.parseFilesWithEnhancedAnalysis(
				projectFiles,
				worktreePath,
				contextOptions
			);
			...
			if (this.options.enablePhase21 && this.contextBuilder) {
				...
				assembledContext = await this.assembleBasicContext(
					parseResults,
					contextOptions
				);
			}
			...
			console.error(
				'[Enhanced AST] Failed to build worktree context:',
				error.message
			);
			return {
				enabled: true,
	...
	async discoverProjectFiles(worktreePath) {
		...
		try {
				...
				if (await fs.pathExists(fullPath)) {
					...
					if (language && this.config.supportedLanguages.includes(language)) {
						files.push({
							...
							estimatedComplexity: this.estimateFileComplexity(
								stats.size,
								language
							),
			...
			console.warn(
				'[Enhanced AST] Error discovering project files:',
				error.message
			);
			return [];
		}
	 ...
	 */
	async parseFilesWithEnhancedAnalysis(
		projectFiles,
		worktreePath,
		contextOptions
	) {
		...
		for (const [language, files] of Object.entries(filesByLanguage)) {
			console.debug(
				`[Enhanced AST] Processing ${files.length} ${language} files`
			);
			...
			languageResults.forEach((result) => {
				if (result.success) {
					...
					if (complexity <= 3) results.summary.complexityDistribution.low++;
						...
						results.summary.complexityDistribution.medium++;
	 ...
	 */
	async parseLanguageFilesEnhanced(
		language,
		files,
		worktreePath,
		contextOptions
		...
		for (const file of files) {
			try {
				...
				const parseResult = await getCachedOrExecute(
					...
					async () => {
						console.debug(
							`[Enhanced AST] Parsing ${language} file:`,
							file.path
						);
						...
						if (this.options.enablePhase22 && this.analyzerDispatcher) {
							try {
								...
								console.warn(
									`[Enhanced AST] Analysis failed for ${file.path}:`,
				...
				if (parseResult.success) {
					...
					console.warn(
						`[Enhanced AST] Failed to parse ${file.path}:`,
						parseResult.error
					);
					results.push({
						file,
				...
				console.warn(
					`[Enhanced AST] Error processing ${file.path}:`,
					error.message
				);
				results.push({
					file,
	...
	async assembleBasicContext(parseResults, contextOptions) {
		const context = {
			...
			successRate:
				parseResults.summary.totalFiles > 0
					? (parseResults.summary.successfulParses /
							parseResults.summary.totalFiles) *
						100
					: 0
		};
	 ...
	 */
	isTestFile(filePath) {
		return /test|spec|__tests__|\.test\.|\.spec\.|_test\.|_spec\./i.test(
			filePath
		);
	}
	...
	isConfigFile(filePath) {
		return /config|settings|\.env|package\.json|tsconfig|webpack|babel/i.test(
			filePath
		);
	}
	...
	async generateEnhancedContext(taskData, options = {}) {
		try {
			...
			const worktreePath =
				options.projectPath || this.projectRoot || process.cwd();
			...
			const context = await this.buildWorktreeContext(
				worktreePath,
				contextOptions
			);
...
class ChangeEventProcessor {
	...
	async handleCriticalChanges(changes) {
		console.log(
			`Processing ${changes.length} critical changes, full context rebuild required`
		);
		...
		if (this.smartInvalidationManager) {
			try {
				const invalidationResult =
					await this.smartInvalidationManager.invalidateContext(changes, {
						priority: 'high',
						strategy: 'CONSERVATIVE'
					});

				console.log(
					`Smart invalidation processed ${invalidationResult.processedFiles.length} files, ` +
						`invalidated ${invalidationResult.invalidatedContexts.length} contexts`
				);
				return;
			} catch (error) {
				console.error(
					'Smart invalidation failed for high impact changes:',
					error
				);
			}
		}
	...
	async handleMediumImpactChanges(changes) {
		...
		if (this.smartInvalidationManager) {
			try {
				...
				console.error(
					'Smart invalidation failed for medium impact changes:',
					error
				);
			}
		}
		...
		for (const batch of batches) {
			await Promise.all(
				batch.map((change) => this.processSingleFileChange(change))
			);
		}
	}
	...
	async processSingleFileChange(change) {
		try {
			...
			console.error(
				`Error processing single file change ${change.path}:`,
				error
			);
		}
	}
	...
	async findAffectedContexts(changes) {
		...
		if (
			this.smartInvalidationManager &&
			this.smartInvalidationManager.isSmartInvalidationAvailable()
		) {
			try {
				const dependencyAnalysis =
					await this.smartInvalidationManager.analyzeDependencies(changes);
	...
	notifyContextInvalidation(contextId, newVersion) {
		...
		if (this.contextBuilder.emit) {
			this.contextBuilder.emit('contextInvalidated', {
				contextId,
				version: newVersion
			});
		}
	...
	async queuePreemptiveAnalysis(filePath, affectedContexts) {
		...
		console.log(
			`Queuing preemptive analysis for ${filePath}, affects ${affectedContexts.length} contexts`
		);
	}
...
class PreemptiveAnalyzer {
	...
	async analyzeFile(analysisTask) {
		...
		try {
			...
			// Perform analysis
			const analysisResult = await this.performFileAnalysis(
				filePath,
				affectedContexts
			);
	 ...
	 */
	isCacheValid(cachedResult, maxAge = 30000) {
		// 30 seconds default
...
class RealTimeASTContextBuilder extends EnhancedASTContextBuilder {
	constructor(options = {}) {
		...
		this.changeProcessor.smartInvalidationManager =
			this.smartInvalidationManager;
	...
	async initializeFileWatching() {
		...
		try {
			...
			if (WatchManager) {
				...
				console.log(
					'Real-time AST context updates enabled with Phase 3.1 integration'
				);
			} else {
			...
			console.warn(
				'Failed to initialize real-time file watching, using fallback mode:',
				error.message
			);
	...
	async generateEnhancedContext(taskData, options = {}) {
		...
		try {
			...
			if (context.relevantFiles && context.relevantFiles.length > 0) {
				this.queueFilesForPreemptiveAnalysis(
					context.relevantFiles,
					taskData.id
				);

				// Register context dependencies with smart invalidation manager
				this.smartInvalidationManager.registerContextDependencies(
					taskData.id,
					context.relevantFiles
				);
			}
	 ...
	 */
	isContextCurrent(preanalyzedContext, maxAge = 60000) {
		// 1 minute default
	...
	async mergeWithRealTimeUpdates(preanalyzedContext, taskData) {
		...
		const baseContext =
			await this.buildContextFromPreanalyzed(preanalyzedContext);
	...
	async buildContextFromPreanalyzed(preanalyzedContext) {
		...
		for (const fileResult of preanalyzedContext.preanalyzedFiles) {
			...
			if (fileResult.functions) {
				context.functions = (context.functions || []).concat(
					fileResult.functions
				);
			}
			if (fileResult.dependencies) {
				context.dependencies = (context.dependencies || []).concat(
					fileResult.dependencies
				);
			}
		}
	...
	async getRealTimeEnhancements(taskData) {
		return {
			...
			preemptiveAnalysisActive:
				this.preemptiveAnalyzer.metrics.analysisCompleted > 0
		};
	}
	...
	queueFilesForPreemptiveAnalysis(filePaths, taskId) {
		...
		for (const filePath of filePaths) {
			this.preemptiveAnalyzer.queuePreemptiveAnalysis(filePath, 'normal', [
				taskId
			]);
		}
	}
...
class SmartInvalidationManager {
	...
	async initializeSmartInvalidation() {
		try {
			if (DependencyTracker && SelectiveInvalidation && BatchInvalidation) {
				...
				this.selectiveInvalidation = new SelectiveInvalidation(
				...
				this.batchInvalidation = new BatchInvalidation(
					this.selectiveInvalidation,
					{
	...
	async invalidateContext(changes, options = {}) {
		...
		try {
			...
			const invalidationPlan =
				await this.createInvalidationPlan(dependencyAnalysis);
	...
	async analyzeDependencies(changes) {
		...
		for (const change of changes) {
			try {
				...
				const dependencies = await this.dependencyTracker.getDependencies(
					change.path
				);
				const dependents = await this.dependencyTracker.getDependents(
					change.path
				);
				...
				const contexts = await this.findAffectedContexts(
					change.path,
					dependencies,
					dependents
				);
				...
				console.error(
					`Error analyzing dependencies for ${change.path}:`,
					error
				);
			}
		}
	...
	determineOptimalStrategy(analysis) {
		...
		const maxImpactScore = Math.max(
			...Array.from(analysis.impactScores.values())
		);
	...
	async executeInvalidation(plan, options = {}) {
		const results = {
			...
			metrics: {
				startTime: Date.now(),
				totalFiles: plan.invalidationGroups.reduce(
		...
		try {
			...
			for (const group of plan.invalidationGroups) {
				try {
					const groupResult = await this.processInvalidationGroup(
						group,
						options
					);
			...
			results.metrics.duration =
				results.metrics.endTime - results.metrics.startTime;
		}
	...
	async processInvalidationGroup(group, options) {
		...
		if (this.batchInvalidation) {
			try {
				...
				const batchResults = await this.batchInvalidation.queueChanges(
					changeEvents,
					{
				...
				console.error(
					`Batch invalidation failed for group ${group.type}:`,
					error
				);
				// Fallback to individual file processing
				result.processedFiles = await this.processFilesIndividually(
					group.files
				);
			}
		} else {
	...
	async fallbackInvalidation(changes) {
		...
		for (const change of changes) {
			try {
				const affectedContexts = await this.findAffectedContextsFallback(
					change.path
				);
				...
				console.error(
					`Fallback invalidation failed for ${change.path}:`,
					error
				);
			}
		}
