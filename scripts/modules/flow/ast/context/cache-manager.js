...
export async function createCacheKey(filePath, language, worktreePath) {
	try {
		...
		const pathHash = crypto.createHash('sha256').update(relativePath).digest('hex').substring(0, 8);
 ...
 */
export async function getCachedOrExecute(cacheKey, executeFunction, options = {}) {
...
export async function initializeWorktreeCache(worktreePath, projectRoot) {
	try {
		...
		console.warn('[AST Cache] Failed to initialize worktree cache:', error.message);
	}
}
...
export async function cleanupWorktreeCache(worktreePath, projectRoot) {
	try {
		...
		try {
			...
			console.debug('[AST Cache] Could not get git context for cleanup, skipping');
			return;
		}
		...
		console.warn('[AST Cache] Failed to cleanup worktree cache:', error.message);
	}
}
