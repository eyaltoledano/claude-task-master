...
export class ComplexityScorer {
    ...
    async calculateComplexity(astData, language, content, context = {}) {
        try {
            ...
            analysis.file = this.analyzeFileComplexity(astData, language, content, context);
    ...
    analyzeFunctionComplexity(func, language, content, context) {
        ...
        if (this.options.enableCognitiveComplexity) {
            analysis.complexity.cognitive = this.calculateCognitiveComplexity(functionBody, language);
        }

        // Calculate Halstead complexity
        if (this.options.enableHalsteadComplexity) {
            analysis.complexity.halstead = this.calculateHalsteadComplexity(functionBody, language);
        }

        // Calculate nesting complexity
        if (this.options.enableNestingComplexity) {
            analysis.complexity.nesting = this.calculateNestingComplexity(functionBody, language);
        }

        // Calculate maintenance complexity
        if (this.options.enableMaintenanceComplexity) {
            analysis.complexity.maintenance = this.calculateMaintenanceComplexity(func, functionBody, language, context);
        }
        ...
        analysis.complexity.overall = this.calculateOverallFunctionScore(analysis.complexity);
        analysis.score = analysis.complexity.overall;

        // Identify complexity factors and issues
        analysis.factors = this.identifyComplexityFactors(analysis, functionBody, language);
    ...
    analyzeClassComplexity(cls, language, content, context) {
        ...
        const methodComplexities = analysis.methods.map(m => m.complexity.overall);
        
        if (methodComplexities.length > 0) {
            analysis.complexity.total = methodComplexities.reduce((sum, c) => sum + c, 0);
            analysis.complexity.average = analysis.complexity.total / methodComplexities.length;
        ...
        analysis.complexity.coupling = this.calculateClassCoupling(cls, content, context);
    ...
    analyzeFileComplexity(astData, language, content, context) {
        ...
        if (functions.length > 0) {
            ...
            analysis.structure.averageFunctionLength = functionLengths.reduce((sum, l) => sum + l, 0) / functionLengths.length;
            analysis.structure.longestFunction = Math.max(...functionLengths);
            analysis.complexity.average = functionComplexities.reduce((sum, c) => sum + c, 0) / functionComplexities.length;
        ...
        analysis.complexity.maintainabilityIndex = this.calculateMaintainabilityIndex(analysis, content);

        // Estimate technical debt
        analysis.complexity.technicalDebt = this.estimateTechnicalDebt(analysis, functions, classes);
    ...
    calculateCognitiveComplexity(code, language) {
        ...
        patterns.forEach(pattern => {
            ...
            matches.forEach(() => {
                complexity += pattern.baseWeight + (nestingLevel * this.options.cognitiveWeights.nesting);
    ...
    calculateOverallComplexity(analysis) {
        ...
        return {
            average: allScores.length > 0 ? allScores.reduce((sum, s) => sum + s, 0) / allScores.length : 1,
    ...
    generateComplexityRecommendations(analysis) {
        ...
        const highComplexityFunctions = analysis.functions.filter(f => f.score > 7);
