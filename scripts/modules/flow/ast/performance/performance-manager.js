...
export class PerformanceManager extends EventEmitter {
    ...
    async initialize(astConfig = {}) {
        ...
        try {
            ...
            this.workerPool = new AdaptiveWorkerPool({
                maxConcurrentOperations: astConfig.performance?.maxConcurrentOperations || 4,
            ...
            this.emit('performance:initialized', {
                components: ['ResourceMonitor', 'AdaptiveWorkerPool', 'SmartPriorityQueue', 'LazyLoadingManager']
            });
    ...
    getCurrentPerformanceLevel() {
        return {
            ...
            analysisDepth: this.resourceMonitor?.getRecommendedAnalysisDepth() || 'full',
     ...
     */
    async forceCleanup() {
        const cleanedCount = this.lazyLoader ? this.lazyLoader.scheduleCleanup(0) : 0;
...
export const performanceCommands = {
    ...
    async status(performanceManager) {
        ...
        if (stats.system.isThrottling) {
            console.log('🚨 THROTTLING ACTIVE - Analysis reduced due to high system load');
        }
        
        // Resource usage
        if (stats.resources) {
            console.log('\n💻 Resource Usage:');
            console.log(`   CPU: ${stats.resources.cpu.percentage.toFixed(1)}% (threshold: ${stats.resources.cpu.threshold}%)`);
            console.log(`   Memory: ${(stats.resources.memory.used / 1024 / 1024).toFixed(1)}MB`);
            console.log(`   Degradation Level: ${stats.resources.degradationLevel}/3`);
        }
        ...
        if (stats.workers) {
            ...
            for (const [language, pool] of Object.entries(stats.workers.pools)) {
                console.log(`   ${language}: ${pool.activeWorkers}/${pool.totalWorkers} active (${pool.queuedTasks} queued)`);
            }
        }
