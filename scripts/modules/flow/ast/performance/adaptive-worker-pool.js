...
export class AdaptiveWorkerPool extends EventEmitter {
	constructor(config = {}) {
		...
		this.config = {
			maxConcurrentOperations:
				config.maxConcurrentOperations || Math.max(1, cpus().length - 1),
			...
			healthCheckInterval: this._parseDuration(
				config.healthCheckInterval || '5s'
			)
		};
	...
	_handleWorkerResult(worker, result) {
		...
		this.metrics.averageTaskTime =
			(this.metrics.averageTaskTime * (this.metrics.completedTasks - 1) +
				taskTime) /
			this.metrics.completedTasks;
	...
	_calculateAverageQueueWait(pool) {
		...
		const now = Date.now();
		const totalWait = pool.queue.reduce(
			(sum, task) => sum + (now - task.submittedAt),
			0
		);
