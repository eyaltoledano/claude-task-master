...
export class DocumentationGenerator extends EventEmitter {
    ...
    async generateFileDocumentation(filePath, ast, language) {
        ...
        try {
            ...
            const content = await this.generateDocumentationContent(elements, filePath, language);
            ...
            throw new DocumentationGenerationError(`Documentation generation failed for ${filePath}: ${error.message}`, error);
        }
    }
    ...
    async generateProjectDocumentation(projectPath, astResults) {
        ...
        try {
            ...
            projectDoc.summary = {
                ...
                totalFunctions: projectDoc.files.reduce((sum, f) => sum + f.summary.functionsDocumented, 0),
                totalClasses: projectDoc.files.reduce((sum, f) => sum + f.summary.classesDocumented, 0)
            };
    ...
    async generateDocumentationContent(elements, filePath, language) {
        const content = {
            ...
            functions: elements.functions.map(func => this.generateFunctionDoc(func)),
    ...
    generateFunctionDoc(func) {
        return {
            ...
            returns: func.documentation?.returns || { type: 'unknown', description: 'No return description' }
        };
    }
    ...
    generateJSExample(func) {
        const params = func.parameters ? func.parameters.map(p => p.name).join(', ') : '';
