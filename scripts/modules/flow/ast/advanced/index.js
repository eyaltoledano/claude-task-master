...
export {
	...
	CrossLanguageAnalysisError
} from './cross-language-analysis.js';
export {
	RefactoringSuggestionEngine,
	RefactoringSuggestionError
} from './refactoring-suggestions.js';
export {
...
export class AdvancedAnalysisEngine {
	constructor(parserRegistry, dependencyMapper, analyzers, options = {}) {
		this.crossLanguageAnalyzer = new CrossLanguageAnalyzer(
			...
			dependencyMapper,
			analyzers,
			options.crossLanguage
		);
		this.refactoringSuggestionEngine = new RefactoringSuggestionEngine(
			analyzers,
			options.refactoring
		);
		...
		try {
			...
			if (this.options.enableCrossLanguage) {
				results.crossLanguage =
					await this.crossLanguageAnalyzer.analyzeCrossLanguageProject(
						projectPath
					);
			...
			if (this.options.enableDocumentation && results.crossLanguage) {
				...
				results.documentation =
					await this.documentationGenerator.generateProjectDocumentation(
						projectPath,
						astResults
					);
			...
			throw new AdvancedAnalysisError(
	...
	async analyzeFile(filePath, ast, language) {
		...
		try {
			...
			if (this.options.enableRefactoring) {
				results.refactoring =
					await this.refactoringSuggestionEngine.analyzeFileForRefactoring(
						filePath,
						ast,
						language
					);
			...
			if (this.options.enablePatterns) {
				results.patterns = await this.patternDetectionEngine.analyzePatterns(
					filePath,
					ast,
					language
				);
			...
			if (this.options.enableDocumentation) {
				results.documentation =
					await this.documentationGenerator.generateFileDocumentation(
						filePath,
						ast,
						language
					);
