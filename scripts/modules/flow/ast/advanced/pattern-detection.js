...
export class PatternDetectionEngine extends EventEmitter {
        ...
        try {
            ...
            return {
                ...
                summary: {
                    ...
                    designPatterns: filteredPatterns.filter(p => p.category === 'design-pattern').length,
                    antiPatterns: filteredPatterns.filter(p => p.category === 'anti-pattern').length,
            ...
            for (const cls of ast.classes) {
                if (this.isSingletonPattern(cls)) {
                    patterns.push({
                        ...
                        confidence: 0.8,
                        implications: ['Ensures single instance', 'May create testing difficulties']
                    });
                }
            }
        }
            ...
            for (const cls of ast.classes) {
                ...
                if (methodCount > 20) {
                    antiPatterns.push({
                        ...
                        refactoringSuggestions: ['Split class using Single Responsibility Principle']
                    });
                }
            }
        }
    ...
    isSingletonPattern(cls) {
        return cls.methods && cls.methods.some(method => 
            method.name.includes('instance') || method.name.includes('getInstance')
        );
    }

    isObserverPattern(cls) {
        return cls.methods && cls.methods.some(method => 
            ['emit', 'on', 'off', 'addListener', 'removeListener'].includes(method.name)
        );
    }
    ...
    getHooksUsed(func) {
        ...
        if (func.calls) {
            for (const call of func.calls) {
                if (call.function && call.function.startsWith('use') && 
                    ['useState', 'useEffect', 'useContext', 'useReducer'].includes(call.function)) {
    ...
    filterPatterns(patterns) {
        return patterns
            .filter(pattern => pattern.confidence >= this.options.confidenceThreshold)
    ...
    calculateAverageConfidence(patterns) {
        ...
        const total = patterns.reduce((sum, pattern) => sum + pattern.confidence, 0);
