...
export class WatchManager extends EventEmitter {
    ...
    async initialize() {
        ...
        try {
            ...
            this.fileWatcher = createFileWatcher(this.projectPath, componentOptions.fileWatcher);
            if (!await this.fileWatcher.initialize()) {
                throw new Error('Failed to initialize file watcher');
            }
            
            // Initialize change processor
            this.changeProcessor = createChangeProcessor(this.projectPath, componentOptions.changeProcessor);
            if (!await this.changeProcessor.initialize()) {
                throw new Error('Failed to initialize change processor');
            }
            
            // Initialize batch processor
            this.batchProcessor = createBatchProcessor(this.projectPath, componentOptions.batchProcessor);
    ...
    async startWatching() {
        ...
        if (this.state !== WatchState.STARTING && this.state !== WatchState.STOPPED) {
        ...
        try {
            ...
            this.state = WatchState.ERROR;
            console.error(`[WatchManager] Failed to start watching: ${error.message}`);
    ...
    async pauseWatching() {
        ...
        try {
            ...
            console.error(`[WatchManager] Failed to pause watching: ${error.message}`);
            return false;
        }
    }
    ...
    async resumeWatching() {
        ...
        try {
            ...
            console.error(`[WatchManager] Failed to resume watching: ${error.message}`);
            return false;
        }
    }
    ...
    _bindEventHandlers() {
        ...
        this.on('changeProcessorReady', () => {
            this.changeProcessor?.on('changeProcessed', this._handleChangeProcessed.bind(this));
            this.changeProcessor?.on('batchProcessed', this._handleBatchProcessed.bind(this));
            this.changeProcessor?.on('processingError', this._handleProcessingError.bind(this));
        });
        
        // Batch processor events
        this.on('batchProcessorReady', () => {
            this.batchProcessor?.on('batchProcessed', this._handleBatchProcessorResult.bind(this));
        });
    }
    ...
    _handleBatchProcessorResult(event) {
        console.debug(`[WatchManager] Batch processor completed: ${event.batchSize} changes`);
    ...
    async _setupGitIntegration() {
        try {
            ...
            if (!await fs.pathExists(gitDir)) {
                console.debug('[WatchManager] No git repository found, skipping git integration');
                return;
            }
