...
export class ChangeProcessor extends EventEmitter {
    ...
    async initialize() {
        try {
            ...
            console.debug(`[ChangeProcessor] Impact analysis: ${this.options.enableImpactAnalysis}`);
            console.debug(`[ChangeProcessor] Content comparison: ${this.options.enableContentComparison}`);
            
            return true;
        } catch (error) {
            console.error(`[ChangeProcessor] Initialization failed: ${error.message}`);
            return false;
        }
    }
    ...
    async processChange(changeEvent) {
        try {
            ...
            if (analysis.shouldIgnore) {
                ...
                console.debug(`[ChangeProcessor] Ignoring change: ${changeEvent.relativePath}`);
                return null;
            }
            ...
            console.debug(`[ChangeProcessor] Processed ${analysis.changeType} change: ${changeEvent.relativePath} (priority: ${analysis.priority})`);
            ...
            console.error(`[ChangeProcessor] Error processing change ${changeEvent.relativePath}: ${error.message}`);
    ...
    async processBatchChanges(changes) {
        ...
        console.debug(`[ChangeProcessor] Processing batch of ${changes.length} changes`);
        ...
        try {
            ...
            console.debug(`[ChangeProcessor] Batch analysis complete: ${analyses.length} changes processed`);
            ...
            console.error(`[ChangeProcessor] Error processing batch: ${error.message}`);
            throw error;
        }
    }
    ...
    async _shouldIgnoreChange(changeEvent) {
        ...
        if (size > this.options.maxFileSize) {
            console.debug(`[ChangeProcessor] Ignoring large file: ${relativePath} (${size} bytes)`);
            return true;
        }
        ...
        if (type === 'change' && this.options.enableContentComparison) {
            ...
            if (!hasContentChanged) {
                console.debug(`[ChangeProcessor] Ignoring metadata-only change: ${relativePath}`);
                return true;
            }
        }
    ...
    async _analyzeContentChange(analysis) {
        try {
            ...
            console.warn(`[ChangeProcessor] Failed to analyze content for ${analysis.relativePath}: ${error.message}`);
        }
    }
    ...
    async _calculatePriority(analysis) {
        ...
        priority = this._adjustPriorityForLanguage(priority, language, relativePath);
    ...
    async _analyzeImpact(analysis) {
        ...
        try {
            ...
            if (dependents.length > 5) {
                analysis.priority = Math.max(analysis.priority, ChangePriority.CRITICAL);
            ...
            console.warn(`[ChangeProcessor] Failed to analyze impact for ${analysis.relativePath}: ${error.message}`);
        }
    }
    ...
    async _analyzeBatchImpact(analyses) {
        ...
        if (batchAnalysis.hasCriticalChanges || batchAnalysis.affectedFilesCount > 20) {
