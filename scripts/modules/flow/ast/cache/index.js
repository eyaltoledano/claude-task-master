...
import { DependencyTracker, DependencyTypes, createDependencyTracker } from './dependency-tracker.js';
import { 
...
export function createSmartInvalidationSystem(options = {}) {
    ...
    const system = {
        ...
        async initialize(projectPath, files) {
            ...
            const graphResult = await dependencyTracker.buildDependencyGraph(projectPath, files);
            
            console.info(`[SmartInvalidation] Built dependency graph with ${graphResult.stats.dependenciesFound} dependencies`);
        ...
        async updateDependencies(filePath, newDependencies) {
            return await dependencyTracker.updateDependencies(filePath, newDependencies);
        },
        ...
        async validateContentHash(filePath, expectedHash) {
            return await contentHasher.validateHashConsistency(filePath, expectedHash);
        },
 ...
 */
export function createInvalidationSystemWithPreset(presetName, customOptions = {}) {
    const preset = InvalidationPresets[presetName];
    
    if (!preset) {
        throw new Error(`Unknown preset: ${presetName}. Available: ${Object.keys(InvalidationPresets).join(', ')}`);
    }
    ...
    const mergedOptions = {
        ...preset,
        ...customOptions,
        contentHasherOptions: { ...preset.contentHasherOptions, ...customOptions.contentHasherOptions },
        dependencyTrackerOptions: { ...preset.dependencyTrackerOptions, ...customOptions.dependencyTrackerOptions },
        selectiveInvalidationOptions: { ...preset.selectiveInvalidationOptions, ...customOptions.selectiveInvalidationOptions },
        batchInvalidationOptions: { ...preset.batchInvalidationOptions, ...customOptions.batchInvalidationOptions }
    };
...
export function validateInvalidationConfig(config) {
    ...
    const validInvalidationStrategies = ['conservative', 'balanced', 'aggressive', 'immediate'];
    const validBatchStrategies = ['time_based', 'count_based', 'size_based', 'hybrid', 'immediate'];
    
    // Validate strategy values
    if (config.strategy && !validInvalidationStrategies.includes(config.strategy)) {
        errors.push(`Invalid invalidation strategy: ${config.strategy}`);
    }
    
    if (config.batchStrategy && !validBatchStrategies.includes(config.batchStrategy)) {
