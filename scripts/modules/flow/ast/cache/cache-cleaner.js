...
export class ASTCacheCleaner {
            ...
            await this.walkCacheDirectory(this.cacheRoot, async (cacheFilePath) => {
                ...
                try {
                    ...
                    if (error.code !== 'ENOENT') {
                        console.warn(`Error cleaning file ${cacheFilePath}:`, error.message);
                        this.stats.errors++;
                    }
                }
            });
    ...
    async cleanupBySize(options = {}) {
        const maxSize = this.parseCacheMaxSize(this.config?.cacheMaxSize || '100MB');
        const results = {
            totalSize: 0,
            filesScanned: 0,
            filesDeleted: 0,
        ...
        try {
            await this.walkCacheDirectory(this.cacheRoot, async (cacheFilePath) => {
                ...
                try {
                    ...
                    if (error.code !== 'ENOENT') {
                        console.warn(`Error checking orphan file ${cacheFilePath}:`, error.message);
                        this.stats.errors++;
                    }
                }
            });
        ...
        try {
            await this.walkCacheDirectory(this.cacheRoot, async (cacheFilePath) => {
                try {
                    ...
                    if (error.code !== 'ENOENT') {
                        console.warn(`Error getting stats for ${cacheFilePath}:`, error.message);
                    }
                }
            });
        } catch (error) {
     ...
     */
    scheduleCleanup(intervalMs = 60 * 60 * 1000) { // Default: 1 hour
