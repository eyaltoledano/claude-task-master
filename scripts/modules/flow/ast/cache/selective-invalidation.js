...
export class SelectiveInvalidation {
    ...
    async invalidateByChange(changeEvent, options = {}) {
        ...
        try {
            ...
            if (oldContent && newContent && this.contentHasher) {
                changeImpact = await this._analyzeChangeImpact(filePath, oldContent, newContent);
            }
            ...
            const scope = await this._calculateInvalidationScope(filePath, strategy, changeImpact);
            ...
            console.error(`[SelectiveInvalidation] Error processing change for ${filePath}: ${error.message}`);
            return { 
                invalidated: [], 
                error: error.message, 
                strategy,
    ...
    async invalidateByDependency(filePath, dependencyGraph, options = {}) {
        ...
        if (!this.dependencyTracker) {
            throw new Error('Dependency tracker required for dependency-based invalidation');
        }
    ...
    async _calculateInvalidationScope(filePath, strategy, changeImpact) {
        ...
        if (config.invalidateAll) {
            return {
                ...
                total: this.dependencyTracker ? 
                    this.dependencyTracker.mainDependencyGraph.size : 1
            };
        }
        ...
        for (const dependent of directDependents) {
            const dependentImpactScore = this.dependencyTracker.calculateImpactScore(dependent);
        ...
        if (config.includeTransitive && effectiveDepth > 1) {
            ...
            scope.transitive = transitive.filter(f => 
                !scope.direct.includes(f) && !scope.dependents.includes(f)
            );
        }
        
        // Include test files if strategy allows
        if (config.includeTestFiles) {
            const allFiles = [...scope.direct, ...scope.dependents, ...scope.transitive];
        ...
        scope.total = scope.direct.length + scope.dependents.length + 
                     scope.transitive.length + scope.testFiles.length;
        ...
        if (scope.type === 'all') {
            ...
            if (this.dependencyTracker) {
                ...
                if (this.dependencyTracker.trackTestFiles) {
                    invalidated.push(...this.dependencyTracker.testDependencyGraph.keys());
                }
            }
        } else {
    ...
    async _invalidateCacheEntry(filePath) {
        ...
        console.debug(`[SelectiveInvalidation] Invalidating cache for: ${filePath}`);
    }
    ...
    async _analyzeChangeImpact(filePath, oldContent, newContent) {
        ...
        const oldHash = await this.contentHasher.generateHash(filePath, oldContent, language);
        const newHash = await this.contentHasher.generateHash(filePath, newContent, language);
        ...
        const changes = this._analyzeContentChanges(oldContent, newContent, language);
    ...
    _extractJSFunctions(content) {
        ...
        const arrowFunctionRegex = /(?:export\s+)?(?:const|let|var)\s+(\w+)\s*=\s*(?:\([^)]*\)|[^=])\s*=>/g;
    ...
    async rollbackInvalidation(invalidationId) {
        const invalidation = this.invalidationHistory.find(inv => inv.id === invalidationId);
    ...
    _updateStats(strategy, filesInvalidated) {
        ...
        this.stats.averageScope = this.stats.filesInvalidated / this.stats.invalidationsPerformed;
    }
