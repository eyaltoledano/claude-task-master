...
export class DependencyTracker {
    ...
    async buildDependencyGraph(projectPath, files) {
        console.debug(`[DependencyTracker] Building dependency graph for ${files.length} files`);
        ...
        for (const filePath of files) {
            try {
                ...
                this.stats.errors++;
                console.error(`[DependencyTracker] Error analyzing ${filePath}: ${error.message}`);
            }
        }
        ...
        console.debug(`[DependencyTracker] Analysis complete: ${this.stats.dependenciesFound} dependencies found`);
    ...
    async updateDependencies(filePath, dependencies) {
        ...
        const graph = isTestFile ? this.testDependencyGraph : this.mainDependencyGraph;
    ...
    async _analyzeFile(filePath, projectRoot) {
        ...
        const dependencies = await this._extractDependencies(filePath, content, language, projectRoot);
    ...
    async _extractDependencies(filePath, content, language, projectRoot) {
        ...
        switch (language) {
            case 'javascript':
            case 'typescript':
                dependencies.push(...this._extractJSDependencies(content, filePath, projectRoot));
                break;
            case 'python':
                dependencies.push(...this._extractPythonDependencies(content, filePath, projectRoot));
                break;
            case 'go':
                dependencies.push(...this._extractGoDependencies(content, filePath, projectRoot));
                break;
            case 'json':
                dependencies.push(...this._extractJSONDependencies(content, filePath, projectRoot));
                break;
        }
        
        // Add cross-language dependencies if enabled
        if (this.crossLanguageSupport) {
            dependencies.push(...this._extractCrossLanguageDependencies(content, filePath, projectRoot));
        }
    ...
    _extractJSDependencies(content, filePath, projectRoot) {
        ...
        while ((match = importRegex.exec(content)) !== null) {
            const modulePath = this._resolveModulePath(match[1], filePath, projectRoot);
            if (modulePath) {
                dependencies.push({
                    path: modulePath,
        ...
        if (this.trackDynamicImports) {
            ...
            while ((match = dynamicImportRegex.exec(content)) !== null) {
                const modulePath = this._resolveModulePath(match[2], filePath, projectRoot);
                if (modulePath) {
                    dependencies.push({
                        path: modulePath,
    ...
    _extractPythonDependencies(content, filePath, projectRoot) {
        ...
        while ((match = importRegex.exec(content)) !== null) {
            ...
            const modulePath = this._resolvePythonModule(fromModule || importedModule, filePath, projectRoot);
            if (modulePath) {
                dependencies.push({
                    path: modulePath,
                    type: DependencyTypes.IMPORT,
        ...
        if (this.trackDynamicImports) {
            ...
            const dynamicImportRegex = /importlib\.import_module\s*\(\s*['"`]([^'"`]+)['"`]/g;
            while ((match = dynamicImportRegex.exec(content)) !== null) {
                const modulePath = this._resolvePythonModule(match[1], filePath, projectRoot);
                if (modulePath) {
                    dependencies.push({
                        path: modulePath,
                        type: DependencyTypes.DYNAMIC,
            ...
            if (match[1]) {
                ...
                for (const imp of imports) {
                    ...
                    if (cleaned && !cleaned.startsWith('//')) {
                        const modulePath = this._resolveGoModule(cleaned, filePath, projectRoot);
                        if (modulePath) {
                            dependencies.push({
                                path: modulePath,
                                type: DependencyTypes.IMPORT,
                ...
                // Single import
                const modulePath = this._resolveGoModule(match[2], filePath, projectRoot);
                if (modulePath) {
                    dependencies.push({
                        path: modulePath,
                        type: DependencyTypes.IMPORT,
    ...
    _extractCrossLanguageDependencies(content, filePath, projectRoot) {
        ...
        const fileRefRegex = /['"`]([^'"`]*\.(?:js|ts|py|go|json|yaml|yml|toml))['"]/g;
        let match;
        
        while ((match = fileRefRegex.exec(content)) !== null) {
            const referencedPath = this._resolveRelativePath(match[1], filePath, projectRoot);
    ...
    _resolvePythonModule(modulePath, fromFile, projectRoot) {
        ...
        if (modulePath.startsWith('.')) {
            return this._resolveRelativePath(modulePath.replace(/\./g, '/') + '.py', fromFile, projectRoot);
        }
        
        // Check if it's a local module in the project
        const possiblePath = path.join(projectRoot, modulePath.replace(/\./g, '/') + '.py');
    ...
    _resolveGoModule(modulePath, fromFile, projectRoot) {
        ...
        if (modulePath.includes('/') && !modulePath.includes('github.com') && !modulePath.includes('golang.org')) {
    ...
    getStats() {
        return {
            ...
            averageDependencies: this.stats.filesAnalyzed > 0 ? 
                this.stats.dependenciesFound / this.stats.filesAnalyzed : 0
        };
    }
}
