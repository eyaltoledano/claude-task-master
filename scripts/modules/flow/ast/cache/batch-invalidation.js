...
export class BatchInvalidation extends EventEmitter {
    ...
    async queueChange(changeEvent, options = {}) {
        ...
        try {
            ...
            console.error(`[BatchInvalidation] Error queueing change: ${error.message}`);
     ...
     */
    getBatchStatus() {
        const totalPending = Object.values(this.priorityQueues)
        ...
        return {
            ...
            nextProcessTime: this.batchTimer ? 
                Date.now() + this.batchWindow : null,
            stats: this.getStats()
        };
    }
    ...
    async _processImmediate(change) {
        try {
            ...
            if (!this.selectiveInvalidation) {
                throw new Error('Selective invalidation required for immediate processing');
            }
            
            const result = await this.selectiveInvalidation.invalidateByChange(change);
            ...
            console.error(`[BatchInvalidation] Error in immediate processing: ${error.message}`);
    ...
    async _shouldProcessSizeBase(config) {
        ...
        try {
            ...
            const preview = await this.selectiveInvalidation.previewInvalidation(pendingChanges);
            const totalScope = preview.reduce((sum, p) => sum + (p.estimatedFiles || 0), 0);
            
            return totalScope >= config.maxScope;
        } catch (error) {
            console.error(`[BatchInvalidation] Error estimating scope: ${error.message}`);
            return false;
        }
    }
    ...
    async _processBatches() {
        ...
        try {
            ...
            for (const priority of [BatchPriority.HIGH, BatchPriority.MEDIUM, BatchPriority.LOW]) {
            ...
            console.error(`[BatchInvalidation] Error processing batches: ${error.message}`);
    ...
    async _processPriorityBatch(changes, priority) {
        ...
        for (const [filePath, fileChanges] of changesByFile.entries()) {
            try {
                ...
                const result = await this.selectiveInvalidation.invalidateByChange(latestChange);
                ...
                console.error(`[BatchInvalidation] Error processing ${filePath}: ${error.message}`);
                results.push({
    ...
    _estimateProcessTime() {
        ...
        return timeToNextBatch + (avgProcessingTime * totalPending / this.maxBatchSize);
    }
