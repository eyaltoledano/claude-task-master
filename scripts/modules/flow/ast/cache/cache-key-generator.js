...
export class CacheKeyGenerator {
     ...
     */
    generateKeyFromContent(content, language, relativePath, branch = 'main', commitHash = null) {
     ...
     */
    async generateKeys(filePaths, projectRoot, branch = 'main', commitHash = null) {
        const results = [];
        
        for (const filePath of filePaths) {
            const cacheKey = await this.generateKey(filePath, projectRoot, branch, commitHash);
            if (cacheKey) {
     ...
     */
    generateCacheDir(language, branch, commitHash, cacheRoot = '.taskmaster/ast-cache') {
        const sanitizedBranch = this.sanitizeBranchName(branch);
