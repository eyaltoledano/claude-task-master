...
export class ASTCacheManager {
    ...
    async get(filePath, projectRoot, branch = 'main', commitHash = null) {
        ...
        try {
            const cacheKey = await this.keyGenerator.generateKey(filePath, projectRoot, branch, commitHash);
            if (!cacheKey) {
                return null;
            }
     ...
     */
    async set(filePath, projectRoot, astResult, branch = 'main', commitHash = null) {
        if (!this.config?.enabled) {
            return false;
        }
        ...
        try {
            ...
            // Write to cache file
            await fs.writeFile(cacheFilePath, JSON.stringify(cacheEntry, null, 2), 'utf8');
    ...
    async invalidateFile(filePath, projectRoot, branch = null) {
        try {
            ...
            if (branch) {
                const cacheKey = await this.keyGenerator.generateKey(filePath, projectRoot, branch);
    ...
    async findCacheEntriesForFile(relativePath) {
        const entries = [];
        try {
            await this.walkCacheDirectory(this.cacheRoot, (cacheFilePath, cacheKey) => {
