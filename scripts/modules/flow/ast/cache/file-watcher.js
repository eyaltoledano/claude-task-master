...
export class ASTFileWatcher extends EventEmitter {
    ...
    async initialize() {
        try {
            ...
            console.warn('Failed to load AST config for file watcher:', error.message);
    ...
    async watch(projectRoot, options = {}) {
        ...
        try {
            ...
            const watcher = watch(projectRoot, watchOptions, (eventType, filename) => {
    ...
    async processFileChange(eventType, filePath, projectRoot) {
        try {
            ...
            console.warn(`Error processing file change for ${filePath}:`, error.message);
        }
    }
