...
export async function listAgentsCommand(options = {}) {
	try {
		...
		for (const agent of agents) {
			...
			const healthStatus = agent.healthy
				? `${c.green}healthy${c.reset}`
				: `${c.red}unhealthy${c.reset}`;

			console.log(
				`\n${statusIcon} ${c.bright}${agent.name}${c.reset} (${agent.type})`
			);
			console.log(
				`   Status: ${getStatusColor(agent.status)}${agent.status}${c.reset} | Health: ${healthStatus}`
			);
			console.log(
				`   Capabilities: ${c.cyan}${agent.capabilities.join(', ')}${c.reset}`
			);

			if (options.verbose && agent.metrics) {
				console.log(
					`   Metrics: ${agent.metrics.totalRequests} requests, ${Math.round(agent.metrics.averageResponseTime)}ms avg`
				);
				if (agent.qualityScore !== null) {
					console.log(
						`   Quality: ${getQualityColor(agent.qualityScore)}${Math.round(agent.qualityScore * 100)}%${c.reset}`
					);
				}
				if (agent.loadFactor !== undefined) {
					console.log(
						`   Load: ${getLoadColor(agent.loadFactor)}${Math.round(agent.loadFactor * 100)}%${c.reset}`
					);
				}
			}
		...
		console.error(
			`${c.red}❌ Failed to list agents: ${error.message}${c.reset}`
		);
		if (options.verbose) {
	...
	try {
		...
		if (options.testStreaming) {
			...
			for await (const chunk of agent.streamResponse(testTask, {})) {
				chunkCount++;
				console.log(
					`   ${chunkCount}. ${chunk.type}: ${chunk.content || chunk.message || 'No content'}`
				);

				if (chunkCount >= 5 && !options.fullStream) {
					console.log(
						`   ... (truncated, use --full-stream for complete output)`
					);
					break;
				}
			}
	...
	try {
		...
		if (provider) {
			...
			if (options.stats) {
				...
				console.log(
					`Success Rate: ${Math.round((stats.successfulRequests / Math.max(stats.totalRequests, 1)) * 100)}%`
				);
				console.log(
					`Average Quality: ${Math.round(stats.averageQuality * 100)}%`
				);
			}
		}
	} catch (error) {
...
export async function generateCodeCommand(task, options = {}) {
	try {
		...
		if (options.stream) {
			...
			for await (const chunk of agent.streamResponse(taskObj, {
				language: options.language
			})) {
				const timestamp = new Date().toISOString().substr(11, 8);
				console.log(
					`${c.gray}[${timestamp}]${c.reset} ${chunk.type}: ${chunk.content || chunk.message || 'Progress update'}`
				);
			...
			if (result.success) {
				console.log(
					`${c.green}✅ Code generation completed (${duration}ms)${c.reset}`
				);
				console.log(
				...
				if (result.metadata && options.verbose) {
					...
					console.log(
						`Agent: ${result.metadata.agentType} (${result.metadata.agentId})`
					);
		...
		console.error(
			`${c.red}❌ Code generation failed: ${error.message}${c.reset}`
		);
		if (options.verbose) {
			console.error(`${c.gray}${error.stack}${c.reset}`);
		}
...
export async function agentStatsCommand(options = {}) {
	try {
		...
		console.log(
			`Healthy Agents: ${getHealthColor(stats.healthyAgents, stats.activeAgents)}${stats.healthyAgents}/${stats.activeAgents}${c.reset}`
		);

		console.log(`\n${c.bright}Usage Statistics:${c.reset}`);
		console.log(`Total Requests: ${c.cyan}${stats.totalRequests}${c.reset}`);
		console.log(
			`Successful Requests: ${c.cyan}${stats.successfulRequests}${c.reset}`
		);
		if (stats.totalRequests > 0) {
			const successRate =
				(stats.successfulRequests / stats.totalRequests) * 100;
			console.log(
				`Success Rate: ${getPercentageColor(successRate)}${Math.round(successRate)}%${c.reset}`
			);
		}
		...
		console.log(
			`Average Quality Score: ${getQualityColor(stats.averageQuality)}${qualityPercentage}%${c.reset}`
		);
		...
		if (options.detailed) {
			...
			for (const agent of agents) {
				if (agent.metrics && agent.metrics.totalRequests > 0) {
					...
					console.log(
						`  Success Rate: ${Math.round((agent.metrics.successfulRequests / agent.metrics.totalRequests) * 100)}%`
					);
					console.log(
						`  Avg Response Time: ${Math.round(agent.metrics.averageResponseTime)}ms`
					);
					console.log(`  Total Cost: $${agent.metrics.totalCost.toFixed(6)}`);
					if (agent.qualityScore !== null) {
						console.log(
							`  Quality Score: ${Math.round(agent.qualityScore * 100)}%`
						);
					}
				}
			}
		...
		console.error(
			`${c.red}❌ Failed to get statistics: ${error.message}${c.reset}`
		);
		if (options.verbose) {
			console.error(`${c.gray}${error.stack}${c.reset}`);
		}
