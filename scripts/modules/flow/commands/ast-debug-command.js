...
export class ASTDebugCommand {
    ...
    async debugFile(filePath, options = {}) {
        try {
            ...
            if (!parser) {
                console.log(`⚠️  No parser available for ${language}, using fallback analysis`);
            ...
            try {
                ...
                // Validate the result
                const validation = await this.validator.validateAST(result, filePath, content, language);
                ...
                const debugResult = await this.debugTools.debugParsingFailure(
                    parseError, filePath, content, language, options
                );
                
                // Attempt error recovery
                const recoveryResult = await this.errorRecovery.recoverFromError(
                    parseError, filePath, content, language
                );
                ...
                const fallbackResult = await this.parserFallbacks.parseWithFallbacks(
                    filePath, content, language, parser
                );
    ...
    async diagnoseDirectory(dirPath = '.', options = {}) {
        try {
            ...
            const files = await this.findCodeFiles(dirPath, options.recursive !== false);
    ...
    async showStats() {
        ...
        console.log(`  Avg issues per file: ${debugStats.averageIssuesPerFile.toFixed(2)}`);
        ...
        console.log(`  Average confidence: ${validatorStats.averageConfidence.toFixed(2)}`);
        console.log(`  Avg validation time: ${validatorStats.averageValidationTime.toFixed(1)}ms`);
        ...
        console.log(`  Avg recovery time: ${recoveryStats.averageRecoveryTime.toFixed(1)}ms`);
        ...
        console.log(`  Avg fallback time: ${fallbackStats.averageFallbackTime.toFixed(1)}ms`);
            ...
            try {
                ...
                for (const entry of entries) {
                    ...
                    if (entry.isDirectory()) {
                        ...
                        if (!['node_modules', '.git', 'dist', 'build', '__pycache__'].includes(entry.name) && recursive) {
    ...
    async runFallbackAnalysis(filePath, content, language) {
        ...
        const fallbackResult = await this.parserFallbacks.parseWithFallbacks(
            filePath, content, language, null
        );
        
        if (fallbackResult.success) {
            console.log(`✅ Fallback analysis successful`);
            console.log(`📋 Strategy: ${fallbackResult.strategy}`);
            console.log(`🔍 Found ${fallbackResult.functions.length} functions, ${fallbackResult.classes.length} classes`);
        } else {
    ...
    displayValidationResults(validation, detailed = false) {
        ...
        if (detailed && validation.checks) {
            console.log(`\n🔍 Detailed Checks:`);
            for (const [checkName, checkResult] of Object.entries(validation.checks)) {
                console.log(`  ${checkName}: ${checkResult.isValid ? '✅' : '❌'} (${(checkResult.confidence * 100).toFixed(1)}%)`);
            }
        }
    }
    ...
    displayDiagnosticSummary(results) {
        ...
        // Success rate
        const successRate = results.totalFiles > 0 ? 
            (results.successful / results.totalFiles) * 100 : 0;
