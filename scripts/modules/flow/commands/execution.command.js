...
function formatStreamingMessage(message, options = {}) {
	...
	switch (message.type) {
		case MESSAGE_TYPES.PROGRESS: {
			const progressBar =
				'█'.repeat(Math.floor(message.data.progress / 5)) +
	...
	try {
		...
		if (options.provider) {
			...
			if (!availableProviders[options.provider]) {
				throw new Error(
					`Provider '${options.provider}' not found. Available: ${Object.keys(availableProviders).join(', ')}`
				);
			}
			...
			if (!config.provider) {
				try {
					const { getFlowSystemConfig } = await import(
						'../services/flow-system-integration.js'
					);
...
export async function statusCommand(executionId = null, options = {}) {
	try {
		if (executionId) {
			...
			console.log(
				`   Started: ${status.startTime ? new Date(status.startTime).toLocaleString() : 'Not started'}`
			);
			console.log(
				`   Completed: ${status.endTime ? new Date(status.endTime).toLocaleString() : 'In progress'}`
			);
			console.log(
			...
			filtered.forEach((exec) => {
				...
				console.log(
					`   ${exec.executionId} | ${exec.taskId} | ${exec.status} | ${exec.progress}% | ${streaming} | ${duration}`
				);
			});
...
export async function streamCommand(executionId, options = {}) {
	try {
		...
		const status = executionService.getExecutionStatus(executionId);
		console.log(
			`📊 Initial Status: ${status.status} (${status.phase}) - ${status.progress}%`
		);
		...
		try {
			for await (const message of executionService.streamExecution(
				executionId
			)) {
				if (shouldStop) break;
			...
			try {
				...
				console.log(
					`📊 Final Status: ${finalStatus.status} - ${finalStatus.progress}%`
				);
			} catch (statusError) {
	...
	try {
		...
		configs.forEach((config, index) => {
			try {
				...
				throw new Error(
					`Task ${index + 1} configuration invalid: ${error.message}`
				);
			}
		});
		...
		results.forEach((result, index) => {
			...
			if (result.status === 'fulfilled') {
				successful++;
				console.log(
					`   ✅ ${config.taskId}: Completed (${result.value.duration}ms)`
				);
			} else {
				failed++;
				console.log(
					`   ❌ ${config.taskId}: Failed - ${result.reason.message}`
				);
			}
		});
...
export async function debugCommand(options = {}) {
	...
	try {
		...
		const execResult = await executionService.executeTask(testConfig, {
			stream: false
		});
		...
		const streamingPromise = executionService.executeTask(streamTestConfig, {
			stream: true
		});
		...
		const streamTimeout = setTimeout(() => {
			console.log(
				`   ⚠️  Stream test timeout (this is expected for completed tasks)`
			);
		}, 2000);
		...
		console.log(
			`   ✅ Streaming test completed (${streamMessages} messages received)`
		);

		// Wait for execution to complete
		const streamResult = await streamingPromise;
		console.log(
			`   ✅ Streaming execution completed: ${streamResult.executionId}`
		);
		...
		if (executions.length > 0) {
			...
			const status = executionService.getExecutionStatus(
	...
	try {
		switch (action) {
			...
			case 'test':
				if (!provider) {
					console.error(
						'Provider name required for testing. Use --provider <name>'
					);
					process.exit(1);
				}
			...
			case 'capabilities':
				if (!provider) {
					console.error(
						'Provider name required for capabilities. Use --provider <name>'
					);
					process.exit(1);
				}
			...
			case 'set':
				if (!provider) {
					console.error(
						'Provider name required for set command. Use: flow provider set <name>'
					);
					process.exit(1);
				}
			...
			case 'switch':
				if (!provider) {
					console.error(
						'Provider name required for switch command. Use: flow provider switch <name>'
					);
					process.exit(1);
				}
...
async function listProviders({ verbose, json }) {
	const { availableProviders, globalRegistry } = await import(
	...
	for (const [key, provider] of Object.entries(availableProviders)) {
		...
		console.log(
			`   Status: ${hasApiKey ? 'Ready' : `Missing ${authConfig.envKey || 'API Key'}`}`
		);
...
async function testProvider({ provider, verbose, json }) {
	...
	try {
		...
		if (!availableProviders[provider]) {
			throw new Error(
				`Provider '${provider}' not found. Available: ${Object.keys(availableProviders).join(', ')}`
			);
		}
		...
		if (healthResult.success) {
			...
			if (verbose) {
				...
				if (healthResult.templatesAvailable !== undefined) {
					console.log(
						`   Templates Available: ${healthResult.templatesAvailable}`
					);
				}
				...
				if (healthResult.profilesAvailable !== undefined) {
					console.log(
						`   Profiles Available: ${healthResult.profilesAvailable}`
					);
				}
			}
		} else {
			...
			if (providerInfo.config.authentication.type === 'api_key') {
				...
				console.log(
					`   API Key (${envKey}): ${hasKey ? 'Configured' : 'Missing/Invalid'}`
				);

				if (!hasKey) {
					console.log(
						`   💡 Set the ${envKey} environment variable to test this provider`
					);
				}
			}
		}
...
async function getProviderCapabilities({ provider, json }) {
	try {
		...
		for (const [feature, supported] of Object.entries(capabilities.features)) {
			...
			console.log(
				`  ${icon} ${feature.replace(/([A-Z])/g, ' $1').toLowerCase()}`
			);
		}
		console.log('');
		console.log('Limits:');
...
async function setDefaultProvider({ provider, verbose, json }) {
	try {
		...
		if (
			...
			providerInfo.config.authentication.type === 'api_key'
		) {
			console.log(`⚠️  Warning: ${providerInfo.name} health check failed`);
			console.log(`   Error: ${healthResult.error}`);
			console.log(
				`   💡 You may need to set the ${providerInfo.config.authentication.envKey} environment variable`
			);
		...
		console.log(
			`✅ Default provider set to: ${providerInfo.name} (${provider})`
		);
		...
		if (verbose) {
			...
			console.log(
				`   Documentation: ${providerInfo.metadata.documentation || 'N/A'}`
			);
		}
