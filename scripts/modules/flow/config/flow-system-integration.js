...
import { flowConfig, initializeFlowConfig } from './flow-config.js'
import { initializeErrorHandling, errorRecoveryManager } from '../errors/flow-errors.js'
...
export async function initializeFlowSystem(options = {}) {
  ...
  try {
    ...
    const stateFile = path.join(projectRoot, '.taskmaster/flow/state/initialized.json')
...
function setupRecoveryStrategies(errorManager, logger) {
  ...
  errorManager.registerRecoveryStrategy('ProviderError', async (error, options) => {
  ...
  errorManager.registerRecoveryStrategy('AgentError', async (error, options) => {
  ...
  errorManager.registerRecoveryStrategy('NetworkError', async (error, options) => {
...
function isFlowSystemInitialized() {
  const stateFile = path.join(process.cwd(), '.taskmaster/flow/state/initialized.json')
  ...
  return {
    ...
    async execute(fn, executionOptions = {}) {
      ...
      try {
        ...
        if (logEnd) {
          await operationLogger.info(`Operation ${name} completed successfully`)
        }
    ...
    async withTimeout(fn, timeout = null) {
      ...
      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
          reject(new Error(`Operation ${name} timed out after ${actualTimeout}ms`))
        }, actualTimeout)
