...
class ASTConfigManager {
    constructor(options = {}) {
        ...
        this.configFile = path.join(this.projectRoot, '.taskmaster', 'ast-config.json');
        ...
        try {
            ...
            try {
                ...
                if (error.code === 'ENOENT') {
                    ...
                    throw new Error(`Failed to parse AST configuration: ${error.message}`);
                }
            }
            
            // Merge with defaults
            this.currentConfig = this.mergeConfigurations(this.defaultConfig, fileConfig);
    ...
    validateConfiguration(config) {
        ...
        try {
            ...
            if (ast.cacheInvalidation) {
                const validStrategies = ['conservative', 'selective', 'aggressive', 'immediate'];
                if (ast.cacheInvalidation.strategy && !validStrategies.includes(ast.cacheInvalidation.strategy)) {
                    errors.push(`cacheInvalidation.strategy must be one of: ${validStrategies.join(', ')}`);
                }
            }
            ...
            if (ast.worktreeManager) {
                ...
                if (ast.worktreeManager.coordinationStrategy && !validCoordStrategies.includes(ast.worktreeManager.coordinationStrategy)) {
                    errors.push(`worktreeManager.coordinationStrategy must be one of: ${validCoordStrategies.join(', ')}`);
                }
            }
    ...
    async resetToDefaults(section = null) {
        try {
            if (section) {
                ...
                configCurrent[finalSectionName] = JSON.parse(JSON.stringify(defaultCurrent[finalSectionName]));
