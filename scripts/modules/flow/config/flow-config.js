...
const FlowConfigSchema = z.object({
	...
	claudeCode: z
		.object({
			...
			permissionMode: z
				.enum(['acceptEdits', 'requireConfirmation', 'denyAll'])
				.default('acceptEdits'),
			...
			systemPrompts: z
				.object({
					...
					subtask: z
						.string()
						.default(
							'You are Claude Code, helping implement a specific subtask. Focus on the task requirements and context provided.'
						)
				})
				.default({})
		})
				...
				.object({
					...
					email: z
						.object({
							...
							events: z
								.array(z.string())
								.default(['error', 'pr-created', 'pr-merged', 'checks-failed'])
						})
						.default({}),
					slack: z
						.object({
					...
					telegram: z
						.object({
							...
							events: z
								.array(z.string())
								.default(['error', 'critical', 'pr-merged'])
						})
						.default({}),
					whatsapp: z
						.object({
				...
				.object({
					...
					rules: z
						...
						.default([
							...
							{
								trigger: 'critical',
								channels: ['app', 'slack', 'email', 'telegram', 'sms']
							}
						])
				})
				.default({}),
				...
				.object({
					prCreated: z
						.object({
							...
							message: z
								.string()
								.default(
									'Pull request #{{prNumber}} has been created for task {{taskId}}'
								),
					...
					prMerged: z
						.object({
							...
							message: z
								.string()
								.default(
									'Pull request #{{prNumber}} has been successfully merged'
								),
							priority: z.enum(['normal', 'high', 'critical']).default('normal')
						})
						.default({}),
					checksFailed: z
						.object({
							...
							message: z
								.string()
								.default(
									'Pull request #{{prNumber}} has failing checks: {{failedChecks}}'
								),
					...
					sessionCompleted: z
						.object({
							...
							message: z
								.string()
								.default('Claude Code session completed for task {{taskId}}'),
							priority: z.enum(['normal', 'high', 'critical']).default('normal')
						})
						.default({}),
					error: z
						.object({
							...
							message: z
								.string()
								.default('An error occurred: {{errorMessage}}'),
							priority: z.enum(['normal', 'high', 'critical']).default('high')
						})
						.default({}),
					critical: z
						.object({
							...
							message: z
								.string()
								.default('Critical issue detected: {{alertMessage}}'),
							priority: z
	...
	// Providers
	defaultProvider: z
		.enum(['mock', 'e2b', 'daytona', 'modal', 'fly'])
		.default('mock'),
