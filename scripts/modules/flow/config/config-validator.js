/**
 * Configuration Validator for Phase 4.1
 * 
 * Provides comprehensive validation for AST configuration with:
 * - Critical validation (prevents startup)
 * - Warning validation (logs warnings)
 ...
 * 
 * Validates AST configuration and determines if issues are critical or warning-level.
 */
        ...
        return {
            critical: [
                ...
                },
                {
                    ...
                    message: 'cacheInvalidation.strategy must be one of: conservative, selective, aggressive, immediate'
                },
                {
                    path: 'ast.worktreeManager.coordinationStrategy',
                    type: 'enum',
                    values: ['safe', 'balanced', 'fast'],
                    message: 'worktreeManager.coordinationStrategy must be one of: safe, balanced, fast'
                }
            ],
            ...
            warning: [
                ...
                },
                {
                    ...
                    type: 'range',
                    max: 10,
                    message: 'fileWatching.maxConcurrentAnalysis > 10 may cause high CPU usage'
                },
                {
                    path: 'ast.fileWatching.batchDelay',
                    type: 'range',
                    min: 100,
                    message: 'fileWatching.batchDelay < 100ms may cause excessive processing'
                },
                {
                    path: 'ast.cacheInvalidation.maxDependencyDepth',
                    type: 'range',
                    max: 10,
                    message: 'cacheInvalidation.maxDependencyDepth > 10 may impact performance'
                },
                {
                    path: 'ast.worktreeManager.maxConcurrentWatchers',
                    type: 'range',
                    max: 20,
                    message: 'worktreeManager.maxConcurrentWatchers > 20 may cause resource exhaustion'
                },
                {
                ...
                },
                {
                    ...
                    type: 'size',
                    max: '1GB',
                    message: 'performance.maxMemoryUsage > 1GB may impact system performance'
                }
            ]
        };
    }
