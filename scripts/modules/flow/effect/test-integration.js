...
import { healthCheck, extendedHealthCheck, quickHealthCheck } from './effects/health.js';
import { getDefaultConfig, validateEffectConfig } from './config.js';
import { isEffectAvailable, EFFECT_FEATURES, EFFECT_MODULE_VERSION } from './index.js';
...
export async function runBasicIntegrationTest() {
  ...
  try {
    ...
    results.tests.effectAvailable = {
      ...
      message: effectAvailable ? 'Effect is available' : 'Effect is not available'
    };
    ...
    results.tests.configSystem = {
      passed: configValid,
      message: configValid ? 'Configuration system working' : 'Configuration validation failed',
      data: defaultConfig
    };
    ...
    results.tests.featureFlags = {
      ...
      message: hasRequiredFeatures ? 'Feature flags configured correctly' : 'Feature flags misconfigured',
