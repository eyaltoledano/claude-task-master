...
const onAdd = (projectRoot, assetsDir) => {
	...
	if (fs.existsSync(sourceCommandsDir)) {
		try {
			...
			console.log(`[INFO] Adding ${commandFiles.length} custom slash commands (/tm-*) to .cursor/commands`);
			let successCount = 0;
			...
			console.log(`[SUCCESS] Added ${successCount} Task Master slash commands to Cursor IDE`);
			return { success: successCount, failed: commandFiles.length - successCount };
		} catch (error) {
	...
	if (fs.existsSync(commandsDir)) {
		try {
			...
			if (commandFiles.length > 0) {
				console.log(`[INFO] Removing ${commandFiles.length} Task Master slash commands (/tm-*) from .cursor/commands`);
				let removeCount = 0;
				commandFiles.forEach(commandFile => {
				...
				console.log(`[SUCCESS] Removed ${removeCount} Task Master slash commands from Cursor IDE`);
				return { success: removeCount, failed: commandFiles.length - removeCount };
			}
		} catch (error) {
