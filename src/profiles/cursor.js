...
const onAdd = (projectRoot, assetsDir) => {
	...
	const transform = content => content
		...
		.replace(/Type '\/project:tm\/' and use tab completion/g, "Type 'tm/' and use tab completion");
	...
	console.log(`[SUCCESS] Added ${count} Task Master slash commands to Cursor IDE`);
...
const onRemove = (projectRoot) => {
	...
	try {
		...
		console.log(`[SUCCESS] Removed ${count} Task Master slash commands from Cursor IDE`);
		return { success: count, failed: 0 };
	} catch (error) {
