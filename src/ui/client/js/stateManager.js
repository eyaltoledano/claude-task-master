...
class StateManager {
    ...
    init() {
        ...
        console.log('StateManager initialized with', Object.keys(this.currentState).length, 'tasks');
    }
     ...
     */
    captureInitialState() {
        const cards = document.querySelectorAll('.task-card, .main-task-card, .subtask-card');
    ...
    updateTaskDOM(change) {
        ...
        if (!targetContainer) {
            console.error(`Target container not found for status: ${change.toStatus}`);
            return;
        }
        ...
        if (previousState && previousState[change.taskId]) {
            ...
            requestAnimationFrame(() => {
                if (taskState.element) {
                    ...
                    let targetContainer = document.querySelector(`[data-column="${prevTaskState.status}"] .task-container`) ||
                    ...
                    if (targetContainer) {
                        ...
                        if (taskState.element.parentNode !== targetContainer) {
                            ...
                            if (prevTaskState.originalIndex >= 0 && targetContainer.children[prevTaskState.originalIndex]) {
    ...
    restore(data) {
        if (data.currentState) {
            ...
            Object.values(this.currentState).forEach(taskState => {
                const element = document.querySelector(`[data-task-id="${taskState.id}"]`);
                if (element) {
    ...
    destroy() {
        ...
        document.removeEventListener('taskStatusChanged', this.handleExternalChange);
