...
class TaskCard {
	...
	static create(task, parentTask = null) {
		...
		const parentProgress = isSubtask
			? this.createParentProgress(parentTask)
			: null;
	...
	static createParentProgress(parentTask) {
		...
		const completed = parentTask.subtasks.filter(
			(st) => st.status === 'done'
		).length;
	...
	static createTaskFooter(task) {
		...
		if (task.dependencies && task.dependencies.length > 0) {
			task.dependencies.forEach((depId) => {
				...
				const isReady =
					task.dependencyStatus && task.dependencyStatus[depId] === 'done';
				if (isReady) {
		...
		const hours =
			task.estimatedHours || (task.complexity ? task.complexity * 0.5 : 2);
	...
	static highlightRelatedTasks(parentId) {
		...
		document
			.querySelectorAll(`[data-parent-id="${parentId}"]`)
			.forEach((card) => {
	...
	static createFromTask(task, allTasks = []) {
		...
		const parentTask = allTasks.find(
			(t) => t.subtasks && t.subtasks.some((st) => st.id === task.id)
		);
	...
	static update(cardElement, task, parentTask = null) {
		...
		const classesToKeep = currentClasses.filter((cls) =>
