...
class ErrorBoundary {
    ...
    handleComponentError(wrapperId, error) {
        ...
        const toastManager = (typeof window !== 'undefined' && window.toast) || (typeof global !== 'undefined' && global.toast);
        if (toastManager) {
    ...
    attachGlobalHandlers() {
        ...
        this.globalErrorHandler = (event) => {
            ...
            const boundary = target?.closest && target.closest('[data-error-boundary]');
            ...
            // Log uncaught errors
            if (this.options.logErrors) {
                console.error('Uncaught error:', error || (message ? new Error(message) : 'Unknown error'));
            }
        };
...
export const errorBoundary = {
    wrap: (selector, componentName, renderFn) => getErrorBoundary().wrap(selector, componentName, renderFn),
