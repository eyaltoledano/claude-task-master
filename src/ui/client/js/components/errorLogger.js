...
class ErrorLogger {
     ...
     * @param {boolean} options.enableRemoteLogging - Send logs to remote endpoint (default: false)
     * @param {string} options.remoteEndpoint - Remote logging endpoint
     * @param {number} options.batchSize - Batch size for remote logging (default: 10)
     */
    constructor(options = {}) {
        this.options = {
            maxLogs: 100,
        ...
        this.currentLevel = this.logLevels[this.options.logLevel] !== undefined ? this.logLevels[this.options.logLevel] : 3;
        ...
        // Set up remote logging interval
        ...
        if (format === 'csv') {
            ...
            this.logs.forEach(log => {
                ...
                const row = [
                    log.timestamp,
                    log.level,
                    message.includes(',') || message.includes('"') ? `"${message}"` : message,
                    `"${metadata}"`
                ];
                rows.push(row.join(','));
            });
    ...
    /**
     * Start remote logging interval
     * @private
     */
    startRemoteLogging() {
        // Check batch immediately if needed
        this.checkRemoteBatch();
    ...
    /**
     * Check if remote batch should be sent
     * @private
     */
    checkRemoteBatch() {
        if (this.pendingRemoteLogs.length >= this.options.batchSize) {
            // Send asynchronously to avoid blocking
            setTimeout(() => this.sendRemoteLogs(), 0);
        }
    }
    ...
    async sendRemoteLogs() {
        if (this.pendingRemoteLogs.length === 0) return;
        ...
        try {
            ...
            if (!response.ok) {
                ...
                this.pendingRemoteLogs.unshift(...logsToSend);
            }
        } catch (error) {
            // Put logs back in queue
            this.pendingRemoteLogs.unshift(...logsToSend);
    ...
    destroy() {
        ...
        if (this.options.enableRemoteLogging && this.pendingRemoteLogs.length > 0) {
            this.sendRemoteLogs();
        }
