...
class Column {
    ...
    static setupColumnHeader(columnElement, columnId, config) {
        ...
        if (menuButton) {
            menuButton.setAttribute('aria-label', `Column options for ${config.title}`);
    ...
    static handleDragEnter(event, columnId) {
        ...
        this.announceToScreenReader(`Entering ${this.config[columnId].title} column`);
    }
    ...
    static handleMenuItemClick(event, columnId, action) {
        ...
        this.announceToScreenReader(`${action.replace('-', ' ')} for ${config.title} column`);
    }
    ...
    static clearColumn(columnId) {
        ...
        const confirmed = confirm(`Are you sure you want to clear all tasks from ${config.title}?`);
        if (confirmed) {
    ...
    static updateTaskCount(columnId, count) {
        ...
        if (countElement) {
            ...
            countElement.setAttribute('aria-label', `${count} task${count !== 1 ? 's' : ''}`);
        }
