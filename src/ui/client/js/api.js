...
class TaskAPI {
	...
	static async request(endpoint, options = {}) {
		...
		try {
			...
			console.log(
				`API Request: ${requestOptions.method} ${url}`,
				requestOptions
			);
	...
	static async getTasks(filters = {}) {
		...
		const endpoint =
			'/tasks' + (queryParams.toString() ? `?${queryParams}` : '');
	...
	static async bulkUpdateTasks(updates) {
		...
		try {
			...
			if (error.status === 0) {
				...
				this.setCachedTasks(cachedTasks);
				console.log(
					'Bulk updated local tasks due to network error:',
					updatedTasks
				);
	...
	static async searchTasks(query, filters = {}) {
		...
		try {
			...
			const cachedTasks = this.getCachedTasks() || [];
			const filteredTasks = this.searchTasksLocally(
				cachedTasks,
				query,
				filters
			);
	...
	static async syncLocalChanges() {
		const cachedTasks = this.getCachedTasks() || [];
		const localTasks = cachedTasks.filter(
			(task) => task.isLocal || task.isLocalUpdate
		);
		...
		console.log(
			`Sync completed: ${syncResults.synced} synced, ${syncResults.errors.length} errors`
		);
		return syncResults;
	}
