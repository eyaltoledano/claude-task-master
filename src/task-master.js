...
export function initTaskMaster(overrides = {}) {
    ...
    const resolvePath = (pathType, override, defaultPaths = [], basePath = null) => {
    ...
    if ('taskMasterDir' in overrides) {
        paths.taskMasterDir = resolvePath('taskmaster directory', overrides.taskMasterDir, [TASKMASTER_DIR], paths.projectRoot);
    } else {
        paths.taskMasterDir = resolvePath('taskmaster directory', false, [TASKMASTER_DIR], paths.projectRoot);
    }
    ...
    if ('statePath' in overrides) {
        paths.statePath = resolvePath('state file', overrides.statePath, ['state.json'], paths.taskMasterDir);
    }
    
    if ('tasksPath' in overrides) {
        paths.tasksPath = resolvePath('tasks file', overrides.tasksPath, [TASKMASTER_TASKS_FILE, LEGACY_TASKS_FILE], paths.projectRoot);
    }
    
    if ('prdPath' in overrides) {
        paths.prdPath = resolvePath('PRD file', overrides.prdPath, [
    ...
    if ('complexityReportPath' in overrides) {
        paths.complexityReportPath = resolvePath('complexity report', overrides.complexityReportPath, [
