	...
	if (bodyObj.response_format && typeof bodyObj.response_format === 'object') {
		...
		if (responseFormat.type === 'json_schema' && responseFormat.json_schema) {
			...
			if (jsonSchema.schema) {
				jsonSchema.schema = removeUnsupportedFeatures(
		...
		if (responseFormat.type === 'json' && responseFormat.schema) {
			responseFormat.schema = removeUnsupportedFeatures(
			...
			if (tool && typeof tool === 'object') {
				...
				if (toolObj.function && typeof toolObj.function === 'object') {
					...
					if (funcObj.parameters) {
						funcObj.parameters = removeUnsupportedFeatures(
 ...
 */
export async function getSnowflakeBaseURL(
	settings: SnowflakeProviderSettings
): Promise<string> {
