...
export const listSkillsInputSchema = z.object({
	directories: z.array(z.string()).default(['.claude/skills', '.cortex/skills', 'assets/cortex-skills'])
...
export const listSkillsTool: ToolDefinition<ListSkillsInput, ListSkillsResult> = {
	description: 'List available skills with metadata from YAML front-matter and complete file inventory. Returns skill name, description, path, and ALL files in the skill folder (including scripts/, references/, assets/ subdirectories). Use file_read to load specific files as needed.',
	parameters: listSkillsInputSchema,
	execute: async (input: ListSkillsInput): Promise<ListSkillsResult> => {
		const { directories = ['.claude/skills', '.cortex/skills', 'assets/cortex-skills'], skillName } = input;
