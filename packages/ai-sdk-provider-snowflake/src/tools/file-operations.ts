...
function getFileType(filename: string): string {
	...
	const typeMap: Record<string, string> = {
		md: 'markdown', txt: 'text', py: 'python', js: 'javascript', ts: 'typescript',
		jsx: 'javascript', tsx: 'typescript', sh: 'shell', bash: 'shell', json: 'json',
		yaml: 'yaml', yml: 'yaml', xml: 'xml', html: 'html', css: 'css', sql: 'sql',
		go: 'go', rs: 'rust', java: 'java', c: 'c', cpp: 'cpp', h: 'header', hpp: 'header'
	};
...
function shouldIgnore(filePath: string, ignore: string[]): boolean {
	...
	return ignore.some(pattern => pathParts.some(part => part === pattern || part.startsWith(pattern)));
}
	...
	try {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				dirCount++;
				const children = await buildTree(fullPath, basePath, maxDepth, currentDepth + 1, includeFiles, ignore, fs, path);
				nodes.push({ name: entry.name, type: 'directory', path: relativePath, children: children.nodes });
		...
		try {
			...
			for (const entry of entries) {
				...
				if (entry.isDirectory()) {
					...
					if (matchPattern(entry.name, pattern) || matchPattern(relativePath, pattern)) {
						try {
							const stat = await fs.stat(fullPath);
							results.push({ path: relativePath, size: stat.size, type: getFileType(entry.name), modified: stat.mtime });
						} catch {
							results.push({ path: relativePath, size: 0, type: getFileType(entry.name) });
						}
					}
				}
			}
 ...
 */
async function grepFile(filePath: string, pattern: RegExp, contextLines: number, fs: typeof import('fs/promises')): Promise<GrepMatch[]> {
...
export const projectTreeInputSchema = z.object({
	...
	ignore: z.array(z.string()).default(['node_modules', '.git', 'dist', 'coverage', '__pycache__', '.next', 'build'])
...
export const grepInputSchema = z.object({
	...
	directory: z.string().default('.').describe('Directory to search in'),
	filePattern: z.string().default('*').describe('Glob pattern for files to search'),
...
export const projectTreeTool: ToolDefinition<ProjectTreeInput, ProjectTreeResult> = {
	...
	execute: async (input: ProjectTreeInput): Promise<ProjectTreeResult> => {
		const { directory = '.', maxDepth = 3, includeFiles = true, ignore = ['node_modules', '.git', 'dist', 'coverage', '__pycache__', '.next', 'build'] } = input;
		const fs = await import('fs/promises');
		const path = await import('path');
		
		const projectRoot = process.env.PROJECT_ROOT || process.cwd();
		const targetDir = path.isAbsolute(directory) ? directory : path.join(projectRoot, directory);
		
		const { nodes, fileCount, dirCount } = await buildTree(targetDir, targetDir, maxDepth, 0, includeFiles, ignore, fs, path);
		
		return { root: path.relative(projectRoot, targetDir) || '.', tree: nodes, totalFiles: fileCount, totalDirectories: dirCount };
	}
};
...
export const fileSearchTool: ToolDefinition<FileSearchInput, FileSearchResult> = {
	description: 'Search for files matching a glob pattern (e.g., "*.ts", "**/*.test.js"). Returns file paths with metadata.',
	...
	execute: async (input: FileSearchInput): Promise<FileSearchResult> => {
		...
		const defaultIgnore = ['node_modules', '.git', 'dist', 'coverage', '__pycache__', '.next', 'build'];
		const matches = await searchFiles(targetDir, targetDir, pattern, defaultIgnore, maxResults + 1, fs, path);
		
		const truncated = matches.length > maxResults;
		const results = matches.slice(0, maxResults);
		
		return { pattern, directory: path.relative(projectRoot, targetDir) || '.', matches: results, totalMatches: results.length, truncated };
	}
};
	...
	execute: async (input: FileReadInput): Promise<FileReadResult> => {
		...
		try {
			...
			if (startLine !== undefined || endLine !== undefined) {
				...
				resultContent = selectedLines.map((line, i) => `${String(actualStartLine + i).padStart(6)}| ${line}`).join('\n');
			} else {
				resultContent = lines.map((line, i) => `${String(i + 1).padStart(6)}| ${line}`).join('\n');
			}
			
			return { path: path.relative(projectRoot, targetPath) || filePath, content: resultContent, startLine: actualStartLine, endLine: actualEndLine, totalLines };
		} catch (error) {
			return { path: filePath, content: `Error reading file: ${error instanceof Error ? error.message : 'Unknown error'}`, totalLines: 0 };
		}
	}
};
...
export const grepTool: ToolDefinition<GrepInput, GrepResult> = {
	...
	execute: async (input: GrepInput): Promise<GrepResult> => {
		...
		const defaultIgnore = ['node_modules', '.git', 'dist', 'coverage', '__pycache__', '.next', 'build'];
		const files = await searchFiles(targetDir, targetDir, filePattern, defaultIgnore, 1000, fs, path);
		...
		return { pattern, directory: path.relative(projectRoot, targetDir) || '.', matches: allMatches.slice(0, maxMatches), totalMatches: allMatches.length, truncated: allMatches.length > maxMatches };
	}
};

export { projectTreeTool as projectTree, fileSearchTool as fileSearch, fileReadTool as fileRead, grepTool as grep };
export default { projectTree: projectTreeTool, fileSearch: fileSearchTool, fileRead: fileReadTool, grep: grepTool };
