...
async function loadTasks(
	...
	const tasksPath = pathModule.join(
		projectRoot,
		'.taskmaster',
		'tasks',
		'tasks.json'
...
export const listTasksInputSchema = z.object({
	status: z
		.enum(['all', 'pending', 'in-progress', 'done', 'blocked', 'cancelled'])
		.default('all')
...
export const getNextTaskInputSchema = z.object({
	tag: z
		.string()
		.optional()
		.describe('Tag context (defaults to current active tag)')
});
...
export const listTasksTool: ToolDefinition<ListTasksInput, ListTasksResult> = {
	description:
		'List TaskMaster tasks with status and priority. Useful for understanding project progress and what needs to be done.',
	...
	execute: async (input: ListTasksInput): Promise<ListTasksResult> => {
		...
		const summaries: TaskSummary[] = tasks.map((task) => {
			...
			if (withSubtasks && task.subtasks) {
				const subtaskStatuses = task.subtasks.reduce(
					(acc, st) => {
				...
				summary.title = `${task.title} (${task.subtasks.length} subtasks: ${Object.entries(
					subtaskStatuses
				)
					.map(([s, c]) => `${c} ${s}`)
					.join(', ')})`;
...
export const getTaskTool: ToolDefinition<GetTaskInput, TaskDetails | null> = {
	description:
		'Get detailed information about a specific task or subtask by ID. Returns full task details including description, implementation details, and test strategy.',
...
> = {
	description:
		'Get the next task to work on based on dependencies and priority. Returns the highest priority task with all dependencies satisfied.',
	...
	execute: async (input: GetNextTaskInput): Promise<TaskDetails | null> => {
		...
		const completedIds = new Set(
			result.tasks.filter((t) => t.status === 'done').map((t) => String(t.id))
		);
...
> = {
	description:
		'Get current TaskMaster context (active tag, in-progress tasks, recently completed). Useful for understanding current project state.',
...
export {
	listTasksTool as listTasks,
	getTaskTool as getTask,
	getNextTaskTool as getNextTask,
	getCurrentContextTool as getCurrentContext
};
export default {
