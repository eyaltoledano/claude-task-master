...
import { ENV_VARS, getEnvVar } from '../config/index.js';
 ...
 * Returns null if required fields (account) are missing
 */
function normalizeTomlConfig(
	config: Record<string, unknown>
): SnowflakeConnectionConfig | null {
	// Validate that account exists - it's a required field
...
/**
 * Load connection configuration from TOML files
 * Checks connections.toml first (preferred), then config.toml
 */
function loadConnectionFromToml(
	connectionName: string
): SnowflakeConnectionConfig | null {
	const snowflakeHome =
	...
	for (const tomlPath of tomlFiles) {
		try {
			...
			if (connectionConfig) {
				const normalized = normalizeTomlConfig(connectionConfig);
				if (normalized) {
					return normalized;
				}
				// Connection found but missing required 'account' field, continue to next source
			}
		} catch {
...
export async function resolveConnectionConfig(
	...
	let config = loadConnectionFromToml(connectionName);
...
function buildAccountUrl(account: string, host?: string): string {
	...
	// Replace all periods with hyphens to handle multi-part identifiers (e.g., org.region.account)
...
export async function authenticate(
	...
	// Validate account exists (defensive check - should already be validated by normalizeTomlConfig)
	if (!config.account) {
		throw new Error(
			'Snowflake account identifier is missing from configuration. ' +
				'Ensure your connection profile or environment variables include the account.'
		);
	}
	...
	const cachedToken = tokenCache.get(config.account, username || 'default');
	if (cachedToken) {
		return {
	...
	if (config.privateKeyPath || config.privateKey) {
		...
		tokenCache.set(config.account, username, {
			accessToken,
			expiresAt,
			baseURL: accountUrl
		});
