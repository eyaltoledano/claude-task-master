...
export const ENV_VARS = {
	...
	home: ['SNOWFLAKE_HOME'],
	// Execution mode
	executionMode: ['CORTEX_EXECUTION_MODE', 'SNOWFLAKE_EXECUTION_MODE'],
	// MCP settings
	enableMcpServers: ['CORTEX_ENABLE_MCP_SERVERS'],
	// Tool calls settings
	dangerouslyAllowAllToolCalls: ['CORTEX_DANGEROUSLY_ALLOW_ALL_TOOL_CALLS']
};
...
/** Default user config */
export const DEFAULT_USER_CONFIG: SnowflakeUserConfig = {
...
/**
 * Get the project root directory, with fallback to current working directory.
 *
 * Priority:
 * 1. PROJECT_ROOT environment variable
 * 2. TASKMASTER_PROJECT_ROOT environment variable
 * 3. Walk up to find .taskmaster directory
 * 4. Current working directory (fallback)
 */
export function getProjectRoot(): string {
	// Check PROJECT_ROOT first (used by tools)
	if (process.env.PROJECT_ROOT) {
		return process.env.PROJECT_ROOT;
	}
	// Try to find TaskMaster project root
	const taskMasterRoot = findProjectRoot();
	if (taskMasterRoot) {
...
export function loadUserConfig(forceReload = false): SnowflakeUserConfig {
	...
	try {
		...
		logger.debug('snowflake:config', 'Error loading config:', error);
...
export function getExecutionMode(): ExecutionMode | undefined {
	...
	if (envMode) {
		...
		if (validModes.includes(normalizedMode as ExecutionMode)) {
			...
			// Invalid env var value - log warning and fall through to config check
			// (treat invalid value as "not set")
			logger.warn(
				`Invalid CORTEX_EXECUTION_MODE/SNOWFLAKE_EXECUTION_MODE value: ${envMode}. Valid values: ${validModes.join(', ')}`
			);
		}
	}
