...
import type { SnowflakeUserConfig, ThinkingLevel, ExecutionMode } from '../types.js';
...
export function getExecutionMode(): ExecutionMode | undefined {
	...
	const envMode = process.env.CORTEX_EXECUTION_MODE || process.env.SNOWFLAKE_EXECUTION_MODE;
	...
	if (envMode) {
		...
		if (validModes.includes(normalizedMode as ExecutionMode)) {
			if (process.env.DEBUG?.includes('snowflake:config')) {
				console.log(`[DEBUG snowflake:config] Execution mode from env: ${normalizedMode}`);
			}
			return normalizedMode as ExecutionMode;
		} else {
			console.warn(`[WARN] Invalid CORTEX_EXECUTION_MODE/SNOWFLAKE_EXECUTION_MODE value: ${envMode}. Valid values: ${validModes.join(', ')}`);
		}
	}
	...
	try {
		...
		if (configMode) {
			...
			if (validModes.includes(normalizedMode as ExecutionMode)) {
				if (process.env.DEBUG?.includes('snowflake:config')) {
					console.log(`[DEBUG snowflake:config] Execution mode from config.json: ${normalizedMode}`);
				}
				return normalizedMode as ExecutionMode;
			}
		}
		...
		if (process.env.DEBUG?.includes('snowflake:config')) {
			console.error('[DEBUG snowflake:config] Error reading execution mode from config:', error);
		}
	}
...
export function getEnableMcpServers(): boolean {
	...
	if (envValue !== undefined) {
		...
		if (process.env.DEBUG?.includes('snowflake:config') && !enabled) {
			console.log('[DEBUG snowflake:config] MCP servers disabled from env (--no-mcp will be used)');
		}
		return enabled;
	}
	...
	try {
		...
		if (snowflakeConfig.enableMcpServers === false) {
			if (process.env.DEBUG?.includes('snowflake:config')) {
				console.log('[DEBUG snowflake:config] MCP servers disabled from config.json (--no-mcp will be used)');
			}
			return false;
		}
...
export function getDangerouslyAllowAllToolCalls(): boolean {
	...
	if (envValue !== undefined) {
		...
		if (process.env.DEBUG?.includes('snowflake:config') && enabled) {
			console.log('[DEBUG snowflake:config] dangerouslyAllowAllToolCalls enabled from env');
		}
		return enabled;
	}
	...
	try {
		...
		if (process.env.DEBUG?.includes('snowflake:config') && enabled) {
			console.log('[DEBUG snowflake:config] dangerouslyAllowAllToolCalls enabled from config.json');
		}
