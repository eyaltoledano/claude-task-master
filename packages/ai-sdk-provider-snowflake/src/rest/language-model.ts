			...
			try {
				...
				if (responseText.startsWith('data:')) {
					...
					try {
						...
						logger.debug('snowflake:rest', 'Parse error:', parseError);
						throw parseError;
					}
				}
			...
			start: async (controller) => {
				try {
					...
					while (true) {
						...
						if (done) {
							// Send finish event only if not already emitted
							if (!finishEmitted) {
								controller.enqueue({
									type: 'finish',
									finishReason: 'stop',
									...
									if (tc.id && tc.function?.name) {
										...
										if (tc.function.arguments) {
											try {
												...
												// Emit warning but continue streaming with empty args
												logger.warn(
													`Failed to parse tool call arguments for toolCallId=${tc.id}. ` +
