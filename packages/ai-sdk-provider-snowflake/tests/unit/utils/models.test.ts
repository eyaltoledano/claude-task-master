...
describe('Model Utilities', () => {
	...
	describe('getModelCapabilities', () => {
		it.concurrent('should return capabilities for known model', async () => {
		...
		it.concurrent('should return defaults for unknown model', async () => {
			const caps = getModelCapabilities('unknown-model' as any);
			expect(caps.supportsStructuredOutput).toBe(false);
			expect(caps.maxTokens).toBe(8192);
		});

		it.concurrent('should handle cortex prefix', async () => {
			const caps = getModelCapabilities('cortex/claude-haiku-4-5' as any);
			expect(caps.supportsStructuredOutput).toBe(true);
		});
	});

	describe('supportsPromptCaching', () => {
		it.concurrent('should return true for Claude models', async () => {
			expect(supportsPromptCaching('claude-sonnet-4-5')).toBe(true);
			expect(supportsPromptCaching('claude-haiku-4-5')).toBe(true);
		});

		it.concurrent('should check OpenAI models from KNOWN_MODELS', async () => {
			// OpenAI models use known model data, not fallback
			const result = supportsPromptCaching('openai-gpt-5');
			expect(typeof result).toBe('boolean');
		});

		it.concurrent('should return false for Llama models', async () => {
			expect(supportsPromptCaching('llama3.1-70b')).toBe(false);
		});

		it.concurrent('should use fallback for unknown Claude model', async () => {
			expect(supportsPromptCaching('claude-unknown-v99')).toBe(true);
		});

		it.concurrent('should use fallback for unknown OpenAI model', async () => {
			expect(supportsPromptCaching('openai-unknown-v99')).toBe(true);
		});

		it.concurrent('should return false for unknown non-Claude/OpenAI model', async () => {
			expect(supportsPromptCaching('completely-unknown-model')).toBe(false);
		});
	});

	describe('supportsThinking', () => {
		it.concurrent('should return true for Claude models with thinking', async () => {
			// Check a known Claude model
			const result = supportsThinking('claude-sonnet-4-5');
			expect(typeof result).toBe('boolean');
		});

		it.concurrent('should return false for Llama models', async () => {
			expect(supportsThinking('llama3.1-70b')).toBe(false);
		});

		it.concurrent('should use fallback for unknown Claude model', async () => {
			expect(supportsThinking('claude-unknown-v99')).toBe(true);
		});

		it.concurrent('should return false for unknown non-Claude model', async () => {
			expect(supportsThinking('completely-unknown-model')).toBe(false);
		});
	});

	describe('supportsReasoning', () => {
		it.concurrent('should return true for OpenAI reasoning models', async () => {
			// Check a known reasoning model
			const result = supportsReasoning('openai-o3-mini');
			expect(typeof result).toBe('boolean');
		});

		it.concurrent('should return false for Claude models', async () => {
			expect(supportsReasoning('claude-sonnet-4-5')).toBe(false);
		});

		it.concurrent('should use fallback for unknown OpenAI model', async () => {
			expect(supportsReasoning('openai-unknown-v99')).toBe(true);
		});

		it.concurrent('should return false for unknown non-OpenAI model', async () => {
			expect(supportsReasoning('completely-unknown-model')).toBe(false);
		});
	});

	describe('supportsStreaming', () => {
		it.concurrent('should return true for most models', async () => {
			expect(supportsStreaming('claude-sonnet-4-5')).toBe(true);
			expect(supportsStreaming('llama3.1-70b')).toBe(true);
		});

		it.concurrent('should default to true for unknown model', async () => {
			expect(supportsStreaming('completely-unknown-model')).toBe(true);
		});
	});

	describe('getPreferredEndpoint', () => {
		it.concurrent('should return rest for known models', async () => {
			expect(getPreferredEndpoint('claude-sonnet-4-5')).toBe('rest');
			expect(getPreferredEndpoint('llama3.1-70b')).toBe('rest');
		});

		it.concurrent('should default to rest for unknown model', async () => {
			expect(getPreferredEndpoint('completely-unknown-model')).toBe('rest');
		});
	});

	describe('getThinkingBudgetTokens', () => {
		it.concurrent('should return low budget for low level', async () => {
			expect(getThinkingBudgetTokens('claude-sonnet-4-5', 'low')).toBe(4096);
		});

		it.concurrent('should return medium budget for medium level', async () => {
			const result = getThinkingBudgetTokens('claude-sonnet-4-5', 'medium');
			expect(result).toBeGreaterThanOrEqual(10000);
		});

		it.concurrent('should return high budget for high level', async () => {
			expect(getThinkingBudgetTokens('claude-sonnet-4-5', 'high')).toBe(32000);
		});

		it.concurrent('should default to medium level', async () => {
			const result = getThinkingBudgetTokens('claude-sonnet-4-5');
			expect(result).toBeGreaterThanOrEqual(10000);
		});

		it.concurrent('should use model-specific budget for medium if available', async () => {
			// For a model with specific budget tokens
			const result = getThinkingBudgetTokens('claude-sonnet-4-5', 'medium');
			expect(typeof result).toBe('number');
			expect(result).toBeGreaterThan(0);
		});

		it.concurrent('should use level budget for unknown model', async () => {
			expect(getThinkingBudgetTokens('unknown-model', 'low')).toBe(4096);
			expect(getThinkingBudgetTokens('unknown-model', 'high')).toBe(32000);
		});
	});
});
