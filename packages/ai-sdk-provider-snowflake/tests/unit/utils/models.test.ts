...
describe('Model Utilities', () => {
	describe('KNOWN_MODELS', () => {
		it.concurrent('should contain Claude models with structured output support', async () => {
			expect(KNOWN_MODELS['claude-haiku-4-5'].supportsStructuredOutput).toBe(true);
			expect(KNOWN_MODELS['claude-sonnet-4-5'].supportsStructuredOutput).toBe(true);
			expect(KNOWN_MODELS['claude-4-sonnet'].supportsStructuredOutput).toBe(true);
		});

		it.concurrent('should contain OpenAI models with structured output support', async () => {
			expect(KNOWN_MODELS['openai-gpt-5'].supportsStructuredOutput).toBe(true);
			expect(KNOWN_MODELS['openai-gpt-4.1'].supportsStructuredOutput).toBe(true);
		});

		it.concurrent('should contain Llama models without structured output support', async () => {
			expect(KNOWN_MODELS['llama3.1-8b'].supportsStructuredOutput).toBe(false);
			expect(KNOWN_MODELS['llama3.1-70b'].supportsStructuredOutput).toBe(false);
		});

		it.concurrent('should contain Mistral models without structured output support', async () => {
			expect(KNOWN_MODELS['mistral-7b'].supportsStructuredOutput).toBe(false);
			expect(KNOWN_MODELS['mistral-large2'].supportsStructuredOutput).toBe(false);
		});
	});
	...
	describe('normalizeModelId', () => {
		it.concurrent.each([
			...
			['cortex/ prefix removed', 'cortex/claude-sonnet-4-5', 'claude-sonnet-4-5'],
			['lowercase conversion', 'CLAUDE-HAIKU-4-5', 'claude-haiku-4-5'],
			['combined prefix and case', 'cortex/CLAUDE-SONNET-4-5', 'claude-sonnet-4-5'],
	...
	describe('prefixModelId', () => {
		...
		it.concurrent('should not duplicate cortex/ prefix', async () => {
			expect(prefixModelId('cortex/claude-haiku-4-5')).toBe('cortex/claude-haiku-4-5');
		});

		it.concurrent('should handle model IDs with special characters', async () => {
	...
	describe('supportsStructuredOutputs', () => {
		...
		it.concurrent('should use fallback for unknown Claude-like models', async () => {
			// Unknown model but contains 'claude'
			expect(supportsStructuredOutputs('claude-unknown-version')).toBe(true);
		});

		it.concurrent('should use fallback for unknown OpenAI-like models', async () => {
			// Unknown model but contains 'openai'
			expect(supportsStructuredOutputs('openai-unknown-version')).toBe(true);
		});

		it.concurrent('should use fallback for unknown GPT-like models', async () => {
	...
	describe('supportsTemperature', () => {
		...
		])('should return true for %s', async (_, modelId, isStructured, expected) => {
			expect(supportsTemperature(modelId as any, isStructured)).toBe(expected);
		});
		...
		])('should return false for %s', async (_, modelId, isStructured, expected) => {
			expect(supportsTemperature(modelId, isStructured)).toBe(expected);
		});

		it.concurrent('should allow temperature for OpenAI without structured output', async () => {
	...
	describe('getAvailableModels', () => {
		...
		it.concurrent('should include OpenAI models', async () => {
			const models = getAvailableModels();
			const openaiModels = models.filter(m => m.includes('openai') || m.includes('gpt-'));
	...
	describe('prepareTokenParam', () => {
		it.concurrent('should use model max tokens when not specified', async () => {
		...
		it.concurrent('should handle null/undefined token values', async () => {
			expect(prepareTokenParam('claude-haiku-4-5', null as any).maxTokens).toBe(8192);
			expect(prepareTokenParam('claude-haiku-4-5', undefined).maxTokens).toBe(8192);
		});
	...
	describe('ModelHelpers class', () => {
		it.concurrent('should expose supportsStructuredOutputs', async () => {
			expect(ModelHelpers.supportsStructuredOutputs).toBe(supportsStructuredOutputs);
			expect(ModelHelpers.supportsStructuredOutputs('claude-haiku-4-5')).toBe(true);
		});
		...
		it.concurrent('should expose getUnsupportedStructuredOutputsWarning', async () => {
			expect(ModelHelpers.getUnsupportedStructuredOutputsWarning).toBe(getUnsupportedStructuredOutputsWarning);
