...
skipIfNoCredentials('Snowflake Provider Integration Tests', () => {
	beforeAll(() => {
	...
	// ===== Feature Detection Tests (from Cortex Code) =====
	...
	describe('Schema Transformation Matrix Tests', () => {
		...
		it.each(schemaTransformationMatrix)(
			'should remove %s',
			(_name, schema, removedKeys) => {
				const cleaned = removeUnsupportedFeatures(schema as any);
				const firstPropKey = Object.keys(cleaned.properties)[0];
				const cleanedProp = cleaned.properties[firstPropKey];

				removedKeys.forEach((key) => {
					expect(cleanedProp[key]).toBeUndefined();
				});
			}
		);
