...
import { THINKING_MODEL_IDS, REASONING_MODEL_IDS } from '../../../src/utils/models.js';
...
skipIfNoCredentials('Thinking & Reasoning Integration Tests', () => {
	beforeAll(async () => {
		clearAuthCache();
		logTestEnvironment('Thinking & Reasoning Tests');
	...
	describe('Claude Extended Thinking', () => {
			...
			async (modelId) => {
				...
				try {
					const result = await generateText({
						model,
						prompt: 'What is the sum of the first 10 prime numbers? Show your work.',
					});

					expect(result.text).toBeTruthy();
					
					// Check if thinking content is present in response
					const hasThinking = result.text.includes('<thinking>') || 
					                    result.text.length > 100; // Longer responses suggest thinking
					
					console.log(`[PASS] ${modelId}: Thinking completed (${result.text.length} chars, thinking: ${hasThinking})`);
				} catch (error) {
					const err = error as Error;
					if (err.message.includes('500') || err.message.includes('unavailable') || err.message.includes('thinking')) {
		...
		it.each(thinkingLevels)(
			...
			async (level) => {
				...
				try {
					...
					expect(result.text).toBeTruthy();
					console.log(`[PASS] ${modelId} with ${level} thinking: ${result.text.substring(0, 50)}...`);
				} catch (error) {
					const err = error as Error;
		...
		it('Claude should work without thinking enabled', async () => {
			if (THINKING_MODEL_IDS.length === 0) {
				console.log('[SKIP] No thinking models available');
				return;
			}
	...
	describe('OpenAI Reasoning Effort', () => {
		...
		it.each(REASONING_MODEL_IDS)(
			...
			async (modelId) => {
				...
				try {
					...
					expect(result.text).toBeTruthy();
					console.log(`[PASS] ${modelId}: Reasoning completed (${result.text.length} chars)`);
				} catch (error) {
					const err = error as Error;
					if (err.message.includes('500') || err.message.includes('unavailable')) {
						console.log(`[INFO] ${modelId}: Server unavailable - ${err.message.substring(0, 100)}`);
						return;
					}
					throw error;
				}
		...
		it.each(reasoningLevels)(
			...
			async (level) => {
				...
				try {
					...
					expect(result.text).toBeTruthy();
					console.log(`[PASS] ${modelId} with ${level} reasoning: ${result.text.substring(0, 50)}...`);
				} catch (error) {
					const err = error as Error;
					if (err.message.includes('500') || err.message.includes('unavailable')) {
						console.log(`[INFO] ${modelId} with ${level}: Server unavailable`);
		...
		it('OpenAI should work without reasoning enabled', async () => {
			if (REASONING_MODEL_IDS.length === 0) {
				console.log('[SKIP] No reasoning models available');
				return;
			}
