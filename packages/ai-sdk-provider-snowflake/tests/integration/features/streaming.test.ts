...
describeWithCredentials('Streaming Integration Tests', () => {
	beforeAll(async () => {
		clearAuthCache();
		...
		it.each(claudeStreamingModels)(
			...
			async (modelId) => {
				...
				try {
					...
					console.log(
						`[PASS] ${modelId}: Streamed ${chunkCount} chunks, total ${fullText.length} chars`
					);
				} catch (error) {
	...
	describe('Streaming vs Non-Streaming Consistency', () => {
		it(
			'streaming and non-streaming should produce similar results',
			async () => {
				...
				const modelId = CLAUDE_MODEL_IDS.find((id) =>
					STREAMING_MODEL_IDS.includes(id)
				);
				if (!modelId) {
				...
				try {
					...
					console.log(
						`  Non-streaming: ${nonStreamResult.text.substring(0, 50)}`
					);
	...
	describe('Streaming Disabled', () => {
		it(
			'should throw error when streaming is disabled',
			async () => {
				if (STREAMING_MODEL_IDS.length === 0) {
					console.log('[SKIP] No streaming models available');
					return;
				...
				try {
					...
					throw new Error(
						'Expected an error when streaming is disabled, but no error was thrown'
					);
				} catch (error) {
					const err = error as Error;
					...
					console.log(
						`[PASS] Streaming disabled error: ${err.message.substring(0, 80)}`
					);
				}
			},
			TEST_TIMEOUT
