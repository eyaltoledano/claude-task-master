...
describeWithCredentials('Tool Calling Integration Tests', () => {
	...
	describe('Basic Tool Calling', () => {
		it('should call a single tool', async () => {
			...
			console.log(
				`📊 Steps: ${steps.length}, Total tool calls: ${allToolCalls.length}`
			);
		...
		it('should call multiple tools', async () => {
			...
			const { text, steps } = await generateText({
				...
				prompt:
					'First multiply 6 by 7, then check the weather in New York. Use both tools.',
	...
	describe('Tool Calling with Different Models', () => {
		const modelsToTest = ['cortex/claude-sonnet-4-5', 'cortex/openai-gpt-4.1'];
		...
		for (const modelId of modelsToTest) {
			it(`should handle tool calls with ${modelId}`, async () => {
				try {
					...
					expect(allToolCalls.length).toBeGreaterThan(0);
					const calcCall = allToolCalls.find(
					...
					if (error instanceof Error && error.message.includes('500')) {
						console.log(
							`[INFO] ${modelId} returned 500, may be unavailable in this region`
						);
						return;
					}
					throw error;
	...
	describe('Tool Execution Flow', () => {
		it('should execute tool and include result in response', async () => {
			...
			const { text, steps } = await generateText({
				...
				tools: { calculator: calculatorTool },
				prompt:
					'What is 15 times 8? Use the calculator and tell me the result.',
				stopWhen: stepCountIs(3)
			});
	...
	describe('Tool Call Round-Trip', () => {
		it('should complete full tool call → execute → response cycle', async () => {
			...
			console.log(
				'  Phase 3: Model received tool result and generated response'
			);
			...
			if (toolResult && 'result' in toolResult) {
				console.log(
					`  ✓ Tool result received: ${JSON.stringify(toolResult.result)}`
				);
		...
		it('should handle tool returning complex nested object', async () => {
			...
			const { text, steps } = await generateText({
				...
				prompt:
					'Get info for user "user123" and tell me their name and how many visits they have.',
				stopWhen: stepCountIs(3)
			});
	...
	describe('Tool Choice', () => {
		...
		it('should respect toolChoice: none (when supported)', async () => {
			...
			if (allToolCalls.length > 0) {
				console.log(
					'[WARN] Cortex may not fully support toolChoice: none - tools were still called'
				);
			}
