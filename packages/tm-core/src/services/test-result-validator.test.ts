...
import type { TestResult, ValidationResult, TestPhase } from './test-result-validator.types.js';
...
describe('TestResultValidator - Input Validation', () => {
  ...
  describe('Schema Validation', () => {
    ...
    it('should reject when totals do not match', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Total tests must equal passed + failed + skipped');
    });
...
describe('TestResultValidator - RED Phase Validation', () => {
  ...
  it('should fail validation when RED phase has zero failures', () => {
    ...
    const result = validator.validateRedPhase(redResult);
    expect(result.valid).toBe(false);
    expect(result.errors).toContain('RED phase must have at least one failing test');
    expect(result.suggestions).toContain('Write failing tests first to follow TDD workflow');
  });
  ...
  it('should fail validation when RED phase has empty test suite', () => {
    ...
    expect(result.suggestions).toContain('Add at least one test to begin TDD cycle');
  });
...
describe('TestResultValidator - GREEN Phase Validation', () => {
  ...
  it('should fail validation when GREEN phase has failures', () => {
    ...
    expect(result.suggestions).toContain('Fix implementation to make all tests pass');
  });
  ...
  it('should fail validation when GREEN phase has no passing tests', () => {
    ...
    const result = validator.validateGreenPhase(greenResult);
    expect(result.valid).toBe(false);
    expect(result.errors).toContain('GREEN phase must have at least one passing test');
  });
  ...
  it('should warn when test count decreases', () => {
    ...
    expect(result.suggestions).toContain('Verify that no tests were accidentally removed');
  });
...
describe('TestResultValidator - Coverage Threshold Validation', () => {
  ...
  it('should fail validation when line coverage is below threshold', () => {
    ...
    expect(validationResult.suggestions).toContain('Add more tests to improve code coverage');
  });
  ...
  it('should fail validation when multiple coverage types are below threshold', () => {
    ...
    expect(validationResult.errors[0]).toContain('function coverage (75% < 85%)');
    expect(validationResult.errors[0]).toContain('statement coverage (65% < 80%)');
  });
  ...
  it('should propagate base validation errors', () => {
    ...
    const validationResult = validator.validateCoverage(invalidResult, thresholds);
    expect(validationResult.valid).toBe(false);
    expect(validationResult.errors).toContain('Total tests must equal passed + failed + skipped');
  });
});
