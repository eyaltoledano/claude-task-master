...
export class TestResultValidator {
  ...
  validateGreenPhase(
    ...
    if (previousTestCount !== undefined && testResult.total < previousTestCount) {
      warnings.push(
  ...
  validateCoverage(
    ...
    if (thresholds.branch !== undefined && coverage.branch < thresholds.branch) {
      gaps.push(`branch coverage (${coverage.branch}% < ${thresholds.branch}%)`);
    }

    if (thresholds.function !== undefined && coverage.function < thresholds.function) {
      gaps.push(`function coverage (${coverage.function}% < ${thresholds.function}%)`);
    }

    if (thresholds.statement !== undefined && coverage.statement < thresholds.statement) {
      gaps.push(`statement coverage (${coverage.statement}% < ${thresholds.statement}%)`);
    }
