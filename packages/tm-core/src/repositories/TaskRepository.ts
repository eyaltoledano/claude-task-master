...
export interface TaskRepository {
  ...
  updateTag(projectId: string, tagName: string, updates: Partial<TaskTag>): Promise<TaskTag>
  deleteTag(projectId: string, tagName: string): Promise<void>
  
  // Bulk operations
  bulkCreateTasks(projectId: string, tasks: Omit<Task, 'id'>[]): Promise<Task[]>
  bulkUpdateTasks(projectId: string, updates: Array<{ id: string; updates: Partial<Task> }>): Promise<Task[]>
